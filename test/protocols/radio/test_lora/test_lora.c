...
void test_frame_parse_rejects_short()
{
    ...
    TEST_ASSERT_FALSE(protocore_lora_frame_parse(raw, 3, &h, NULL, NULL)); // shorter than the header
}
...
void test_frame_build_bounds()
{
    ...
    uint8_t pay[8] = {0};
    uint8_t small[5];
    TEST_ASSERT_EQUAL_UINT16(0, protocore_lora_frame_build(&h, pay, 8, small, sizeof(small))); // 12 > cap 5
}
...
{
    protocore_lora_config c = default_cfg(); // 915 MHz
...
{
    TEST_ASSERT_FALSE(protocore_lora_tx_done(&g_bus)); // no flag
...
{
    uint8_t buf[16];
    TEST_ASSERT_EQUAL_INT(-1, protocore_lora_recv(&g_bus, buf, sizeof(buf), NULL)); // no RxDone
}
...
void test_frame_parse_build_guards()
{
    ...
    TEST_ASSERT_FALSE(protocore_lora_frame_parse(too_short, sizeof(too_short), &hdr, &payload, &payload_len)); // too short
    uint8_t out[4];
    uint8_t pay[8] = {0};
    TEST_ASSERT_EQUAL_UINT16(0, protocore_lora_frame_build(&hdr, pay, sizeof(pay), out, 2)); // cap too small
}
...
// protocore_lora_frame_build() refuses a null header, a null output buffer, and a payload longer than
...
// protocore_lora_init() refuses a null bus, a bus missing either access callback, and a null config -
