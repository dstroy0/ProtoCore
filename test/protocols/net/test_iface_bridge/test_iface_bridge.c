...
void test_map_and_find()
{
    ...
    TEST_ASSERT_EQUAL(PROTOCORE_IP_V4, r->listen_ip.family); // full bind address preserved
    TEST_ASSERT_EQUAL_UINT8(192, r->listen_ip.bytes[0]);
    TEST_ASSERT_EQUAL_UINT8(50, r->listen_ip.bytes[3]);

    TEST_ASSERT_NULL(protocore_iface_bridge_find(4001, BRIDGE_PROTO_UDP)); // proto must match
    TEST_ASSERT_NULL(protocore_iface_bridge_find(4002, BRIDGE_PROTO_TCP)); // port must match
}

void test_any_interface_and_dedup()
{
    BridgeTarget i = i2c_target(0x40);
    TEST_ASSERT_TRUE(protocore_iface_bridge_map(NULL, 5000, BRIDGE_PROTO_TCP, &i)); // any interface
...
void test_table_full()
{
    ...
    TEST_ASSERT_FALSE(protocore_iface_bridge_map(NULL, 9999, BRIDGE_PROTO_TCP, &u)); // full
    protocore_iface_bridge_clear();
    TEST_ASSERT_EQUAL_UINT8(0, protocore_iface_bridge_count());
}
