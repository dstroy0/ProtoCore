...
// 9114). Drives protocore_h3_conn through the protocore_quic_conn callback seam (qc->cb.on_stream_data /
...
// Two transports, matching the two connection slots. The engine's borrow is bound to the object,
// so it is carried across the reset the way protocore_quic_conn_init carries it.
...
void test_client_control_stream_settings()
{
    ...
    uint8_t s[64];
    size_t sp = protocore_quic_varint_encode(s, sizeof(s), 0x00); // control stream type
    ...
    g_qc.cb.on_stream_data(g_qc.cb.app, &g_qc, 2, s, sp, PROTO_FALSE); // client-initiated uni stream (id 2)
...
void test_client_uni_stream_types()
{
    ...
    n = protocore_quic_varint_encode(&t, 1, 0x1f); // an unknown stream type
...
void test_handshake_done_idempotent()
{
    ...
    g_qc.cb.on_handshake_done(g_qc.cb.app, &g_qc); // control_opened -> no-op
...
void test_stream_pool_full()
{
    ...
    {
        g_qc.cb.on_stream_data(g_qc.cb.app, &g_qc, i * 4, &b, 1, PROTO_FALSE); // partial request streams, no FIN
    }
...
void test_overlong_field_truncated()
{
    ...
    uint8_t block[128];
    size_t bp = protocore_qpack_encode_prefix(block, sizeof(block));
    const char *m = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; // 26 chars > PROTOCORE_H3_METHOD_LEN (16)
    ...
    TEST_ASSERT_EQUAL_INT(1, g_requests);
    TEST_ASSERT_EQUAL_UINT(PROTOCORE_H3_METHOD_LEN - 1, strlen(g_method)); // truncated to fit
}
...
void test_h3_request_unknown_frame_and_empty_data()
{
    ...
    uint8_t req[512];
    size_t rp = protocore_h3_build_headers(req, sizeof(req), block, bp);
    rp += protocore_h3_build_data(req + rp, sizeof(req) - rp, NULL, 0); // empty DATA
...
void test_h3_control_only_frames_on_a_request_stream()
{
    ...
    for (size_t i = 0; i < sizeof(only_control) / sizeof(only_control[0]); i++)
    {
        ...
        TEST_ASSERT_TRUE(g_qc.close_is_app); // an HTTP/3 code needs the 0x1d variant
...
{
    minimal_qc(&g_qc); // deliberately not established: no 1-RTT keys
    protocore_h3_conn_init(&g_h3, &g_qc, on_request, NULL);
...
void test_h3_stream_buffer_overflow_clamped()
{
    ...
    H3Stream *st = find_h3(&g_h3, 0);
    TEST_ASSERT_NOT_NULL(st);
    TEST_ASSERT_EQUAL_UINT(PROTOCORE_H3_STREAM_BUF, st->buf_len); // clamped to capacity
}
...
void test_h3_control_stream_frame_guards()
{
    ...
    sp3 += protocore_h3_frame_write_header(s3 + sp3, sizeof(s3) - sp3, 0x07 /*GOAWAY*/, 1);
    s3[sp3++] = 0x00;
    ...
    sp4 += protocore_h3_frame_write_header(s4 + sp4, sizeof(s4) - sp4, 0x07 /*GOAWAY*/, 1);
    s4[sp4++] = 0x00;
