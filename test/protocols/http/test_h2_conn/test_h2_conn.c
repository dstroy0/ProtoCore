...
// frame reassembly, and protocore_h2_conn_respond serializing a HEADERS + DATA response we can decode back.
//
...
void test_init_and_request(void)
{
    ...
    protocore_h2_conn_init(&g_conn, &cb); // must emit our SETTINGS
    int acks = 0;
...
// Every other request test here synthesizes its block with protocore_hpack_encode_header and then asserts
// what protocore_hpack_decode returns, so a symmetric codec fault would pass. This one carries the RFC 7541
...
void test_h2_stream_table_full_rst(void)
{
    ...
    TEST_ASSERT_TRUE(protocore_h2_conn_recv(&g_conn, hf, hn)); // kept alive
...
void test_h2_goaway_then_ignore(void)
{
    ...
    TEST_ASSERT_TRUE(protocore_h2_conn_recv(&g_conn, junk, sizeof junk)); // ignored while closing
}
...
// respond() to an unknown stream fails; protocore_h2_conn_goaway emits a GOAWAY; a body
// larger than the peer's max frame size is split across DATA frames.
void test_h2_respond_paths_and_goaway(void)
{
    static Cap cap;
    establish(&g_conn, &cap);
    TEST_ASSERT_FALSE(protocore_h2_conn_respond(&g_conn, 99, 200, "text/plain", "x", 1)); // no such stream
...
void test_h2_null_callbacks(void)
{
    ...
    protocore_h2_conn_init(&g_conn, &cb); // send_our_settings has nowhere to write; must not crash
...
// sec 6.10: PROTOCORE_H2_HDR_BLOCK bounds the bytes a header block may carry, but an empty CONTINUATION
...
void test_h2_continuation_after_stream_freed(void)
{
    ...
    TEST_ASSERT_TRUE(protocore_h2_conn_respond(&g_conn, 1, 200, NULL, "x", 1));  // frees the slot
    TEST_ASSERT_TRUE(feed_frame(&g_conn, H2_CONTINUATION, H2_FLAG_END_HEADERS, 1, block + half, blen - half));
