...
| `native_h2server` | `PC_ENABLE_HTTP2=1`, `PC_ENABLE_TLS=1`, `PC_HAS_VENDOR_TLS=1` | `protocols/http/test_h2_server` | HTTP/2 -> request-pipeline bridge (network_drivers/presentation/http/http2/h2_server): the RFC 9113 sec 8.2 / 8.3 validation a request header block must survive before the route dispatcher sees it - t... |
...
| `native_ssh_client` | `PC_ENABLE_SSH=1`, `PC_ENABLE_SSH_CLIENT=1`, `PC_SSH_MAX_CHANNELS=4` | `integration/ssh/test_ssh_client` | SSH client role (ssh_client.c) built per file: nothing else in the matrix compiles it. |
...
| `native_ssh_packet` | `PC_SSH_MAX_CHANNELS=3`, `PC_ENABLE_SSH=1`, `PC_ENABLE_MNT=1`, `PC_ENABLE_SSH_SFTP=1`, `PC_ENABLE_SSH_SCP=1` | `integration/ssh/test_ssh_packet` | The RFC 4253 sec 6 binary packet protocol (ssh/transport/ssh_packet.c): packet_length, the 4..255 padding and its block-size rule, the 16-byte minimum, the sec 6.4 sequence counter, and the SshDir the... |
| `native_ssh_pqc` | `PC_ENABLE_SSH=1`, `PC_SSH_MAX_CHANNELS=3`, `PC_ENABLE_PQC_KEX=1` | `integration/ssh/test_ssh_pqc` | mlkem768x25519-sha256 SSH hybrid KEX (draft-ietf-sshm-mlkem-hybrid-kex) end to end: the full SSH transport built with PC_ENABLE_PQC_KEX=1 plus the ML-KEM-768 / SHA-3 core. |
