...
void dbench_run(void)
{
    ...
    for (;;)
    {
        ...
        static char buf[64];
        DBENCH_OP("protocore_scpi_build (2 args)", 200000,
                  sink += protocore_scpi_build(buf, sizeof(buf), "SOUR:VOLT", args2, 2));
        ...
        DBENCH_OP("protocore_scpi_match (short/long)", 200000,
                  sink += protocore_scpi_match("SOUR:VOLT", 9, "SOURce:VOLTage"));
        (void)sink;
        DBENCH_DONE();
    }
}
