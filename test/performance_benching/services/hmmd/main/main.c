...
static int hmmd_reassemble(const uint8_t *frame, HmmdReport *out)
{
    ...
    for (int i = 0; i < PROTOCORE_HMMD_FRAME_MAX; i++)
    {
        ...
        HmmdV.stream_push_args.out = out;
...
void dbench_run(void)
{
    ...
    for (;;)
    {
        ...
        HmmdV.parse_report_args.frame = REPORT;
        HmmdV.parse_report_args.len = sizeof(REPORT);
        ...
        DBENCH_BULK("Hmmd.parse_report", 100000, sizeof(REPORT),
                    (Hmmd.parse_report(protocore_hmmd_span()), bsink ^= HmmdV.ok));
        ...
        HmmdV.cmd_open_args.buf = cmd;
        HmmdV.cmd_open_args.cap = sizeof(cmd);
        DBENCH_OP("Hmmd.cmd_open", 100000, (Hmmd.cmd_open(protocore_hmmd_span()), sink += HmmdV.n));
        // decode one command-ACK frame
        HmmdV.parse_ack_args.frame = ACK;
        HmmdV.parse_ack_args.len = sizeof(ACK);
        HmmdV.parse_ack_args.out = &a;
        DBENCH_OP("Hmmd.parse_ack", 100000, (Hmmd.parse_ack(protocore_hmmd_span()), bsink ^= HmmdV.ok));
