...
void dbench_run(void)
{
    ...
    for (;;)
    {
        ...
        DBENCH_OP("Cia402.state", 200000, sink8 += (uint8_t)(Cia402V.state(cia402_work), Cia402V.value));
        Cia402V.enable_sequence_args.state = CIA402_STATE_SWITCHED_ON;
        DBENCH_OP("Cia402.enable_sequence", 200000, sink16 += (Cia402V.enable_sequence(cia402_work), Cia402V.u16));
        ...
        DBENCH_OP("Cia402.sdo_set_controlword", 100000,
                  sinkb |= (Cia402V.sdo_set_controlword(cia402_work), Cia402V.ok));
        ...
        DBENCH_OP("Cia402.sdo_get_u16", 100000, sinkb |= (Cia402V.sdo_get_u16(cia402_work), Cia402V.ok));
        ...
        DBENCH_BULK("Cia402.pack_command", 100000, 6, sinksz += (Cia402V.pack_command(cia402_work), Cia402V.n));
        ...
        DBENCH_BULK("Cia402.unpack_status", 100000, 6, sinkb |= (Cia402V.unpack_status(cia402_work), Cia402V.ok));
