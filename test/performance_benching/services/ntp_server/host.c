...
//   -DPC_ENABLE_TIME_SOURCE=1 -DPC_ENABLE_NTP_SERVER=1
...
int main(void)
{
    ...
    // build one mode-4 server reply per client request (the whole per-query server cost).
    {
        ...
        double ns = 0.0;
        HBENCH_NS(5000000,
                  sink += pc_ntp_server_build_response(req, sizeof(req), 2, PC_NTP_REFID_LOCL, 0xE9A1B2C3u, 0x80000000u,
                                                       out, sizeof(out)),
                  ns);
        hbench_row("ntp", "build_response (48-octet)", ns, (double)(PC_NTP_PACKET_LEN * 2)); // request + reply moved
        (void)sink;
    }
