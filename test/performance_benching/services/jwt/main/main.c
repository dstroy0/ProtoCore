...
void dbench_run(void)
{
    ...
    for (;;)
    {
        ...
        DBENCH_OP("Jwt.verify_mac", 2000, Jwt.verify_mac(jwt_work); sink += JwtV.ok ? 1 : 0);

        JwtV.token.credentials = auth_hdr;
        DBENCH_OP("Jwt.verify_bearer", 2000, Jwt.verify_bearer(jwt_work); sink += JwtV.ok ? 1 : 0);

        // --- Codec path: base64url-decode the payload + scan JSON for a claim (medium N). ---
        JwtV.claim.name = "exp";
        DBENCH_OP("Jwt.claim_int", 10000, Jwt.claim_int(jwt_work); lsink = JwtV.num; sink += JwtV.ok ? 1 : 0);

        JwtV.claim.name = "sub";
        JwtV.claim.out = strbuf;
        JwtV.claim.out_cap = sizeof(strbuf);
        DBENCH_OP("Jwt.claim_str", 10000, Jwt.claim_str(jwt_work); sink += JwtV.ok ? 1 : 0);

        // --- Pure string path: whole-token scope match, no decode (large N). ---
        JwtV.scope.claim = scope_claim;
        JwtV.scope.required = "admin";
        DBENCH_OP("Jwt.scope_allows", 50000, Jwt.scope_allows(jwt_work); sink += JwtV.ok ? 1 : 0);
