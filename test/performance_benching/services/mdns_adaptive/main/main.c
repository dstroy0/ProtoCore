...
void dbench_run(void)
{
    ...
    MdnsAdaptiveV.beacon_init_args.base_ms = 60000;
    MdnsAdaptiveV.beacon_init_args.max_ms = 480000;
    MdnsAdaptiveV.beacon_init_args.hi_thresh = 5;
    ...
    for (;;)
    {
        ...
        DBENCH_OP("MdnsAdaptive.refresh_interval", 200000,
                  sink32 += (MdnsAdaptive.refresh_interval(work), MdnsAdaptiveV.ms));

        MdnsAdaptiveV.beacon_adapt_args.b = &b;
        MdnsAdaptiveV.beacon_adapt_args.contention = 8;
        // RF-contention backoff/recovery step (mutates cur_ms in place; settles at the ceiling clamp).
        DBENCH_OP("MdnsAdaptive.beacon_adapt", 200000, sink32 += (MdnsAdaptive.beacon_adapt(work), MdnsAdaptiveV.ms));

        MdnsAdaptiveV.beacon_due_args.b = &b;
        MdnsAdaptiveV.beacon_due_args.last_ms = 1000;
        MdnsAdaptiveV.beacon_due_args.now_ms = 1000 + 60000;
        // Wrap-safe announce-due check (evaluated every tick in production).
        DBENCH_OP("MdnsAdaptive.beacon_due", 200000, sinkb += (MdnsAdaptive.beacon_due(work), MdnsAdaptiveV.ok));

        MdnsAdaptiveV.beacon_presleep_due_args.b = &b;
        MdnsAdaptiveV.beacon_presleep_due_args.last_ms = 0;
        MdnsAdaptiveV.beacon_presleep_due_args.now_ms = 10000;
        MdnsAdaptiveV.beacon_presleep_due_args.sleep_ms = 60000;
        ...
        DBENCH_OP("MdnsAdaptive.beacon_presleep_due", 200000,
                  sinkb += (MdnsAdaptive.beacon_presleep_due(work), MdnsAdaptiveV.ok));

        MdnsAdaptiveV.contention_init_args.w = &w;
        MdnsAdaptiveV.contention_init_args.window_ms = 1000;
        MdnsAdaptiveV.contention_init_args.frames_now = 0;
        MdnsAdaptiveV.contention_init_args.now_ms = 0;
        MdnsAdaptiveV.contention_sample_args.w = &w;
        MdnsAdaptiveV.contention_sample_args.frames_now = 500;
        MdnsAdaptiveV.contention_sample_args.now_ms = 100000;
        ...
        DBENCH_OP("MdnsAdaptive.contention_sample", 200000,
                  sink16 +=
                  (MdnsAdaptive.contention_init(work), MdnsAdaptive.contention_sample(work), MdnsAdaptiveV.ok));
