...
//   -DPC_ENABLE_GOOSE=1 -DPC_ENABLE_RAWL2=1 test/performance_benching/services/goose/host.c
...
int main(void)
{
    ...
    // pc_goose_pdu: BER-encode the IECGoosePdu (11 control fields + the allData blob) - the publish core.
    {
        ...
        double ns = 0.0;
        HBENCH_NS(5000000, sink += pc_goose_pdu(&g, buf, sizeof(buf)), ns);
    ...
    // pc_goose_frame: wrap the PDU in the Ethernet header + 8-octet GOOSE header (the full L2 datagram).
    {
        ...
        volatile size_t sink = 0;
        double ns = 0.0;
        HBENCH_NS(5000000, sink += pc_goose_frame(dst, src, 0x0001, &g, buf, sizeof(buf)), ns);
