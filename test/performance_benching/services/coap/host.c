...
//   -DPC_ENABLE_COAP=1 test/performance_benching/services/coap/host.c src/services/iot/coap/coap.c
...
int main(void)
{
    ...
    pc_coap_server_add_resource("/info", COAP_ALLOW_GET, h_info);
    pc_coap_server_add_resource("/a/b/c", COAP_ALLOW_GET, h_info);
...

    {
        ...
        double ns = 0.0;
        HBENCH_NS(1000000, sink += pc_coap_server_process(get_info, sizeof(get_info), resp, sizeof(resp)), ns);
    ...
    }
    {
        volatile size_t sink = 0;
        double ns = 0.0;
        HBENCH_NS(1000000, sink += pc_coap_server_process(get_abc, sizeof(get_abc), resp, sizeof(resp)), ns);
