...
//   -DPC_ENABLE_SSE=1 test/performance_benching/network_drivers/presentation/sse/host.c
...
// sse.c's pc_sse_write() (not exercised here) references the transport layer; the bench only calls
...
int main(void)
{
    ...
    // data-only: the common broadcast shape (`data: <payload>\n\n`).
    {
        volatile int sink = 0;
        double ns = 0.0;
        HBENCH_NS(2000000, sink += pc_sse_format(buf, sizeof(buf), "sensor=21.4C rh=48%", NULL, NULL), ns);
        int bytes = pc_sse_format(buf, sizeof(buf), "sensor=21.4C rh=48%", NULL, NULL);
    ...
    // event + id + data: the fully-addressed record (named event, resumable id).
    {
        volatile int sink = 0;
        double ns = 0.0;
        HBENCH_NS(2000000, sink += pc_sse_format(buf, sizeof(buf), "sensor=21.4C rh=48%", "telemetry", "12345"), ns);
