...
//   -DPC_ENABLE_FILE_SERVING=1 -DPC_ENABLE_WEBDAV=1
...
int main(void)
{
    ...
    // A whole Depth-1 directory listing: prolog + 8 children + epilog (a realistic PROPFIND body).
    {
        ...
        HBENCH_NS(
            200000,
            {
                ...
                for (int k = 0; k < 8; k++)
                {
                    len = pc_webdav_ms_entry(buf, sizeof(buf), len, "/dav/sensor-log.csv", false, 12800, mtime,
                                             "text/csv");
                }
    ...
    // XML-escape an href containing the five escapable characters (per href / property tag).
    {
        ...
        volatile size_t sink = 0;
        double ns = 0.0;
        HBENCH_NS(2000000, sink += pc_webdav_xml_escape(esc, sizeof(esc), "/dav/a&b<c>\"d'e.txt"), ns);
