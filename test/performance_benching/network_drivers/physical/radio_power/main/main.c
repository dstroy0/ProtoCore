...
void dbench_run(void)
{
    for (;;)
    {
        ...
        DBENCH_OP("Radio.ps_name", 200000, Radio.ps_name(protocore_radio_power_span()); sink += (uintptr_t)RadioV.text);
        DBENCH_OP("Radio.ps_mode", 200000, Radio.ps_mode(protocore_radio_power_span()); sink += (uintptr_t)RadioV.mode);
        (void)sink;
        DBENCH_DONE();
    }
}
