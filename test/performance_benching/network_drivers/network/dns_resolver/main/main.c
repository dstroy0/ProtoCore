...
void dbench_run(void)
{
    ...
    for (;;)
    {
        ...
        ResolverV.addr.ip = ip_public;
        DBENCH_OP("Resolver.classify (public)", 200000, Resolver.classify(protocore_dns_resolver_span());
                  sink8 += (uint8_t)ResolverV.cls);

        ResolverV.addr.ip = ip_private;
        DBENCH_OP("Resolver.classify (private)", 200000, Resolver.classify(protocore_dns_resolver_span());
                  sink8 += (uint8_t)ResolverV.cls);

        ResolverV.addr.ip = ip_public;
        DBENCH_OP("Resolver.verify (accept public)", 200000, ResolverV.verify(protocore_dns_resolver_span());
                  sinkb ^= ResolverV.ok);

        ResolverV.addr.ip = ip_loopback;
        DBENCH_OP("Resolver.verify (reject loopback)", 200000, ResolverV.verify(protocore_dns_resolver_span());
                  sinkb ^= ResolverV.ok);
        ResolverV.addr.ip = ip_multicast;
        DBENCH_OP("Resolver.verify (reject multicast)", 200000, ResolverV.verify(protocore_dns_resolver_span());
                  sinkb ^= ResolverV.ok);
        (void)sink8;
        (void)sinkb;
        DBENCH_DONE();
