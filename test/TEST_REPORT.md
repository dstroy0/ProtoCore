...
**Generated:** 2026-08-14 04:51:49
**Command:** `pio test` over 439 auto-discovered native envs (excludes native_pentest, native_codeql, native_tsan)
**Result:** ❌ 4953 passed, 93 failed - 5067s
...
*Host tests for the AD9238 SPI configuration-port codec (server/peripherals/ad9238/ad9238.h).*
...
*Host tests for the CDN edge-cache pure engine (server/web/edge_cache/edge_cache.h) and the shared*
...
*Host tests for the EUROMAP 77 IMM_MES_Interface model (services/machine_tool/euromap77/euromap77.h).*
...
*Host tests for the FANUC Stream Motion (J519) UDP codec*
...
*Unit tests for the HTTP/2 connection engine (network_drivers/presentation/http/http2/protocore_h2_conn,*
...
*Unit tests for the HTTP/3 application engine (network_drivers/presentation/http/http3/protocore_h3_conn; RFC*
...
*Host tests for the HTTP delivery optimizations (services/file_transfer/http_delivery/http_delivery.h).*
...
*Host tests for the SAE J2735 V2X UPER codec (services/transportation/j2735/j2735.h).*
...
| 9 | `test_a_full_volume_refuses_rather_than_pretending` | ✅ | The reason this fixture is the real filesystem: a hand-rolled tree never runs out, so the |
...
*Host tests for the LonWorks / LON-IP network-variable codec (services/fieldbus/lonworks/lonworks.h).*
...
*Host tests for the NTP/SNTP server role (network_drivers/application/ntp_server/ntp_server.h).*
...
*Host tests for the NTRIP caster protocol codec (services/timing_position/gnss/ntrip_caster.h).*
...
*Host tests for the OPC UA Binary server core (services/fieldbus/opcua/opcua.h).*
...
*Host tests for the CPU clock governor (server/core/power_mgmt.h).*
...
*Host tests for the HAProxy PROXY protocol codec (network_drivers/transport/proxy_protocol/proxy_protocol.h).*
...
*Host tests for the PTPv2 message codec and slave clock math (network_drivers/application/ptp/ptp.h).*
...
*Host tests for the OPC UA for Robotics model (services/machine_tool/robotics/robotics.h).*
...
*Host tests for the SERCOS III telegram + IDN codec (services/fieldbus/sercos/sercos.h).*
...
*Host tests for the sleep-window scheduler (server/core/sleep_sched.h).*
...
*Adversarial host tests for the SMB-family parsers that consume bytes off the wire*
...
| 9 | `test_find_skips_driver_mutated_name_null` | ✅ | protocore_southbound_find() stores a *borrowed* pointer (const SouthboundDriver *), not a copy: the |
...
*Host tests for the TLS 1.3 handshake driver (network_drivers/tls/handshake/handshake.h).*
