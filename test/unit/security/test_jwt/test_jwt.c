...
{
    TEST_ASSERT_TRUE(protocore_jwt_verify_hs256(TOKEN, strlen(TOKEN), sec(), seclen()));
}

void test_wrong_secret_rejects()
{
    const char *bad = "wrong-key";
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256(TOKEN, strlen(TOKEN), (const uint8_t *)bad, strlen(bad)));
}
...
void test_tampered_payload_rejects()
{
    ...
    char *dot = strchr(t, '.');
    dot[3] = (dot[3] == 'A') ? 'B' : 'A';
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256(t, strlen(t), sec(), seclen()));
}
...
{
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256("not-a-jwt", 9, sec(), seclen()));
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256("only.one-dot", 12, sec(), seclen()));
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256("", 0, sec(), seclen()));
}
...
void test_alg_not_hs256_rejected()
{
    ...
    protocore_hmac_sha256(tw, sec(), seclen(), (const uint8_t *)signing, (size_t)sl, mac);
    char sig[48];
    ...
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256(token, strlen(token), sec(), seclen()));
...
void test_bearer_header()
{
    ...
    TEST_ASSERT_TRUE(protocore_jwt_bearer_valid(hdr, sec(), seclen()));
    // case-insensitive scheme
    snprintf(hdr, sizeof(hdr), "bearer %s", TOKEN);
    TEST_ASSERT_TRUE(protocore_jwt_bearer_valid(hdr, sec(), seclen()));
    // missing scheme
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid(TOKEN, sec(), seclen()));
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid(NULL, sec(), seclen()));
}

void test_claim_int()
{
    long v = 0;
    TEST_ASSERT_TRUE(protocore_jwt_claim_int(TOKEN, strlen(TOKEN), "exp", &v));
    TEST_ASSERT_EQUAL_INT32(2000000000, v);
    TEST_ASSERT_TRUE(protocore_jwt_claim_int(TOKEN, strlen(TOKEN), "iat", &v));
...
{
    long v = 0;
    TEST_ASSERT_FALSE(protocore_jwt_claim_int(TOKEN, strlen(TOKEN), "nbf", &v));
}

void test_claim_str()
{
    char buf[32];
    TEST_ASSERT_TRUE(protocore_jwt_claim_str(TOKEN, strlen(TOKEN), "sub", buf, sizeof(buf)));
    TEST_ASSERT_EQUAL_STRING("alice", buf);
    TEST_ASSERT_TRUE(protocore_jwt_claim_str(TOKEN, strlen(TOKEN), "role", buf, sizeof(buf)));
    TEST_ASSERT_EQUAL_STRING("admin", buf);
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(TOKEN, strlen(TOKEN), "nope", buf, sizeof(buf)));
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(TOKEN, strlen(TOKEN), "exp", buf, sizeof(buf))); // numeric, not a string
}

void test_scope_allows()
{
    TEST_ASSERT_TRUE(protocore_jwt_scope_allows("read write admin", "read"));
    TEST_ASSERT_TRUE(protocore_jwt_scope_allows("read write admin", "write"));
    TEST_ASSERT_TRUE(protocore_jwt_scope_allows("read write admin", "admin"));
    TEST_ASSERT_TRUE(protocore_jwt_scope_allows("admin", "admin")); // single scope
    TEST_ASSERT_FALSE(protocore_jwt_scope_allows("read write admin", "delete"));
    TEST_ASSERT_FALSE(protocore_jwt_scope_allows("read write admin", "adm")); // whole-token match only
    TEST_ASSERT_FALSE(protocore_jwt_scope_allows("read write admin", ""));
    TEST_ASSERT_FALSE(protocore_jwt_scope_allows(NULL, "read"));
}
...
void test_verify_malformed_headers()
{
    ...
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256("aaaa.b.c.d", 10, sec(), seclen()));
...
{
    long v = 0;
    TEST_ASSERT_FALSE(protocore_jwt_claim_int(NULL, 5, "x", &v));
    TEST_ASSERT_FALSE(protocore_jwt_claim_int(TOKEN, strlen(TOKEN), NULL, &v));
    TEST_ASSERT_FALSE(protocore_jwt_claim_int(TOKEN, strlen(TOKEN), "exp", NULL));
    TEST_ASSERT_FALSE(protocore_jwt_claim_int("nodots", 6, "x", &v));  // no first dot
    TEST_ASSERT_FALSE(protocore_jwt_claim_int("a.b", 3, "x", &v));     // no second dot
    TEST_ASSERT_FALSE(protocore_jwt_claim_int("x.@@@.y", 7, "x", &v)); // payload not base64url
    char longname[60];
    memset(longname, 'a', sizeof(longname) - 1);
    longname[sizeof(longname) - 1] = '\0';
    TEST_ASSERT_FALSE(protocore_jwt_claim_int(TOKEN, strlen(TOKEN), longname, &v)); // name too long for key buffer

    char t[256];
    mk_token(t, sizeof(t), "{\"alg\":\"HS256\"}", "{\"neg\":-42}", "sig");
    TEST_ASSERT_TRUE(protocore_jwt_claim_int(t, strlen(t), "neg", &v));
    TEST_ASSERT_EQUAL_INT(-42, v);
    mk_token(t, sizeof(t), "{\"alg\":\"HS256\"}", "{\"x\":\"str\"}", "sig");
    TEST_ASSERT_FALSE(protocore_jwt_claim_int(t, strlen(t), "x", &v)); // value is not a number
}

void test_claim_str_edges()
{
    char buf[32];
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(NULL, 5, "x", buf, sizeof(buf)));
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(TOKEN, strlen(TOKEN), "sub", buf, 0)); // zero cap
    TEST_ASSERT_FALSE(protocore_jwt_claim_str("nodots", 6, "x", buf, sizeof(buf)));
    TEST_ASSERT_FALSE(protocore_jwt_claim_str("a.b", 3, "x", buf, sizeof(buf)));
    TEST_ASSERT_FALSE(protocore_jwt_claim_str("x.@@@.y", 7, "x", buf, sizeof(buf)));
    char longname[60];
    memset(longname, 'a', sizeof(longname) - 1);
    longname[sizeof(longname) - 1] = '\0';
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(TOKEN, strlen(TOKEN), longname, buf, sizeof(buf)));

    char t[256];
    mk_token(t, sizeof(t), "{\"alg\":\"HS256\"}", "{\"s\":\"a\\/b\"}", "sig");
    TEST_ASSERT_TRUE(protocore_jwt_claim_str(t, strlen(t), "s", buf, sizeof(buf)));
    TEST_ASSERT_EQUAL_STRING("a/b", buf); // backslash unescape
    mk_token(t, sizeof(t), "{\"alg\":\"HS256\"}", "{\"s\":\"abc", "sig");
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(t, strlen(t), "s", buf, sizeof(buf))); // unterminated string
}
...
// expired token passes protocore_jwt_time_valid, and the signed verify reduces to the sig check.
void test_time_no_clock_skips_claims()
{
    char t[400];
    mk_signed(t, sizeof(t), "{\"sub\":\"a\",\"exp\":1000}"); // exp far in the past
    TEST_ASSERT_TRUE(protocore_jwt_time_valid(t, strlen(t), 0, 0));
    TEST_ASSERT_TRUE(protocore_jwt_verify_hs256_at(t, strlen(t), sec(), seclen(), 0, 0));
}
...
void test_time_exp_enforced()
{
    ...
    TEST_ASSERT_TRUE(protocore_jwt_time_valid(t, strlen(t), 1699999000, 0));  // before exp
    TEST_ASSERT_FALSE(protocore_jwt_time_valid(t, strlen(t), 1700000100, 0)); // past exp
    TEST_ASSERT_TRUE(protocore_jwt_time_valid(t, strlen(t), 1700000030, 60)); // within leeway of exp
    // The signed verifier enforces it too (valid signature, time decides).
    TEST_ASSERT_TRUE(protocore_jwt_verify_hs256_at(t, strlen(t), sec(), seclen(), 1699999000, 0));
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256_at(t, strlen(t), sec(), seclen(), 1700000100, 0));
}
...
void test_time_nbf_enforced()
{
    ...
    TEST_ASSERT_FALSE(protocore_jwt_time_valid(t, strlen(t), 1699999000, 0)); // before nbf
    TEST_ASSERT_TRUE(protocore_jwt_time_valid(t, strlen(t), 1700000100, 0));  // after nbf
    TEST_ASSERT_TRUE(protocore_jwt_time_valid(t, strlen(t), 1699999970, 60)); // within leeway before nbf
}
...
void test_time_no_claims_valid()
{
    ...
    TEST_ASSERT_TRUE(protocore_jwt_time_valid(t, strlen(t), 1700000000, 0));
}

// protocore_jwt_bearer_valid_at gates on both the signature and the time window.
void test_bearer_valid_at()
{
    char t[400], hdr[470];
    mk_signed(t, sizeof(t), "{\"exp\":1700000000}");
    snprintf(hdr, sizeof(hdr), "Bearer %s", t);
    TEST_ASSERT_TRUE(protocore_jwt_bearer_valid_at(hdr, sec(), seclen(), 1699999000, 0));  // valid sig, in window
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid_at(hdr, sec(), seclen(), 1700000100, 0)); // valid sig, expired
    // A bad signature fails even inside the time window (signature is the primary gate).
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid_at(hdr, (const uint8_t *)"wrong", 5, 1699999000, 0));
}

void test_bearer_header_guards()
{
    uint8_t secret[8] = {1, 2, 3, 4, 5, 6, 7, 8};
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid(NULL, secret, sizeof(secret)));                  // null header
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid("Basic abc", secret, sizeof(secret)));           // not a Bearer
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid("Bearer    not.a.jwt", secret, sizeof(secret))); // leading spaces skipped
}
...
void test_header_alg_whitespace_and_prefix()
{
    ...
    TEST_ASSERT_TRUE(protocore_jwt_verify_hs256(t, strlen(t), sec(), seclen()));
    mk_signed_hdr(t, sizeof(t), "{\"alg\":\t\"HS256\"}", "{\"sub\":\"a\"}");
    TEST_ASSERT_TRUE(protocore_jwt_verify_hs256(t, strlen(t), sec(), seclen()));
...
// protocore_jwt_verify_hs256() refuses a null token and one longer than PROTOCORE_JWT_MAX_LEN before doing any
// parsing or HMAC work.
void test_verify_length_guards()
{
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256(NULL, 20, sec(), seclen()));
    static char big[PROTOCORE_JWT_MAX_LEN + 8];
    memset(big, 'a', sizeof(big) - 1);
    big[sizeof(big) - 1] = '\0';
    TEST_ASSERT_FALSE(protocore_jwt_verify_hs256(big, PROTOCORE_JWT_MAX_LEN + 1, sec(), seclen()));
}

// With no token there are no time claims to evaluate, so protocore_jwt_time_valid() reports valid -
// exactly as it does with no wall clock.
void test_time_valid_null_token()
{
    TEST_ASSERT_TRUE(protocore_jwt_time_valid(NULL, 0, 1700000000, 0));
}

// protocore_jwt_bearer_valid_at() applies the same header guards as the untimed variant: a null header
// and a non-Bearer scheme are refused, and spaces after the scheme are skipped.
void test_bearer_valid_at_header_guards()
{
    char t[400], hdr[470];
    mk_signed(t, sizeof(t), "{\"exp\":1700000000}");
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid_at(NULL, sec(), seclen(), 1699999000, 0));
    TEST_ASSERT_FALSE(protocore_jwt_bearer_valid_at("Basic abc", sec(), seclen(), 1699999000, 0));
    snprintf(hdr, sizeof(hdr), "Bearer    %s", t); // extra spaces after the scheme
    TEST_ASSERT_TRUE(protocore_jwt_bearer_valid_at(hdr, sec(), seclen(), 1699999000, 0));
}

// protocore_jwt_claim_int() skips spaces and tabs around the colon, and rejects a value whose first
...
void test_claim_int_value_whitespace_and_non_numeric()
{
    ...
    TEST_ASSERT_TRUE(protocore_jwt_claim_int(t, strlen(t), "exp", &v));
    TEST_ASSERT_EQUAL_INT32(1700000000, v);
    ...
    TEST_ASSERT_FALSE(protocore_jwt_claim_int(t, strlen(t), "exp", &v));
}

// protocore_jwt_claim_str() refuses a null claim name and a null output, skips spaces and tabs around
...
{
    char buf[32], t[256];
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(TOKEN, strlen(TOKEN), NULL, buf, sizeof(buf)));
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(TOKEN, strlen(TOKEN), "sub", NULL, sizeof(buf)));
    ...
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(t, strlen(t), "s", small, sizeof(small)));
    ...
    TEST_ASSERT_FALSE(protocore_jwt_claim_str(t, strlen(t), "s", buf, sizeof(buf)));
}

// protocore_jwt_scope_allows() refuses a null required scope as firmly as a null claim or an empty
// requirement.
void test_scope_allows_null_required()
{
    TEST_ASSERT_FALSE(protocore_jwt_scope_allows("read write", NULL));
}
