...
void tearDown()
{
    Udp.listener->close(PROTOCORE_NTP_CLIENT_PORT);
}
...
static proto_bool sync_with(const char *server)
{
    ...
    return protocore_ntp_begin(NULL, server, NULL);
}

// The request the client just sent, taken off the wire.
static const protocore_net_host_dgram *request(void)
{
    TEST_ASSERT_EQUAL_INT(1, (int)protocore_net_host_udp_count());
    return protocore_net_host_udp_at(0);
}
...
static void reply_with(uint8_t mode, uint8_t stratum, uint32_t origin, uint32_t unix_secs)
{
    ...
    r[1] = stratum;
    wr_be32(r + 24, origin);
    wr_be32(r + PROTOCORE_NTP_OFF_TX_SEC, unix_secs + PROTOCORE_NTP_UNIX_OFFSET);
    protocore_net_host_udp_deliver(PROTOCORE_NTP_CLIENT_PORT, SERVER_IP, 123, r, sizeof(r));
    Udp.listener->poll();
}
...
void test_begin_sends_a_client_request()
{
    ...
    const protocore_net_host_dgram *d = request();
...
void test_a_short_datagram_is_refused()
{
    ...
    protocore_net_host_udp_deliver(PROTOCORE_NTP_CLIENT_PORT, SERVER_IP, 123, runt, sizeof(runt));
    Udp.listener->poll();
    TEST_ASSERT_FALSE(protocore_ntp_synced());
}
...
void test_the_epoch_advances_with_the_tick()
{
    ...
    TEST_ASSERT_EQUAL_UINT32(GOOD_EPOCH + 5, (uint32_t)protocore_ntp_epoch());
...
{
    TEST_ASSERT_FALSE(protocore_ntp_synced());
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)protocore_ntp_epoch());
...
void test_a_seeded_epoch_formats_a_date()
{
    ...
    TEST_ASSERT_TRUE(protocore_ntp_synced());
    char buf[40];
    size_t n = protocore_ntp_http_date(buf, sizeof(buf));
