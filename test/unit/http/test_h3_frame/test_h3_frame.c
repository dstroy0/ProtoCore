...
//
// Unit tests for the HTTP/3 framing layer (network_drivers/presentation/http/http3/protocore_h3_frame, RFC 9114
...
void test_header_roundtrip()
{
    ...
    TEST_ASSERT_EQUAL_INT(2, (int)protocore_h3_frame_write_header(b, sizeof b, H3_SETTINGS, 0));
    const uint8_t exp[2] = {0x04, 0x00};
    TEST_ASSERT_EQUAL_UINT8_ARRAY(exp, b, 2);
    H3Frame f;
    TEST_ASSERT_TRUE(protocore_h3_frame_parse(b, 2, &f));
    TEST_ASSERT_TRUE(f.type == H3_SETTINGS && f.length == 0 && f.header_len == 2);

    // HEADERS(1), length 1000 -> a 2-byte length varint (0x43E8).
    TEST_ASSERT_EQUAL_INT(3, (int)protocore_h3_frame_write_header(b, sizeof b, H3_HEADERS, 1000));
    const uint8_t exp2[3] = {0x01, 0x43, 0xE8};
    TEST_ASSERT_EQUAL_UINT8_ARRAY(exp2, b, 3);
    TEST_ASSERT_TRUE(protocore_h3_frame_parse(b, 3, &f));
...
{
    uint8_t b[32];
    TEST_ASSERT_EQUAL_INT(7, (int)protocore_h3_build_data(b, sizeof b, (const uint8_t *)"hello", 5));
    const uint8_t data[7] = {0x00, 0x05, 'h', 'e', 'l', 'l', 'o'};
    TEST_ASSERT_EQUAL_UINT8_ARRAY(data, b, 7);

    TEST_ASSERT_EQUAL_INT(3, (int)protocore_h3_build_goaway(b, sizeof b, 8));
...
void test_settings_roundtrip()
{
    ...
    uint8_t b[32];
    size_t n = protocore_h3_build_settings(b, sizeof b, ids, vals, 2);
    ...
    H3Frame f;
    TEST_ASSERT_TRUE(protocore_h3_frame_parse(b, n, &f));
    TEST_ASSERT_TRUE(f.type == H3_SETTINGS && f.length == 8);
    H3Settings s;
    protocore_h3_settings_defaults(&s);
    TEST_ASSERT_TRUE(protocore_h3_parse_settings(b + f.header_len, (size_t)f.length, &s));
    TEST_ASSERT_TRUE(s.protocore_qpack_max_table_capacity == 4096);
    TEST_ASSERT_TRUE(s.max_field_section_size == 1048576);
    TEST_ASSERT_TRUE(s.protocore_qpack_blocked_streams == 0);
}

void test_reserved()
{
    TEST_ASSERT_TRUE(protocore_h3_frame_type_reserved(0x02));
    TEST_ASSERT_TRUE(protocore_h3_frame_type_reserved(0x06));
    TEST_ASSERT_TRUE(protocore_h3_frame_type_reserved(0x08));
    TEST_ASSERT_TRUE(protocore_h3_frame_type_reserved(0x09));
    TEST_ASSERT_FALSE(protocore_h3_frame_type_reserved(H3_DATA));
    TEST_ASSERT_FALSE(protocore_h3_frame_type_reserved(H3_HEADERS));
    TEST_ASSERT_FALSE(protocore_h3_frame_type_reserved(H3_SETTINGS));
    ...
    {
        TEST_ASSERT_FALSE(protocore_h3_frame_type_reserved(0x1f * n + 0x21));
    }
    ...
    for (size_t i = 0; i < sizeof reserved_ids / sizeof reserved_ids[0]; i++)
    {
        ...
        TEST_ASSERT_FALSE(protocore_h3_parse_settings(bad, 2, &s));
    }
}
...
void test_build_headers()
{
    ...
    uint8_t b[8];
    size_t n = protocore_h3_build_headers(b, sizeof b, block, 2);
...
void test_builder_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_frame_write_header(b, 0, H3_DATA, 0));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_data(b, 2, data, 5)); // header fits, payload does not
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_headers(b, 2, data, 5));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_settings(b, 1, ids, vals, 1));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_goaway(b, 1, 0x4000)); // stream id needs a 2-byte varint
}
...
{
    H3Frame f;
    TEST_ASSERT_FALSE(protocore_h3_frame_parse((const uint8_t *)"", 0, &f)); // no type
    const uint8_t no_len[1] = {0x04};                                 // type ok, no length varint
    TEST_ASSERT_FALSE(protocore_h3_frame_parse(no_len, 1, &f));
    const uint8_t trunc_len[1] = {0x40}; // a 2-byte type varint with only 1 byte present
    TEST_ASSERT_FALSE(protocore_h3_frame_parse(trunc_len, 1, &f));

    H3Settings s;
    protocore_h3_settings_defaults(&s);
    const uint8_t id_no_val[1] = {0x01}; // QPACK_MAX_TABLE_CAPACITY id, no value
    TEST_ASSERT_FALSE(protocore_h3_parse_settings(id_no_val, 1, &s));
    // An unknown / GREASE settings id is ignored (not an error).
    const uint8_t grease[2] = {0x21, 0x00};
    TEST_ASSERT_TRUE(protocore_h3_parse_settings(grease, 2, &s));
}
...
void test_settings_blocked_streams()
{
    ...
    uint8_t b[8];
    size_t n = protocore_h3_build_settings(b, sizeof b, ids, vals, 1);
    ...
    protocore_h3_settings_defaults(&s);
    TEST_ASSERT_TRUE(protocore_h3_parse_settings(b + f.header_len, (size_t)f.length, &s));
    TEST_ASSERT_TRUE(s.protocore_qpack_blocked_streams == 9);
    // The other two fields stay at their defaults.
    TEST_ASSERT_TRUE(s.protocore_qpack_max_table_capacity == 0);
...
void test_parse_settings_id_decode_fails()
{
    ...
    TEST_ASSERT_FALSE(protocore_h3_parse_settings(bad_id, 1, &s));
}

// protocore_h3_build_data / protocore_h3_build_headers: a zero-length payload skips the memcpy, and a cap too
...
void test_build_data_and_headers_edge_caps()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_data(b, 0, block, 0));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_headers(b, 0, block, 0));
}

// protocore_h3_build_settings: cap can be exactly enough for the header but run out mid-loop, either
...
void test_build_settings_partial_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_settings(b, 2, ids, vals, 1));
    // One more byte fits the id varint but leaves none for the value varint.
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_settings(b, 3, ids, vals, 1));
}

// protocore_h3_build_goaway: cap can be exactly enough for the header but too small for the stream id
// varint itself (a == 0).
void test_build_goaway_partial_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_h3_build_goaway(b, 2, 64));
}
