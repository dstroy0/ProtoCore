...
//
// Unit tests for the QUIC frame codec (network_drivers/presentation/http/http3/protocore_quic_frame, RFC 9000
...
void test_simple_frames()
{
    uint8_t b[4];
    QuicFrame f;
    TEST_ASSERT_EQUAL_INT(1, (int)protocore_quic_build_ping(b, sizeof b));
    TEST_ASSERT_EQUAL_INT(1, (int)protocore_quic_frame_parse(b, 1, &f));
    TEST_ASSERT_EQUAL_UINT(QUIC_FT_PING, (unsigned)f.type);

    TEST_ASSERT_EQUAL_INT(1, (int)protocore_quic_build_handshake_done(b, sizeof b));
    TEST_ASSERT_EQUAL_INT(1, (int)protocore_quic_frame_parse(b, 1, &f));
    ...
    TEST_ASSERT_EQUAL_INT(1, (int)protocore_quic_frame_parse(pad, 1, &f));
...
{
    uint8_t b[16];
    size_t n = protocore_quic_build_ack(b, sizeof b, 1000, 42, 3);
    TEST_ASSERT_TRUE(n > 0);
    QuicFrame f;
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(b, n, &f));
    ...
    TEST_ASSERT_EQUAL_INT(10, (int)protocore_quic_frame_parse(ecn, sizeof ecn, &f));
...
void test_crypto()
{
    ...
    size_t n = protocore_quic_build_crypto(b, sizeof b, 7, data, 5);
    TEST_ASSERT_TRUE(n > 0);
    QuicFrame f;
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(b, n, &f));
...
void test_stream()
{
    ...
    // With offset + FIN.
    size_t n = protocore_quic_build_stream(b, sizeof b, 4, 100, data, 3, PROTO_TRUE);
    QuicFrame f;
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(b, n, &f));
    ...
    // No offset, no FIN.
    n = protocore_quic_build_stream(b, sizeof b, 0, 0, data, 3, PROTO_FALSE);
    ...
    TEST_ASSERT_EQUAL_INT(5, (int)protocore_quic_frame_parse(to_end, sizeof to_end, &f));
...
{
    uint8_t b[64];
    size_t n = protocore_quic_build_max_data(b, sizeof b, 65536);
    QuicFrame f;
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(b, n, &f));
    TEST_ASSERT_TRUE(f.type == QUIC_FT_MAX_DATA && f.max_data.max == 65536);

    n = protocore_quic_build_connection_close(b, sizeof b, PROTO_FALSE, 0x0a, QUIC_FT_STREAM, "bad", 3);
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(b, n, &f));
    ...
    TEST_ASSERT_EQUAL_INT(4, (int)protocore_quic_frame_parse(appclose, sizeof appclose, &f));
...
void test_sequence_and_truncation()
{
    ...
    o += protocore_quic_build_crypto(buf + o, sizeof buf - o, 0, data, 2);

    size_t pos = 0;
    QuicFrame f;
    size_t c = protocore_quic_frame_parse(buf + pos, o - pos, &f);
    TEST_ASSERT_TRUE(c == 1 && f.type == QUIC_FT_PADDING);
    pos += c;
    c = protocore_quic_frame_parse(buf + pos, o - pos, &f);
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(bad, sizeof bad, &f));
}
...
void test_builder_overflow()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_ping(b, 0));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_handshake_done(b, 0));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_padding(b, 1, 3)); // n > cap
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_ack(b, 1, 1000, 42, 3));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_crypto(b, 1, 7, d, 4)); // header fits, data does not
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_crypto(b, 0, 0, d, 4)); // type varint does not fit
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_stream(b, 1, 4, 100, d, 4, PROTO_TRUE));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_stream(b, 0, 0, 0, d, 4, PROTO_FALSE));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_max_data(b, 1, 1u << 30));
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_connection_close(b, 1, PROTO_FALSE, 0x0a, 0, "x", 1));
    TEST_ASSERT_EQUAL_INT(
        0, (int)protocore_quic_build_connection_close(b, 4, PROTO_FALSE, 0x0a, 0, "hello", 5)); // reason overflows
}
...
{
    QuicFrame f;
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse((const uint8_t *)"", 0, &f)); // no type byte

    const uint8_t ack_trunc[1] = {QUIC_FT_ACK}; // no Largest/Delay/...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(ack_trunc, 1, &f));
    const uint8_t ack_ranges_trunc[5] = {0x02, 60, 5, 1, 3}; // range_count 1 but no Gap/Length
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(ack_ranges_trunc, 5, &f));
    const uint8_t ack_ecn_trunc[7] = {0x03, 60, 5, 0, 3, 1, 2}; // ECN needs 3 counts, only 2
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(ack_ecn_trunc, 7, &f));

    const uint8_t crypto_trunc[2] = {QUIC_FT_CRYPTO, 0x00}; // offset ok, no length
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(crypto_trunc, 2, &f));

    const uint8_t stream_noid[1] = {0x08}; // STREAM, no Stream ID
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(stream_noid, 1, &f));
    const uint8_t stream_off_trunc[2] = {0x0c, 0x00}; // OFF set, id ok, no Offset
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(stream_off_trunc, 2, &f));
    const uint8_t stream_len_over[3] = {0x0a, 0x00, 0x08}; // LEN set, length 8 > remaining
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(stream_len_over, 3, &f));

    const uint8_t max_trunc[1] = {QUIC_FT_MAX_DATA};
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(max_trunc, 1, &f));

    const uint8_t close_trunc[1] = {QUIC_FT_CONNECTION_CLOSE};
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(close_trunc, 1, &f));
    const uint8_t close_reason_over[4] = {0x1c, 0x00, 0x00, 0x08}; // reason len 8 > remaining
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(close_reason_over, 4, &f));
    const uint8_t appclose_trunc[2] = {0x1d, 0x00}; // error code ok, no reason length
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(appclose_trunc, 2, &f));
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(new_cid_trunc, 1, &f));
}
...
void test_frame_edge_guards()
{
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(stream_len_trunc, 2, &f));
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(close_ft_trunc, 2, &f));

    uint8_t b[8];
    const uint8_t d[3] = {1, 2, 3};
    // protocore_quic_build_padding success: n <= cap zeroes n bytes and returns n.
    TEST_ASSERT_EQUAL_INT(3, (int)protocore_quic_build_padding(b, sizeof b, 3));
    TEST_ASSERT_EQUAL_HEX8(0, b[0]);
    TEST_ASSERT_EQUAL_HEX8(0, b[2]);
    // CRYPTO: type+offset+length varints fit but the data does not.
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_crypto(b, 4, 7, d, 3));
    // STREAM: type+id fit but the Offset varint does not.
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_stream(b, 2, 4, 100, d, 3, PROTO_TRUE));
    // STREAM (no offset): type+id fit but the Length varint does not.
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_stream(b, 2, 0, 0, d, 3, PROTO_FALSE));
    // STREAM: the header fits but the stream data does not.
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_stream(b, 3, 0, 0, d, 3, PROTO_FALSE));
}
...
void test_ack_multi_range()
{
    ...
    QuicFrame f;
    TEST_ASSERT_EQUAL_INT(12, (int)protocore_quic_frame_parse(ecn2, sizeof ecn2, &f));
...
void test_skip_and_extra_frames()
{
    ...
    for (size_t i = 0; i < sizeof one_varint; i++)
    {
        ...
        TEST_ASSERT_EQUAL_INT(2, (int)protocore_quic_frame_parse(ok, sizeof ok, &f));
        TEST_ASSERT_EQUAL_UINT(one_varint[i], (unsigned)f.type);
        const uint8_t trunc[1] = {one_varint[i]}; // the lone varint is absent
        TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(trunc, sizeof trunc, &f));
    }
    ...
    for (size_t i = 0; i < sizeof two_varint; i++)
    {
        ...
        TEST_ASSERT_EQUAL_INT(3, (int)protocore_quic_frame_parse(ok, sizeof ok, &f));
    ...
    TEST_ASSERT_EQUAL_INT(4, (int)protocore_quic_frame_parse(reset, sizeof reset, &f));
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(reset_trunc, sizeof reset_trunc, &f));

    // NEW_TOKEN: token length + token bytes.
    const uint8_t new_token[5] = {QUIC_FT_NEW_TOKEN, 0x03, 0xAA, 0xBB, 0xCC};
    TEST_ASSERT_EQUAL_INT(5, (int)protocore_quic_frame_parse(new_token, sizeof new_token, &f));
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(new_token_no_len, sizeof new_token_no_len, &f));
    const uint8_t new_token_over[3] = {QUIC_FT_NEW_TOKEN, 0x05, 0xAA}; // token len 5 > remaining
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(new_token_over, sizeof new_token_over, &f));
    ...
    TEST_ASSERT_EQUAL_INT(24, (int)protocore_quic_frame_parse(ncid, sizeof ncid, &f));
    TEST_ASSERT_EQUAL_UINT(QUIC_FT_NEW_CONNECTION_ID, (unsigned)f.type);
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(ncid, sizeof ncid - 1, &f)); // one byte short of CID + token
    const uint8_t ncid_hdr_only[3] = {QUIC_FT_NEW_CONNECTION_ID, 0x01, 0x00};      // no CID length byte
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(ncid_hdr_only, sizeof ncid_hdr_only, &f));

    // PATH_CHALLENGE / PATH_RESPONSE: 8 opaque bytes.
    uint8_t path[9];
    memset(path, 0x11, sizeof path);
    path[0] = QUIC_FT_PATH_CHALLENGE;
    TEST_ASSERT_EQUAL_INT(9, (int)protocore_quic_frame_parse(path, sizeof path, &f));
    ...
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(path_trunc, sizeof path_trunc, &f));

    // A genuinely unknown / reserved frame type falls through to the final reject.
    const uint8_t unknown[1] = {0x1f};
    TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(unknown, sizeof unknown, &f));
}
...
static void reject_every_prefix(const uint8_t *buf, size_t n)
{
    ...
    {
        TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_frame_parse(buf, k, &f));
    }
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(buf, n, &f)); // the whole frame parses
}
...
void test_frame_truncation_sweep()
{
    ...
    uint8_t ack[16];
    size_t an = protocore_quic_build_ack(ack, sizeof ack, 1000, 42, 3);
    ...
    uint8_t cr[32];
    const uint8_t data[5] = {'h', 'e', 'l', 'l', 'o'};
    size_t cn = protocore_quic_build_crypto(cr, sizeof cr, 1000, data, sizeof data);
...
void test_builder_capacity_sweep()
{
    ...
    size_t need = protocore_quic_build_ack(out, sizeof out, 1000, 500, 3);
    TEST_ASSERT_TRUE(need > 0);
    for (size_t cap = 0; cap < need; cap++)
    {
        TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_ack(out, cap, 1000, 500, 3));
    }
    TEST_ASSERT_EQUAL_INT((int)need, (int)protocore_quic_build_ack(out, need, 1000, 500, 3));

    // CRYPTO with a multi-octet offset.
    need = protocore_quic_build_crypto(out, sizeof out, 1000, data, sizeof data);
    TEST_ASSERT_TRUE(need > 0);
    for (size_t cap = 0; cap < need; cap++)
    {
        TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_crypto(out, cap, 1000, data, sizeof data));
    }

    // STREAM with an offset and FIN.
    need = protocore_quic_build_stream(out, sizeof out, 1000, 2000, data, sizeof data, PROTO_TRUE);
    TEST_ASSERT_TRUE(need > 0);
    for (size_t cap = 0; cap < need; cap++)
    {
        TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_stream(out, cap, 1000, 2000, data, sizeof data, PROTO_TRUE));
    }

    // MAX_DATA.
    need = protocore_quic_build_max_data(out, sizeof out, 1000000);
    TEST_ASSERT_TRUE(need > 0);
    for (size_t cap = 0; cap < need; cap++)
    {
        TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_max_data(out, cap, 1000000));
    }

    // CONNECTION_CLOSE with a multi-octet error code, frame type and reason.
    need = protocore_quic_build_connection_close(out, sizeof out, PROTO_FALSE, 1000, 2000, "reason", 6);
    TEST_ASSERT_TRUE(need > 0);
    for (size_t cap = 0; cap < need; cap++)
    {
        TEST_ASSERT_EQUAL_INT(0, (int)protocore_quic_build_connection_close(out, cap, PROTO_FALSE, 1000, 2000, "reason", 6));
    }
}
...
void test_builders_with_empty_bodies()
{
    ...
    size_t n = protocore_quic_build_crypto(out, sizeof out, 4, NULL, 0);
    TEST_ASSERT_TRUE(n > 0);
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(out, n, &f));
    TEST_ASSERT_EQUAL_UINT(QUIC_FT_CRYPTO, (unsigned)f.type);
    ...
    n = protocore_quic_build_stream(out, sizeof out, 8, 0, NULL, 0, PROTO_TRUE);
    TEST_ASSERT_TRUE(n > 0);
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(out, n, &f));
    ...
    n = protocore_quic_build_connection_close(out, sizeof out, PROTO_FALSE, 0x0a, 0, NULL, 0);
    TEST_ASSERT_TRUE(n > 0);
    TEST_ASSERT_EQUAL_INT((int)n, (int)protocore_quic_frame_parse(out, n, &f));
