...
//
// Unit tests for the QUIC transport-parameters codec (network_drivers/presentation/http/http3/protocore_quic_tp;
...
void test_roundtrip()
{
    ...
    uint8_t buf[256];
    size_t n = protocore_quic_tp_encode(&tp, buf, sizeof(buf));
    TEST_ASSERT_TRUE(n > 0);

    QuicTransportParams out;
    TEST_ASSERT_TRUE(protocore_quic_tp_parse(buf, n, &out));
...
void test_parse_bytes()
{
    ...
    QuicTransportParams tp;
    TEST_ASSERT_TRUE(protocore_quic_tp_parse(enc, sizeof(enc), &tp));
...
void test_reject_duplicate()
{
    ...
    QuicTransportParams tp;
    TEST_ASSERT_FALSE(protocore_quic_tp_parse(enc, sizeof(enc), &tp));
}
...
void test_reject_bad_values()
{
    ...
    TEST_ASSERT_FALSE(protocore_quic_tp_parse(h, sizeof(h), &tp));
    ...
    TEST_ASSERT_TRUE(protocore_quic_tp_parse(bidi_max, sizeof(bidi_max), &tp));
    ...
    TEST_ASSERT_FALSE(protocore_quic_tp_parse(bidi_over, sizeof(bidi_over), &tp));

    static const uint8_t uni_max[] = {0x09, 0x08, 0xd0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    TEST_ASSERT_TRUE(protocore_quic_tp_parse(uni_max, sizeof(uni_max), &tp));
    ...
    TEST_ASSERT_FALSE(protocore_quic_tp_parse(uni_over, sizeof(uni_over), &tp));
}
...
void test_quic_tp_more_paths()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, protocore_quic_tp_encode(&tp, buf, 0)); // ID varint has no room
    TEST_ASSERT_EQUAL_size_t(0, protocore_quic_tp_encode(&tp, buf, 1)); // length varint has no room
    TEST_ASSERT_EQUAL_size_t(0, protocore_quic_tp_encode(&tp, buf, 6)); // value (8 octets) overruns

    // Encode: a varint value beyond the 62-bit range fails the inner varint encode.
    protocore_quic_tp_defaults(&tp);
    tp.initial_max_data = 0xFFFFFFFFFFFFFFFFull;
    TEST_ASSERT_EQUAL_size_t(0, protocore_quic_tp_encode(&tp, buf, sizeof(buf)));
    ...
    TEST_ASSERT_FALSE(protocore_quic_tp_parse(badlen, sizeof(badlen), &out));
    ...
    TEST_ASSERT_FALSE(protocore_quic_tp_parse(bigscid, sizeof(bigscid), &out));
    ...
    for (unsigned i = 0; i < sizeof(ids); i++)
    {
        ...
        TEST_ASSERT_FALSE(protocore_quic_tp_parse(bad, sizeof(bad), &out));
    }
    ...
    TEST_ASSERT_TRUE(protocore_quic_tp_parse(ade, sizeof(ade), &out));
    static const uint8_t mad[3] = {0x0b, 0x01, 0x19}; // max_ack_delay = 25
    TEST_ASSERT_TRUE(protocore_quic_tp_parse(mad, sizeof(mad), &out));
}
...
void test_encode_cid_ok_chain_gaps()
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, protocore_quic_tp_encode(&tp, buf, 0));
    ...
    TEST_ASSERT_EQUAL_size_t(0, protocore_quic_tp_encode(&tp, buf, 5));
...
void test_encode_disable_migration_gaps()
{
    ...
    protocore_quic_tp_defaults(&tp);
    tp.disable_active_migration = PROTO_TRUE;
    TEST_ASSERT_EQUAL_size_t(0, protocore_quic_tp_encode(&tp, buf, 30));
}
...
void test_parse_id_decode_and_large_id()
{
    ...
    TEST_ASSERT_FALSE(protocore_quic_tp_parse(bad_id, sizeof(bad_id), &tp));

    // id = 32 (>= 32, so the dup-guard bitmask is skipped), zero-length value; unknown -> skipped.
    static const uint8_t big_id[] = {0x20, 0x00};
    TEST_ASSERT_TRUE(protocore_quic_tp_parse(big_id, sizeof(big_id), &tp));
}
...
void test_parse_range_check_value_decode_gaps()
{
    ...
    {
        const uint8_t bad[4] = {ids[i], 0x02, 0x00, 0x00}; // 2-octet value, but the varint consumes 1
        TEST_ASSERT_FALSE(protocore_quic_tp_parse(bad, sizeof(bad), &tp));
    }
}
