...
void test_malformed_guards()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0, DnsServer.build_response(q, 11, 60, resolve_foo, out, sizeof(out)));
...
void test_dns_opcode_notimp()
{
    ...
    TEST_ASSERT_EQUAL_UINT(0, DnsServer.build_response(q, qlen, 60, resolve_foo, out, 8));
}
...
void test_dns_add_and_lookup_guards()
{
    ...
    TEST_ASSERT_FALSE(DnsServer.add("overflow.lan", 10, 0, 0, 99));
