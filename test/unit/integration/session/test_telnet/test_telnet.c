...
void test_accept_negotiates_echo_and_sga()
{
    ...
    TEST_ASSERT_EQUAL_UINT8(1, TelnetV.u8);
}
...
void test_accept_no_capacity()
{
    ...
    {
        TelnetV.slot = (uint8_t)(s);
        Telnet.accept(protocore_telnet_span());
    }
    Telnet.client_count(protocore_telnet_span());
    TEST_ASSERT_EQUAL_UINT8(MAX_TELNET_CONNS, TelnetV.u8);
    TelnetV.slot = (uint8_t)(MAX_TELNET_CONNS);
    Telnet.accept(protocore_telnet_span());
    Telnet.client_count(protocore_telnet_span());
    TEST_ASSERT_EQUAL_UINT8(MAX_TELNET_CONNS, TelnetV.u8);
    for (uint8_t s = 0; s < MAX_TELNET_CONNS; s++)
...
void test_output_escaping_and_printf()
{
    ...
    TelnetV.out.spec = NEQ;
    TelnetV.out.val = (const protocore_fval[]){PROTOCORE_VU32(7u)};
    TelnetV.out.nv = 1;
...
void test_print_println_null_and_printf_empty()
{
    ...
    TelnetV.out.spec = EMPTY;
    TelnetV.out.val = NULL;
    TelnetV.out.nv = 0;
    Telnet.frame(protocore_telnet_span());
    TEST_ASSERT_EQUAL_UINT(0, tcp_captured_len());
}
...
void test_protocore_handler_accessor()
{
    ...
    const ProtoHandler *h = (Telnet.proto_handler(protocore_telnet_span()), TelnetV.handler);
