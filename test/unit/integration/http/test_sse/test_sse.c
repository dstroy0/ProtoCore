...
void test_sse_alloc_pool_full_returns_null()
{
    ...
    TEST_ASSERT_NULL(sse_alloc(2, "/c"));
}
...
{
    SseConn *sse = sse_alloc(0, "/events");
    TEST_ASSERT_FALSE(sse_write(sse, NULL, NULL, NULL));
}
...
void test_sse_write_returns_false_when_conn_not_active()
{
    ...
    TEST_ASSERT_FALSE(sse_write(sse, "hello", NULL, NULL));
}
...
void test_sse_write_returns_false_when_pcb_null()
{
    ...
    TEST_ASSERT_FALSE(sse_write(sse, "data", NULL, NULL));
}

void test_sse_write_data_only_returns_true()
{
    SseConn *sse = sse_alloc(0, "/events");
    TEST_ASSERT_TRUE(sse_write(sse, "hello", NULL, NULL));
}

void test_sse_write_with_event_returns_true()
{
    SseConn *sse = sse_alloc(0, "/events");
    TEST_ASSERT_TRUE(sse_write(sse, "payload", "update", NULL));
}

void test_sse_write_with_id_returns_true()
{
    SseConn *sse = sse_alloc(0, "/events");
    TEST_ASSERT_TRUE(sse_write(sse, "payload", NULL, "42"));
}
...
{
    char buf[64];
    int n = sse_format(buf, sizeof(buf), "hello", NULL, NULL);
...
{
    char buf[64];
    int n = sse_format(buf, sizeof(buf), "payload", "update", NULL);
...
{
    char buf[64];
    int n = sse_format(buf, sizeof(buf), "payload", NULL, "42");
...
{
    char buf[64];
    TEST_ASSERT_EQUAL(0, sse_format(buf, sizeof(buf), NULL, "x", "1"));
}
...
{
    char buf[8];
    TEST_ASSERT_EQUAL(0, sse_format(buf, 0, "data", NULL, NULL));
}

void test_sse_format_event_prefix_itself_overflows()
{
    char buf[5];
    TEST_ASSERT_EQUAL(0, sse_format(buf, sizeof(buf), "y", "x", NULL));
}

void test_sse_format_event_newline_overflows()
{
    char buf[10];
    TEST_ASSERT_EQUAL(0, sse_format(buf, sizeof(buf), "unused", "ab", NULL));
}

void test_sse_format_id_block_failure_arms()
{
    char a[4];
    TEST_ASSERT_EQUAL(0, sse_format(a, sizeof(a), "d", NULL, "z"));

    char b[7];
    TEST_ASSERT_EQUAL(0, sse_format(b, sizeof(b), "d", NULL, "XYZ"));

    char c[7];
    TEST_ASSERT_EQUAL(0, sse_format(c, sizeof(c), "d", NULL, "XY"));
}

void test_sse_format_data_block_failure_arms()
{
    char a[5];
    TEST_ASSERT_EQUAL(0, sse_format(a, sizeof(a), "abcdef", NULL, NULL));

    char b[10];
    TEST_ASSERT_EQUAL(0, sse_format(b, sizeof(b), "abcdef", NULL, NULL));

    char c[9];
    TEST_ASSERT_EQUAL(0, sse_format(c, sizeof(c), "ab", NULL, NULL));
}
...
void stress_sse_write_100_calls()
{
    ...
    for (int i = 0; i < 100; i++)
    {
        proto_bool ok = sse_write(sse, "data", "update", "1");
