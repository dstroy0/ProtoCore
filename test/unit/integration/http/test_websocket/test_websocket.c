...
void test_ws_alloc_pool_full_returns_null()
{
    ...
    TEST_ASSERT_NULL(ws_alloc(2));
}

void test_ws_active_reflects_pool_state()
{
    TEST_ASSERT_FALSE(ws_active(0));
    ws_alloc(0);
    TEST_ASSERT_TRUE(ws_active(0));
    TEST_ASSERT_FALSE(ws_active((uint8_t)MAX_WS_CONNS));
}

void test_ws_payload_returns_buf_or_null()
{
    TEST_ASSERT_NULL(ws_payload(0));
    WsConn *ws = ws_alloc(0);
    TEST_ASSERT_EQUAL_PTR((const char *)ws->buf, ws_payload(0));
    TEST_ASSERT_NULL(ws_payload((uint8_t)MAX_WS_CONNS));
}
...
void test_ws_outbound_pmd_zero_len_and_control()
{
    ...
    tcp_capture_reset();
    TEST_ASSERT_TRUE(ws_send_frame(ws, WS_OP_TEXT, NULL, 0));
    tcp_capture_disable();

    tcp_capture_reset();
    TEST_ASSERT_TRUE(ws_send_frame(ws, WS_OP_PING, (const uint8_t *)"AAAA", 4));
    tcp_capture_disable();
}
