...
// (services/machine_tool/euromap77/euromap77.h).
//
...
//
// TWO CASES ARE EXPECTED TO FAIL, both against that NodeSet:
//
//   test_the_namespace_uris_are_the_published_model_uris - euromap77.h:57 and :59 define the two
//   URIs as http://www.euromap.org/euromap77/ and .../euromap83/. The namespaces were moved to the
//   OPC Foundation when the recommendation was published as OPC 40077 / OPC 40083; the NodeSet
//   ModelUri values above are the ones a client resolves, and a grep for "euromap" over both
//   NodeSets returns 0 hits. A server advertising the old URI lands in a different namespace index,
//   so a conforming client never matches the model.
//
...
//   leaves are HasComponent (47) / BaseDataVariableType (63). euromap77.c:117 and :125-126 emit
//   HasComponent and BaseDataVariableType for every leaf, so a client browsing HasProperty finds
//   nothing.
//
...
static uint32_t node_at(const char *a, const char *b)
{
    ...
    {
        const int32_t n = browse(PROTOCORE_EM77_NS, id);
...
{
    const int32_t n = browse(PROTOCORE_EM77_NS, node_at(l->container, l->sub));
    const OpcUaReference *r = child(g_refs, n, l->name);
    TEST_ASSERT_NOT_NULL_MESSAGE(r, l->name);
    return r;
}

// FAILS AGAINST THE CURRENT SOURCE. The namespace URI is what a client resolves the model by, and
// the NodeSets publish these two ModelUri values; neither NodeSet mentions euromap.org at all.
...
void test_the_browse_hierarchy_carries_the_published_browsenames(void)
{
    ...
    int32_t n = browse(PROTOCORE_EM77_NS, interface_id());
    ...
    n = browse(PROTOCORE_EM77_NS, node_at("Jobs", NULL));
    ...
    TEST_ASSERT_EQUAL_INT32(8, browse(PROTOCORE_EM77_NS, node_at("MachineInformation", NULL)));
    TEST_ASSERT_EQUAL_INT32(2, browse(PROTOCORE_EM77_NS, node_at("MachineStatus", NULL)));
    TEST_ASSERT_EQUAL_INT32(8, browse(PROTOCORE_EM77_NS, node_at("Jobs", "ActiveJob")));
    TEST_ASSERT_EQUAL_INT32(8, browse(PROTOCORE_EM77_NS, node_at("Jobs", "ActiveJobValues")));
...
void test_the_containers_are_objects_and_the_leaves_are_variables(void)
{
    ...
    const int32_t n = browse(PROTOCORE_EM77_NS, interface_id());
    ...
    const int32_t jn = browse(PROTOCORE_EM77_NS, node_at("Jobs", NULL));
...
// FAILS AGAINST THE CURRENT SOURCE. Per the NodeSet, 19 of the 26 leaves are HasProperty (46)
// references to PropertyType (68) nodes and only the seven HasComponent ones under ActiveJobValues
// are BaseDataVariableType (63); euromap77.c:133 emits HasComponent / BaseDataVariableType for all.
...
void test_every_value_comes_from_the_bound_model(void)
{
    ...
    {
        const int32_t n = browse(PROTOCORE_EM77_NS, node_at(STR[i].container, STR[i].sub));
    ...
    {
        const int32_t n = browse(PROTOCORE_EM77_NS, node_at("Jobs", U64[i].sub));
    ...
    {
        const int32_t n = browse(PROTOCORE_EM77_NS, node_at("Jobs", F64[i].sub));
    ...
    OpcUaVariant v;
    const int32_t sn = browse(PROTOCORE_EM77_NS, node_at("MachineStatus", NULL));
    ...
    const int32_t an = browse(PROTOCORE_EM77_NS, node_at("Jobs", "ActiveJob"));
    TEST_ASSERT_TRUE(read_value(child(g_refs, an, "NumCavities")->target_id, &v));
    TEST_ASSERT_EQUAL_UINT32(4u, v.u32);

    const int32_t vn = browse(PROTOCORE_EM77_NS, node_at("Jobs", "ActiveJobValues"));
...
void test_absent_strings_read_as_empty(void)
{
    ...
    const int32_t n = browse(PROTOCORE_EM77_NS, node_at("MachineInformation", NULL));
    OpcUaVariant v;
...
void test_every_reference_is_forward_and_in_the_models_namespace(void)
{
    ...
    {
        const int32_t n = browse(PROTOCORE_EM77_NS, container[c]);
...
void test_reads_outside_the_model_are_refused(void)
{
    ...
    TEST_ASSERT_FALSE(protocore_em77_read(PROTOCORE_EM77_NS, root, OPCUA_ATTR_VALUE, &v));
...
void test_browse_outside_the_model_is_refused(void)
{
    ...
    TEST_ASSERT_EQUAL_INT32(-1, browse((uint16_t)(PROTOCORE_EM77_NS + 1u), root));
}
...
void test_browse_respects_the_caller_bound(void)
{
    ...
    TEST_ASSERT_EQUAL_INT32(0, protocore_em77_browse(PROTOCORE_EM77_NS, info, one, 0u));
