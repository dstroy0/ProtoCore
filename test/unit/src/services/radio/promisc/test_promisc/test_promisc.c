...
void test_ieee80211_address_fields_by_ds_bits(void)
{
    ...
    proto_bool promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 24, &w);
    TEST_ASSERT_TRUE(promisc_ok);
    ...
    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 24, &w);
    TEST_ASSERT_TRUE(promisc_ok);
    ...
    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 30, &w);
    TEST_ASSERT_TRUE(promisc_ok);
...
void test_ieee80211_header_length(void)
{
    ...
    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 26, &w);
    TEST_ASSERT_TRUE(promisc_ok);
    ...
    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 32, &w);
    TEST_ASSERT_TRUE(promisc_ok);
...
void test_ieee80211_control_frame(void)
{
    ...
    proto_bool promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 10, &w);
    TEST_ASSERT_TRUE(promisc_ok);
...
void test_parse_refuses_a_short_frame(void)
{
    ...
    header(f, 0x08, 0x00, 0);
    proto_bool promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 9, &w);
    TEST_ASSERT_FALSE(promisc_ok); // shorter than Frame Control + Duration + A1
    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 23, &w);
    TEST_ASSERT_FALSE(promisc_ok); // a 3-address header needs 24

    header(f, 0x88, 0x00, 0);
    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 25, &w);
    TEST_ASSERT_FALSE(promisc_ok); // QoS Data needs 26

    header(f, 0x08, 0x03, 0);
    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 29, &w);
    TEST_ASSERT_FALSE(promisc_ok); // WDS needs 30

    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), NULL, 24, &w);
    TEST_ASSERT_FALSE(promisc_ok);
    promisc_ok = Promisc.wifi_frame_parse(protocore_promisc_span(), f, 24, NULL);
    TEST_ASSERT_FALSE(promisc_ok);
}
...
{
    proto_bool promisc_ok = Promisc.begin(protocore_promisc_span(), 6, NULL);
    TEST_ASSERT_FALSE(promisc_ok);
}
...
void test_capture_delivers_frames_through_the_radio(void)
{
    ...
    g_seen_count = 0;
    proto_bool promisc_ok = Promisc.begin(protocore_promisc_span(), 6, capture_sink);
    TEST_ASSERT_TRUE(promisc_ok);
    ...
    Promisc.set_channel(protocore_promisc_span(), 11);
