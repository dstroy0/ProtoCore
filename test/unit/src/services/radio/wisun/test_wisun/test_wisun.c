...
void test_rfc7252_figure_16_request(void)
{
    static const uint8_t WANT[16] = {0x40, 0x01, 0x7D, 0x34, 0xBB, 't', 'e', 'm',
                                     'p',  'e',  'r',  'a',  't',  'u', 'r', 'e'};
    uint8_t out[64];
    size_t wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 0x7d34, NULL, 0, "temperature", NULL,
                                      0, out, sizeof(out));
    const size_t n = wisun_n;
...
void test_leading_slash_is_not_a_segment(void)
{
    ...
    size_t wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 0x7d34, NULL, 0, "/temperature", NULL,
                                      0, with, sizeof(with));
    const size_t a = wisun_n;
    wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 0x7d34, NULL, 0, "temperature", NULL, 0,
                               without, sizeof(without));
    const size_t b = wisun_n;
...
void test_type_field_selects_confirmable_or_not(void)
{
    uint8_t out[32];
    size_t wisun_n =
        Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 1, NULL, 0, NULL, NULL, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(4, wisun_n);
    TEST_ASSERT_EQUAL_HEX8(0x40, out[0]);
    wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_NON, WISUN_COAP_GET, 1, NULL, 0, NULL, NULL, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(4, wisun_n);
    ...
    wisun_n =
        Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_PUT, 0xABCD, NULL, 0, NULL, NULL, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(4, wisun_n);
...
void test_token_length_and_placement(void)
{
    ...
    uint8_t out[64];
    size_t wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 0x0001, TOKEN, sizeof(TOKEN), "temp",
    ...
    for (uint8_t tkl = 9; tkl < 16; tkl++)
    {
        uint8_t big[16] = {0};
        size_t wisun_n =
            Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 1, big, tkl, "a", NULL, 0, out, sizeof(out));
        TEST_ASSERT_EQUAL_size_t(0, wisun_n);
    }
    // A non-zero length with no token is refused rather than read from nothing.
    wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 1, NULL, 4, "a", NULL, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
}
...
void test_each_path_segment_is_its_own_option(void)
{
    ...
    uint8_t out[64];
    size_t wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 42, NULL, 0, "sensors/temp", NULL, 0,
                                      out, sizeof(out));
    const size_t n = wisun_n;
    ...
    wisun_n =
        Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 42, NULL, 0, "a/b/c", NULL, 0, out, sizeof(out));
    const size_t m = wisun_n;
...
void test_option_length_extension(void)
{
    ...
    uint8_t out[512];
    size_t wisun_n =
        Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 1, NULL, 0, SEG12, NULL, 0, out, sizeof(out));
    size_t n = wisun_n;
    ...
    wisun_n =
        Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 1, NULL, 0, SEG13, NULL, 0, out, sizeof(out));
    n = wisun_n;
    ...
    wisun_n =
        Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 1, NULL, 0, SEG20, NULL, 0, out, sizeof(out));
    n = wisun_n;
...
void test_payload_marker(void)
{
    ...
    uint8_t out[64];
    size_t wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_PUT, 0x7d34, NULL, 0, "temp", BODY,
    ...
    wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_PUT, 0x7d34, NULL, 0, "temp", NULL, 0, out,
                               sizeof(out));
    const size_t m = wisun_n;
    TEST_ASSERT_EQUAL_size_t(9, m); // header + option, no marker
    wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_PUT, 1, NULL, 0, "a", NULL, 4, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
}
...
void test_build_refuses_a_short_buffer(void)
{
    ...
    size_t wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 0x7d34, NULL, 0, "temperature", NULL,
                                      0, out, sizeof(out));
    const size_t exact = wisun_n;
    TEST_ASSERT_EQUAL_size_t(16, exact);
    for (size_t cap = 0; cap < exact; cap++)
    {
        size_t wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 0x7d34, NULL, 0, "temperature",
                                          NULL, 0, out, cap);
        TEST_ASSERT_EQUAL_size_t(0, wisun_n);
    }
    wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 0x7d34, NULL, 0, "temperature", NULL, 0, out,
                               exact);
    TEST_ASSERT_EQUAL_size_t(exact, wisun_n);
    wisun_n = Wisun.build_coap(wisun_work, WISUN_COAP_CON, WISUN_COAP_GET, 1, NULL, 0, "a", NULL, 0, NULL, 32);
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
}
...
void test_node_registry(void)
{
    ...
    Wisun.init(wisun_work, &fan, &br, storage, 3);
    TEST_ASSERT_EQUAL_size_t(0, fan.count);
    TEST_ASSERT_EQUAL_size_t(3, fan.cap);
    size_t wisun_n = Wisun.joined_count(wisun_work, &fan);
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
    ...
    int wisun_i32 = Wisun.node_register(wisun_work, &fan, &a, 100);
    TEST_ASSERT_EQUAL_INT(0, wisun_i32);
    wisun_i32 = Wisun.node_register(wisun_work, &fan, &b, 200);
    TEST_ASSERT_EQUAL_INT(1, wisun_i32);
    TEST_ASSERT_EQUAL_size_t(2, fan.count);

    // The same address again is the same entry, with a fresh last_seen.
    wisun_i32 = Wisun.node_register(wisun_work, &fan, &a, 300);
    TEST_ASSERT_EQUAL_INT(0, wisun_i32);
    TEST_ASSERT_EQUAL_size_t(2, fan.count);
    ...
    size_t idx = 99;
    proto_bool wisun_ok = Wisun.node_find(wisun_work, &fan, &b, &idx);
    TEST_ASSERT_TRUE(wisun_ok);
    TEST_ASSERT_EQUAL_size_t(1, idx);
    wisun_ok = Wisun.node_find(wisun_work, &fan, &c, &idx);
    TEST_ASSERT_FALSE(wisun_ok);
    TEST_ASSERT_EQUAL_size_t(1, idx); // untouched on a miss

    wisun_i32 = Wisun.node_register(wisun_work, &fan, &c, 400);
    TEST_ASSERT_EQUAL_INT(2, wisun_i32);
    wisun_n = Wisun.joined_count(wisun_work, &fan);
    TEST_ASSERT_EQUAL_size_t(3, wisun_n);
    wisun_i32 = Wisun.node_register(wisun_work, &fan, &d, 500);
    TEST_ASSERT_EQUAL_INT(-1, wisun_i32); // full
    TEST_ASSERT_EQUAL_size_t(3, fan.count);

    // The border router is kept as given, and it is not itself a mesh node.
    IpV.args.ip = &fan.border_router;
    IpV.args.b = &br;
    Ip.equal(ip_work);
    TEST_ASSERT_TRUE(IpV.ok);
    wisun_ok = Wisun.node_find(wisun_work, &fan, &br, NULL);
    TEST_ASSERT_FALSE(wisun_ok);
}
...
void test_registry_without_storage(void)
{
    WisunFan fan;
    const protocore_ip br = addr("fd00::1");
    Wisun.init(wisun_work, &fan, &br, NULL, 8);
    TEST_ASSERT_EQUAL_size_t(0, fan.cap);

    const protocore_ip a = addr("fd00::a");
    int wisun_i32 = Wisun.node_register(wisun_work, &fan, &a, 1);
    TEST_ASSERT_EQUAL_INT(-1, wisun_i32);
    proto_bool wisun_ok = Wisun.node_find(wisun_work, &fan, &a, NULL);
    TEST_ASSERT_FALSE(wisun_ok);
    size_t wisun_n = Wisun.joined_count(wisun_work, &fan);
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
    ...
    WisunNode storage[1];
    Wisun.init(wisun_work, &fan, NULL, storage, 1);
    IpV.args.ip = &fan.border_router;
    Ip.is_unspecified(ip_work);
    TEST_ASSERT_TRUE(IpV.ok);

    Wisun.init(wisun_work, NULL, &br, storage, 1); // no state to write, no crash
    wisun_i32 = Wisun.node_register(wisun_work, &fan, NULL, 1);
    TEST_ASSERT_EQUAL_INT(-1, wisun_i32);
    wisun_i32 = Wisun.node_register(wisun_work, NULL, &a, 1);
    TEST_ASSERT_EQUAL_INT(-1, wisun_i32);
    wisun_ok = Wisun.node_find(wisun_work, NULL, &a, NULL);
    TEST_ASSERT_FALSE(wisun_ok);
    wisun_n = Wisun.joined_count(wisun_work, NULL);
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
}
...
void test_nodes_json(void)
{
    ...
    WisunFan fan;
    const protocore_ip br = addr("fd00::1");
    Wisun.init(wisun_work, &fan, &br, storage, 2);

    char out[128];
    size_t wisun_n = Wisun.nodes_json(wisun_work, &fan, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(2, wisun_n);
    ...
    int wisun_i32 = Wisun.node_register(wisun_work, &fan, &a, 1);
    TEST_ASSERT_EQUAL_INT(0, wisun_i32);
    wisun_i32 = Wisun.node_register(wisun_work, &fan, &b, 2);
    TEST_ASSERT_EQUAL_INT(1, wisun_i32);

    wisun_n = Wisun.nodes_json(wisun_work, &fan, out, sizeof(out));
    const size_t n = wisun_n;
    ...
    for (size_t cap = 1; cap <= n; cap++)
    {
        ...
        size_t wisun_n = Wisun.nodes_json(wisun_work, &fan, small, cap);
        TEST_ASSERT_EQUAL_size_t(0, wisun_n);
    }
    wisun_n = Wisun.nodes_json(wisun_work, &fan, out, 0);
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
    wisun_n = Wisun.nodes_json(wisun_work, NULL, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
    wisun_n = Wisun.nodes_json(wisun_work, &fan, NULL, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, wisun_n);
}
