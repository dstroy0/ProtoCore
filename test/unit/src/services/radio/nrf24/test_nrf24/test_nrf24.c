...
void test_init_configures_and_powers_up()
{
    ...
    proto_bool nrf24_ok = Nrf24.init(nrf24_work, &g_bus, &c);
    TEST_ASSERT_TRUE(nrf24_ok);
...
void test_init_fails_when_absent()
{
    ...
    nrf_config c = default_cfg();
    proto_bool nrf24_ok = Nrf24.init(nrf24_work, &g_bus, &c);
    TEST_ASSERT_FALSE(nrf24_ok);
}

void test_send_pads_to_width_and_keys_tx()
{
    nrf_config c = default_cfg();
    Nrf24.init(nrf24_work, &g_bus, &c);
    const uint8_t data[3] = {0xAB, 0xCD, 0xEF};
    proto_bool nrf24_ok = Nrf24.send(nrf24_work, &g_bus, data, 3);
    TEST_ASSERT_TRUE(nrf24_ok);
...
void test_send_rejects_oversize()
{
    ...
    proto_bool nrf24_ok = Nrf24.send(nrf24_work, &g_bus, big, 9);
    TEST_ASSERT_FALSE(nrf24_ok);
}

void test_tx_done_flag()
{
    proto_bool nrf24_ok = Nrf24.tx_done(nrf24_work, &g_bus);
    TEST_ASSERT_FALSE(nrf24_ok);
    g.reg[0x07] = 0x20;
    nrf24_ok = Nrf24.tx_done(nrf24_work, &g_bus);
    TEST_ASSERT_TRUE(nrf24_ok);
    nrf24_ok = Nrf24.tx_done(nrf24_work, &g_bus);
    TEST_ASSERT_FALSE(nrf24_ok);
}
...
void test_recv_reads_payload_and_pipe()
{
    ...
    int nrf24_n = Nrf24.recv(nrf24_work, &g_bus, buf, sizeof(buf), &pipe);
    int n = nrf24_n;
...
{
    uint8_t buf[16];
    int nrf24_n = Nrf24.recv(nrf24_work, &g_bus, buf, sizeof(buf), NULL);
    TEST_ASSERT_EQUAL_INT(-1, nrf24_n);
}
...
void test_data_rate_variants()
{
    ...
    c2.data_rate = 2;
    nrf24_ok = Nrf24.init(nrf24_work, &g_bus, &c2);
    TEST_ASSERT_TRUE(nrf24_ok);
}
...
{
    nrf_config c = default_cfg();
    proto_bool nrf24_ok = Nrf24.init(nrf24_work, NULL, &c);
    TEST_ASSERT_FALSE(nrf24_ok);

    nrf_bus no_spi = {NULL, mock_ce, NULL};
    nrf24_ok = Nrf24.init(nrf24_work, &no_spi, &c);
    TEST_ASSERT_FALSE(nrf24_ok);

    nrf_bus no_ce = {mock_spi, NULL, NULL};
    nrf24_ok = Nrf24.init(nrf24_work, &no_ce, &c);
    TEST_ASSERT_FALSE(nrf24_ok);

    nrf24_ok = Nrf24.init(nrf24_work, &g_bus, NULL);
    TEST_ASSERT_FALSE(nrf24_ok);

    nrf_config no_addr = default_cfg();
    no_addr.address = NULL;
    nrf24_ok = Nrf24.init(nrf24_work, &g_bus, &no_addr);
    TEST_ASSERT_FALSE(nrf24_ok);
}
...
void test_send_rejects_null_args_and_zero_len()
{
    ...
    proto_bool nrf24_ok = Nrf24.send(nrf24_work, NULL, data, 3);
    TEST_ASSERT_FALSE(nrf24_ok);
    nrf24_ok = Nrf24.send(nrf24_work, &g_bus, NULL, 3);
    TEST_ASSERT_FALSE(nrf24_ok);
    nrf24_ok = Nrf24.send(nrf24_work, &g_bus, data, 0);
    TEST_ASSERT_FALSE(nrf24_ok);
}

void test_tx_done_null_bus()
{
    proto_bool nrf24_ok = Nrf24.tx_done(nrf24_work, NULL);
    TEST_ASSERT_FALSE(nrf24_ok);
}

void test_set_rx_null_bus_is_noop()
{
    Nrf24.set_rx(nrf24_work, NULL);
}

void test_recv_rejects_null_args()
{
    uint8_t buf[16];
    int nrf24_n = Nrf24.recv(nrf24_work, NULL, buf, sizeof(buf), NULL);
    TEST_ASSERT_EQUAL_INT(-1, nrf24_n);
    nrf24_n = Nrf24.recv(nrf24_work, &g_bus, NULL, sizeof(buf), NULL);
    TEST_ASSERT_EQUAL_INT(-1, nrf24_n);
}
