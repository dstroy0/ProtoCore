...
{
    uint16_t thread_value = Thread.spinel_fcs(thread_work, CHECK9, sizeof(CHECK9));
    TEST_ASSERT_EQUAL_HEX16(0x906Eu, thread_value);
}
...
void test_rfc1662_good_fcs_residue(void)
{
    ...
    {
        uint8_t framed[8];
        uint16_t thread_value = Thread.spinel_fcs(thread_work, PAYLOADS[i], 6);
        const uint16_t fcs = thread_value;
        memcpy(framed, PAYLOADS[i], 6);
        framed[6] = (uint8_t)(fcs & 0xFF); // low byte first on the wire
        framed[7] = (uint8_t)(fcs >> 8);
        uint16_t thread_value2 = Thread.spinel_fcs(thread_work, framed, 8);
        TEST_ASSERT_EQUAL_HEX16(0x0F47u, thread_value2);
    }
}
...
void test_rfc1662_escape_table(void)
{
    ...
    uint8_t out[32];
    uint16_t thread_value = Thread.spinel_frame_encode(thread_work, RESERVED, 4, out, sizeof(out));
    const uint16_t n = thread_value;
...
void test_frame_round_trip(void)
{
    ...
    uint8_t frame[64];
    uint16_t thread_value = Thread.spinel_frame_encode(thread_work, PAYLOAD, sizeof(PAYLOAD), frame, sizeof(frame));
    const uint16_t n = thread_value;
    TEST_ASSERT_TRUE(n > 0);

    uint8_t back[64];
    uint16_t back_len = 0;
    int thread_n = Thread.spinel_frame_decode(thread_work, frame, n, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT((int)n, thread_n);
...
void test_decode_rejects_a_corrupted_frame(void)
{
    ...
    for (uint16_t i = 0; i + 1 < n; i++)
    {
        ...
        int thread_n = Thread.spinel_frame_decode(thread_work, bad, n, back, sizeof(back), &back_len);
        TEST_ASSERT_EQUAL_INT(-1, thread_n);
    }
}
...
void test_decode_framing_faults(void)
{
    ...
    int thread_n = Thread.spinel_frame_decode(thread_work, NO_FLAG, sizeof(NO_FLAG), back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(0, thread_n);

    static const uint8_t DANGLING[3] = {0x01, HDLC_ESCAPE, HDLC_FLAG};
    int thread_n2 = Thread.spinel_frame_decode(thread_work, DANGLING, sizeof(DANGLING), back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(-1, thread_n2);

    static const uint8_t TOO_SHORT[2] = {0x01, HDLC_FLAG}; // one byte cannot hold a two-byte FCS
    int thread_n3 =
        Thread.spinel_frame_decode(thread_work, TOO_SHORT, sizeof(TOO_SHORT), back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(-1, thread_n3);
    ...
    uint16_t thread_value = Thread.spinel_frame_encode(thread_work, PAYLOAD, sizeof(PAYLOAD), frame, sizeof(frame));
    const uint16_t n = thread_value;
    int thread_n4 = Thread.spinel_frame_decode(thread_work, frame, n, back, 4, &back_len);
    TEST_ASSERT_EQUAL_INT(-1, thread_n4);
}
...
void test_encode_bounds(void)
{
    ...
    uint16_t thread_value = Thread.spinel_frame_encode(thread_work, big, sizeof(big), out, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(0, thread_value);

    // The frame is all-or-nothing: one octet short of the exact length writes nothing.
    static const uint8_t SMALL[4] = {1, 2, 3, 4};
    uint16_t thread_value2 = Thread.spinel_frame_encode(thread_work, SMALL, sizeof(SMALL), out, sizeof(out));
    const uint16_t exact = thread_value2;
    TEST_ASSERT_TRUE(exact >= 7); // 4 payload + FCS(2) + flag, more if the FCS needs stuffing
    uint16_t thread_value3 = Thread.spinel_frame_encode(thread_work, SMALL, sizeof(SMALL), out, (uint16_t)(exact - 1));
    TEST_ASSERT_EQUAL_UINT16(0, thread_value3);
    uint16_t thread_value4 = Thread.spinel_frame_encode(thread_work, SMALL, sizeof(SMALL), out, exact);
    TEST_ASSERT_EQUAL_UINT16(exact, thread_value4);
}
...
void test_spinel_packed_uint_vectors(void)
{
    ...
    {
        uint8_t out[5];
        uint8_t thread_u8 = Thread.spinel_pack_uint(thread_work, CASES[i].value, out, sizeof(out));
        TEST_ASSERT_EQUAL_UINT8(CASES[i].len, thread_u8);
        TEST_ASSERT_EQUAL_HEX8_ARRAY(CASES[i].bytes, out, CASES[i].len);

        uint32_t back = 0;
        int thread_n = Thread.spinel_unpack_uint(thread_work, out, CASES[i].len, &back);
        TEST_ASSERT_EQUAL_INT(CASES[i].len, thread_n);
...
void test_spinel_packed_uint_faults(void)
{
    ...
    uint32_t v = 0;
    int thread_n = Thread.spinel_unpack_uint(thread_work, TRUNCATED, sizeof(TRUNCATED), &v);
    TEST_ASSERT_EQUAL_INT(0, thread_n);

    static const uint8_t OVERFLOWS[6] = {0x80, 0x80, 0x80, 0x80, 0x80, 0x01};
    int thread_n2 = Thread.spinel_unpack_uint(thread_work, OVERFLOWS, sizeof(OVERFLOWS), &v);
    TEST_ASSERT_EQUAL_INT(-1, thread_n2);

    // A four-byte buffer cannot hold the five-byte encoding of a 32-bit value.
    uint8_t out[4];
    uint8_t thread_u8 = Thread.spinel_pack_uint(thread_work, 0xFFFFFFFFu, out, sizeof(out));
    TEST_ASSERT_EQUAL_UINT8(0, thread_u8);
}
...
void test_spinel_command_build_and_parse(void)
{
    ...
    uint8_t out[16];
    uint16_t thread_value =
        Thread.spinel_command_build(thread_work, protocore_spinel_header(0, 1), SPINEL_CMD_PROP_VALUE_GET,
    ...
    int thread_n = Thread.spinel_command_parse(thread_work, out, n, &header, &cmd, &prop, &value, &value_len);
    TEST_ASSERT_EQUAL_INT(3, thread_n);
    ...
    uint16_t thread_value2 =
        Thread.spinel_command_build(thread_work, 0x82, SPINEL_CMD_PROP_VALUE_SET, SPINEL_PROP_MAC_15_4_PANID,
                                    (const uint8_t *)"\x34\x12", 2, out, sizeof(out));
    const uint16_t m = thread_value2;
    TEST_ASSERT_EQUAL_UINT16(5, m); // header + cmd(1) + prop(1) + value(2)
    int thread_n2 = Thread.spinel_command_parse(thread_work, out, m, &header, &cmd, &prop, &value, &value_len);
    TEST_ASSERT_EQUAL_INT(3, thread_n2);
    ...
    uint16_t thread_value3 = Thread.spinel_command_build(thread_work, 0x83, SPINEL_CMD_PROP_VALUE_IS, 0x100u,
                                                         (const uint8_t *)"\xAA", 1, out, sizeof(out));
    const uint16_t k = thread_value3;
    ...
    int thread_n3 = Thread.spinel_command_parse(thread_work, out, k, &header, &cmd, &prop, &value, &value_len);
    TEST_ASSERT_EQUAL_INT(4, thread_n3);
...
void test_spinel_command_through_hdlc(void)
{
    uint8_t cmdbuf[32];
    uint16_t thread_value =
        Thread.spinel_command_build(thread_work, protocore_spinel_header(0, 3), SPINEL_CMD_PROP_VALUE_SET,
                                    SPINEL_PROP_PHY_CHAN, (const uint8_t *)"\x0B", 1, cmdbuf, sizeof(cmdbuf));
    const uint16_t clen = thread_value;
    TEST_ASSERT_EQUAL_UINT16(4, clen);

    uint8_t frame[64];
    uint16_t thread_value2 = Thread.spinel_frame_encode(thread_work, cmdbuf, clen, frame, sizeof(frame));
    const uint16_t flen = thread_value2;
    TEST_ASSERT_TRUE(flen > 0);

    uint8_t payload[64];
    uint16_t plen = 0;
    int thread_n = Thread.spinel_frame_decode(thread_work, frame, flen, payload, sizeof(payload), &plen);
    TEST_ASSERT_EQUAL_INT((int)flen, thread_n);

    uint32_t cmd = 0;
    uint32_t prop = 0;
    const uint8_t *value = NULL;
    uint16_t value_len = 0;
    int thread_n2 = Thread.spinel_command_parse(thread_work, payload, plen, NULL, &cmd, &prop, &value, &value_len);
    TEST_ASSERT_TRUE(thread_n2 > 0);
...
void test_spinel_value_wire_layout(void)
{
    uint8_t buf[64];
    SpinelWriter w;
    Thread.spinel_writer_init(thread_work, &w, buf, sizeof(buf));
    Thread.spinel_put_u16(thread_work, &w, 0x1234);
    Thread.spinel_put_u32(thread_work, &w, 0x89ABCDEFu);
    Thread.spinel_put_i8(thread_work, &w, -1);
    Thread.spinel_put_i16(thread_work, &w, -2);
    uint16_t thread_value = Thread.spinel_writer_len(thread_work, &w);
    TEST_ASSERT_EQUAL_UINT16(9, thread_value);

    static const uint8_t WANT[9] = {0x34, 0x12, 0xEF, 0xCD, 0xAB, 0x89, 0xFF, 0xFE, 0xFF};
    TEST_ASSERT_EQUAL_MEMORY(WANT, buf, 9);

    SpinelReader r;
    Thread.spinel_reader_init(thread_work, &r, buf, 9);
    uint16_t u16 = 0;
    uint32_t u32 = 0;
    int8_t i8 = 0;
    int16_t i16 = 0;
    proto_bool thread_ok = Thread.spinel_get_u16(thread_work, &r, &u16);
    TEST_ASSERT_TRUE(thread_ok);
    proto_bool thread_ok2 = Thread.spinel_get_u32(thread_work, &r, &u32);
    TEST_ASSERT_TRUE(thread_ok2);
    proto_bool thread_ok3 = Thread.spinel_get_i8(thread_work, &r, &i8);
    TEST_ASSERT_TRUE(thread_ok3);
    proto_bool thread_ok4 = Thread.spinel_get_i16(thread_work, &r, &i16);
    TEST_ASSERT_TRUE(thread_ok4);
    ...
    proto_bool thread_ok5 = Thread.spinel_reader_ok(thread_work, &r);
    TEST_ASSERT_TRUE(thread_ok5);
}
...
void test_spinel_value_round_trip(void)
{
    ...
    SpinelWriter w;
    Thread.spinel_writer_init(thread_work, &w, buf, sizeof(buf));
    Thread.spinel_put_bool(thread_work, &w, PROTO_TRUE);
    Thread.spinel_put_uint(thread_work, &w, 1337u);
    Thread.spinel_put_eui64(thread_work, &w, EUI);
    Thread.spinel_put_ipv6(thread_work, &w, LL);
    Thread.spinel_put_utf8(thread_work, &w, "OPENTHREAD");
    Thread.spinel_put_data_wlen(thread_work, &w, RAW, sizeof(RAW));
    Thread.spinel_put_data(thread_work, &w, RAW, sizeof(RAW));
    uint16_t thread_value = Thread.spinel_writer_len(thread_work, &w);
    const uint16_t n = thread_value;
    TEST_ASSERT_TRUE(n > 0);

    SpinelReader r;
    Thread.spinel_reader_init(thread_work, &r, buf, n);
    ...
    uint16_t d2len = 0;
    proto_bool thread_ok = Thread.spinel_get_bool(thread_work, &r, &b);
    TEST_ASSERT_TRUE(thread_ok);
    proto_bool thread_ok2 = Thread.spinel_get_uint(thread_work, &r, &packed);
    TEST_ASSERT_TRUE(thread_ok2);
    proto_bool thread_ok3 = Thread.spinel_get_eui64(thread_work, &r, &eui);
    TEST_ASSERT_TRUE(thread_ok3);
    proto_bool thread_ok4 = Thread.spinel_get_ipv6(thread_work, &r, &ip6);
    TEST_ASSERT_TRUE(thread_ok4);
    proto_bool thread_ok5 = Thread.spinel_get_utf8(thread_work, &r, &s, &slen);
    TEST_ASSERT_TRUE(thread_ok5);
    proto_bool thread_ok6 = Thread.spinel_get_data_wlen(thread_work, &r, &d1, &d1len);
    TEST_ASSERT_TRUE(thread_ok6);
    proto_bool thread_ok7 = Thread.spinel_get_data(thread_work, &r, &d2, &d2len);
    TEST_ASSERT_TRUE(thread_ok7);
    ...
    proto_bool thread_ok8 = Thread.spinel_reader_ok(thread_work, &r);
    TEST_ASSERT_TRUE(thread_ok8);
}
...
void test_spinel_cursor_bounds_latch(void)
{
    ...
    SpinelReader r;
    Thread.spinel_reader_init(thread_work, &r, VALUE, sizeof(VALUE));
    uint32_t u32 = 0;
    proto_bool thread_ok = Thread.spinel_get_u32(thread_work, &r, &u32);
    TEST_ASSERT_FALSE(thread_ok); // 4 bytes wanted, 3 present
    proto_bool thread_ok2 = Thread.spinel_reader_ok(thread_work, &r);
    TEST_ASSERT_FALSE(thread_ok2);
    uint8_t u8 = 0;
    proto_bool thread_ok3 = Thread.spinel_get_u8(thread_work, &r, &u8);
    TEST_ASSERT_FALSE(thread_ok3); // stays latched
    proto_bool thread_ok4 = Thread.spinel_reader_ok(thread_work, &r);
    TEST_ASSERT_FALSE(thread_ok4);

    // A value with no NUL is not a UTF8 field.
    SpinelReader s;
    Thread.spinel_reader_init(thread_work, &s, (const uint8_t *)"abc", 3);
    const char *str = NULL;
    uint16_t slen = 0;
    proto_bool thread_ok5 = Thread.spinel_get_utf8(thread_work, &s, &str, &slen);
    TEST_ASSERT_FALSE(thread_ok5);
    proto_bool thread_ok6 = Thread.spinel_reader_ok(thread_work, &s);
    TEST_ASSERT_FALSE(thread_ok6);

    uint8_t small[3];
    SpinelWriter w;
    Thread.spinel_writer_init(thread_work, &w, small, sizeof(small));
    proto_bool thread_ok7 = Thread.spinel_put_u16(thread_work, &w, 0xBEEF);
    TEST_ASSERT_TRUE(thread_ok7);
    proto_bool thread_ok8 = Thread.spinel_put_u32(thread_work, &w, 0);
    TEST_ASSERT_FALSE(thread_ok8); // 4 more, 1 left
    uint16_t thread_value = Thread.spinel_writer_len(thread_work, &w);
    TEST_ASSERT_EQUAL_UINT16(0, thread_value);
}
...
{
    const SpinelPropInfo *thread_ptr = Thread.spinel_prop_lookup(thread_work, SPINEL_PROP_NCP_VERSION);
    ...
    const SpinelPropInfo *thread_ptr2 = Thread.spinel_prop_lookup(thread_work, SPINEL_PROP_IPV6_LL_ADDR);
    e = thread_ptr2;
    TEST_ASSERT_NOT_NULL(e);
    TEST_ASSERT_EQUAL_CHAR('6', e->type); // a 16-octet IPv6 address

    const SpinelPropInfo *thread_ptr3 = Thread.spinel_prop_lookup(thread_work, 0xFFFFu);
    TEST_ASSERT_NULL(thread_ptr3);
    const char *thread_text = Thread.spinel_prop_name(thread_work, 0xFFFFu);
    TEST_ASSERT_EQUAL_STRING("UNKNOWN", thread_text);
    const char *thread_text2 = Thread.spinel_prop_name(thread_work, SPINEL_PROP_LAST_STATUS);
    TEST_ASSERT_EQUAL_STRING("LAST_STATUS", thread_text2);
}
...
{
    const char *thread_text = Thread.spinel_status_name(thread_work, SPINEL_STATUS_OK);
    TEST_ASSERT_EQUAL_STRING("OK", thread_text);
    const char *thread_text2 = Thread.spinel_status_name(thread_work, SPINEL_STATUS_PARSE_ERROR);
    TEST_ASSERT_EQUAL_STRING("PARSE_ERROR", thread_text2);
    const char *thread_text3 = Thread.spinel_status_name(thread_work, SPINEL_STATUS_EMPTY);
    TEST_ASSERT_EQUAL_STRING("EMPTY", thread_text3);
    ...
    {
        const char *thread_text = Thread.spinel_status_name(thread_work, s);
        TEST_ASSERT_EQUAL_STRING("RESET", thread_text);
    }
    const char *thread_text4 = Thread.spinel_status_name(thread_work, 114u);
    TEST_ASSERT_EQUAL_STRING("RESET", thread_text4); // RESET_SOFTWARE
    const char *thread_text5 = Thread.spinel_status_name(thread_work, 120u);
    TEST_ASSERT_EQUAL_STRING("RESET", thread_text5); // RESET_WATCHDOG

    const char *thread_text6 = Thread.spinel_status_name(thread_work, SPINEL_STATUS_RESET_POWER_ON - 1);
    TEST_ASSERT_EQUAL_STRING("UNKNOWN", thread_text6);
    const char *thread_text7 = Thread.spinel_status_name(thread_work, SPINEL_STATUS_RESET_END);
    TEST_ASSERT_EQUAL_STRING("UNKNOWN", thread_text7);
}
...
{
    uint8_t value[5];
    uint8_t thread_u8 = Thread.spinel_pack_uint(thread_work, SPINEL_STATUS_RESET_POWER_ON, value, sizeof(value));
    const uint8_t vlen = thread_u8;
    TEST_ASSERT_EQUAL_UINT8(1, vlen); // 112 < 128, so one byte

    uint8_t cmdbuf[16];
    uint16_t thread_value =
        Thread.spinel_command_build(thread_work, protocore_spinel_header(0, 0), SPINEL_CMD_PROP_VALUE_IS,
    ...
    uint16_t val_len = 0;
    int thread_n2 = Thread.spinel_command_parse(thread_work, payload, plen, NULL, &cmd, &prop, &val, &val_len);
    TEST_ASSERT_TRUE(thread_n2 > 0);
    TEST_ASSERT_EQUAL_UINT32(SPINEL_CMD_PROP_VALUE_IS, cmd);
    TEST_ASSERT_EQUAL_UINT32(SPINEL_PROP_LAST_STATUS, prop);

    SpinelReader r;
    Thread.spinel_reader_init(thread_work, &r, val, val_len);
    uint32_t status = 0;
    proto_bool thread_ok = Thread.spinel_get_uint(thread_work, &r, &status);
    TEST_ASSERT_TRUE(thread_ok);
    TEST_ASSERT_EQUAL_UINT32(SPINEL_STATUS_RESET_POWER_ON, status);
    const char *thread_text = Thread.spinel_status_name(thread_work, status);
    TEST_ASSERT_EQUAL_STRING("RESET", thread_text);
}
...
{
    uint8_t out[16];
    uint8_t thread_u8 = Thread.spinel_pack_uint(thread_work, 1, NULL, 4);
    TEST_ASSERT_EQUAL_UINT8(0, thread_u8);
    int thread_n = Thread.spinel_unpack_uint(thread_work, NULL, 4, NULL);
    TEST_ASSERT_EQUAL_INT(0, thread_n);
    uint16_t thread_value = Thread.spinel_frame_encode(thread_work, NULL, 4, out, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(0, thread_value);
    uint16_t thread_value2 = Thread.spinel_frame_encode(thread_work, out, 4, NULL, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(0, thread_value2);
    int thread_n2 = Thread.spinel_frame_decode(thread_work, NULL, 4, out, sizeof(out), NULL);
    TEST_ASSERT_EQUAL_INT(0, thread_n2);
    uint16_t thread_value3 = Thread.spinel_command_build(thread_work, 0x80, 1, 1, NULL, 3, out, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(0, thread_value3);
    int thread_n3 = Thread.spinel_command_parse(thread_work, NULL, 4, NULL, NULL, NULL, NULL, NULL);
    TEST_ASSERT_EQUAL_INT(-1, thread_n3);
    int thread_n4 = Thread.spinel_command_parse(thread_work, out, 0, NULL, NULL, NULL, NULL, NULL);
    TEST_ASSERT_EQUAL_INT(-1, thread_n4);
}
