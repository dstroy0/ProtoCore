...
{
    uint16_t zigbee_value = Zigbee.ash_crc16(zigbee_work, CHECK9, sizeof(CHECK9));
    TEST_ASSERT_EQUAL_HEX16(0x29B1u, zigbee_value);
}
...
void test_ug101_rst_frame(void)
{
    ...
    uint16_t zigbee_value = Zigbee.ash_crc16(zigbee_work, WANT, 1);
    TEST_ASSERT_EQUAL_HEX16(0x38BCu, zigbee_value);

    uint8_t out[16];
    zigbee_value = Zigbee.ash_frame_encode(zigbee_work, ASH_RST, NULL, 0, out, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(4, zigbee_value);
...
void test_crc_covers_control_then_payload_msb_first(void)
{
    ...
    uint16_t zigbee_value = Zigbee.ash_crc16(zigbee_work, JOINED, sizeof(JOINED));
    const uint16_t crc = zigbee_value;
    ...
    uint8_t back[32];
    uint16_t back_len = 0;
    int zigbee_n = Zigbee.ash_frame_decode(zigbee_work, good, g, &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT((int)g, zigbee_n);
    ...
    swapped[s++] = ASH_FLAG;
    zigbee_n = Zigbee.ash_frame_decode(zigbee_work, swapped, s, &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(-1, zigbee_n);

    // And the encoder lays out the same octets the hand-built frame does.
    uint8_t out[32];
    zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x42, JOINED + 1, 3, out, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(g, zigbee_value);
...
void test_reserved_octets_are_escaped(void)
{
    ...
    uint8_t out[48];
    uint16_t zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x00, RESERVED, sizeof(RESERVED), out, sizeof(out));
    const uint16_t n = zigbee_value;
...
void test_frame_round_trip(void)
{
    ...
    uint8_t frame[64];
    uint16_t zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x25, PAYLOAD, sizeof(PAYLOAD), frame, sizeof(frame));
    const uint16_t n = zigbee_value;
    ...
    uint8_t back[64];
    uint16_t back_len = 0;
    int zigbee_n = Zigbee.ash_frame_decode(zigbee_work, frame, n, &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT((int)n, zigbee_n);
...
{
    uint8_t frame[16];
    uint16_t zigbee_value = Zigbee.ash_frame_encode(zigbee_work, ASH_RSTACK, NULL, 0, frame, sizeof(frame));
    const uint16_t n = zigbee_value;
    ...
    int zigbee_n = Zigbee.ash_frame_decode(zigbee_work, frame, n, &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(4, zigbee_n);
...
void test_decode_rejects_a_corrupted_frame(void)
{
    ...
    uint8_t frame[32];
    uint16_t zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x35, PAYLOAD, sizeof(PAYLOAD), frame, sizeof(frame));
    const uint16_t n = zigbee_value;
    TEST_ASSERT_TRUE(n > 0);
    ...
    for (uint16_t i = 0; i + 1 < n; i++)
    {
        ...
        int zigbee_n = Zigbee.ash_frame_decode(zigbee_work, bad, n, &control, back, sizeof(back), &back_len);
        TEST_ASSERT_EQUAL_INT(-1, zigbee_n);
    }
}
...
void test_decode_framing_faults(void)
{
    ...
    int zigbee_n =
        Zigbee.ash_frame_decode(zigbee_work, NO_FLAG, sizeof(NO_FLAG), &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(0, zigbee_n);
    zigbee_n = Zigbee.ash_frame_decode(zigbee_work, NO_FLAG, 0, &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(0, zigbee_n);

    static const uint8_t DANGLING[3] = {0xC0, ASH_ESCAPE, ASH_FLAG};
    zigbee_n =
        Zigbee.ash_frame_decode(zigbee_work, DANGLING, sizeof(DANGLING), &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(-1, zigbee_n);

    // Two octets cannot hold a control byte plus a two-octet CRC.
    static const uint8_t TOO_SHORT[3] = {0xC0, 0x38, ASH_FLAG};
    zigbee_n =
        Zigbee.ash_frame_decode(zigbee_work, TOO_SHORT, sizeof(TOO_SHORT), &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(-1, zigbee_n);
}
...
void test_decode_refuses_a_short_payload_buffer(void)
{
    ...
    uint8_t frame[32];
    uint16_t zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x01, PAYLOAD, sizeof(PAYLOAD), frame, sizeof(frame));
    const uint16_t n = zigbee_value;

    uint8_t control = 0;
    uint8_t small[4];
    uint16_t back_len = 0;
    int zigbee_n = Zigbee.ash_frame_decode(zigbee_work, frame, n, &control, small, sizeof(small), &back_len);
    TEST_ASSERT_EQUAL_INT(-1, zigbee_n);
}
...
void test_decode_consumes_one_frame_from_a_stream(void)
{
    ...
    uint8_t stream[64];
    uint16_t zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x10, A, sizeof(A), stream, sizeof(stream));
    const uint16_t na = zigbee_value;
    zigbee_value =
        Zigbee.ash_frame_encode(zigbee_work, 0x20, B, sizeof(B), stream + na, (uint16_t)(sizeof(stream) - na));
    const uint16_t nb = zigbee_value;
    ...
    uint16_t back_len = 0;
    int zigbee_n =
        Zigbee.ash_frame_decode(zigbee_work, stream, (uint16_t)(na + nb), &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT((int)na, zigbee_n);
    TEST_ASSERT_EQUAL_HEX8(0x10, control);
    TEST_ASSERT_EQUAL_MEMORY(A, back, sizeof(A));

    zigbee_n = Zigbee.ash_frame_decode(zigbee_work, stream + na, nb, &control, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT((int)nb, zigbee_n);
...
void test_encode_bounds(void)
{
    ...
    uint16_t zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x00, big, sizeof(big), out, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(0, zigbee_value);
    zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x00, NULL, 4, out, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(0, zigbee_value);
    zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x00, big, 4, NULL, sizeof(out));
    TEST_ASSERT_EQUAL_UINT16(0, zigbee_value);

    static const uint8_t SMALL[4] = {1, 2, 3, 4};
    zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x00, SMALL, sizeof(SMALL), out, sizeof(out));
    const uint16_t exact = zigbee_value;
    TEST_ASSERT_TRUE(exact >= 8); // control + 4 payload + CRC(2) + flag
    zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x00, SMALL, sizeof(SMALL), out, (uint16_t)(exact - 1));
    TEST_ASSERT_EQUAL_UINT16(0, zigbee_value);
    zigbee_value = Zigbee.ash_frame_encode(zigbee_work, 0x00, SMALL, sizeof(SMALL), out, exact);
    TEST_ASSERT_EQUAL_UINT16(exact, zigbee_value);
}
...
void test_decode_null_input(void)
{
    uint8_t back[8];
    uint16_t back_len = 0;
    int zigbee_n = Zigbee.ash_frame_decode(zigbee_work, NULL, 8, NULL, back, sizeof(back), &back_len);
    TEST_ASSERT_EQUAL_INT(0, zigbee_n);
}
