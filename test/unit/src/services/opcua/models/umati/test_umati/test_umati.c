...
//
// Host tests for the umati / OPC UA for Machine Tools model (services/opcua/models/umati/umati.h).
//
...
#include "services/opcua/models/umati/umati.c"
...
{
    return protocore_opcua_namespace_index(UMATI_NS_URI);
}
...
{
    int32_t n = umati_browse(model_ns(), id, g_ref, REF_MAX);
    TEST_ASSERT_EQUAL_INT32(want, n);
    return n;
}

// One reference: its BrowseName, NodeClass, and the reference and TypeDefinition the NodeSet
// publishes for that edge - which is HasProperty to PropertyType for everything under
// Identification and HasComponent to BaseDataVariableType for the monitoring leaves.
static void assert_ref_as(int32_t i, const char *name, uint32_t node_class, uint32_t ref_type, uint32_t type_def)
{
    ...
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(ref_type, g_ref[i].ref_type_id, name);
    TEST_ASSERT_TRUE_MESSAGE(g_ref[i].is_forward, name);
    TEST_ASSERT_EQUAL_UINT16_MESSAGE(model_ns(), g_ref[i].target_ns, name);
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(type_def, g_ref[i].type_def_id, name);
}

// A property leaf, as MachineryItemIdentificationType reaches every one of its.
static void assert_prop(int32_t i, const char *name)
{
    assert_ref_as(i, name, OPCUA_NODECLASS_VARIABLE, OPCUA_REFTYPE_HAS_PROPERTY, OPCUA_TYPEDEF_PROPERTY);
}

// A component leaf, as the monitoring types reach theirs.
static void assert_ref(int32_t i, const char *name, uint32_t node_class)
{
    assert_ref_as(i, name, node_class, OPCUA_REFTYPE_HAS_COMPONENT,
                  node_class == OPCUA_NODECLASS_VARIABLE ? OPCUA_TYPEDEF_BASE_DATA_VARIABLE
...
static OpcUaVariant read_ok(uint32_t id)
{
    OpcUaVariant v;
    memset(&v, 0, sizeof(v));
    TEST_ASSERT_TRUE(umati_read(model_ns(), id, OPCUA_ATTR_VALUE, &v));
    return v;
}
...
void test_machine_tool_components(void)
{
    ...
    for (int32_t i = 0; i < 4; i++)
    {
        ...
        // Identification is an add-in (Part 3), which is how Machinery and MachineTool both attach
        // it; the other three are components. All four carry their own ObjectType.
        TEST_ASSERT_EQUAL_UINT32(i == 0 ? OPCUA_REFTYPE_HAS_ADDIN : OPCUA_REFTYPE_HAS_COMPONENT, g_ref[i].ref_type_id);
        TEST_ASSERT_EQUAL_UINT16(model_ns(), g_ref[i].type_def_ns);
        TEST_ASSERT_NOT_EQUAL_UINT32(OPCUA_TYPEDEF_BASE_OBJECT, g_ref[i].type_def_id);
    }
}
...
void test_monitoring_sub_objects(void)
{
    ...
    TEST_ASSERT_EQUAL_STRING("Override", g_ref[1].browse_name);
...
void test_production_and_notification(void)
{
    ...
    // ProductionType holds two Objects, not two values: ActiveProgram is a
    // ProductionActiveProgramType whose Name carries the program, and the part counter is
    // Statistics/PartsProducedInLifetime.
    browse(prod, 2);
    TEST_ASSERT_EQUAL_STRING("ActiveProgram", g_ref[0].browse_name);
    TEST_ASSERT_EQUAL_UINT32(OPCUA_NODECLASS_OBJECT, g_ref[0].node_class);
    TEST_ASSERT_EQUAL_STRING("Statistics", g_ref[1].browse_name);
    TEST_ASSERT_EQUAL_UINT32(OPCUA_NODECLASS_OBJECT, g_ref[1].node_class);
    uint32_t active_program = g_ref[0].target_id;
    uint32_t statistics = g_ref[1].target_id;

    browse(active_program, 1);
    TEST_ASSERT_EQUAL_STRING("Name", g_ref[0].browse_name);
    TEST_ASSERT_EQUAL_UINT32(OPCUA_REFTYPE_HAS_PROPERTY, g_ref[0].ref_type_id);
    TEST_ASSERT_EQUAL_STRING("O1234.NC", read_ok(g_ref[0].target_id).str);

    browse(statistics, 1);
    TEST_ASSERT_EQUAL_STRING("PartsProducedInLifetime", g_ref[0].browse_name);
    OpcUaVariant parts = read_ok(g_ref[0].target_id);
...
void test_reads_outside_the_model_are_refused(void)
{
    ...
    TEST_ASSERT_FALSE(umati_read(model_ns(), 1u, OPCUA_ATTR_VALUE, &v));
    TEST_ASSERT_FALSE(umati_read(model_ns(), root, OPCUA_ATTR_VALUE, &v)); // an Object, not a value
    TEST_ASSERT_FALSE(umati_read((uint16_t)(model_ns() + 1), root + 101u, OPCUA_ATTR_VALUE, &v));
    TEST_ASSERT_FALSE(umati_read(model_ns(), root + 101u, OPCUA_ATTR_VALUE + 1u, &v));

    // a leaf Variable has no children, and an unknown node is not in the model
    TEST_ASSERT_EQUAL_INT32(-1, umati_browse(model_ns(), root + 101u, g_ref, REF_MAX));
    TEST_ASSERT_EQUAL_INT32(-1, umati_browse(model_ns(), 1u, g_ref, REF_MAX));
    TEST_ASSERT_EQUAL_INT32(-1, umati_browse((uint16_t)(model_ns() + 1), root, g_ref, REF_MAX));
}
...
void test_browse_respects_the_reference_cap(void)
{
    ...
    TEST_ASSERT_EQUAL_INT32(2, umati_browse(model_ns(), root, g_ref, 2));
    TEST_ASSERT_EQUAL_STRING("Identification", g_ref[0].browse_name);
    TEST_ASSERT_EQUAL_STRING("Monitoring", g_ref[1].browse_name);
    TEST_ASSERT_EQUAL_INT32(0, umati_browse(model_ns(), root, g_ref, 0));
}
...
void test_every_reference_resolves(void)
{
    ...
    while (head < tail)
    {
        ...
        for (int32_t i = 0; i < n; i++)
        {
            ...
            {
                OpcUaVariant v;
                TEST_ASSERT_TRUE_MESSAGE(umati_read(model_ns(), kids[i].target_id, OPCUA_ATTR_VALUE, &v),
                                         kids[i].browse_name);
                variables++;
            }
            else
