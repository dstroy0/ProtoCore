...
void tearDown(void)
{
    protocore_robotics_bind(NULL);
}

static int32_t browse(uint32_t id, int32_t want)
{
    int32_t n = protocore_robotics_browse(PROTOCORE_ROBOTICS_NS, id, g_ref, REF_MAX);
...
static OpcUaVariant read_ok(uint32_t id)
{
    OpcUaVariant v;
    memset(&v, 0, sizeof(v));
    TEST_ASSERT_TRUE(protocore_robotics_read(PROTOCORE_ROBOTICS_NS, id, OPCUA_ATTR_VALUE, &v));
    return v;
}
...
{
    TEST_ASSERT_EQUAL_INT32(1, protocore_robotics_browse(0, 85, g_ref, REF_MAX));
...
void test_objects_folder_organizes_the_motion_device_system(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT16(PROTOCORE_ROBOTICS_NS, g_ref[0].target_ns);
...
void test_motion_device_system_components(void)
{
    ...
    for (int32_t i = 0; i < 3; i++)
    {
        ...
        TEST_ASSERT_EQUAL_UINT32(OPCUA_TYPEDEF_BASE_OBJECT, g_ref[i].type_def_id);
    }
}
...
void test_axis_beyond_the_bound_count_is_absent(void)
{
    ...
    OpcUaVariant v;
    TEST_ASSERT_TRUE(protocore_robotics_read(PROTOCORE_ROBOTICS_NS, axis3 + 1u, OPCUA_ATTR_VALUE, &v));
    TEST_ASSERT_FALSE(protocore_robotics_read(PROTOCORE_ROBOTICS_NS, axis4 + 1u, OPCUA_ATTR_VALUE, &v));
    TEST_ASSERT_EQUAL_INT32(-1, protocore_robotics_browse(PROTOCORE_ROBOTICS_NS, axis4, g_ref, REF_MAX));

    // and a sub-id past the four axis variables is not one either
    TEST_ASSERT_FALSE(protocore_robotics_read(PROTOCORE_ROBOTICS_NS, axis3 + 5u, OPCUA_ATTR_VALUE, &v));
}
...
void test_reads_outside_the_model_are_refused(void)
{
    ...
    TEST_ASSERT_FALSE(protocore_robotics_read(PROTOCORE_ROBOTICS_NS, 1u, OPCUA_ATTR_VALUE, &v));
    TEST_ASSERT_FALSE(protocore_robotics_read(PROTOCORE_ROBOTICS_NS, r, OPCUA_ATTR_VALUE, &v)); // an Object
    TEST_ASSERT_FALSE(
        protocore_robotics_read((uint16_t)(PROTOCORE_ROBOTICS_NS + 1), manufacturer, OPCUA_ATTR_VALUE, &v));
    TEST_ASSERT_FALSE(protocore_robotics_read(PROTOCORE_ROBOTICS_NS, manufacturer, OPCUA_ATTR_VALUE + 1u, &v));

    TEST_ASSERT_EQUAL_INT32(-1, protocore_robotics_browse(PROTOCORE_ROBOTICS_NS, manufacturer, g_ref, REF_MAX));
    TEST_ASSERT_EQUAL_INT32(-1, protocore_robotics_browse(PROTOCORE_ROBOTICS_NS, 1u, g_ref, REF_MAX));
    TEST_ASSERT_EQUAL_INT32(-1, protocore_robotics_browse((uint16_t)(PROTOCORE_ROBOTICS_NS + 1), r, g_ref, REF_MAX));
}
...
void test_nothing_is_served_before_bind(void)
{
    ...
    protocore_robotics_bind(NULL);
    TEST_ASSERT_EQUAL_INT32(-1, protocore_robotics_browse(0, 85, g_ref, REF_MAX));
    TEST_ASSERT_EQUAL_INT32(-1, protocore_robotics_browse(PROTOCORE_ROBOTICS_NS, r, g_ref, REF_MAX));
    TEST_ASSERT_FALSE(protocore_robotics_read(PROTOCORE_ROBOTICS_NS, manufacturer, OPCUA_ATTR_VALUE, &v));
}
...
{
    uint32_t r = root();
    TEST_ASSERT_EQUAL_INT32(2, protocore_robotics_browse(PROTOCORE_ROBOTICS_NS, r, g_ref, 2));
    TEST_ASSERT_EQUAL_STRING("MotionDevices", g_ref[0].browse_name);
    TEST_ASSERT_EQUAL_STRING("Controllers", g_ref[1].browse_name);
    TEST_ASSERT_EQUAL_INT32(0, protocore_robotics_browse(PROTOCORE_ROBOTICS_NS, r, g_ref, 0));
}
        ...
        for (int32_t i = 0; i < n; i++)
        {
            if (kids[i].node_class == OPCUA_NODECLASS_VARIABLE)
            {
                ...
                TEST_ASSERT_TRUE_MESSAGE(
                    protocore_robotics_read(PROTOCORE_ROBOTICS_NS, kids[i].target_id, OPCUA_ATTR_VALUE, &v),
                    kids[i].browse_name);
                variables++;
            }
            else
