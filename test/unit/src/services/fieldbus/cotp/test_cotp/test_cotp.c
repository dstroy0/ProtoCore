...
void test_smallest_tpkt_is_seven_octets(void)
{
    ...
    uint8_t dt[8];
    Cotp.build_dt_args.buf = dt;
    Cotp.build_dt_args.cap = sizeof(dt);
    Cotp.build_dt_args.data = NULL;
    Cotp.build_dt_args.data_len = 0;
    Cotp.build_dt_args.eot = PROTO_TRUE;
    ...
    Cotp.tpkt_build_args.buf = frame;
    Cotp.tpkt_build_args.cap = sizeof(frame);
    Cotp.tpkt_build_args.payload = dt;
    Cotp.tpkt_build_args.payload_len = dlen;
...
void test_tpkt_length_includes_the_header(void)
{
    ...
    Cotp.tpkt_build_args.buf = frame;
    Cotp.tpkt_build_args.cap = sizeof(frame);
    Cotp.tpkt_build_args.payload = PAYLOAD;
    Cotp.tpkt_build_args.payload_len = sizeof(PAYLOAD);
    Cotp.tpkt_build(cotp_work);
    size_t n = Cotp.n;
    ...
    Cotp.tpkt_parse_args.buf = frame;
    Cotp.tpkt_parse_args.len = n;
    Cotp.tpkt_parse_args.payload = &slice;
    Cotp.tpkt_parse_args.payload_len = &slice_len;
    Cotp.tpkt_parse_args.consumed = &consumed;
    Cotp.tpkt_parse(cotp_work);
    TEST_ASSERT_TRUE(Cotp.ok);
...
void test_consumed_advances_past_one_packet_in_a_stream(void)
{
    ...
    uint8_t stream[32];
    Cotp.tpkt_build_args.buf = stream;
    Cotp.tpkt_build_args.cap = sizeof(stream);
    Cotp.tpkt_build_args.payload = A;
    Cotp.tpkt_build_args.payload_len = sizeof(A);
    Cotp.tpkt_build(cotp_work);
    size_t n = Cotp.n;
    Cotp.tpkt_build_args.buf = stream + n;
    Cotp.tpkt_build_args.cap = sizeof(stream) - n;
    Cotp.tpkt_build_args.payload = B;
    Cotp.tpkt_build_args.payload_len = sizeof(B);
    Cotp.tpkt_build(cotp_work);
    size_t m = Cotp.n;
    ...
    Cotp.tpkt_parse_args.buf = stream;
    Cotp.tpkt_parse_args.len = n + m;
    Cotp.tpkt_parse_args.payload = &slice;
    Cotp.tpkt_parse_args.payload_len = &slice_len;
    Cotp.tpkt_parse_args.consumed = &consumed;
    Cotp.tpkt_parse(cotp_work);
    ...
    Cotp.tpkt_parse_args.buf = stream + consumed;
    Cotp.tpkt_parse_args.len = n + m - consumed;
    Cotp.tpkt_parse_args.payload = &slice;
    Cotp.tpkt_parse_args.payload_len = &slice_len;
    Cotp.tpkt_parse_args.consumed = &consumed;
    Cotp.tpkt_parse(cotp_work);
...
void test_tpkt_refusals(void)
{
    ...
    TEST_ASSERT_TRUE(Cotp.ok);
    Cotp.tpkt_parse_args.buf = frame;
    Cotp.tpkt_parse_args.len = 7;
    Cotp.tpkt_parse_args.payload = &slice;
    Cotp.tpkt_parse_args.payload_len = &slice_len;
    Cotp.tpkt_parse_args.consumed = &consumed;
    Cotp.tpkt_parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok); // one octet short
    ...
    Cotp.tpkt_parse_args.buf = frame;
    Cotp.tpkt_parse_args.len = 3;
    Cotp.tpkt_parse_args.payload = &slice;
    Cotp.tpkt_parse_args.payload_len = &slice_len;
    Cotp.tpkt_parse_args.consumed = &consumed;
    Cotp.tpkt_parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok);
    Cotp.tpkt_parse_args.buf = NULL;
    Cotp.tpkt_parse_args.len = 8;
    Cotp.tpkt_parse_args.payload = &slice;
    Cotp.tpkt_parse_args.payload_len = &slice_len;
    Cotp.tpkt_parse_args.consumed = &consumed;
    ...
    Cotp.tpkt_build_args.buf = out;
    Cotp.tpkt_build_args.cap = 8;
    Cotp.tpkt_build_args.payload = PAYLOAD;
    Cotp.tpkt_build_args.payload_len = sizeof(PAYLOAD);
    Cotp.tpkt_build(cotp_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cotp.n); // 4+5 > 8
    Cotp.tpkt_build_args.buf = NULL;
    Cotp.tpkt_build_args.cap = sizeof(out);
    Cotp.tpkt_build_args.payload = PAYLOAD;
    Cotp.tpkt_build_args.payload_len = sizeof(PAYLOAD);
    Cotp.tpkt_build(cotp_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cotp.n);
    Cotp.tpkt_build_args.buf = out;
    Cotp.tpkt_build_args.cap = sizeof(out);
    Cotp.tpkt_build_args.payload = NULL;
    Cotp.tpkt_build_args.payload_len = 2;
    Cotp.tpkt_build(cotp_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cotp.n);
}
...
void test_data_tpdu_layout(void)
{
    ...
    uint8_t buf[16];
    Cotp.build_dt_args.buf = buf;
    Cotp.build_dt_args.cap = sizeof(buf);
    Cotp.build_dt_args.data = DATA;
    Cotp.build_dt_args.data_len = sizeof(DATA);
    Cotp.build_dt_args.eot = PROTO_TRUE;
    Cotp.build_dt(cotp_work);
    size_t n = Cotp.n;
    ...
    CotpHeader h;
    Cotp.parse_args.buf = buf;
    Cotp.parse_args.len = n;
    ...
    Cotp.build_dt_args.data = DATA;
    Cotp.build_dt_args.data_len = sizeof(DATA);
    Cotp.build_dt_args.eot = PROTO_FALSE;
    Cotp.build_dt(cotp_work);
    n = Cotp.n;
    ...
    Cotp.build_dt_args.buf = buf;
    Cotp.build_dt_args.cap = 2;
    Cotp.build_dt_args.data = DATA;
    Cotp.build_dt_args.data_len = sizeof(DATA);
    Cotp.build_dt_args.eot = PROTO_TRUE;
    Cotp.build_dt(cotp_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cotp.n);
    Cotp.build_dt_args.buf = NULL;
    Cotp.build_dt_args.cap = sizeof(buf);
    Cotp.build_dt_args.data = DATA;
    Cotp.build_dt_args.data_len = sizeof(DATA);
    Cotp.build_dt_args.eot = PROTO_TRUE;
...
{
    uint8_t buf[32];
    Cotp.build_cr_args.buf = buf;
    Cotp.build_cr_args.cap = sizeof(buf);
    Cotp.build_cr_args.src_ref = 0x0001u;
    Cotp.build_cr_args.tpdu_size_code = 0x0Au;
    ...
    Cotp.build_cr_args.buf = buf;
    Cotp.build_cr_args.cap = sizeof(buf);
    Cotp.build_cr_args.src_ref = 0x1234u;
    Cotp.build_cr_args.tpdu_size_code = 0x0Au;
    Cotp.build_cr_args.extra_params = NULL;
    Cotp.build_cr_args.extra_len = 0;
    Cotp.build_cr(cotp_work);
...
void test_connection_request_with_tsap_parameters(void)
{
    ...
    uint8_t cr[32];
    Cotp.build_cr_args.buf = cr;
    Cotp.build_cr_args.cap = sizeof(cr);
    Cotp.build_cr_args.src_ref = 0x0001u;
    Cotp.build_cr_args.tpdu_size_code = 0x0Au;
    ...
    Cotp.tpkt_build_args.buf = frame;
    Cotp.tpkt_build_args.cap = sizeof(frame);
    Cotp.tpkt_build_args.payload = cr;
    Cotp.tpkt_build_args.payload_len = n;
    Cotp.tpkt_build(cotp_work);
    size_t flen = Cotp.n;
    ...
    TEST_ASSERT_TRUE(Cotp.ok);
    CotpHeader h;
    Cotp.parse_args.buf = slice;
    Cotp.parse_args.len = slice_len;
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_TRUE(Cotp.ok);
    TEST_ASSERT_EQUAL_HEX8(COTP_CR, h.code);
...
{
    uint8_t buf[32];
    Cotp.build_cc_args.buf = buf;
    Cotp.build_cc_args.cap = sizeof(buf);
    Cotp.build_cc_args.dst_ref = 0x1234u;
    Cotp.build_cc_args.src_ref = 0xABCDu;
    Cotp.build_cc_args.tpdu_size_code = 0x0Au;
    Cotp.build_cc_args.extra_params = NULL;
    Cotp.build_cc_args.extra_len = 0;
    ...
    Cotp.build_cr_args.buf = cr;
    Cotp.build_cr_args.cap = sizeof(cr);
    Cotp.build_cr_args.src_ref = 0x0055u;
    Cotp.build_cr_args.tpdu_size_code = 0x0Au;
    Cotp.build_cr_args.extra_params = NULL;
    Cotp.build_cr_args.extra_len = 0;
    Cotp.build_cr(cotp_work);
    TEST_ASSERT_EQUAL_size_t(10u, Cotp.n);
    CotpHeader req;
    Cotp.parse_args.buf = cr;
    Cotp.parse_args.len = 10;
    Cotp.parse_args.out = &req;
    Cotp.parse(cotp_work);
    TEST_ASSERT_TRUE(Cotp.ok);
    Cotp.build_cc_args.buf = buf;
    Cotp.build_cc_args.cap = sizeof(buf);
    Cotp.build_cc_args.dst_ref = req.src_ref;
    Cotp.build_cc_args.src_ref = 0x0077u;
    Cotp.build_cc_args.tpdu_size_code = 0x0Au;
    Cotp.build_cc_args.extra_params = NULL;
    Cotp.build_cc_args.extra_len = 0;
    Cotp.build_cc(cotp_work);
    ...
    Cotp.build_cc_args.buf = buf;
    Cotp.build_cc_args.cap = 9;
    Cotp.build_cc_args.dst_ref = 1;
    Cotp.build_cc_args.src_ref = 2;
    Cotp.build_cc_args.tpdu_size_code = 0x0Au;
    Cotp.build_cc_args.extra_params = NULL;
    Cotp.build_cc_args.extra_len = 0;
    Cotp.build_cc(cotp_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cotp.n);
    Cotp.build_cc_args.buf = NULL;
    Cotp.build_cc_args.cap = sizeof(buf);
    Cotp.build_cc_args.dst_ref = 1;
    Cotp.build_cc_args.src_ref = 2;
    Cotp.build_cc_args.tpdu_size_code = 0x0Au;
    ...
    Cotp.build_cc_args.tpdu_size_code = 0x0Au;
    Cotp.build_cc_args.extra_params = NULL;
    Cotp.build_cc_args.extra_len = 4;
    Cotp.build_cc(cotp_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cotp.n);
    Cotp.build_cr_args.buf = buf;
    Cotp.build_cr_args.cap = 9;
    Cotp.build_cr_args.src_ref = 1;
    Cotp.build_cr_args.tpdu_size_code = 0x0Au;
    Cotp.build_cr_args.extra_params = NULL;
    Cotp.build_cr_args.extra_len = 0;
    Cotp.build_cr(cotp_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cotp.n);
    Cotp.build_cr_args.buf = NULL;
    Cotp.build_cr_args.cap = sizeof(buf);
    Cotp.build_cr_args.src_ref = 1;
    Cotp.build_cr_args.tpdu_size_code = 0x0Au;
    Cotp.build_cr_args.extra_params = NULL;
    ...
    Cotp.build_cr_args.tpdu_size_code = 0x0Au;
    Cotp.build_cr_args.extra_params = NULL;
    Cotp.build_cr_args.extra_len = 4;
    Cotp.build_cr(cotp_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cotp.n);
}
...
void test_type_is_the_high_nibble(void)
{
    ...
    Cotp.parse_args.buf = DR;
    Cotp.parse_args.len = sizeof(DR);
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_TRUE(Cotp.ok);
    ...
    Cotp.parse_args.buf = ER;
    Cotp.parse_args.len = sizeof(ER);
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_TRUE(Cotp.ok);
    ...
    Cotp.parse_args.buf = DT_WITH_CDT;
    Cotp.parse_args.len = sizeof(DT_WITH_CDT);
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_TRUE(Cotp.ok);
    TEST_ASSERT_EQUAL_HEX8(COTP_DT, h.code);
...
void test_cotp_refusals(void)
{
    ...
    Cotp.parse_args.buf = LI_ZERO;
    Cotp.parse_args.len = sizeof(LI_ZERO);
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok);

    static const uint8_t LI_TOO_BIG[3] = {0x20, 0xF0, 0x80}; // header of 33 octets in a 3-octet TPDU
    Cotp.parse_args.buf = LI_TOO_BIG;
    Cotp.parse_args.len = sizeof(LI_TOO_BIG);
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok);

    static const uint8_t DT_SHORT[2] = {0x01, 0xF0}; // DT needs the EOT/TPDU-NR octet
    Cotp.parse_args.buf = DT_SHORT;
    Cotp.parse_args.len = sizeof(DT_SHORT);
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok);

    static const uint8_t CR_SHORT[6] = {0x05, 0xE0, 0x00, 0x00, 0x00, 0x01}; // CR needs LI >= 6
    Cotp.parse_args.buf = CR_SHORT;
    Cotp.parse_args.len = sizeof(CR_SHORT);
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok);

    static const uint8_t ONE[1] = {0x02};
    Cotp.parse_args.buf = ONE;
    Cotp.parse_args.len = sizeof(ONE);
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok);
    Cotp.parse_args.buf = NULL;
    Cotp.parse_args.len = 3;
    Cotp.parse_args.out = &h;
    Cotp.parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok);
    Cotp.parse_args.buf = LI_ZERO;
    Cotp.parse_args.len = 3;
    Cotp.parse_args.out = NULL;
    Cotp.parse(cotp_work);
    TEST_ASSERT_FALSE(Cotp.ok);
}
...
void test_stack_round_trip(void)
{
    ...
    Cotp.build_dt_args.buf = dt;
    Cotp.build_dt_args.cap = sizeof(dt);
    Cotp.build_dt_args.data = S7;
    Cotp.build_dt_args.data_len = sizeof(S7);
    Cotp.build_dt_args.eot = PROTO_TRUE;
    Cotp.build_dt(cotp_work);
    size_t dlen = Cotp.n;
    uint8_t frame[64];
