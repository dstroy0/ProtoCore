...
void test_memory_area_read_octets(void)
{
    ...
    uint8_t buf[32];
    FinsV.build_memory_area_read_args.buf = buf;
    FinsV.build_memory_area_read_args.cap = sizeof(buf);
    FinsV.build_memory_area_read_args.h = &h;
    FinsV.build_memory_area_read_args.area = FINS_AREA_DM_WORD;
    FinsV.build_memory_area_read_args.address = 100;
    FinsV.build_memory_area_read_args.bit = 0;
    FinsV.build_memory_area_read_args.count = 10;
    Fins.build_memory_area_read(fins_work);
    size_t n = FinsV.n;
...
void test_memory_area_write_octets(void)
{
    ...
    FinsHeader h = master_header();
    uint8_t buf[32];
    FinsV.build_memory_area_write_args.buf = buf;
    FinsV.build_memory_area_write_args.cap = sizeof(buf);
    FinsV.build_memory_area_write_args.h = &h;
    FinsV.build_memory_area_write_args.area = FINS_AREA_DM_WORD;
    FinsV.build_memory_area_write_args.address = 100;
    FinsV.build_memory_area_write_args.bit = 0;
    FinsV.build_memory_area_write_args.count = 2;
    FinsV.build_memory_area_write_args.data = DATA;
    FinsV.build_memory_area_write_args.data_len = sizeof(DATA);
    Fins.build_memory_area_write(fins_work);
    size_t n = FinsV.n;
    TEST_ASSERT_EQUAL_size_t(sizeof(WANT), n);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(WANT, buf, sizeof(WANT));
}
...
void test_operating_mode_commands(void)
{
    ...
    FinsV.build_run_args.buf = buf;
    FinsV.build_run_args.cap = sizeof(buf);
    FinsV.build_run_args.h = &h;
    FinsV.build_run_args.mode = FINS_RUN_MODE_RUN;
    Fins.build_run(fins_work);
    TEST_ASSERT_EQUAL_size_t(15u, FinsV.n);
    ...
    FinsV.build_run_args.cap = sizeof(buf);
    FinsV.build_run_args.h = &h;
    FinsV.build_run_args.mode = FINS_RUN_MODE_MONITOR;
    Fins.build_run(fins_work);
    TEST_ASSERT_EQUAL_size_t(15u, FinsV.n);
    TEST_ASSERT_EQUAL_HEX8(0x02u, buf[14]);

    FinsV.build_stop_args.buf = buf;
    FinsV.build_stop_args.cap = sizeof(buf);
    FinsV.build_stop_args.h = &h;
    Fins.build_stop(fins_work);
    TEST_ASSERT_EQUAL_size_t(12u, FinsV.n);
...
void test_header_round_trip(void)
{
    ...
    FinsV.build_command_args.cap = sizeof(buf);
    FinsV.build_command_args.h = &h;
    FinsV.build_command_args.mrc = 0x05;
    FinsV.build_command_args.src = 0x01;
    ...
    FinsCommand c;
    memset(&c, 0, sizeof(c));
    FinsV.parse_command_args.buf = buf;
    FinsV.parse_command_args.len = n;
    FinsV.parse_command_args.out = &c;
    Fins.parse_command(fins_work);
    TEST_ASSERT_TRUE(FinsV.ok);
...
void test_response_end_code(void)
{
    ...
    FinsResponse r;
    memset(&r, 0, sizeof(r));
    FinsV.parse_response_args.buf = OK;
    FinsV.parse_response_args.len = sizeof(OK);
    FinsV.parse_response_args.out = &r;
    Fins.parse_response(fins_work);
    TEST_ASSERT_TRUE(FinsV.ok);
    ...
    FinsV.parse_response_args.buf = err;
    FinsV.parse_response_args.len = sizeof(err);
    FinsV.parse_response_args.out = &r;
    Fins.parse_response(fins_work);
    TEST_ASSERT_TRUE(FinsV.ok);
...
void test_parsers_refuse_short_frames(void)
{
    ...
    FinsCommand c;
    FinsV.parse_command_args.buf = buf;
    FinsV.parse_command_args.len = 11;
    FinsV.parse_command_args.out = &c;
    Fins.parse_command(fins_work);
    TEST_ASSERT_FALSE(FinsV.ok);
    FinsV.parse_command_args.buf = buf;
    FinsV.parse_command_args.len = 12;
    FinsV.parse_command_args.out = &c;
    Fins.parse_command(fins_work);
    TEST_ASSERT_TRUE(FinsV.ok);
    TEST_ASSERT_EQUAL_size_t(0u, c.params_len);
    FinsV.parse_command_args.buf = NULL;
    FinsV.parse_command_args.len = 12;
    FinsV.parse_command_args.out = &c;
    Fins.parse_command(fins_work);
    TEST_ASSERT_FALSE(FinsV.ok);
    FinsV.parse_command_args.buf = buf;
    FinsV.parse_command_args.len = 12;
    FinsV.parse_command_args.out = NULL;
    Fins.parse_command(fins_work);
    TEST_ASSERT_FALSE(FinsV.ok);

    FinsResponse r;
    FinsV.parse_response_args.buf = buf;
    FinsV.parse_response_args.len = 13;
    FinsV.parse_response_args.out = &r;
    Fins.parse_response(fins_work);
    TEST_ASSERT_FALSE(FinsV.ok);
    FinsV.parse_response_args.buf = buf;
    FinsV.parse_response_args.len = 14;
    FinsV.parse_response_args.out = &r;
    Fins.parse_response(fins_work);
    TEST_ASSERT_TRUE(FinsV.ok);
    TEST_ASSERT_EQUAL_size_t(0u, r.data_len);
    FinsV.parse_response_args.buf = NULL;
    FinsV.parse_response_args.len = 14;
    FinsV.parse_response_args.out = &r;
    Fins.parse_response(fins_work);
    TEST_ASSERT_FALSE(FinsV.ok);
    FinsV.parse_response_args.buf = buf;
    FinsV.parse_response_args.len = 14;
    FinsV.parse_response_args.out = NULL;
    Fins.parse_response(fins_work);
    TEST_ASSERT_FALSE(FinsV.ok);
}
...
void test_builders_refuse_a_short_buffer(void)
{
    ...
    FinsV.build_memory_area_read_args.buf = buf;
    FinsV.build_memory_area_read_args.cap = 17;
    FinsV.build_memory_area_read_args.h = &h;
    FinsV.build_memory_area_read_args.area = FINS_AREA_DM_WORD;
    FinsV.build_memory_area_read_args.address = 100;
    FinsV.build_memory_area_read_args.bit = 0;
    FinsV.build_memory_area_read_args.count = 10;
    Fins.build_memory_area_read(fins_work);
    TEST_ASSERT_EQUAL_size_t(0u, FinsV.n);
    FinsV.build_memory_area_read_args.buf = buf;
    FinsV.build_memory_area_read_args.cap = 18;
    FinsV.build_memory_area_read_args.h = &h;
    FinsV.build_memory_area_read_args.area = FINS_AREA_DM_WORD;
    FinsV.build_memory_area_read_args.address = 100;
    FinsV.build_memory_area_read_args.bit = 0;
    ...
    FinsV.build_memory_area_write_args.bit = 0;
    FinsV.build_memory_area_write_args.count = 2;
    FinsV.build_memory_area_write_args.data = NULL;
    FinsV.build_memory_area_write_args.data_len = 4;
    Fins.build_memory_area_write(fins_work);
    TEST_ASSERT_EQUAL_size_t(0u, FinsV.n);

    FinsV.build_stop_args.buf = buf;
    FinsV.build_stop_args.cap = 11;
    FinsV.build_stop_args.h = &h;
    Fins.build_stop(fins_work);
    TEST_ASSERT_EQUAL_size_t(0u, FinsV.n);
    FinsV.build_run_args.buf = buf;
    FinsV.build_run_args.cap = 14;
    FinsV.build_run_args.h = &h;
    FinsV.build_run_args.mode = FINS_RUN_MODE_RUN;
    Fins.build_run(fins_work);
    TEST_ASSERT_EQUAL_size_t(0u, FinsV.n);
    FinsV.build_command_args.buf = NULL;
    FinsV.build_command_args.cap = sizeof(buf);
    FinsV.build_command_args.h = &h;
    FinsV.build_command_args.mrc = 1;
    FinsV.build_command_args.src = 1;
    ...
    FinsV.build_command_args.buf = buf;
    FinsV.build_command_args.cap = sizeof(buf);
    FinsV.build_command_args.h = NULL;
    FinsV.build_command_args.mrc = 1;
    FinsV.build_command_args.src = 1;
    FinsV.build_command_args.params = NULL;
    FinsV.build_command_args.params_len = 0;
    ...
    FinsV.build_command_args.src = 1;
    FinsV.build_command_args.params = NULL;
    FinsV.build_command_args.params_len = 3;
    Fins.build_command(fins_work);
    TEST_ASSERT_EQUAL_size_t(0u, FinsV.n);
}
