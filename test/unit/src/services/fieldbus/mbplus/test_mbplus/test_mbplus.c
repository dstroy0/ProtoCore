...
void test_published_check_value(void)
{
    ...
    MbplusV.crc_args.bytes = CHECK;
    MbplusV.crc_args.len = sizeof(CHECK);
    Mbplus.crc(mbplus_work);
    TEST_ASSERT_EQUAL_HEX16(0x906Eu, MbplusV.value);
    // init=0xffff and xorout=0xffff cancel over an empty message.
    MbplusV.crc_args.bytes = CHECK;
    MbplusV.crc_args.len = 0;
    Mbplus.crc(mbplus_work);
    TEST_ASSERT_EQUAL_HEX16(0x0000u, MbplusV.value);
}
...
void test_frame_layout(void)
{
    ...
    MbplusV.crc_args.bytes = out + 1;
    MbplusV.crc_args.len = 5;
    Mbplus.crc(mbplus_work);
    uint16_t crc = MbplusV.value;                 // address + control + payload
    TEST_ASSERT_EQUAL_HEX8((uint8_t)crc, out[6]); // low octet first, the HDLC order
    TEST_ASSERT_EQUAL_HEX8((uint8_t)(crc >> 8), out[7]);

    MbPlusFrame f;
    memset(&f, 0, sizeof(f));
    MbplusV.parse_args.frame = out;
    MbplusV.parse_args.len = n;
    MbplusV.parse_args.out = &f;
    Mbplus.parse(mbplus_work);
    TEST_ASSERT_TRUE(MbplusV.ok);
...
{
    uint8_t out[16];
    MbplusV.build_args.address = 1;
    MbplusV.build_args.control = MBPLUS_CTRL_TOKEN;
    MbplusV.build_args.payload = NULL;
    MbplusV.build_args.payload_len = 0;
    MbplusV.build_args.out = out;
    MbplusV.build_args.cap = sizeof(out);
    Mbplus.build(mbplus_work);
    size_t n = MbplusV.n;
...
void test_round_trip(void)
{
    ...
    for (uint8_t addr = 1; addr <= MBPLUS_MAX_STATION; addr++)
    {
        ...
        {
            uint8_t frame[24];
            MbplusV.build_args.address = addr;
            MbplusV.build_args.control = MBPLUS_CTRL_DATA;
            MbplusV.build_args.payload = len ? PAYLOAD : NULL;
            MbplusV.build_args.payload_len = len;
...
void test_single_bit_corruption_is_refused(void)
{
    ...
    for (size_t i = 1; i + 1 < len; i++) // everything between the two flags
    {
        for (int bit = 0; bit < 8; bit++)
        {
            ...
            MbPlusFrame f;
            MbplusV.parse_args.frame = bad;
            MbplusV.parse_args.len = len;
            MbplusV.parse_args.out = &f;
            Mbplus.parse(mbplus_work);
            TEST_ASSERT_FALSE(MbplusV.ok);
        }
    }
}
...
void test_parse_rejects_bad_framing(void)
{
    ...
    MbplusV.parse_args.out = &f;
    Mbplus.parse(mbplus_work);
    TEST_ASSERT_FALSE(MbplusV.ok); // no opening flag
    ...
    MbplusV.parse_args.out = &f;
    Mbplus.parse(mbplus_work);
    TEST_ASSERT_FALSE(MbplusV.ok); // no closing flag

    MbplusV.parse_args.frame = frame;
    MbplusV.parse_args.len = 5;
    MbplusV.parse_args.out = &f;
    Mbplus.parse(mbplus_work);
    TEST_ASSERT_FALSE(MbplusV.ok); // shorter than the smallest frame
    MbplusV.parse_args.frame = NULL;
    MbplusV.parse_args.len = len;
    MbplusV.parse_args.out = &f;
    Mbplus.parse(mbplus_work);
    TEST_ASSERT_FALSE(MbplusV.ok);
...
void test_token_ring_rotation(void)
{
    ...
    for (size_t m = 0; m < sizeof(MAX_STATIONS) / sizeof(MAX_STATIONS[0]); m++)
    {
        ...
        for (uint8_t start = 1; start <= max; start++)
        {
            ...
            for (uint8_t step = 0; step < max; step++)
            {
                ...
                seen[at] = 1;
                MbplusV.next_token_args.current = at;
                MbplusV.next_token_args.max_station = max;
    ...
    MbplusV.next_token_args.current = 1;
    MbplusV.next_token_args.max_station = MBPLUS_MAX_STATION;
    Mbplus.next_token(mbplus_work);
    TEST_ASSERT_EQUAL_UINT8(2u, MbplusV.value);
    MbplusV.next_token_args.current = MBPLUS_MAX_STATION;
    MbplusV.next_token_args.max_station = MBPLUS_MAX_STATION;
    Mbplus.next_token(mbplus_work);
    TEST_ASSERT_EQUAL_UINT8(1u, MbplusV.value);
    MbplusV.next_token_args.current = 0;
    MbplusV.next_token_args.max_station = 0;
    Mbplus.next_token(mbplus_work);
    TEST_ASSERT_EQUAL_UINT8(1u, MbplusV.value);
    // A station above the segment's highest active one hands the token back to the first.
    MbplusV.next_token_args.current = 50;
    MbplusV.next_token_args.max_station = 8;
    Mbplus.next_token(mbplus_work);
    TEST_ASSERT_EQUAL_UINT8(1u, MbplusV.value);
}
...
void test_build_refuses_bad_arguments(void)
{
    ...
    MbplusV.build_args.address = 0;
    MbplusV.build_args.control = MBPLUS_CTRL_DATA;
    MbplusV.build_args.payload = PAYLOAD;
    MbplusV.build_args.payload_len = 2;
    MbplusV.build_args.out = out;
    MbplusV.build_args.cap = sizeof(out);
    Mbplus.build(mbplus_work);
    TEST_ASSERT_EQUAL_size_t(0u, MbplusV.n);
    MbplusV.build_args.address = 1;
    MbplusV.build_args.control = MBPLUS_CTRL_DATA;
    MbplusV.build_args.payload = PAYLOAD;
    MbplusV.build_args.payload_len = 2;
    ...
    MbplusV.build_args.cap = sizeof(out);
    Mbplus.build(mbplus_work);
    TEST_ASSERT_EQUAL_size_t(8u, MbplusV.n);
    MbplusV.build_args.address = 64;
    MbplusV.build_args.control = MBPLUS_CTRL_DATA;
    MbplusV.build_args.payload = PAYLOAD;
    MbplusV.build_args.payload_len = 2;
    MbplusV.build_args.out = out;
