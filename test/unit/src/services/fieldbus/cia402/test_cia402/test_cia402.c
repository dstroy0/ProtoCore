...
void test_statusword_mask_value_table(void)
{
    ...
    for (size_t i = 0; i < sizeof(ROWS) / sizeof(ROWS[0]); i++)
    {
        ...
        Cia402.state_args.statusword = bare;
        Cia402.state(cia402_work);
        TEST_ASSERT_EQUAL_INT_MESSAGE(ROWS[i].state, Cia402.value, "bare statusword");
        Cia402.state_args.statusword = decorated;
        Cia402.state(cia402_work);
        TEST_ASSERT_EQUAL_INT_MESSAGE(ROWS[i].state, Cia402.value, "don't-care bits set");
    }
}
...
{
    Cia402.state_args.statusword = 0x0027u;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_OPERATION_ENABLED, Cia402.value);
    Cia402.state_args.statusword = 0x0007u;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_QUICK_STOP_ACTIVE, Cia402.value);
    // The same pair with a realistic upper byte: voltage enabled, remote, target reached.
    Cia402.state_args.statusword = 0x0637u;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_OPERATION_ENABLED, Cia402.value);
    Cia402.state_args.statusword = 0x0617u;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_QUICK_STOP_ACTIVE, Cia402.value);
}
...
{
    Cia402.state_args.statusword = 0x0041u;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_UNKNOWN, Cia402.value); // x1xx 0001 is no row
    Cia402.state_args.statusword = 0x0002u;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_UNKNOWN, Cia402.value); // switched-on bit alone
    Cia402.state_args.statusword = 0x0004u;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_UNKNOWN, Cia402.value);
    Cia402.state_args.statusword = 0x0043u;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_UNKNOWN, Cia402.value);
}
...
void test_controlword_command_table(void)
{
    ...
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(CIA402_CW_ENABLE_VOLTAGE | CIA402_CW_QUICK_STOP, Cia402.u16);
    Cia402.controlword_args.cmd = CIA402_COMMAND_SHUTDOWN;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0006u, Cia402.u16);

    Cia402.controlword_args.cmd = CIA402_COMMAND_SWITCH_ON;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(CIA402_CW_SWITCH_ON | CIA402_CW_ENABLE_VOLTAGE | CIA402_CW_QUICK_STOP, Cia402.u16);
    Cia402.controlword_args.cmd = CIA402_COMMAND_SWITCH_ON;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0007u, Cia402.u16);

    Cia402.controlword_args.cmd = CIA402_COMMAND_ENABLE_OPERATION;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x000Fu, Cia402.u16);
    Cia402.controlword_args.cmd = CIA402_COMMAND_ENABLE_OPERATION;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(
        CIA402_CW_SWITCH_ON | CIA402_CW_ENABLE_VOLTAGE | CIA402_CW_QUICK_STOP | CIA402_CW_ENABLE_OPERATION, Cia402.u16);
    ...
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0000u, Cia402.u16);
    Cia402.controlword_args.cmd = CIA402_COMMAND_QUICK_STOP;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0002u, Cia402.u16);
    Cia402.controlword_args.cmd = CIA402_COMMAND_QUICK_STOP;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0u, Cia402.u16 & CIA402_CW_QUICK_STOP);

    // Disable Operation drops back to the Switched On word.
    Cia402.controlword_args.cmd = CIA402_COMMAND_DISABLE_OPERATION;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0007u, Cia402.u16);
    ...
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(CIA402_CW_FAULT_RESET, Cia402.u16);
    Cia402.controlword_args.cmd = CIA402_COMMAND_FAULT_RESET;
    Cia402.controlword(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0080u, Cia402.u16);
}
...
{
    Cia402.enable_sequence_args.state = CIA402_STATE_SWITCH_ON_DISABLED;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0006u, Cia402.u16);
    Cia402.enable_sequence_args.state = CIA402_STATE_READY_TO_SWITCH_ON;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0007u, Cia402.u16);
    Cia402.enable_sequence_args.state = CIA402_STATE_SWITCHED_ON;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x000Fu, Cia402.u16);
    Cia402.enable_sequence_args.state = CIA402_STATE_OPERATION_ENABLED;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x000Fu, Cia402.u16);
    Cia402.enable_sequence_args.state = CIA402_STATE_QUICK_STOP_ACTIVE;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x000Fu, Cia402.u16);

    // A fault must be reset before the sequence can restart.
    Cia402.enable_sequence_args.state = CIA402_STATE_FAULT;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0080u, Cia402.u16);
    Cia402.enable_sequence_args.state = CIA402_STATE_FAULT_REACTION_ACTIVE;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0080u, Cia402.u16);

    // Nothing is commanded until the drive has left Not Ready To Switch On.
    Cia402.enable_sequence_args.state = CIA402_STATE_NOT_READY_TO_SWITCH_ON;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0000u, Cia402.u16);
    Cia402.enable_sequence_args.state = CIA402_STATE_UNKNOWN;
    Cia402.enable_sequence(cia402_work);
    TEST_ASSERT_EQUAL_HEX16(0x0000u, Cia402.u16);
    ...
    {
        Cia402.state_args.statusword = WALK[i];
        Cia402.state(cia402_work);
        Cia402.enable_sequence_args.state = Cia402.value;
        Cia402.enable_sequence(cia402_work);
        TEST_ASSERT_EQUAL_HEX16(WANT[i], Cia402.u16);
    }
}
...
void test_sdo_setters_target_the_right_objects(void)
{
    ...
    Cia402.sdo_set_controlword_args.controlword = 0x000Fu;
    Cia402.sdo_set_controlword(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    ...
    Cia402.sdo_set_mode_args.out = &f;
    Cia402.sdo_set_mode_args.node = 5;
    Cia402.sdo_set_mode_args.mode = CIA402_MODE_CYCLIC_SYNC_POSITION;
    Cia402.sdo_set_mode(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    ...
    Cia402.sdo_set_target_position_args.out = &f;
    Cia402.sdo_set_target_position_args.node = 5;
    Cia402.sdo_set_target_position_args.position = -1000;
    Cia402.sdo_set_target_position(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    static const uint8_t POS[8] = {0x23, 0x7A, 0x60, 0x00, 0x18, 0xFC, 0xFF, 0xFF};
    TEST_ASSERT_EQUAL_HEX8_ARRAY(POS, f.data, 8);

    // 100000 = 0x000186A0
    Cia402.sdo_set_target_velocity_args.out = &f;
    Cia402.sdo_set_target_velocity_args.node = 5;
    Cia402.sdo_set_target_velocity_args.velocity = 100000;
    Cia402.sdo_set_target_velocity(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    ...
    Cia402.sdo_set_target_torque_args.out = &f;
    Cia402.sdo_set_target_torque_args.node = 5;
    Cia402.sdo_set_target_torque_args.torque = -500;
    Cia402.sdo_set_target_torque(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    ...
    Cia402.sdo_read_args.out = &f;
    Cia402.sdo_read_args.node = 5;
    Cia402.sdo_read_args.index = CIA402_OD_STATUSWORD;
    Cia402.sdo_read_args.sub = 0;
    Cia402.sdo_read(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    ...
    Cia402.sdo_set_controlword_args.out = &f;
    Cia402.sdo_set_controlword_args.node = 0;
    Cia402.sdo_set_controlword_args.controlword = 0x000Fu;
    Cia402.sdo_set_controlword(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    Cia402.sdo_set_controlword_args.out = &f;
    Cia402.sdo_set_controlword_args.node = 128;
    Cia402.sdo_set_controlword_args.controlword = 0x000Fu;
    Cia402.sdo_set_controlword(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    Cia402.sdo_read_args.out = &f;
    Cia402.sdo_read_args.node = 0;
    Cia402.sdo_read_args.index = CIA402_OD_STATUSWORD;
    Cia402.sdo_read_args.sub = 0;
    Cia402.sdo_read(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
...
void test_sdo_get_u16_checks_the_index(void)
{
    ...
    uint16_t value = 0;
    Cia402.sdo_get_u16_args.f = &f;
    Cia402.sdo_get_u16_args.want_index = CIA402_OD_STATUSWORD;
    Cia402.sdo_get_u16_args.value = &value;
    Cia402.sdo_get_u16(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    TEST_ASSERT_EQUAL_HEX16(0x0637u, value);
    Cia402.state_args.statusword = value;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_OPERATION_ENABLED, Cia402.value);
    ...
    value = 0;
    Cia402.sdo_get_u16_args.f = &f;
    Cia402.sdo_get_u16_args.want_index = 0;
    Cia402.sdo_get_u16_args.value = &value;
    Cia402.sdo_get_u16(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    TEST_ASSERT_EQUAL_HEX16(0x0637u, value);
    ...
    Cia402.sdo_get_u16_args.f = &f;
    Cia402.sdo_get_u16_args.want_index = CIA402_OD_ERROR_CODE;
    Cia402.sdo_get_u16_args.value = &value;
    Cia402.sdo_get_u16(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    Cia402.sdo_get_u16_args.f = &f;
    Cia402.sdo_get_u16_args.want_index = CIA402_OD_STATUSWORD;
    Cia402.sdo_get_u16_args.value = NULL;
    Cia402.sdo_get_u16(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    ...
    Cia402.sdo_get_u16_args.f = &abort_frame;
    Cia402.sdo_get_u16_args.want_index = CIA402_OD_STATUSWORD;
    Cia402.sdo_get_u16_args.value = &value;
    Cia402.sdo_get_u16(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    ...
    Cia402.sdo_get_u16_args.f = &narrow;
    Cia402.sdo_get_u16_args.want_index = CIA402_OD_STATUSWORD;
    Cia402.sdo_get_u16_args.value = &value;
    Cia402.sdo_get_u16(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
...
void test_sdo_get_i32_is_signed(void)
{
    ...
    int32_t value = 0;
    Cia402.sdo_get_i32_args.f = &f;
    Cia402.sdo_get_i32_args.want_index = CIA402_OD_POSITION_ACTUAL;
    Cia402.sdo_get_i32_args.value = &value;
    Cia402.sdo_get_i32(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    ...
    Cia402.sdo_get_i32_args.f = &f;
    Cia402.sdo_get_i32_args.want_index = CIA402_OD_VELOCITY_ACTUAL;
    Cia402.sdo_get_i32_args.value = &value;
    Cia402.sdo_get_i32(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    ...
    TEST_ASSERT_FALSE(Cia402.ok);
    Cia402.sdo_get_i32_args.f = &f;
    Cia402.sdo_get_i32_args.want_index = 0;
    Cia402.sdo_get_i32_args.value = NULL;
    Cia402.sdo_get_i32(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    ...
    Cia402.sdo_get_i32_args.f = &narrow;
    Cia402.sdo_get_i32_args.want_index = CIA402_OD_POSITION_ACTUAL;
    Cia402.sdo_get_i32_args.value = &value;
    Cia402.sdo_get_i32(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
...
void test_pdo_pack_and_unpack_round_trip(void)
{
    ...
    Cia402.pack_command_args.buf = buf;
    Cia402.pack_command_args.cap = sizeof(buf);
    Cia402.pack_command_args.controlword = 0x000Fu;
    Cia402.pack_command_args.target = -1000;
    Cia402.pack_command(cia402_work);
    size_t n = Cia402.n;
    ...
    uint16_t sw = 0;
    int32_t actual = 0;
    Cia402.unpack_status_args.buf = buf;
    Cia402.unpack_status_args.len = n;
    Cia402.unpack_status_args.statusword = &sw;
    Cia402.unpack_status_args.actual = &actual;
    Cia402.unpack_status(cia402_work);
    TEST_ASSERT_TRUE(Cia402.ok);
    ...
    Cia402.state_args.statusword = sw;
    Cia402.state(cia402_work);
    TEST_ASSERT_EQUAL_INT(CIA402_STATE_OPERATION_ENABLED, Cia402.value);
    TEST_ASSERT_TRUE(protocore_cia402_target_reached(sw));

    Cia402.pack_command_args.buf = buf;
    Cia402.pack_command_args.cap = 5;
    Cia402.pack_command_args.controlword = 0x000Fu;
    Cia402.pack_command_args.target = 0;
    Cia402.pack_command(cia402_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cia402.n);
    Cia402.pack_command_args.buf = NULL;
    Cia402.pack_command_args.cap = sizeof(buf);
    Cia402.pack_command_args.controlword = 0x000Fu;
    Cia402.pack_command_args.target = 0;
    Cia402.pack_command(cia402_work);
    TEST_ASSERT_EQUAL_size_t(0u, Cia402.n);
    Cia402.unpack_status_args.buf = TPDO;
    Cia402.unpack_status_args.len = 5;
    Cia402.unpack_status_args.statusword = &sw;
    Cia402.unpack_status_args.actual = &actual;
    Cia402.unpack_status(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    Cia402.unpack_status_args.buf = NULL;
    Cia402.unpack_status_args.len = 6;
    Cia402.unpack_status_args.statusword = &sw;
    Cia402.unpack_status_args.actual = &actual;
    Cia402.unpack_status(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    Cia402.unpack_status_args.buf = TPDO;
    Cia402.unpack_status_args.len = 6;
    Cia402.unpack_status_args.statusword = NULL;
    Cia402.unpack_status_args.actual = &actual;
    Cia402.unpack_status(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
    Cia402.unpack_status_args.buf = TPDO;
    Cia402.unpack_status_args.len = 6;
    Cia402.unpack_status_args.statusword = &sw;
    Cia402.unpack_status_args.actual = NULL;
    Cia402.unpack_status(cia402_work);
    TEST_ASSERT_FALSE(Cia402.ok);
}
