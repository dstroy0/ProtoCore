...
void test_published_x_attach_block_check_codes(void)
{
    ...
    SnpV.bcc_args.bytes = X_ATTACH_REQUEST;
    SnpV.bcc_args.len = sizeof(X_ATTACH_REQUEST);
    Snp.bcc(snp_work);
    TEST_ASSERT_EQUAL_HEX8(0xB2, SnpV.value);
    SnpV.bcc_args.bytes = X_ATTACH_RESPONSE;
    SnpV.bcc_args.len = sizeof(X_ATTACH_RESPONSE);
    Snp.bcc(snp_work);
    TEST_ASSERT_EQUAL_HEX8(0xA2, SnpV.value);
    SnpV.bcc_args.bytes = X_ATTACH_BROADCAST;
    SnpV.bcc_args.len = sizeof(X_ATTACH_BROADCAST);
    Snp.bcc(snp_work);
    TEST_ASSERT_EQUAL_HEX8(0x79, SnpV.value);
}
...
void test_rotate_wraps_the_top_bit_into_the_bottom(void)
{
    ...
    SnpV.bcc_args.bytes = ONE;
    SnpV.bcc_args.len = sizeof(ONE);
    Snp.bcc(snp_work);
    TEST_ASSERT_EQUAL_HEX8(0x01, SnpV.value);
    ...
    SnpV.bcc_args.bytes = ABC;
    SnpV.bcc_args.len = sizeof(ABC);
    Snp.bcc(snp_work);
    TEST_ASSERT_EQUAL_HEX8(0x85, SnpV.value);
}

// The seed is zero, so an empty range checks to zero.
void test_empty_range_is_the_seed(void)
{
    SnpV.bcc_args.bytes = NULL;
    SnpV.bcc_args.len = 0;
    Snp.bcc(snp_work);
    TEST_ASSERT_EQUAL_HEX8(0x00, SnpV.value);
}
...
void test_byte_order_changes_the_code(void)
{
    ...
    SnpV.bcc_args.bytes = A;
    SnpV.bcc_args.len = sizeof(A);
    Snp.bcc(snp_work);
    const uint8_t code_a = SnpV.value;

    SnpV.bcc_args.bytes = B;
    SnpV.bcc_args.len = sizeof(B);
    Snp.bcc(snp_work);
    TEST_ASSERT_NOT_EQUAL(code_a, SnpV.value);
}
...
void test_frame_layout(void)
{
    ...
    uint8_t out[16];
    SnpV.build_args.control = SNP_SOH;
    SnpV.build_args.data = DATA;
    SnpV.build_args.data_len = sizeof(DATA);
    SnpV.build_args.out = out;
    SnpV.build_args.cap = sizeof(out);
    Snp.build(snp_work);
    size_t n = SnpV.n;
    ...
    SnpV.bcc_args.len = 5;
    Snp.bcc(snp_work);
    TEST_ASSERT_EQUAL_HEX8(SnpV.value, out[5]);
}
...
void test_round_trip(void)
{
    ...
    {
        SnpFrame f;
        SnpV.build_args.control = SNP_ENQ;
        SnpV.build_args.data = LENS[k] ? data : NULL;
        SnpV.build_args.data_len = LENS[k];
        SnpV.build_args.out = frame;
        SnpV.build_args.cap = sizeof(frame);
        Snp.build(snp_work);
        size_t n = SnpV.n;
        TEST_ASSERT_EQUAL_UINT(LENS[k] + 3u, n);
        SnpV.parse_args.frame = frame;
        SnpV.parse_args.len = n;
        SnpV.parse_args.out = &f;
        Snp.parse(snp_work);
        TEST_ASSERT_TRUE(SnpV.ok);
...
void test_any_single_bit_flip_is_refused(void)
{
    ...
    SnpV.build_args.data = DATA;
    SnpV.build_args.data_len = sizeof(DATA);
    SnpV.build_args.out = good;
    SnpV.build_args.cap = sizeof(good);
    Snp.build(snp_work);
    size_t n = SnpV.n;
    for (size_t i = 0; i < n; i++)
    {
        for (int bit = 0; bit < 8; bit++)
        {
            ...
            SnpV.parse_args.frame = bad;
            SnpV.parse_args.len = n;
            SnpV.parse_args.out = &f;
            Snp.parse(snp_work);
            TEST_ASSERT_FALSE(SnpV.ok);
        }
    }
}
...
void test_trailing_bytes_are_ignored(void)
{
    ...
    uint8_t frame[16];
    SnpFrame f;
    SnpV.build_args.control = SNP_ACK;
    SnpV.build_args.data = DATA;
    SnpV.build_args.data_len = sizeof(DATA);
    SnpV.build_args.out = frame;
    SnpV.build_args.cap = sizeof(frame);
...
void test_builder_refuses_bad_arguments(void)
{
    ...
    SnpV.build_args.data_len = sizeof(DATA);
    SnpV.build_args.out = out;
    SnpV.build_args.cap = 6;
    Snp.build(snp_work);
    TEST_ASSERT_EQUAL_UINT(6u, SnpV.n);
    SnpV.build_args.control = SNP_SOH;
    SnpV.build_args.data = DATA;
    SnpV.build_args.data_len = sizeof(DATA);
    SnpV.build_args.out = out;
    SnpV.build_args.cap = 5;
    Snp.build(snp_work);
    TEST_ASSERT_EQUAL_UINT(0u, SnpV.n);
    SnpV.build_args.control = SNP_SOH;
    SnpV.build_args.data = DATA;
    SnpV.build_args.data_len = sizeof(DATA);
    SnpV.build_args.out = NULL;
    SnpV.build_args.cap = 16;
    Snp.build(snp_work);
    TEST_ASSERT_EQUAL_UINT(0u, SnpV.n);
    SnpV.build_args.control = SNP_SOH;
    SnpV.build_args.data = NULL;
    SnpV.build_args.data_len = 3;
    SnpV.build_args.out = out;
    SnpV.build_args.cap = 16;
    Snp.build(snp_work);
    TEST_ASSERT_EQUAL_UINT(0u, SnpV.n);
    SnpV.build_args.control = SNP_SOH;
    SnpV.build_args.data = big;
    SnpV.build_args.data_len = 256;
    SnpV.build_args.out = out;
    SnpV.build_args.cap = sizeof(out);
    Snp.build(snp_work);
    TEST_ASSERT_EQUAL_UINT(0u, SnpV.n);
...
void test_parser_refuses_bad_arguments(void)
{
    uint8_t frame[8];
    SnpFrame f;
    SnpV.build_args.control = SNP_EOT;
    SnpV.build_args.data = NULL;
    SnpV.build_args.data_len = 0;
    SnpV.build_args.out = frame;
    SnpV.build_args.cap = sizeof(frame);
    Snp.build(snp_work);
    size_t n = SnpV.n;
    TEST_ASSERT_EQUAL_UINT(3u, n);
    SnpV.parse_args.frame = NULL;
    SnpV.parse_args.len = n;
    SnpV.parse_args.out = &f;
    Snp.parse(snp_work);
    TEST_ASSERT_FALSE(SnpV.ok);
    SnpV.parse_args.frame = frame;
    SnpV.parse_args.len = n;
    SnpV.parse_args.out = NULL;
    Snp.parse(snp_work);
    TEST_ASSERT_FALSE(SnpV.ok);
    SnpV.parse_args.frame = frame;
    SnpV.parse_args.len = 2;
    SnpV.parse_args.out = &f;
    Snp.parse(snp_work);
    TEST_ASSERT_FALSE(SnpV.ok);
}
