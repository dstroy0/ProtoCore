...
void setUp(void)
{
    ...
    HttpClientV.target.host = g_host;
    HttpClientV.target.host_cap = sizeof(g_host);
    HttpClientV.target.path = g_path;
    HttpClientV.target.path_cap = sizeof(g_path);
}
...
static void parse(const char *text)
{
    ...
    memcpy(g_msg, text, n);
    HttpClientV.message.buf = g_msg;
    HttpClientV.message.len = n;
    HttpClient.parse_response(http_client_work());
}

static void assert_body(const char *want)
{
    TEST_ASSERT_EQUAL_size_t(strlen(want), HttpClientV.body_len);
    TEST_ASSERT_EQUAL_MEMORY(want, g_msg + HttpClientV.body_off, HttpClientV.body_len);
}
...
void test_target_uri_split(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT16(80u, HttpClientV.target.port);
    TEST_ASSERT_FALSE(HttpClientV.target.https);

    TEST_ASSERT_TRUE(split("https://example.com/path"));
    TEST_ASSERT_EQUAL_UINT16(443u, HttpClientV.target.port);
    TEST_ASSERT_TRUE(HttpClientV.target.https);
    ...
    TEST_ASSERT_EQUAL_UINT16(8080u, HttpClientV.target.port);
    ...
    TEST_ASSERT_EQUAL_UINT16(1u, HttpClientV.target.port);
    TEST_ASSERT_TRUE(split("http://h:65535/"));
    TEST_ASSERT_EQUAL_UINT16(65535u, HttpClientV.target.port);
}
...
void test_get_request_message(void)
{
    ...
    HttpClientV.request.out = g_req;
    HttpClientV.request.cap = sizeof(g_req);
    ...
    static const char WANT[] = "GET /path?q=1 HTTP/1.1\r\n"
                               ...
                               "Connection: close\r\n"
                               "\r\n";
    TEST_ASSERT_EQUAL_size_t(sizeof(WANT) - 1, HttpClientV.n);
    TEST_ASSERT_EQUAL_MEMORY(WANT, g_req, HttpClientV.n);
...
void test_post_request_message(void)
{
    ...
    HttpClientV.request.content_type = "text/plain";
    HttpClientV.request.body = BODY;
    HttpClientV.request.body_len = 5;
    HttpClientV.request.out = g_req;
    HttpClientV.request.cap = sizeof(g_req);
    HttpClient.build_request(http_client_work());
...
void test_post_defaults_the_content_type(void)
{
    ...
    HttpClientV.request.body = BODY;
    HttpClientV.request.body_len = sizeof(BODY);
    HttpClientV.request.out = g_req;
    HttpClientV.request.cap = sizeof(g_req);
    HttpClient.build_request(http_client_work());

    TEST_ASSERT_TRUE(HttpClientV.n > 0);
    ...
    TEST_ASSERT_EQUAL_MEMORY(BODY, g_req + HttpClientV.n - sizeof(BODY), sizeof(BODY));
}
...
void test_build_refuses_a_short_buffer(void)
{
    ...
    HttpClientV.request.cap = sizeof(g_req);
    HttpClient.build_request(http_client_work());
    size_t full = HttpClientV.n;
    TEST_ASSERT_TRUE(full > 0);

    HttpClientV.request.cap = full; // one octet short of the message plus its NUL
    HttpClient.build_request(http_client_work());
    TEST_ASSERT_EQUAL_size_t(0u, HttpClientV.n);
    ...
    HttpClientV.request.cap = sizeof(g_req);
    HttpClient.build_request(http_client_work());
    size_t post_full = HttpClientV.n;
    TEST_ASSERT_TRUE(post_full > 5);

    HttpClientV.request.cap = post_full - 1;
    HttpClient.build_request(http_client_work());
    TEST_ASSERT_EQUAL_size_t(0u, HttpClientV.n);

    HttpClientV.request.out = NULL;
    HttpClientV.request.cap = sizeof(g_req);
    HttpClient.build_request(http_client_work());
    TEST_ASSERT_EQUAL_size_t(0u, HttpClientV.n);
    HttpClientV.request.out = g_req;
    HttpClientV.request.method = NULL;
    HttpClient.build_request(http_client_work());
    TEST_ASSERT_EQUAL_size_t(0u, HttpClientV.n);
}
...
void test_status_line(void)
{
    ...
    TEST_ASSERT_EQUAL_INT32(200, HttpClientV.status);

    parse("HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\n\r\n");
    TEST_ASSERT_EQUAL_INT32(404, HttpClientV.status);

    // RFC 9110 sec 15: the range is 100..599, at both ends
    parse("HTTP/1.1 100 Continue\r\n\r\n");
    TEST_ASSERT_EQUAL_INT32(100, HttpClientV.status);
    parse("HTTP/1.1 599 Whatever\r\n\r\n");
    TEST_ASSERT_EQUAL_INT32(599, HttpClientV.status);

    // HTTP/1.0 is still an HTTP-version this parser reads
    parse("HTTP/1.0 204 No Content\r\n\r\n");
    TEST_ASSERT_EQUAL_INT32(204, HttpClientV.status);

    // an empty reason-phrase is allowed by the grammar
    parse("HTTP/1.1 301 \r\nLocation: /x\r\n\r\n");
    TEST_ASSERT_EQUAL_INT32(301, HttpClientV.status);
}
...
void test_malformed_responses_are_refused(void)
{
    ...
    {
        parse(BAD[i]);
        TEST_ASSERT_EQUAL_INT32_MESSAGE((int32_t)HTTP_CLIENT_ERR_RESPONSE, HttpClientV.status, BAD[i]);
        TEST_ASSERT_EQUAL_size_t(0u, HttpClientV.body_len);
    }

    HttpClientV.message.buf = NULL;
    HttpClientV.message.len = 64;
    HttpClient.parse_response(http_client_work());
    TEST_ASSERT_EQUAL_INT32((int32_t)HTTP_CLIENT_ERR_RESPONSE, HttpClientV.status);
}
...
void test_body_offset_is_past_the_field_section(void)
{
    ...
    parse(MSG);
    TEST_ASSERT_EQUAL_size_t(strlen(MSG) - 3, HttpClientV.body_off);
    assert_body("abc");
}
