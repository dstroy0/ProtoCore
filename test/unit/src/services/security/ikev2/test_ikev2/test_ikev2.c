...
void test_rfc7296_ike_header_layout(void)
{
    ...
    memcpy(IkeV.hdr.init_spi, SPI_I, 8);
    memcpy(IkeV.hdr.resp_spi, SPI_R, 8);
    IkeV.hdr.next_payload = IKE_PL_SA;
    IkeV.hdr.version = PROTOCORE_IKE_VERSION;
    IkeV.hdr.exchange = IKE_SA_INIT;
    IkeV.hdr.flags = PROTOCORE_IKE_FLAG_INITIATOR;
    IkeV.hdr.message_id = 0;
    IkeV.hdr.length = 0x0000012Cu; // 300
    IkeV.out.buf = g_out;
    IkeV.out.cap = sizeof(g_out);
    Ike.hdr_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(28, IkeV.n);
    ...
    memset(&IkeV.hdr, 0, sizeof(IkeV.hdr));
    IkeV.wire.msg = WANT;
    IkeV.wire.len = sizeof(WANT);
    Ike.hdr_parse(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(SPI_I, IkeV.hdr.init_spi, 8);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(SPI_R, IkeV.hdr.resp_spi, 8);
    TEST_ASSERT_EQUAL_INT(IKE_PL_SA, IkeV.hdr.next_payload);
    TEST_ASSERT_EQUAL_HEX8(0x20, IkeV.hdr.version);
    TEST_ASSERT_EQUAL_INT(IKE_SA_INIT, IkeV.hdr.exchange);
    TEST_ASSERT_EQUAL_HEX8(PROTOCORE_IKE_FLAG_INITIATOR, IkeV.hdr.flags);
    TEST_ASSERT_EQUAL_UINT32(0u, IkeV.hdr.message_id);
    TEST_ASSERT_EQUAL_UINT32(300u, IkeV.hdr.length);
...
void test_rfc7296_generic_payload_header(void)
{
    ...
    IkeV.pl.critical = PROTO_FALSE;
    IkeV.pl.data = BODY;
    IkeV.pl.data_len = sizeof(BODY);
    Ike.payload_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(10, IkeV.n); // 4 + 6
    ...
    IkeV.pl.data_len = 0;
    Ike.payload_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(4, IkeV.n);
    ...
    IkeV.out.cap = 9;
    Ike.payload_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(0, IkeV.n);
    IkeV.out.cap = sizeof(g_out);
    IkeV.out.buf = NULL;
    Ike.payload_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(0, IkeV.n);
...
void test_payload_chain_is_walked_forward(void)
{
    ...
    IkePayloadIter it;
    IkeV.walk.chain = &it;
    IkeV.walk.first_type = IKE_PL_NONCE;
    IkeV.wire.msg = AREA;
    IkeV.wire.len = sizeof(AREA);
    Ike.payload_iter_init(ikev2_work);

    Ike.payload_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_PL_NONCE, IkeV.payload.type);
    TEST_ASSERT_EQUAL_INT(IKE_PL_NOTIFY, IkeV.payload.next_payload);
    TEST_ASSERT_FALSE(IkeV.payload.critical);
    TEST_ASSERT_EQUAL_size_t(4, IkeV.payload.body_len); // Payload Length counts the header
    TEST_ASSERT_EQUAL_HEX8(0x01, IkeV.payload.body[0]);

    Ike.payload_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_PL_NOTIFY, IkeV.payload.type);
    TEST_ASSERT_EQUAL_INT(IKE_PL_DELETE, IkeV.payload.next_payload);
    TEST_ASSERT_EQUAL_size_t(8, IkeV.payload.body_len);

    Ike.payload_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_PL_DELETE, IkeV.payload.type);
    TEST_ASSERT_EQUAL_INT(IKE_PL_NONE, IkeV.payload.next_payload);
...
void test_rfc7296_sa_proposal_transform_tree(void)
{
    ...
    IkeV.prop.transforms = TRANSFORMS;
    IkeV.prop.num_transforms = 3;
    ...
    TEST_ASSERT_EQUAL_size_t(40, IkeV.n);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(WANT, g_out, 40);

    // The same octets read back as the same tree.
    IkeV.wire.msg = g_out + PROTOCORE_IKE_PAYLOAD_HDR_LEN;
    IkeV.wire.len = IkeV.n - PROTOCORE_IKE_PAYLOAD_HDR_LEN;
    Ike.sa_first_proposal(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_TRUE(IkeV.proposal.last);
    TEST_ASSERT_EQUAL_UINT8(1, IkeV.proposal.proposal_num);
    TEST_ASSERT_EQUAL_INT(IKE_PROTO_IKE, IkeV.proposal.protocol_id);
    TEST_ASSERT_EQUAL_UINT8(0, IkeV.proposal.spi_size);
    TEST_ASSERT_EQUAL_UINT8(3, IkeV.proposal.num_transforms);

    IkeProposalRef prop = IkeV.proposal;
    IkeTransformIter tit;
    IkeV.walk.transforms = &tit;
    IkeV.walk.proposal = &prop;
    Ike.transform_iter_init(ikev2_work);

    Ike.transform_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_TRANSFORM_ENCR, IkeV.transform.type);
    TEST_ASSERT_EQUAL_UINT16(IKE_ENCR_AES_GCM_16, IkeV.transform.id);
    TEST_ASSERT_EQUAL_INT32(256, IkeV.transform.key_length);
    TEST_ASSERT_FALSE(IkeV.transform.last);

    Ike.transform_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_TRANSFORM_PRF, IkeV.transform.type);
    TEST_ASSERT_EQUAL_UINT16(IKE_PRF_HMAC_SHA2_256, IkeV.transform.id);
    TEST_ASSERT_EQUAL_INT32(-1, IkeV.transform.key_length); // absent, not zero

    Ike.transform_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_TRANSFORM_DH, IkeV.transform.type);
    TEST_ASSERT_EQUAL_UINT16(IKE_DH_CURVE25519, IkeV.transform.id);
    TEST_ASSERT_TRUE(IkeV.transform.last);
...
void test_sa_proposal_with_an_spi(void)
{
    ...
    IkeV.prop.proposal_num = 1;
    IkeV.prop.protocol_id = IKE_PROTO_ESP;
    IkeV.prop.spi = ESP_SPI;
    IkeV.prop.spi_size = 4;
    IkeV.prop.transforms = ONE;
    IkeV.prop.num_transforms = 1;
    Ike.sa_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(4 + 8 + 4 + 12, IkeV.n);
    ...
    IkeV.wire.msg = g_out + 4;
    IkeV.wire.len = IkeV.n - 4;
    Ike.sa_first_proposal(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_UINT8(4, IkeV.proposal.spi_size);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(ESP_SPI, IkeV.proposal.spi, 4);
    TEST_ASSERT_EQUAL_INT(IKE_PROTO_ESP, IkeV.proposal.protocol_id);
...
void test_rfc7296_ke_payload(void)
{
    ...
    IkeV.ke.dh_group = IKE_DH_CURVE25519;
    Ike.ke_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(4 + 4 + 32, IkeV.n);
    ...
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_UINT16(IKE_DH_CURVE25519, IkeV.ke_ref.dh_group);
    TEST_ASSERT_EQUAL_size_t(32, IkeV.ke_ref.ke_len);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(KE_DATA, IkeV.ke_ref.ke_data, 32);

    // A body with no room for the Group Num and RESERVED is not a KE payload.
    static const uint8_t SHORT[3] = {0, 31, 0};
    IkeV.wire.msg = SHORT;
    IkeV.wire.len = sizeof(SHORT);
    Ike.ke_parse(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
}
...
void test_rfc7296_nonce_id_and_auth_payloads(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(20, IkeV.n);
    ...
    IkeV.pl.next_payload = IKE_PL_AUTH;
    IkeV.pl.data = (const uint8_t *)EMAIL;
    IkeV.pl.data_len = sizeof(EMAIL) - 1;
    IkeV.id.id_type = IKE_ID_RFC822_ADDR;
    Ike.id_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(4 + 4 + 15, IkeV.n);
    ...
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_ID_RFC822_ADDR, IkeV.id_ref.id_type);
    TEST_ASSERT_EQUAL_size_t(15, IkeV.id_ref.id_len);
    ...
    IkeV.pl.next_payload = IKE_PL_NONE;
    IkeV.pl.data = AUTH_DATA;
    IkeV.pl.data_len = sizeof(AUTH_DATA);
    IkeV.auth.auth_method = IKE_AUTH_PSK;
    ...
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_AUTH_PSK, IkeV.auth_ref.auth_method);
    TEST_ASSERT_EQUAL_size_t(32, IkeV.auth_ref.auth_len);
    ...
    Ike.id_parse(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_ID_RESERVED, IkeV.id_ref.id_type);
    Ike.auth_parse(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_AUTH_RESERVED, IkeV.auth_ref.auth_method);
}
...
void test_rfc7296_notify_payload(void)
{
    ...
    IkeV.pl.next_payload = IKE_PL_NONE;
    IkeV.pl.data = DATA;
    IkeV.pl.data_len = sizeof(DATA);
    IkeV.prop.protocol_id = IKE_PROTO_ESP;
    IkeV.prop.spi = ESP_SPI;
    IkeV.prop.spi_size = 4;
    IkeV.notify.notify_type = PROTOCORE_IKE_N_FRAGMENTATION_SUPPORTED;
    Ike.notify_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(4 + 4 + 4 + 3, IkeV.n);
    ...
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_PROTO_ESP, IkeV.notify_ref.protocol_id);
    TEST_ASSERT_EQUAL_UINT8(4, IkeV.notify_ref.spi_size);
    TEST_ASSERT_EQUAL_UINT16(16430, IkeV.notify_ref.notify_type);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(ESP_SPI, IkeV.notify_ref.spi, 4);
    TEST_ASSERT_EQUAL_size_t(3, IkeV.notify_ref.data_len);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(DATA, IkeV.notify_ref.data, 3);
    ...
    IkeV.pl.data_len = 0;
    Ike.notify_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(8, IkeV.n);
    IkeV.wire.msg = g_out + 4;
    IkeV.wire.len = 4;
    Ike.notify_parse(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_NULL(IkeV.notify_ref.spi);
    TEST_ASSERT_EQUAL_size_t(0, IkeV.notify_ref.data_len);

    // An SPI Size the body cannot hold is malformed.
    static const uint8_t LIES[4] = {0x01, 0x08, 0x40, 0x06}; // SPI Size 8, body is 4
    IkeV.wire.msg = LIES;
    IkeV.wire.len = sizeof(LIES);
    Ike.notify_parse(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
}
...
void test_rfc7296_delete_payload(void)
{
    ...
    IkeV.prop.protocol_id = IKE_PROTO_ESP;
    IkeV.prop.spi_size = 4;
    IkeV.prop.num_spis = 2;
    Ike.delete_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(4 + 4 + 8, IkeV.n);
    ...
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_PROTO_ESP, IkeV.delete_ref.protocol_id);
    TEST_ASSERT_EQUAL_UINT8(4, IkeV.delete_ref.spi_size);
    TEST_ASSERT_EQUAL_UINT16(2, IkeV.delete_ref.num_spis);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(SPIS, IkeV.delete_ref.spis, 8);

    // Deleting the IKE SA itself names no SPI (sec 3.11).
    IkeV.prop.protocol_id = IKE_PROTO_IKE;
    IkeV.prop.spi_size = 0;
    IkeV.prop.num_spis = 0;
    Ike.delete_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(8, IkeV.n);
    IkeV.wire.msg = g_out + 4;
    IkeV.wire.len = 4;
    Ike.delete_parse(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_NULL(IkeV.delete_ref.spis);
...
void test_rfc7296_traffic_selectors(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(4 + 4 + 16 + 40, IkeV.n);
    ...
    TEST_ASSERT_EQUAL_UINT8(2, IkeV.u8);

    IkeV.ts.index = 0;
    Ike.ts_get(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_TS_IPV4_ADDR_RANGE, IkeV.sel.ts_type);
    TEST_ASSERT_EQUAL_UINT8(6, IkeV.sel.ip_protocol);
    TEST_ASSERT_EQUAL_UINT16(0, IkeV.sel.start_port);
    TEST_ASSERT_EQUAL_UINT16(65535, IkeV.sel.end_port);
    TEST_ASSERT_EQUAL_size_t(4, IkeV.sel.addr_len);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(LO4, IkeV.sel.start_addr, 4);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(HI4, IkeV.sel.end_addr, 4);

    IkeV.ts.index = 1;
    Ike.ts_get(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_TS_IPV6_ADDR_RANGE, IkeV.sel.ts_type);
    TEST_ASSERT_EQUAL_UINT16(443, IkeV.sel.start_port);
    TEST_ASSERT_EQUAL_size_t(16, IkeV.sel.addr_len);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(HI6, IkeV.sel.end_addr, 16);

    IkeV.ts.index = 2; // past Number of TSs
    Ike.ts_get(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
...
void test_rfc7296_configuration_payload(void)
{
    ...
    IkeV.out.cap = sizeof(g_out);
    IkeV.pl.next_payload = IKE_PL_NONE;
    IkeV.cp.cfg_type = IKE_CFG_REPLY;
    IkeV.cp.attrs = ATTRS;
    IkeV.cp.num_attrs = 3;
    Ike.cp_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(4 + 4 + 8 + 8 + 4, IkeV.n);
    ...
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_INT(IKE_CFG_REPLY, IkeV.cp_ref.cfg_type);

    IkeCfgAttrIter it;
    IkeV.walk.attrs = &it;
    IkeV.wire.msg = IkeV.cp_ref.attrs;
    IkeV.wire.len = IkeV.cp_ref.attrs_len;
    Ike.cp_attr_iter_init(ikev2_work);

    Ike.cp_attr_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_UINT16(PROTOCORE_IKE_CFG_INTERNAL_IP4_ADDRESS, IkeV.attr.type);
    TEST_ASSERT_EQUAL_UINT16(4, IkeV.attr.value_len);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(IP4, IkeV.attr.value, 4);

    Ike.cp_attr_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_UINT16(PROTOCORE_IKE_CFG_INTERNAL_IP4_DNS, IkeV.attr.type);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(DNS, IkeV.attr.value, 4);

    Ike.cp_attr_next(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_UINT16(PROTOCORE_IKE_CFG_INTERNAL_IP4_NETMASK, IkeV.attr.type);
    TEST_ASSERT_EQUAL_UINT16(0, IkeV.attr.value_len);
    TEST_ASSERT_NULL(IkeV.attr.value);
...
void test_rfc5282_sk_payload_envelope(void)
{
    ...
    IkeV.out.buf = g_out;
    IkeV.out.cap = sizeof(g_out);
    IkeV.pl.next_payload = IKE_PL_IDI; // the Encrypted payload's Next Payload names the inner chain
    IkeV.sk.iv = IV;
    IkeV.sk.iv_len = sizeof(IV);
    IkeV.sk.ct = CT;
    IkeV.sk.ct_len = sizeof(CT);
    IkeV.sk.icv = ICV;
    IkeV.sk.icv_len = sizeof(ICV);
    Ike.sk_build(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(4 + 8 + 10 + 16, IkeV.n);
    ...
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(IV, IkeV.sk_ref.iv, 8);
    TEST_ASSERT_EQUAL_size_t(10, IkeV.sk_ref.ct_len);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(CT, IkeV.sk_ref.ct, 10);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(ICV, IkeV.sk_ref.icv, 16);

    // A body too short to hold the IV and the ICV is malformed.
    IkeV.wire.len = 23;
    Ike.sk_parse(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
    IkeV.wire.len = 24; // exactly IV + ICV, with no ciphertext, is still well formed
    Ike.sk_parse(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    TEST_ASSERT_EQUAL_size_t(0, IkeV.sk_ref.ct_len);
}
...
void test_rfc7748_curve25519_key_exchange(void)
{
    ...
    uint8_t pub[32];
    IkeV.ke.dh_group = IKE_DH_CURVE25519;
    IkeV.ke.our_priv = A_PRIV;
    IkeV.ke.our_priv_len = 32;
    IkeV.out.buf = pub;
    IkeV.out.cap = sizeof(pub);
    Ike.dh_public(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(32, IkeV.n);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(A_PUB, pub, 32);

    IkeV.ke.our_priv = B_PRIV;
    Ike.dh_public(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(32, IkeV.n);
    ...
    uint8_t secret[32];
    IkeV.out.buf = secret;
    IkeV.out.cap = sizeof(secret);
    IkeV.ke.our_priv = A_PRIV;
    IkeV.ke.our_priv_len = 32;
    IkeV.ke.peer_pub = B_PUB;
    IkeV.ke.peer_pub_len = 32;
...
void test_dh_refuses_other_groups_and_lengths(void)
{
    ...
    uint8_t out[32];
    IkeV.out.buf = out;
    IkeV.out.cap = sizeof(out);
    IkeV.ke.our_priv = PRIV;
    IkeV.ke.our_priv_len = 32;
    IkeV.ke.peer_pub = PUB;
    IkeV.ke.peer_pub_len = 32;

    static const uint16_t OTHER_GROUPS[3] = {IKE_DH_MODP2048, IKE_DH_ECP256, 0};
    for (size_t i = 0; i < 3; i++)
    {
        IkeV.ke.dh_group = OTHER_GROUPS[i];
        Ike.dh_public(ikev2_work);
        TEST_ASSERT_EQUAL_size_t(0, IkeV.n);
        Ike.dh_compute(ikev2_work);
        TEST_ASSERT_EQUAL_size_t(0, IkeV.n);
...
void test_rfc7296_suite_key_lengths(void)
{
    ...
    IkeV.keymat.suite = &suite;
    IkeV.keymat.lens = &lens;
...
void test_rfc7296_prf_plus_is_one_stream(void)
{
    ...
    IkeV.work = g_work;
    IkeV.keymat.prf_key = KEY;
    IkeV.keymat.prf_key_len = sizeof(KEY);
    IkeV.keymat.seed = SEED;
    IkeV.keymat.seed_len = sizeof(SEED);

    IkeV.out.buf = short_out;
    IkeV.out.cap = sizeof(short_out);
    Ike.prf_plus(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);

    IkeV.out.buf = long_out;
    IkeV.out.cap = sizeof(long_out);
    Ike.prf_plus(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    ...
    uint8_t other[32];
    IkeV.out.buf = other;
    IkeV.out.cap = sizeof(other);
    static const uint8_t SEED2[8] = {'H', 'i', ' ', 'T', 'h', 'e', 'r', 'f'};
    IkeV.keymat.seed = SEED2;
    Ike.prf_plus(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    ...
    TEST_ASSERT_FALSE(IkeV.ok);
    IkeV.out.buf = other;
    IkeV.keymat.prf_key = NULL;
    Ike.prf_plus(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
    IkeV.keymat.prf_key = KEY;
    IkeV.keymat.seed = NULL;
    Ike.prf_plus(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
}
...
void test_rfc7296_stateless_cookie(void)
{
    ...
    IkeV.work = g_work;
    IkeV.notify.version = 7;
    IkeV.notify.secret = SECRET;
    IkeV.notify.secret_len = sizeof(SECRET);
    IkeV.notify.ni = NI;
    IkeV.notify.ni_len = sizeof(NI);
    IkeV.notify.ipi = IPI;
    IkeV.notify.ipi_len = sizeof(IPI);
    IkeV.notify.spii = SPI_I;
    IkeV.out.buf = cookie;
    IkeV.out.cap = sizeof(cookie);
    Ike.cookie_compute(ikev2_work);
    TEST_ASSERT_EQUAL_size_t(33, IkeV.n);
    TEST_ASSERT_EQUAL_HEX8(7, cookie[0]); // VersionIDofSecret leads, so the secret can be rotated

    IkeV.notify.cookie = cookie;
    IkeV.notify.cookie_len = sizeof(cookie);
    ...
    IkeV.notify.ipi = OTHER_IP;
    Ike.cookie_verify(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
    IkeV.notify.ipi = IPI;

    static const uint8_t OTHER_SECRET[16] = {0xB0};
    IkeV.notify.secret = OTHER_SECRET;
    Ike.cookie_verify(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
    IkeV.notify.secret = SECRET;
    ...
    IkeV.notify.cookie = reversioned;
    IkeV.notify.cookie_len = sizeof(reversioned);
    Ike.cookie_verify(ikev2_work);
    TEST_ASSERT_TRUE(IkeV.ok);
    ...
    for (size_t i = 1; i < PROTOCORE_IKE_COOKIE_LEN; i++)
    {
        ...
        IkeV.notify.cookie = bad;
        IkeV.notify.cookie_len = sizeof(bad);
        Ike.cookie_verify(ikev2_work);
        TEST_ASSERT_FALSE(IkeV.ok);
    }
    ...
    IkeV.notify.cookie = cookie;
    IkeV.notify.cookie_len = 32;
    Ike.cookie_verify(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
    IkeV.notify.cookie = NULL;
    IkeV.notify.cookie_len = sizeof(cookie);
    Ike.cookie_verify(ikev2_work);
    TEST_ASSERT_FALSE(IkeV.ok);
    ...
    TEST_ASSERT_EQUAL_size_t(4 + 4 + 33, IkeV.n);
