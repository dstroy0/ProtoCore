...
{
    AesBlock.key_expand(g_ws, key, nk, rk);
}

static void encrypt(const uint32_t *rk, int nr, const uint8_t *in, uint8_t *out)
{
    AesBlock.encrypt_block(g_ws, rk, nr, in, out);
}
...
void test_null_operands_are_refused(void)
{
    ...
    proto_bool aes_block_ok = AesBlock.key_expand(g_ws, NULL, 4, rk);
    TEST_ASSERT_FALSE(aes_block_ok);

    aes_block_ok = AesBlock.encrypt_block(g_ws, NULL, 10, PT, out);
    TEST_ASSERT_FALSE(aes_block_ok);
