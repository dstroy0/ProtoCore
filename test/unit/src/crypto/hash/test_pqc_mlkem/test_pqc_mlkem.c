...
void test_acvp_keygen(void)
{
    ...
    protocore_mlkem768_keygen(d, z, g_ek, g_dk);
...
void test_acvp_encaps(void)
{
    ...
    TEST_ASSERT_TRUE(protocore_mlkem768_encaps(g_ek, m, g_ct, g_ss));
...
void test_acvp_decaps(void)
{
    ...
    protocore_mlkem768_decaps(g_dk, g_ct, g_ss);
    TEST_ASSERT_EQUAL_HEX8_ARRAY(g_want_ss, g_ss, MLKEM768_SS_BYTES);
}
...
void test_encaps_decaps_agree(void)
{
    ...
    for (int round = 0; round < 4; round++)
    {
        ...
        TEST_ASSERT_TRUE(protocore_mlkem768_encaps(g_ek, m, g_ct, g_ss));
        protocore_mlkem768_decaps(g_dk, g_ct, back);
...
void test_tampered_ciphertext_implicitly_rejects(void)
{
    ...
    TEST_ASSERT_TRUE(protocore_mlkem768_encaps(g_ek, m, g_ct, real));
    ...
    protocore_mlkem768_decaps(g_dk, g_ct, rej3);
    ...
    protocore_mlkem768_decaps(g_dk, g_ct, rej1); // restored: the real key comes back
...
void test_encaps_refuses_a_malformed_encapsulation_key(void)
{
    ...
    unhex(ACVP_ENCAPS_M, m);
    TEST_ASSERT_TRUE(protocore_mlkem768_encaps(g_ek, m, g_ct, g_ss)); // sanity: the real key works
    ...
    TEST_ASSERT_FALSE(protocore_mlkem768_encaps(g_ek, m, g_ct, g_ss));
...
void test_seeds_determine_the_key_pair(void)
{
    ...
    d[0] ^= 0x01;
    protocore_mlkem768_keygen(d, z, ek2, dk2);
