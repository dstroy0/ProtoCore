...
static uint32_t epoch_of(int y, int mo, int d, int h, int mi, int s)
{
    ...
    proto_bool rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), r, &e);
...
void test_twelve_hour_encoding(void)
{
    ...
    for (size_t i = 0; i < 8; i++)
    {
        ...
        proto_bool rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), r, &got);
        TEST_ASSERT_TRUE(rtc_ok);
...
void test_clock_halt_and_century_bits_are_masked(void)
{
    ...
    proto_bool rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), r, &plain);
    TEST_ASSERT_TRUE(rtc_ok);

    r[0] |= 0x80; // DS1307 CH
    r[5] |= 0x80; // DS3231 century
    rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), r, &flagged);
    TEST_ASSERT_TRUE(rtc_ok);
...
void test_out_of_range_fields_are_refused(void)
{
    ...
    proto_bool rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), r, &e);
    TEST_ASSERT_FALSE(rtc_ok);

    regs24(r, 2020, 1, 1, 0, 0, 0);
    r[1] = bcd(60); // minutes 00-59
    rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), r, &e);
    TEST_ASSERT_FALSE(rtc_ok);
    ...
    regs24(r, 2020, 1, 1, 0, 0, 0);
    rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), NULL, &e);
    TEST_ASSERT_FALSE(rtc_ok);
    rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), r, NULL);
    TEST_ASSERT_FALSE(rtc_ok);
}
...
void test_epoch_to_regs_is_bcd_and_24_hour(void)
{
    ...
    Rtc.epoch_to_regs(protocore_rtc_span(), 946684800u + 45296u, r);
    ...
    Rtc.epoch_to_regs(protocore_rtc_span(), 946684800u + 23u * 3600u, r);
...
void test_round_trip_over_the_register_range(void)
{
    ...
    for (uint32_t e = 946684800u; e < 4102444799u; e += 1234567u)
    {
        uint8_t r[RTC_REG_COUNT];
        uint32_t back = 0;
        Rtc.epoch_to_regs(protocore_rtc_span(), e, r);
        proto_bool rtc_ok = Rtc.regs_to_epoch(protocore_rtc_span(), r, &back);
        TEST_ASSERT_TRUE(rtc_ok);
