...
{
    proto_bool rcwl0516_ok = Rcwl0516.presence_update(protocore_rcwl0516_span(), &g_core, level, now);
    return rcwl0516_ok;
}
...
{
    Rcwl0516.presence_init(protocore_rcwl0516_span(), &g_core, DEBOUNCE, HOLD, 1000);
    proto_bool rcwl0516_ok = Rcwl0516.presence_get(protocore_rcwl0516_span(), &g_core);
    TEST_ASSERT_FALSE(rcwl0516_ok);
    rcwl0516_ok = Rcwl0516.presence_take_event(protocore_rcwl0516_span(), &g_core);
    TEST_ASSERT_FALSE(rcwl0516_ok);
...
{
    Rcwl0516.presence_init(protocore_rcwl0516_span(), &g_core, DEBOUNCE, HOLD, 0);
...
void test_hold_bridges_the_retrigger_gap(void)
{
    ...
    proto_bool rcwl0516_ok = Rcwl0516.presence_take_event(protocore_rcwl0516_span(), &g_core);
    TEST_ASSERT_TRUE(rcwl0516_ok);
...
void test_presence_clears_exactly_one_hold_after_the_last_high(void)
{
    ...
    rcwl0516_ok = Rcwl0516.presence_take_event(protocore_rcwl0516_span(), &g_core);
    TEST_ASSERT_FALSE(rcwl0516_ok); // no edge yet
    TEST_ASSERT_FALSE(feed(PROTO_FALSE, 100 + HOLD));
    rcwl0516_ok = Rcwl0516.presence_get(protocore_rcwl0516_span(), &g_core);
    TEST_ASSERT_FALSE(rcwl0516_ok);
    rcwl0516_ok = Rcwl0516.presence_take_event(protocore_rcwl0516_span(), &g_core);
    TEST_ASSERT_TRUE(rcwl0516_ok);
}
...
{
    Rcwl0516.presence_init(protocore_rcwl0516_span(), &g_core, 0, 0, 0);
...
{
    Rcwl0516.presence_init(protocore_rcwl0516_span(), &g_core, DEBOUNCE, 0, 0);
    TEST_ASSERT_FALSE(feed(PROTO_TRUE, 0));
    TEST_ASSERT_FALSE(feed(PROTO_TRUE, DEBOUNCE - 1));
    TEST_ASSERT_TRUE(feed(PROTO_TRUE, DEBOUNCE));
...
void test_timing_survives_the_millis_rollover(void)
{
    ...
    Rcwl0516.presence_init(protocore_rcwl0516_span(), &g_core, DEBOUNCE, HOLD, t0);
...
{
    Rcwl0516.presence_init(protocore_rcwl0516_span(), &g_core, 1, 1, 0); // deliberately wrong, then overwritten
    Rcwl0516.core_init(protocore_rcwl0516_span(), &g_core, 12345);
    ...
    proto_bool rcwl0516_ok = Rcwl0516.presence_get(protocore_rcwl0516_span(), &g_core);
    TEST_ASSERT_FALSE(rcwl0516_ok); // still fail-safe
}
...
{
    Rcwl0516.presence_init(protocore_rcwl0516_span(), NULL, DEBOUNCE, HOLD, 0); // must not fault
    proto_bool rcwl0516_ok = Rcwl0516.presence_update(protocore_rcwl0516_span(), NULL, PROTO_TRUE, 0);
    TEST_ASSERT_FALSE(rcwl0516_ok);
    rcwl0516_ok = Rcwl0516.presence_get(protocore_rcwl0516_span(), NULL);
    TEST_ASSERT_FALSE(rcwl0516_ok);
    rcwl0516_ok = Rcwl0516.presence_take_event(protocore_rcwl0516_span(), NULL);
    TEST_ASSERT_FALSE(rcwl0516_ok);
}
