...
void test_negotiation_picks_the_highest_common_version(void)
{
    ...
    TlsPolicyV.negotiate_args.client_max = TLS_VERSION_1_3;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_3;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(TLS_VERSION_1_3, TlsPolicyV.version);
    // the client tops out at 1.2, so 1.2 it is
    TlsPolicyV.negotiate_args.client_max = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_3;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(TLS_VERSION_1_2, TlsPolicyV.version);
    // the server is pinned to 1.3 only and the client offers 1.3
    TlsPolicyV.negotiate_args.client_max = TLS_VERSION_1_3;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_3;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_3;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(TLS_VERSION_1_3, TlsPolicyV.version);
    ...
    TlsPolicyV.negotiate_args.client_max = TLS_VERSION_1_3;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_2;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(TLS_VERSION_1_2, TlsPolicyV.version);
}
...
{
    TlsPolicyV.negotiate_args.client_max = 0x0305;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_3;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(TLS_VERSION_1_3, TlsPolicyV.version);
    TlsPolicyV.negotiate_args.client_max = 0x03FF;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_2;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(TLS_VERSION_1_2, TlsPolicyV.version);
...
{
    TlsPolicyV.negotiate_args.client_max = 0x0302;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_3;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.version);
    TlsPolicyV.negotiate_args.client_max = 0x0301;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_3;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.version);
    TlsPolicyV.negotiate_args.client_max = 0x0300;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_3;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.version);
    TlsPolicyV.negotiate_args.client_max = 0x0000;
    TlsPolicyV.negotiate_args.server_min = TLS_VERSION_1_2;
    TlsPolicyV.negotiate_args.server_max = TLS_VERSION_1_3;
    TlsPolicy.negotiate(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.version);
...
void test_the_negotiated_version_is_always_inside_both_ranges(void)
{
    for (uint16_t client = 0x0300; client <= 0x0308; client++)
    {
        for (uint16_t lo = TLS_VERSION_1_2; lo <= TLS_VERSION_1_3; lo++)
        {
            ...
            {
                TlsPolicyV.negotiate_args.client_max = client;
                TlsPolicyV.negotiate_args.server_min = lo;
                TlsPolicyV.negotiate_args.server_max = hi;
                TlsPolicy.negotiate(tw);
                uint16_t got = TlsPolicyV.version;
                if (got == 0)
                {
                    continue;
                }
...
{
    TlsPolicyV.name_args.version = TLS_VERSION_1_2;
    TlsPolicy.name(tw);
    TEST_ASSERT_EQUAL_STRING("TLS 1.2", TlsPolicyV.text);
    TlsPolicyV.name_args.version = TLS_VERSION_1_3;
    TlsPolicy.name(tw);
    TEST_ASSERT_EQUAL_STRING("TLS 1.3", TlsPolicyV.text);
    TlsPolicyV.name_args.version = 0x0302;
    TlsPolicy.name(tw);
    TEST_ASSERT_EQUAL_STRING("unknown", TlsPolicyV.text); // TLS 1.1
    TlsPolicyV.name_args.version = 0x0301;
    TlsPolicy.name(tw);
    TEST_ASSERT_EQUAL_STRING("unknown", TlsPolicyV.text); // TLS 1.0
    TlsPolicyV.name_args.version = 0x0000;
    TlsPolicy.name(tw);
    TEST_ASSERT_EQUAL_STRING("unknown", TlsPolicyV.text);
    TlsPolicyV.name_args.version = 0xFFFF;
    TlsPolicy.name(tw);
    TEST_ASSERT_EQUAL_STRING("unknown", TlsPolicyV.text);
}
...
void test_selection_follows_server_preference_not_client_order(void)
{
    ...
    TlsPolicyV.select_args.client_offered = CLIENT_A;
    TlsPolicyV.select_args.n_client = 3;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 3;
    TlsPolicy.select(tw);
    TEST_ASSERT_EQUAL_HEX16(0x1301, TlsPolicyV.suite);
    TlsPolicyV.select_args.client_offered = CLIENT_B;
    TlsPolicyV.select_args.n_client = 3;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 3;
    TlsPolicy.select(tw);
    ...
    TlsPolicyV.select_args.client_offered = CLIENT_C;
    TlsPolicyV.select_args.n_client = 2;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 3;
    TlsPolicy.select(tw);
    TEST_ASSERT_EQUAL_HEX16(0xC02F, TlsPolicyV.suite);

    // and only the third is left
    static const uint16_t CLIENT_D[1] = {0xCCA8};
    TlsPolicyV.select_args.client_offered = CLIENT_D;
    TlsPolicyV.select_args.n_client = 1;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 3;
    TlsPolicy.select(tw);
    TEST_ASSERT_EQUAL_HEX16(0xCCA8, TlsPolicyV.suite);
}
...
void test_no_overlap_selects_nothing(void)
{
    ...
    TlsPolicyV.select_args.client_offered = LEGACY;
    TlsPolicyV.select_args.n_client = 3;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 2;
    TlsPolicy.select(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.suite);

    // an empty list on either side is no overlap
    TlsPolicyV.select_args.client_offered = LEGACY;
    TlsPolicyV.select_args.n_client = 0;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 2;
    TlsPolicy.select(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.suite);
    ...
    TlsPolicyV.select_args.n_client = 3;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 0;
    TlsPolicy.select(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.suite);
}
...
void test_selection_refuses_a_null_list(void)
{
    ...
    TlsPolicyV.select_args.client_offered = NULL;
    TlsPolicyV.select_args.n_client = 1;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 1;
    TlsPolicy.select(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.suite);
    TlsPolicyV.select_args.client_offered = PINNED;
    TlsPolicyV.select_args.n_client = 1;
    TlsPolicyV.select_args.server_pinned = NULL;
    TlsPolicyV.select_args.n_server = 1;
    TlsPolicy.select(tw);
    TEST_ASSERT_EQUAL_HEX16(0, TlsPolicyV.suite);
    TlsPolicyV.select_args.client_offered = NULL;
...
void test_a_selected_suite_was_always_both_pinned_and_offered(void)
{
    ...
    for (unsigned cm = 0; cm < 16u; cm++)
    {
        ...
        for (unsigned sm = 0; sm < 16u; sm++)
        {
            ...
            TlsPolicyV.select_args.client_offered = client;
            TlsPolicyV.select_args.n_client = nc;
            TlsPolicyV.select_args.server_pinned = server;
            TlsPolicyV.select_args.n_server = ns;
            TlsPolicy.select(tw);
            uint16_t got = TlsPolicyV.suite;
            if (got == 0)
            {
...
void test_the_aead_suites(void)
{
    ...
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_AES_128_GCM_SHA256
    TlsPolicyV.aead_args.suite = 0x1302;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_AES_256_GCM_SHA384
    TlsPolicyV.aead_args.suite = 0x1303;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_CHACHA20_POLY1305_SHA256
    TlsPolicyV.aead_args.suite = 0xC02B;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256
    TlsPolicyV.aead_args.suite = 0xC02C;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384
    TlsPolicyV.aead_args.suite = 0xC02F;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256
    TlsPolicyV.aead_args.suite = 0xC030;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384
    TlsPolicyV.aead_args.suite = 0xCCA8;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256
    TlsPolicyV.aead_args.suite = 0xCCA9;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead); // TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256
}
...
{
    TlsPolicyV.aead_args.suite = 0x0000;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead); // TLS_NULL_WITH_NULL_NULL
    TlsPolicyV.aead_args.suite = 0x0005;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead); // TLS_RSA_WITH_RC4_128_SHA
    TlsPolicyV.aead_args.suite = 0x000A;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead); // TLS_RSA_WITH_3DES_EDE_CBC_SHA
    TlsPolicyV.aead_args.suite = 0x002F;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead); // TLS_RSA_WITH_AES_128_CBC_SHA
    TlsPolicyV.aead_args.suite = 0x0035;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead); // TLS_RSA_WITH_AES_256_CBC_SHA
    TlsPolicyV.aead_args.suite = 0x003C;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead); // TLS_RSA_WITH_AES_128_CBC_SHA256
    TlsPolicyV.aead_args.suite = 0xC013;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead); // TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA
    TlsPolicyV.aead_args.suite = 0xC027;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead); // TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256
    // a code point next to a pinned one is not pinned by proximity
    TlsPolicyV.aead_args.suite = 0xC02E;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead);
    TlsPolicyV.aead_args.suite = 0xC031;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead);
    TlsPolicyV.aead_args.suite = 0x1300;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead);
    TlsPolicyV.aead_args.suite = 0xFFFF;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_FALSE(TlsPolicyV.aead);
}
...
void test_an_aead_only_pin_can_only_select_aead(void)
{
    ...
    TlsPolicyV.select_args.client_offered = CLIENT;
    TlsPolicyV.select_args.n_client = 6;
    TlsPolicyV.select_args.server_pinned = PINNED;
    TlsPolicyV.select_args.n_server = 4;
    TlsPolicy.select(tw);
    uint16_t got = TlsPolicyV.suite;
    TEST_ASSERT_EQUAL_HEX16(0x1303, got); // the first pinned suite the client offered
    TlsPolicyV.aead_args.suite = got;
    TlsPolicy.is_aead(tw);
    TEST_ASSERT_TRUE(TlsPolicyV.aead);
}
