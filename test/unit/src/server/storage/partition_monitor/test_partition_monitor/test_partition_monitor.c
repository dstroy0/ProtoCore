...
{
    // Type 0x00 (APP).
    TEST_ASSERT_EQUAL_STRING("factory", protocore_partition_kind(0x00, 0x00));
    TEST_ASSERT_EQUAL_STRING("ota", protocore_partition_kind(0x00, 0x10));  // OTA_0
    TEST_ASSERT_EQUAL_STRING("ota", protocore_partition_kind(0x00, 0x1F));  // OTA_15
    TEST_ASSERT_EQUAL_STRING("test", protocore_partition_kind(0x00, 0x20)); // APP_TEST
    ...
    TEST_ASSERT_EQUAL_STRING("app", protocore_partition_kind(0x00, 0x0F));
    TEST_ASSERT_EQUAL_STRING("app", protocore_partition_kind(0x00, 0x21));

    // Type 0x01 (DATA).
    TEST_ASSERT_EQUAL_STRING("otadata", protocore_partition_kind(0x01, 0x00));
    TEST_ASSERT_EQUAL_STRING("phy", protocore_partition_kind(0x01, 0x01));
    TEST_ASSERT_EQUAL_STRING("nvs", protocore_partition_kind(0x01, 0x02));
    TEST_ASSERT_EQUAL_STRING("coredump", protocore_partition_kind(0x01, 0x03));
    ...
    TEST_ASSERT_EQUAL_STRING("spiffs", protocore_partition_kind(0x01, 0x82));
    TEST_ASSERT_EQUAL_STRING("littlefs", protocore_partition_kind(0x01, 0x83));

    // Registry entries this build names no kind for still report a kind rather than nothing.
    TEST_ASSERT_EQUAL_STRING("data", protocore_partition_kind(0x01, 0x05)); // EFUSE_EM
    TEST_ASSERT_EQUAL_STRING("data", protocore_partition_kind(0x01, 0x06)); // DATA_UNDEFINED
    TEST_ASSERT_EQUAL_STRING("data", protocore_partition_kind(0x01, 0x80)); // ESPHTTPD
}
...
{
    TEST_ASSERT_EQUAL_STRING("nvs", protocore_partition_kind(0x02, 0x02));
    TEST_ASSERT_EQUAL_STRING("data", protocore_partition_kind(0xFF, 0xFF));
}
...
void test_report_is_an_rfc8259_document(void)
{
    ...
    char out[512];
    int32_t n = protocore_partition_json(parts, 2, out, sizeof(out));
...
void test_an_empty_table_is_still_an_array(void)
{
    ...
    char out[64];
    int32_t n = protocore_partition_json(parts, 0, out, sizeof(out));
...
void test_numbers_span_the_whole_32_bit_range(void)
{
    ...
    char out[256];
    (void)protocore_partition_json(parts, 1, out, sizeof(out));
    TEST_ASSERT_EQUAL_STRING("{\"partitions\":["
...
void test_a_short_buffer_fails_closed(void)
{
    ...
    char full[512];
    int32_t n = protocore_partition_json(parts, 2, full, sizeof(full));
    TEST_ASSERT_TRUE(n > 0);

    // One byte short of the whole document, and short enough to fail on the opening frame.
    char tight[512];
    TEST_ASSERT_EQUAL_INT32(0, protocore_partition_json(parts, 2, tight, (uint32_t)n));
    TEST_ASSERT_EQUAL_STRING("", tight);

    char tiny[8];
    TEST_ASSERT_EQUAL_INT32(0, protocore_partition_json(parts, 2, tiny, sizeof(tiny)));
...
void test_missing_arguments_are_refused(void)
{
    ...
    char out[128];
    TEST_ASSERT_EQUAL_INT32(0, protocore_partition_json(parts, 1, NULL, sizeof(out)));
    TEST_ASSERT_EQUAL_INT32(0, protocore_partition_json(NULL, 1, out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("", out);

    char sentinel[8] = {'z', '\0'};
    TEST_ASSERT_EQUAL_INT32(0, protocore_partition_json(parts, 1, sentinel, 0));
...
void test_the_flash_walk_reports_nothing_off_target(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT8(0, protocore_partition_collect(out, 4));
    TEST_ASSERT_EQUAL_UINT8(0, protocore_partition_collect(NULL, 4));
    TEST_ASSERT_EQUAL_UINT8(0, protocore_partition_collect(out, 0));
}
