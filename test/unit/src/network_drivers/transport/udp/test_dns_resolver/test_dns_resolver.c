...
void setUp(void)
{
    ...
    Resolver.set_server(Resolver.internal);
}
...
void tearDown(void)
{
    Resolver.busy(Resolver.internal);
    if (Resolver.ok)
    {
...
static size_t query_build(uint8_t *out, size_t cap, uint16_t id, const char *host)
{
    ...
    Resolver.query_build(Resolver.internal);
    return Resolver.n;
}
...
static proto_bool answer_parse(const uint8_t *pkt, size_t len, uint16_t id, uint32_t *out)
{
    ...
    Resolver.answer_parse(Resolver.internal);
    *out = Resolver.u32;
    return Resolver.ok;
}
...
{
    Resolver.addr.ip = ip;
    Resolver.classify(Resolver.internal);
    return Resolver.cls;
}
...
{
    Resolver.addr.ip = ip;
    Resolver.verify(Resolver.internal);
    return Resolver.ok;
}
...
{
    Resolver.query.host = host;
    Resolver.resolve_verified(Resolver.internal);
    return Resolver.state;
}
