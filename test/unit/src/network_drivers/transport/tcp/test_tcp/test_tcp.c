...
//
// Checked by what each half BINDS, not by its address. A namespace table is `static const` in its
// header, so every translation unit that takes its address gets its own copy of an identical
// object: `&ConnPool` here is not the pointer tcp.c stored, and comparing the two asserts an
// identity the shape deliberately does not provide. Comparing a member proves the stored pointer
// names the same table, and survives the difference.
void test_each_half_points_at_the_module_that_owns_it(void)
{
    TEST_ASSERT_NOT_NULL(Tcp.conn);
