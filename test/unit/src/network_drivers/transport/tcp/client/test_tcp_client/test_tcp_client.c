...
{
    TcpClient.dial.host = "10.0.0.5";
    TcpClient.dial.port = 8080;
    TcpClient.dial.timeout_ms = 5000;
...
void test_open_rejects_a_null_host(void)
{
    ...
    TcpClient.open(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_INT(-2, TcpClient.i32);
}
...
void test_open_reports_a_full_pool(void)
{
    ...
    TcpClient.dial.timeout_ms = 1000;
    TcpClient.open(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_INT(-1, TcpClient.i32);
}
...
void test_slots_are_distinct_and_returned_on_close(void)
{
    ...
    TcpClient.dial.host = "10.0.0.7";
    TcpClient.dial.port = 80;
    TcpClient.dial.timeout_ms = 1000;
    TcpClient.open(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_INT(-1, TcpClient.i32); // full at two
    TcpClient.cid = a;
    TcpClient.close(protocore_tcp_client_span());
    TcpClient.dial.host = "10.0.0.7";
    TcpClient.dial.port = 80;
...
{
    TcpClient.dial.host = "example.invalid";
    TcpClient.dial.port = 80;
    TcpClient.dial.timeout_ms = 2000;
    TcpClient.open(protocore_tcp_client_span());
    int cid = TcpClient.i32;
    TEST_ASSERT_TRUE(cid >= 0);
    TcpClient.cid = cid;
    ...
    TcpClient.cid = cid;
    TcpClient.is_closed(protocore_tcp_client_span());
    TEST_ASSERT_FALSE(TcpClient.ok); // still resolving

    set_millis(1999);
    TcpClient.cid = cid;
    TcpClient.is_closed(protocore_tcp_client_span());
    TEST_ASSERT_FALSE(TcpClient.ok); // inside the caller's budget
    set_millis(2000);
    TcpClient.cid = cid;
    TcpClient.is_closed(protocore_tcp_client_span());
    TEST_ASSERT_TRUE(TcpClient.ok); // out of time
    TcpClient.cid = cid;
    TcpClient.connected(protocore_tcp_client_span());
    TEST_ASSERT_FALSE(TcpClient.ok);
}
...
void test_an_unknown_connection_id_is_inert(void)
{
    ...
    for (size_t i = 0; i < sizeof(bad) / sizeof(bad[0]); i++)
    {
        ...
        TEST_ASSERT_EQUAL_UINT(0, TcpClient.n);
        TcpClient.cid = bad[i];
...
void test_delivered_bytes_are_readable_in_order(void)
{
    ...
    TcpClient.cid = cid;
    TcpClient.available(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(128, TcpClient.n);

    uint8_t got[128];
    TcpClient.cid = cid;
    TcpClient.io.buf = got;
    TcpClient.io.cap = sizeof(got);
    TcpClient.read(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(128, TcpClient.n);
...
void test_arrival_alone_reopens_no_window(void)
{
    ...
    TcpClient.cid = cid;
    TcpClient.available(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(64, TcpClient.n);
}
...
void test_the_window_reopens_by_exactly_what_was_read(void)
{
    ...
    TcpClient.io.cap = sizeof(got);
    TcpClient.read(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(70, TcpClient.n);
    TEST_ASSERT_EQUAL_INT(1, mock_recved_call_count());
    TEST_ASSERT_EQUAL_UINT16(70, mock_recved_last());

    TcpClient.cid = cid;
    TcpClient.available(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(130, TcpClient.n); // the rest still charged
...
void test_a_segment_that_will_not_fit_is_refused_whole(void)
{
    ...
    TcpClient.cid = cid;
    TcpClient.available(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(filled, TcpClient.n); // untouched
}
...
void test_a_multi_buffer_segment_is_reassembled_in_order(void)
{
    ...
    TcpClient.io.cap = sizeof(got);
    TcpClient.read(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(90, TcpClient.n);
...
void test_a_short_read_leaves_the_remainder_queued(void)
{
    ...
    TcpClient.io.cap = sizeof(got);
    TcpClient.read(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(40, TcpClient.n);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(g_payload, got, 40);
    TcpClient.cid = cid;
    TcpClient.available(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(60, TcpClient.n);
...
void test_a_peer_fin_closes_the_slot(void)
{
    ...
    TcpClient.cid = cid;
    TcpClient.connected(protocore_tcp_client_span());
    TEST_ASSERT_FALSE(TcpClient.ok); // connected is false once closed
}
...
void test_a_reopened_slot_starts_clean(void)
{
    ...
    TcpClient.cid = again;
    TcpClient.available(protocore_tcp_client_span());
    TEST_ASSERT_EQUAL_UINT(0, TcpClient.n); // no carry-over bytes
    TcpClient.cid = again;
    TcpClient.is_closed(protocore_tcp_client_span());
    TEST_ASSERT_FALSE(TcpClient.ok);         // no carry-over close
}
