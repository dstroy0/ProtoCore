...
void test_full_handshake_on_rfc8448_key_material(void)
{
    ...
    Sha256.update(g_cli_transcript, ch, ch_len);
    ...
    Sha256.update(g_cli_transcript, view.fragment, view.frag_len);
    ...
    Sha256.final(g_cli_transcript, ch_sh_hash);
    ...
    while (off < (size_t)wrote)
    {
        ...
        if (msg[0] == TLS_HS_CERTIFICATE_VERIFY)
        {
            ...
            Sha256.final(g_cli_transcript, cert_verify_hash);
        ...
        if (msg[0] == TLS_HS_FINISHED)
        {
            ...
            uint8_t hash[32];
            uint8_t expect[32];
            Sha256.final(g_cli_transcript, hash);
            Tls13KsV.bind.ks = &g_cli_ks;
        ...
        Sha256.update(g_cli_transcript, msg, info.pt_len);
        seen++;
    }
