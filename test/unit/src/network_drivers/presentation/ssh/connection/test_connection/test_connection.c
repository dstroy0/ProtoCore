...
static uint32_t open_session(uint32_t peer_id, uint32_t peer_window)
{
    ...
    uint8_t out[64];
    size_t olen = 0;
    TEST_ASSERT_EQUAL_INT(0, protocore_ssh_channel_handle_open(0, pkt, n, out, &olen, sizeof(out)));
...
{
    SshFlow f;
    protocore_ssh_flow_init(&f, 1024u, 10u, 32768u);

    TEST_ASSERT_TRUE(protocore_ssh_flow_send_allows(&f, 10u));
    TEST_ASSERT_FALSE(protocore_ssh_flow_send_allows(&f, 11u));
}

static void test_sec5_2_sending_decrements_the_window(void)
{
    SshFlow f;
    protocore_ssh_flow_init(&f, 1024u, 100u, 32768u);

    protocore_ssh_flow_send_take(&f, 40u);
    TEST_ASSERT_EQUAL_UINT32(60u, f.peer_window);

    protocore_ssh_flow_send_take(&f, 60u);
    TEST_ASSERT_EQUAL_UINT32(0u, f.peer_window);
    TEST_ASSERT_FALSE(protocore_ssh_flow_send_allows(&f, 1u));
}
...
static void test_sec5_2_send_cap_is_the_smaller_of_window_and_max_packet(void)
{
    ...
    protocore_ssh_flow_init(&f, 1024u, 200u, 32768u);
    TEST_ASSERT_EQUAL_UINT32(200u, protocore_ssh_flow_send_cap(&f, 4096u));

    // Maximum packet size is the binding limit.
    protocore_ssh_flow_init(&f, 1024u, 100000u, 512u);
    TEST_ASSERT_EQUAL_UINT32(512u, protocore_ssh_flow_send_cap(&f, 4096u));

    // Neither binds: the caller gets what it asked for.
    protocore_ssh_flow_init(&f, 1024u, 100000u, 32768u);
    TEST_ASSERT_EQUAL_UINT32(4096u, protocore_ssh_flow_send_cap(&f, 4096u));
}

static void test_sec5_2_window_adjust_increments_the_peer_window(void)
{
    SshFlow f;
    protocore_ssh_flow_init(&f, 1024u, 0u, 32768u);
    TEST_ASSERT_FALSE(protocore_ssh_flow_send_allows(&f, 1u));

    protocore_ssh_flow_peer_add(&f, 4096u);
    TEST_ASSERT_EQUAL_UINT32(4096u, f.peer_window);
    TEST_ASSERT_TRUE(protocore_ssh_flow_send_allows(&f, 4096u));
}

static void test_sec5_2_window_of_2_pow_32_minus_1_is_handled(void)
{
    SshFlow f;
    protocore_ssh_flow_init(&f, 1024u, 0xFFFFFFFFu, 32768u);

    TEST_ASSERT_EQUAL_UINT32(0xFFFFFFFFu, f.peer_window);
    TEST_ASSERT_TRUE(protocore_ssh_flow_send_allows(&f, 32768u));
    TEST_ASSERT_EQUAL_UINT32(32768u, protocore_ssh_flow_send_cap(&f, 0xFFFFFFFFu));

    // Spending against it decrements exactly, with no borrow off the top of the field.
    protocore_ssh_flow_send_take(&f, 32768u);
...
static void test_sec5_2_max_packet_binds_even_with_a_wide_window(void)
{
    ...
    TEST_ASSERT_TRUE(protocore_ssh_flow_send_allows(&f, 32768u));
    TEST_ASSERT_FALSE(protocore_ssh_flow_send_allows(&f, 32769u));
}

static void test_sec5_2_window_must_not_be_increased_above_2_pow_32_minus_1(void)
{
    SshFlow f;
    protocore_ssh_flow_init(&f, 1024u, 0xFFFFFF00u, 32768u);

    protocore_ssh_flow_peer_add(&f, 0x1000u); // would wrap to 0x0FFF if it were a bare addition
    TEST_ASSERT_EQUAL_UINT32(0xFFFFFFFFu, f.peer_window);
}
...
{
    SshFlow f;
    protocore_ssh_flow_init(&f, 64u, 1024u, 32768u);

    TEST_ASSERT_TRUE(protocore_ssh_flow_recv_take(&f, 64u));
    TEST_ASSERT_EQUAL_UINT32(0u, f.local_window);

    // One byte past the advertised window: refused, and the window stays at zero.
    TEST_ASSERT_FALSE(protocore_ssh_flow_recv_take(&f, 1u));
    TEST_ASSERT_EQUAL_UINT32(0u, f.local_window);
}
...
static void test_sec5_2_replenish_restores_the_advertised_window(void)
{
    SshFlow f;
    uint32_t add = 0;
    protocore_ssh_flow_init(&f, 2048u, 1024u, 32768u);

    TEST_ASSERT_TRUE(protocore_ssh_flow_recv_take(&f, 2048u));
    TEST_ASSERT_TRUE(protocore_ssh_flow_replenish_due(&f, &add));

    protocore_ssh_flow_local_credit(&f, add);
...
static void test_sec5_2_replenish_not_due_on_a_full_window(void)
{
    ...
    TEST_ASSERT_FALSE(protocore_ssh_flow_replenish_due(&f, &add));
}
...
static void test_sec6_5_one_string_present(void)
{
    ...
    TEST_ASSERT_TRUE(ssh_req_strings_present(p, n, 0, 1));
}
...
static void test_sec6_4_two_strings_present(void)
{
    ...
    TEST_ASSERT_TRUE(ssh_req_strings_present(p, n, 0, 2));
}
...
static void test_zero_strings_is_satisfied(void)
{
    ...
    TEST_ASSERT_TRUE(ssh_req_strings_present(p, sizeof(p), 0, 0));
}
...
static void test_second_string_missing_is_refused(void)
{
    ...
    TEST_ASSERT_FALSE(ssh_req_strings_present(p, n, 0, 2));
}
...
static void test_string_body_truncated_is_refused(void)
{
    ...
    TEST_ASSERT_FALSE(ssh_req_strings_present(p, 8, 0, 1));
}
...
static void test_string_header_truncated_is_refused(void)
{
    ...
    TEST_ASSERT_FALSE(ssh_req_strings_present(p, 3, 0, 1));
}
...
static void test_offset_is_honoured(void)
{
    ...
    TEST_ASSERT_TRUE(ssh_req_strings_present(p, n, first, 1));
    TEST_ASSERT_FALSE(ssh_req_strings_present(p, n, first, 2));
}
...
void test_s5_3_build_eof_keeps_the_channel_open_and_latches_eof_sent()
{
    ...
    uint8_t out[16];
    size_t olen = 0;
    TEST_ASSERT_EQUAL_INT(0, protocore_ssh_channel_build_eof(0, id, out, &olen, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(5, olen);
    TEST_ASSERT_EQUAL(SSH_MSG_CHANNEL_EOF, out[0]);
...
void test_s5_3_sending_close_alone_does_not_free_the_channel_number()
{
    ...
    uint8_t out[16];
    size_t olen = 0;
    TEST_ASSERT_EQUAL_INT(0, protocore_ssh_channel_build_close(0, id, out, &olen, sizeof(out)));
    TEST_ASSERT_EQUAL_size_t(5, olen);
    TEST_ASSERT_EQUAL(SSH_MSG_CHANNEL_CLOSE, out[0]);
    TEST_ASSERT_EQUAL_UINT32(5, rd_u32(out + 1));
...
void test_s5_3_both_closes_free_the_channel_number()
{
    ...
    wr_u32(pkt + 1, id);
    olen = 0;
    TEST_ASSERT_EQUAL_INT(0, protocore_ssh_channel_handle_close(0, pkt, sizeof(pkt), out, &olen, sizeof(out)));
    ...
    TEST_ASSERT_EQUAL_INT((int)id, protocore_ssh_chan_alloc(0));
}
...
void test_s5_3_inbound_eof_marks_the_peer_done_and_keeps_the_channel_open()
{
    ...
    wr_u32(pkt + 1, id);
    TEST_ASSERT_EQUAL_INT(0, protocore_ssh_channel_handle_eof(0, pkt, sizeof(pkt)));
...
void test_s5_3_inbound_eof_rejects_a_bad_slot_type_or_length()
{
    ...
    TEST_ASSERT_LESS_THAN_INT(0, protocore_ssh_channel_handle_eof(MAX_SSH_CONNS, pkt, sizeof(pkt)));
    TEST_ASSERT_LESS_THAN_INT(0, protocore_ssh_channel_handle_eof(0, pkt, 4));
    pkt[0] = SSH_MSG_CHANNEL_CLOSE;
    TEST_ASSERT_LESS_THAN_INT(0, protocore_ssh_channel_handle_eof(0, pkt, sizeof(pkt)));
}
...
void test_s5_3_close_after_our_eof_is_still_answered_with_close()
{
    ...
    uint8_t eof_out[16];
    size_t eof_len = 0;
    TEST_ASSERT_EQUAL_INT(0, protocore_ssh_channel_build_eof(0, id, eof_out, &eof_len, sizeof(eof_out)));
    TEST_ASSERT_TRUE(ssh_chan[0][id].eof_sent);
...
static void test_sec8_empty_stream_is_valid(void)
{
    ...
    TEST_ASSERT_TRUE(ssh_pty_modes_valid(NULL, 0, &used));
...
static void test_sec8_tty_op_end_terminates(void)
{
    ...
    uint32_t used = 0;
    TEST_ASSERT_TRUE(ssh_pty_modes_valid(s, sizeof(s), &used));
...
static void test_sec8_opcode_159_takes_an_argument(void)
{
    ...
    TEST_ASSERT_TRUE(ssh_pty_modes_valid(s, sizeof(s), NULL));
}
...
static void test_sec8_truncated_argument_is_refused(void)
{
    ...
    TEST_ASSERT_FALSE(ssh_pty_modes_valid(s, sizeof(s), NULL));
}
...
static void test_sec6_2_every_field_is_read(void)
{
    ...
    SshPtyRequest pty;
    TEST_ASSERT_TRUE(ssh_pty_req_parse(p, n, 0, &pty));
...
static void test_sec6_2_missing_term_is_refused(void)
{
    uint8_t p[8] = {0, 0, 0, 0};
    SshPtyRequest pty;
    TEST_ASSERT_FALSE(ssh_pty_req_parse(p, 2, 0, &pty));
}
...
static void test_sec6_2_truncated_dimensions_are_refused(void)
{
    ...
    SshPtyRequest pty;
    TEST_ASSERT_FALSE(ssh_pty_req_parse(p, n, 0, &pty));
}
...
static void test_sec6_2_offset_is_honoured(void)
{
    ...
    SshPtyRequest pty;
    TEST_ASSERT_TRUE(ssh_pty_req_parse(p, n, skip, &pty));
...
static void test_sec6_7_four_dimensions_are_read(void)
{
    ...
    SshPtyRequest dim;
    TEST_ASSERT_TRUE(ssh_window_change_parse(p, n, 0, &dim));
...
static void test_sec6_7_truncated_is_refused(void)
{
    ...
    SshPtyRequest dim;
    TEST_ASSERT_FALSE(ssh_window_change_parse(p, n, 0, &dim));
}
