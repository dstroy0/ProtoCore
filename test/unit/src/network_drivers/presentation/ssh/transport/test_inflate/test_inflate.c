...
static void test_garbage_input_is_refused(void)
{
    ...
    size_t out = 0;
    InflateV.packet_args.z = &s_inf;
    InflateV.packet_args.src = junk;
    InflateV.packet_args.src_len = sizeof(junk);
    InflateV.packet_args.dst = back;
    InflateV.packet_args.dst_cap = sizeof(back);
    InflateV.packet_args.out_len = &out;
    Inflate.packet(inflate_work);
    TEST_ASSERT_NOT_EQUAL(0, InflateV.n);
}
