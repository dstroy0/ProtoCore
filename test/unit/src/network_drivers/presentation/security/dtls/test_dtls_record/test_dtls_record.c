...
void setUp(void)
{
    ...
    DtlsRecord.keys_derive(dtls_record_work, &g_keys, DTLS_CIPHER_AES_128_GCM_SHA256, 3, SECRET);
}
void tearDown(void)
{
}
...
{
    size_t dtls_record_n = DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_HANDSHAKE, 0x0102,
                                                      0x0000AABBCCDDull, MSG, sizeof(MSG), g_out, sizeof(g_out));
    size_t n = dtls_record_n;
    ...
    DtlsPlaintext view;
    dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, g_out, n, &view);
    TEST_ASSERT_EQUAL_UINT(n, dtls_record_n);
...
{
    size_t dtls_record_n = DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_HANDSHAKE, 0, 0, MSG,
                                                      sizeof(MSG), g_out, sizeof(g_out));
    size_t n = dtls_record_n;
    g_out[1] = 0xFE;
...
{
    size_t dtls_record_n = DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_ACK, 0, 1, MSG, sizeof(MSG),
                                                      g_out, sizeof(g_out));
    size_t n = dtls_record_n;
    DtlsPlaintext view;
    ...
    {
        size_t dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, g_out, take, &view);
        TEST_ASSERT_EQUAL_UINT(0u, dtls_record_n);
    }
    dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, g_out, n, &view);
    TEST_ASSERT_EQUAL_UINT(n, dtls_record_n);
    dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, g_out, n + 40, &view);
    TEST_ASSERT_EQUAL_UINT(n, dtls_record_n);

    // A destination too small writes nothing.
    dtls_record_n =
        DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_ACK, 0, 1, MSG, sizeof(MSG), g_out, n - 1);
    TEST_ASSERT_EQUAL_UINT(0u, dtls_record_n);
    dtls_record_n =
        DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_ACK, 0, 1, MSG, sizeof(MSG), g_out, n);
    TEST_ASSERT_EQUAL_UINT(n, dtls_record_n);
}
...
{
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 0x1234, PROTOCORE_DTLS_CT_APPLICATION_DATA,
                                              MSG, sizeof(MSG), g_out, sizeof(g_out), NULL, 0);
    size_t n = dtls_record_n;
    ...
    DtlsRecord.keys_derive(dtls_record_work, &e4, DTLS_CIPHER_AES_128_GCM_SHA256, 4, SECRET);
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &e4, 1, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG, sizeof(MSG),
                                       g_out, sizeof(g_out), NULL, 0);
...
void test_rfc9147_4_2_3_sequence_number_encryption(void)
{
    ...
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, SEQ, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG,
                                              sizeof(MSG), g_out, sizeof(g_out), NULL, 0);
    size_t n = dtls_record_n;
    ...
    proto_bool dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &g_keys, SEQ, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    ...
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, SEQ, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG2,
                                       sizeof(MSG2), other, sizeof(other), NULL, 0);
    size_t m = dtls_record_n;
...
void test_rfc9147_4_2_2_sequence_reconstruction(void)
{
    ...
    for (size_t i = 0; i < sizeof(SEQS) / sizeof(SEQS[0]); i++)
    {
        size_t dtls_record_n =
            DtlsRecord.protect(dtls_record_work, &g_keys, SEQS[i], PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG, sizeof(MSG),
                               g_out, sizeof(g_out), NULL, 0);
        size_t n = dtls_record_n;
        TEST_ASSERT_TRUE(n > 0);
        // The receiver expects the next record after the previous one.
        proto_bool dtls_record_ok =
            DtlsRecord.unprotect(dtls_record_work, &g_keys, SEQS[i], g_out, n, pt, sizeof(pt), &info, NULL, 0);
        TEST_ASSERT_TRUE(dtls_record_ok);
    ...
    size_t dtls_record_n =
        DtlsRecord.protect(dtls_record_work, &g_keys, 0x10000 - 100, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG,
                           sizeof(MSG), g_out, sizeof(g_out), NULL, 0);
    size_t n = dtls_record_n;
    proto_bool dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &g_keys, 0x10000, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_TRUE(dtls_record_ok);
...
void test_a_wrong_sequence_number_fails_deprotection(void)
{
    uint8_t pt[64];
    DtlsCiphertext info;
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 0x10000, PROTOCORE_DTLS_CT_APPLICATION_DATA,
                                              MSG, sizeof(MSG), g_out, sizeof(g_out), NULL, 0);
    size_t n = dtls_record_n;
    TEST_ASSERT_TRUE(n > 0);
    ...
    proto_bool dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &g_keys, 0x30000, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_FALSE(dtls_record_ok);
    // The right expectation still opens it.
    dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &g_keys, 0x10000, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_TRUE(dtls_record_ok);
}
...
void test_epoch_bits_select_the_keys(void)
{
    uint8_t pt[64];
    DtlsCiphertext info;
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 7, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG,
                                              sizeof(MSG), g_out, sizeof(g_out), NULL, 0);
    size_t n = dtls_record_n;

    DtlsRecordKeys other;
    memset(&other, 0, sizeof(other));
    DtlsRecord.keys_derive(dtls_record_work, &other, DTLS_CIPHER_AES_128_GCM_SHA256, 2, SECRET); // low bits 10, not 11
    proto_bool dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &other, 7, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_FALSE(dtls_record_ok);
    ...
    DtlsRecord.keys_derive(dtls_record_work, &e7, DTLS_CIPHER_AES_128_GCM_SHA256, 7, OTHER_SECRET);
    dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &e7, 7, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_FALSE(dtls_record_ok);

    dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &g_keys, 7, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_TRUE(dtls_record_ok);
}
...
void test_rfc9146_connection_id(void)
{
    ...
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 5, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG,
                                              sizeof(MSG), g_out, sizeof(g_out), CID, sizeof(CID));
    size_t n = dtls_record_n;
    ...
    proto_bool dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &g_keys, 5, g_out, n, pt, sizeof(pt), &info, CID, sizeof(CID));
    TEST_ASSERT_TRUE(dtls_record_ok);
    TEST_ASSERT_EQUAL_UINT(sizeof(MSG), info.pt_len);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(MSG, pt, sizeof(MSG));

    // Another endpoint's CID is not ours.
    dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &g_keys, 5, g_out, n, pt, sizeof(pt), &info, WRONG_CID,
                                          sizeof(WRONG_CID));
    TEST_ASSERT_FALSE(dtls_record_ok);
    // A receiver expecting no CID must refuse a record that carries one, and vice versa.
    dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &g_keys, 5, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_FALSE(dtls_record_ok);

    dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 5, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG,
                                       sizeof(MSG), g_out, sizeof(g_out), NULL, 0);
    size_t m = dtls_record_n;
    dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &g_keys, 5, g_out, m, pt, sizeof(pt), &info, CID, sizeof(CID));
    TEST_ASSERT_FALSE(dtls_record_ok);
    ...
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 5, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG,
                                       sizeof(MSG), g_out, sizeof(g_out), big, sizeof(big));
    TEST_ASSERT_EQUAL_UINT(0u, dtls_record_n);
    dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &g_keys, 5, g_out, m, pt, sizeof(pt), &info, big, sizeof(big));
    TEST_ASSERT_FALSE(dtls_record_ok);
}
...
void test_inner_content_type_and_padding(void)
{
    ...
    {
        size_t dtls_record_n =
            DtlsRecord.protect(dtls_record_work, &g_keys, i, TYPES[i], MSG, sizeof(MSG), g_out, sizeof(g_out), NULL, 0);
        size_t n = dtls_record_n;
        TEST_ASSERT_TRUE(n > 0);
        proto_bool dtls_record_ok =
            DtlsRecord.unprotect(dtls_record_work, &g_keys, i, g_out, n, pt, sizeof(pt), &info, NULL, 0);
        TEST_ASSERT_TRUE(dtls_record_ok);
    ...
    size_t dtls_record_n =
        DtlsRecord.protect(dtls_record_work, &g_keys, 9, 0x00, INNER, sizeof(INNER), g_out, sizeof(g_out), NULL, 0);
    size_t n = dtls_record_n;
    TEST_ASSERT_TRUE(n > 0);
    proto_bool dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &g_keys, 9, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_TRUE(dtls_record_ok);
    TEST_ASSERT_EQUAL_HEX8(PROTOCORE_DTLS_CT_APPLICATION_DATA, info.content_type);
    ...
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 10, 0x00, &ZERO, 1, g_out, sizeof(g_out), NULL, 0);
    n = dtls_record_n;
    TEST_ASSERT_TRUE(n > 0);
    dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &g_keys, 10, g_out, n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_FALSE(dtls_record_ok);
}
...
void test_rfc9147_4_5_2_invalid_records(void)
{
    uint8_t pt[64];
    DtlsCiphertext info;
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 11, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG,
                                              sizeof(MSG), g_out, sizeof(g_out), NULL, 0);
    size_t n = dtls_record_n;
    uint8_t rec[128];
    memcpy(rec, g_out, n);
    ...
    for (uint8_t top = 0; top < 8; top++)
    {
        ...
        proto_bool dtls_record_ok =
            DtlsRecord.unprotect(dtls_record_work, &g_keys, 11, rec, n, pt, sizeof(pt), &info, NULL, 0);
        proto_bool ok = dtls_record_ok;
        if (top == 1)
        {
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        proto_bool dtls_record_ok =
            DtlsRecord.unprotect(dtls_record_work, &g_keys, 11, rec, n, pt, sizeof(pt), &info, NULL, 0);
        TEST_ASSERT_FALSE_MESSAGE(dtls_record_ok, "a flipped bit must not deprotect");
    }
    ...
    {
        proto_bool dtls_record_ok =
            DtlsRecord.unprotect(dtls_record_work, &g_keys, 11, g_out, take, pt, sizeof(pt), &info, NULL, 0);
        TEST_ASSERT_FALSE(dtls_record_ok);
    }

    // A destination too small for the recovered plaintext, and one too small for the record.
    proto_bool dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &g_keys, 11, g_out, n, pt, 4, &info, NULL, 0);
    TEST_ASSERT_FALSE(dtls_record_ok);
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, 11, PROTOCORE_DTLS_CT_APPLICATION_DATA, MSG,
                                       sizeof(MSG), g_out, n - 1, NULL, 0);
    TEST_ASSERT_EQUAL_UINT(0u, dtls_record_n);
}
...
{
    DtlsReplayWindow w;
    DtlsRecord.replay_init(dtls_record_work, &w);
    ...
    proto_bool dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 0);
    TEST_ASSERT_TRUE(dtls_record_ok);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 1000);
    TEST_ASSERT_TRUE(dtls_record_ok);
    DtlsRecord.replay_mark(dtls_record_work, &w, 0);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 0);
    TEST_ASSERT_FALSE(dtls_record_ok); // now a duplicate

    // Ahead of the right edge is new; a gap is allowed and the skipped numbers stay acceptable.
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 5);
    TEST_ASSERT_TRUE(dtls_record_ok);
    DtlsRecord.replay_mark(dtls_record_work, &w, 5);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 5);
    TEST_ASSERT_FALSE(dtls_record_ok);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 3);
    TEST_ASSERT_TRUE(dtls_record_ok); // reordered, inside the window, not yet seen
    DtlsRecord.replay_mark(dtls_record_work, &w, 3);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 3);
    TEST_ASSERT_FALSE(dtls_record_ok);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 0);
    TEST_ASSERT_FALSE(dtls_record_ok);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 4);
    TEST_ASSERT_TRUE(dtls_record_ok);
    ...
    // still inside.
    DtlsRecord.replay_mark(dtls_record_work, &w, 69);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 5);
    TEST_ASSERT_FALSE(dtls_record_ok); // 69 - 5 = 64: outside
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 6);
    TEST_ASSERT_TRUE(dtls_record_ok); // 69 - 6 = 63: the oldest bit
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 69);
    TEST_ASSERT_FALSE(dtls_record_ok);

    // A jump of more than the window width clears everything behind it.
    DtlsRecord.replay_mark(dtls_record_work, &w, 1000);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 1000);
    TEST_ASSERT_FALSE(dtls_record_ok);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 6);
    TEST_ASSERT_FALSE(dtls_record_ok);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 936);
    TEST_ASSERT_FALSE(dtls_record_ok); // 1000 - 936 = 64: outside
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 937);
    TEST_ASSERT_TRUE(dtls_record_ok);
    dtls_record_ok = DtlsRecord.replay_check(dtls_record_work, &w, 1001);
    TEST_ASSERT_TRUE(dtls_record_ok);
...
void test_round_trip_over_lengths(void)
{
    ...
    {
        size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &g_keys, len, PROTOCORE_DTLS_CT_APPLICATION_DATA,
                                                  src, len, g_out, sizeof(g_out), NULL, 0);
        size_t n = dtls_record_n;
        TEST_ASSERT_TRUE(n > 0);
        proto_bool dtls_record_ok =
            DtlsRecord.unprotect(dtls_record_work, &g_keys, len, g_out, n, pt, sizeof(pt), &info, NULL, 0);
        TEST_ASSERT_TRUE(dtls_record_ok);
