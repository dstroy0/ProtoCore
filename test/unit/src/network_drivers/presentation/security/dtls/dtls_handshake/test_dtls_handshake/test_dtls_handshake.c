...
void test_hs_header_roundtrip(void)
{
    ...
    size_t dtls_handshake_n =
        DtlsHandshake.frag_build(dtls_handshake_work, 1, 7, 100, 40, frag, sizeof(frag), out, sizeof(out));
    size_t n = dtls_handshake_n;
    ...
    DtlsHsHeader h;
    dtls_handshake_n = DtlsHandshake.header_parse(dtls_handshake_work, out, n, &h);
    size_t consumed = dtls_handshake_n;
...
void test_hs_header_parse_rejects(void)
{
    ...
    size_t dtls_handshake_n = DtlsHandshake.header_parse(dtls_handshake_work, buf, 11, &h);
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);
    ...
    buf[8] = 8;
    buf[11] = 5;
    dtls_handshake_n = DtlsHandshake.header_parse(dtls_handshake_work, buf, sizeof(buf), &h);
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);
    ...
    buf[3] = 40;
    buf[11] = 20;
    dtls_handshake_n = DtlsHandshake.header_parse(dtls_handshake_work, buf, PROTOCORE_DTLS_HS_HDR_LEN + 4, &h);
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);
}
...
{
    uint8_t rec[512];
    size_t dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, msg_type, msg_seq, full_len, off,
                                                       body + off, flen, rec, sizeof(rec));
    size_t n = dtls_handshake_n;
    TEST_ASSERT_TRUE(n > 0);
    DtlsHsHeader h;
    dtls_handshake_n = DtlsHandshake.header_parse(dtls_handshake_work, rec, n, &h);
    TEST_ASSERT_EQUAL_size_t(n, dtls_handshake_n);
    dtls_handshake_n = DtlsHandshake.reasm_add(dtls_handshake_work, r, &h);
    return dtls_handshake_n;
}
...
void test_hs_reasm_single_fragment(void)
{
    ...
    uint8_t buf[80];
    DtlsHsReasm r;
    DtlsHandshake.reasm_init(dtls_handshake_work, &r, 2, buf, sizeof(buf));
...
void test_hs_reasm_in_order(void)
{
    ...
    uint8_t buf[100];
    DtlsHsReasm r;
    DtlsHandshake.reasm_init(dtls_handshake_work, &r, 0, buf, sizeof(buf));
...
void test_hs_reasm_out_of_order(void)
{
    ...
    uint8_t buf[100];
    DtlsHsReasm r;
    DtlsHandshake.reasm_init(dtls_handshake_work, &r, 4, buf, sizeof(buf));
...
void test_hs_reasm_overlap_and_duplicate(void)
{
    ...
    uint8_t buf[100];
    DtlsHsReasm r;
    DtlsHandshake.reasm_init(dtls_handshake_work, &r, 1, buf, sizeof(buf));
...
void test_hs_reasm_wrong_msg_seq_ignored(void)
{
    ...
    uint8_t buf[40];
    DtlsHsReasm r;
    DtlsHandshake.reasm_init(dtls_handshake_work, &r, 5, buf, sizeof(buf));
...
void test_hs_reasm_rejects(void)
{
...

    {
        ...
        size_t dtls_handshake_n = DtlsHandshake.reasm_add(dtls_handshake_work, &r, &h);
        TEST_ASSERT_EQUAL_INT(-1, dtls_handshake_n);
    }
...
void test_ack_roundtrip(void)
{
    ...
    uint8_t out[64];
    size_t dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, in, 3, out, sizeof(out));
    size_t n = dtls_handshake_n;
    ...
    size_t count = 0;
    proto_bool dtls_handshake_ok = DtlsHandshake.ack_parse(dtls_handshake_work, out, n, back, 4, &count);
    TEST_ASSERT_TRUE(dtls_handshake_ok);
    ...
    dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, NULL, 0, out, sizeof(out));
    n = dtls_handshake_n;
    TEST_ASSERT_EQUAL_size_t(2, n);
    dtls_handshake_ok = DtlsHandshake.ack_parse(dtls_handshake_work, out, n, back, 4, &count);
    TEST_ASSERT_TRUE(dtls_handshake_ok);
...
void test_ack_parse_rejects(void)
{
    ...
    proto_bool dtls_handshake_ok = DtlsHandshake.ack_parse(dtls_handshake_work, buf, 1, out, 4, &count);
    TEST_ASSERT_FALSE(dtls_handshake_ok);

    buf[0] = 0x00;
    buf[1] = 0x08;
    dtls_handshake_ok = DtlsHandshake.ack_parse(dtls_handshake_work, buf, 10, out, 4, &count);
    TEST_ASSERT_FALSE(dtls_handshake_ok);
    ...
    size_t dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, many, 3, buf, sizeof(buf));
    size_t n = dtls_handshake_n;
    dtls_handshake_ok = DtlsHandshake.ack_parse(dtls_handshake_work, buf, n, out, 2, &count);
    TEST_ASSERT_FALSE(dtls_handshake_ok);
}
...
void test_cookie_kat(void)
{
    ...
    size_t dtls_handshake_n =
        DtlsHandshake.cookie_make(dtls_handshake_work, tw, COOKIE_KEY, COOKIE_TS, COOKIE_PAYLOAD,
                                  sizeof(COOKIE_PAYLOAD), COOKIE_ADDR, sizeof(COOKIE_ADDR), out, sizeof(out));
    size_t n = dtls_handshake_n;
...
void test_cookie_verify_accept_and_payload(void)
{
    ...
    proto_bool dtls_handshake_ok =
        DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 0, 0, COOKIE_ADDR, sizeof(COOKIE_ADDR),
                                    COOKIE_WIRE, sizeof(COOKIE_WIRE), payload, sizeof(payload), &plen);
    TEST_ASSERT_TRUE(dtls_handshake_ok);
...
void test_cookie_verify_rejects(void)
{
    ...
    proto_bool dtls_handshake_ok =
        DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 0, 0, other_addr, sizeof(other_addr),
                                    COOKIE_WIRE, sizeof(COOKIE_WIRE), payload, sizeof(payload), &plen);
    TEST_ASSERT_FALSE(dtls_handshake_ok);
    ...
    bad[20] ^= 0x01;
    dtls_handshake_ok =
        DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 0, 0, COOKIE_ADDR, sizeof(COOKIE_ADDR), bad,
                                    sizeof(bad), payload, sizeof(payload), &plen);
    TEST_ASSERT_FALSE(dtls_handshake_ok);

    dtls_handshake_ok =
        DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 0, 0, COOKIE_ADDR, sizeof(COOKIE_ADDR),
                                    COOKIE_WIRE, 20, payload, sizeof(payload), &plen);
    TEST_ASSERT_FALSE(dtls_handshake_ok);
}
...
void test_cookie_freshness(void)
{
    ...
    size_t dtls_handshake_n =
        DtlsHandshake.cookie_make(dtls_handshake_work, tw, COOKIE_KEY, 1000, payload, sizeof(payload), COOKIE_ADDR,
    ...
    proto_bool dtls_handshake_ok =
        DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 1005, 10, COOKIE_ADDR, sizeof(COOKIE_ADDR),
                                    cookie, n, out, sizeof(out), &plen);
    TEST_ASSERT_TRUE(dtls_handshake_ok);

    dtls_handshake_ok = DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 2000, 10, COOKIE_ADDR,
                                                    sizeof(COOKIE_ADDR), cookie, n, out, sizeof(out), &plen);
    TEST_ASSERT_FALSE(dtls_handshake_ok);

    dtls_handshake_ok = DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 999, 10, COOKIE_ADDR,
                                                    sizeof(COOKIE_ADDR), cookie, n, out, sizeof(out), &plen);
    TEST_ASSERT_FALSE(dtls_handshake_ok);
}
...
void test_hs_frag_build_rejects(void)
{
    ...
    size_t dtls_handshake_n =
        DtlsHandshake.frag_build(dtls_handshake_work, 1, 0, 0x1000000, 0, body, 8, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);
    dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, 1, 0, 100, 0x1000000, body, 8, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);
    dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, 1, 0, 100, 0, body, 0x1000000, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);

    dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, 1, 0, 10, 8, body, 5, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);

    dtls_handshake_n =
        DtlsHandshake.frag_build(dtls_handshake_work, 1, 0, 8, 0, body, 8, out, PROTOCORE_DTLS_HS_HDR_LEN + 7);
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);

    dtls_handshake_n =
        DtlsHandshake.frag_build(dtls_handshake_work, 1, 0, 8, 0, body, 8, out, PROTOCORE_DTLS_HS_HDR_LEN + 8);
    TEST_ASSERT_EQUAL_size_t(PROTOCORE_DTLS_HS_HDR_LEN + 8, dtls_handshake_n);
}
...
void test_hs_reasm_header_guards(void)
{
...

    {
        ...
        size_t dtls_handshake_n = DtlsHandshake.reasm_add(dtls_handshake_work, &r, &empty);
        TEST_ASSERT_EQUAL_INT(0, dtls_handshake_n);
...
void test_ack_build_rejects(void)
{
    ...
    size_t dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, NULL, 4096, out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);

    DtlsRecordNumber rns[2] = {{2, 1}, {2, 2}};
    dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, rns, 2, out, 2 + 2 * 16 - 1);
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);
    dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, rns, 2, out, 2 + 2 * 16);
    TEST_ASSERT_EQUAL_size_t(2 + 2 * 16, dtls_handshake_n);
}
...
void test_cookie_make_rejects(void)
{
    ...
    size_t dtls_handshake_n = DtlsHandshake.cookie_make(dtls_handshake_work, tw, COOKIE_KEY, 1, NULL, 0x10000,
                                                        COOKIE_ADDR, sizeof(COOKIE_ADDR), out, sizeof(out));
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);

    dtls_handshake_n = DtlsHandshake.cookie_make(dtls_handshake_work, tw, COOKIE_KEY, 1, payload, sizeof(payload),
                                                 COOKIE_ADDR, sizeof(COOKIE_ADDR), out, 20);
    TEST_ASSERT_EQUAL_size_t(0, dtls_handshake_n);

    uint8_t big[128];
    memset(big, 0x5A, sizeof(big));
    dtls_handshake_n = DtlsHandshake.cookie_make(dtls_handshake_work, tw, COOKIE_KEY, 1, big, sizeof(big), COOKIE_ADDR,
...
{
    uint8_t cookie[PROTOCORE_DTLS_COOKIE_MAX];
    size_t dtls_handshake_n = DtlsHandshake.cookie_make(dtls_handshake_work, tw, COOKIE_KEY, 4242, NULL, 0, COOKIE_ADDR,
                                                        sizeof(COOKIE_ADDR), cookie, sizeof(cookie));
    size_t n = dtls_handshake_n;
    ...
    size_t plen = 123;
    proto_bool dtls_handshake_ok =
        DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 4242, 0, COOKIE_ADDR, sizeof(COOKIE_ADDR),
                                    cookie, n, payload, sizeof(payload), &plen);
    TEST_ASSERT_TRUE(dtls_handshake_ok);
...
void test_cookie_verify_structural_rejects(void)
{
    ...
    dtls_handshake_ok =
        DtlsHandshake.cookie_verify(dtls_handshake_work, tw, COOKIE_KEY, 0, 0, COOKIE_ADDR, sizeof(COOKIE_ADDR),
                                    COOKIE_WIRE, sizeof(COOKIE_WIRE), small, sizeof(small), &plen);
    TEST_ASSERT_FALSE(dtls_handshake_ok);
}
