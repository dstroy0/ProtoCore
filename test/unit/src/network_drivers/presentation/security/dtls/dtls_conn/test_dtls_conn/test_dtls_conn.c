...
{
    DtlsHsHeader hh;
    size_t dtls_handshake_n = DtlsHandshake.header_parse(dtls_handshake_work, payload, plen, &hh);
    if (!dtls_handshake_n || hh.frag_offset != 0 || hh.frag_length != hh.length)
    {
        return 0;
    }
                                           ...
                                           proto_bool expect_rpk, uint64_t first_seq)
{
    ...
    DtlsPlaintext pt;
    size_t dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, flight + off, fl - off, &pt);
    size_t rl = dtls_record_n;
    TEST_ASSERT_TRUE(rl > 0);
    off += rl;
    uint8_t sh[512];
    size_t sh_len = frag_to_tls(pt.fragment, pt.frag_len, sh);
    TEST_ASSERT_TRUE(sh_len > 0);
    Sha256.update(tr, sh, sh_len);
    ...
    DtlsRecord.keys_derive(dtls_record_work, &srv_read, DTLS_CIPHER_AES_128_GCM_SHA256, 2, cks.s + TLS13_KS_SERVER_HS);
    ...
    while (off < fl)
    {
        ...
        uint8_t inner[512];
        DtlsCiphertext info;
        proto_bool dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &srv_read, exp_seq, flight + off, crl, inner,
                                                         sizeof(inner), &info, client_cid, client_cid_len);
        TEST_ASSERT_TRUE(dtls_record_ok);
        exp_seq = info.seq + 1;
        off += crl;
        ...
        if (msg[0] == 15)
        {
            ...
            proto_bool ed25519_ok = Ed25519.verify(tw, cert_pub, content, clen, sig);
        ...
        if (msg[0] == 20)
        {
            ...
            uint8_t expect[32];
            Tls13KsV.bind.ks = &cks;
            Tls13KsV.finished_args.base_secret = cks.s + TLS13_KS_SERVER_HS;
            Tls13KsV.finished_args.transcript_hash = hcv;
    ...
    DtlsRecord.keys_derive(dtls_record_work, &cli_write, DTLS_CIPHER_AES_128_GCM_SHA256, 2, cks.s + TLS13_KS_CLIENT_HS);

    uint8_t cfin_frag[80];
    size_t dtls_handshake_n =
        DtlsHandshake.frag_build(dtls_handshake_work, cfin[0], cfin_msg_seq, (uint32_t)(cfin_len - 4), 0, cfin + 4,
                                 (uint32_t)(cfin_len - 4), cfin_frag, sizeof(cfin_frag));
    size_t cff = dtls_handshake_n;
    uint8_t cfin_rec[128];
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &cli_write, 0, PROTOCORE_DTLS_CT_HANDSHAKE, cfin_frag, cff,
                                       cfin_rec, sizeof(cfin_rec), scid_len ? scid : NULL, scid_len);
    size_t cfr = dtls_record_n;
    ...
    DtlsRecord.keys_derive(dtls_record_work, &cli_app_read, DTLS_CIPHER_AES_128_GCM_SHA256, 3,
                           cks.s + TLS13_KS_SERVER_AP);
    DtlsRecord.keys_derive(dtls_record_work, &cli_app_write, DTLS_CIPHER_AES_128_GCM_SHA256, 3,
                           cks.s + TLS13_KS_CLIENT_AP);

    uint8_t ack_pt[64];
    DtlsCiphertext ackinfo;
    proto_bool dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &cli_app_read, 0, out2, (size_t)r2, ack_pt,
                                                     sizeof(ack_pt), &ackinfo, client_cid, client_cid_len);
    TEST_ASSERT_TRUE(dtls_record_ok);
    TEST_ASSERT_EQUAL_UINT8(PROTOCORE_DTLS_CT_ACK, ackinfo.content_type);

    DtlsServerV.app_write_keys_args.c = conn;
    DtlsServer.app_write_keys(dtls_server_work);
    DtlsRecordKeys *const srv_app_write = DtlsServerV.ptr;
    DtlsServerV.app_read_keys_args.c = conn;
    DtlsServer.app_read_keys(dtls_server_work);
    DtlsRecordKeys *const srv_app_read = DtlsServerV.ptr;
    ...
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &cli_write, 1, PROTOCORE_DTLS_CT_HANDSHAKE, cfin_frag, cff,
                                       cfin_rec2, sizeof(cfin_rec2), scid_len ? scid : NULL, scid_len);
    size_t cfr2 = dtls_record_n;
    uint8_t out3[64];
    DtlsServerV.process_args.c = conn;
    DtlsServerV.process_args.dgram = cfin_rec2;
    DtlsServerV.process_args.len = cfr2;
    DtlsServerV.process_args.out = out3;
    DtlsServerV.process_args.out_cap = sizeof(out3);
    DtlsServer.process(dtls_server_work);
    int r3 = DtlsServerV.n;
    ...
    dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &cli_app_read, 1, out3, (size_t)r3, ack_pt3,
                                          sizeof(ack_pt3), &ackinfo3, client_cid, client_cid_len);
    TEST_ASSERT_TRUE(dtls_record_ok);
...
void test_full_handshake(void)
{
    ...
    tr = tw_tr;
    Sha256.init(tr);
    Sha256.update(tr, ch, ch_len);

    uint8_t ch_frag[300];
    size_t dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, ch[0], 0, (uint32_t)(ch_len - 4), 0, ch + 4,
                                                       (uint32_t)(ch_len - 4), ch_frag, sizeof(ch_frag));
    size_t ch_fl = dtls_handshake_n;
    uint8_t ch_rec[320];
    size_t dtls_record_n = DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_HANDSHAKE, 0, 0, ch_frag,
                                                      ch_fl, ch_rec, sizeof(ch_rec));
    size_t ch_rl = dtls_record_n;

    uint8_t flight[2048];
    DtlsServerV.process_args.c = &g_dtls;
...
void test_cid_handshake(void)
{
    ...
    DtlsPlaintext sh_pt;
    dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, flight, (size_t)fl, &sh_pt);
    size_t shrl = dtls_record_n;
...
void test_hrr_group_renegotiation(void)
{
    ...
    DtlsServerV.init_args.c = &g_dtls;
    DtlsServerV.init_args.cfg = &cfg;
    DtlsServerV.init_args.peer_addr = TEST_PEER_ADDR;
    DtlsServerV.init_args.peer_addr_len = sizeof(TEST_PEER_ADDR);
    DtlsServer.init(dtls_server_work);
    ...
    uint8_t f1[300];
    size_t dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, ch1[0], 0, (uint32_t)(ch1_len - 4), 0,
                                                       ch1 + 4, (uint32_t)(ch1_len - 4), f1, sizeof(f1));
    size_t f1l = dtls_handshake_n;
    uint8_t r1[320];
    size_t dtls_record_n =
        DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_HANDSHAKE, 0, 0, f1, f1l, r1, sizeof(r1));
    size_t r1l = dtls_record_n;

    uint8_t hrr_flight[512];
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = r1;
    DtlsServerV.process_args.len = r1l;
    DtlsServerV.process_args.out = hrr_flight;
    DtlsServerV.process_args.out_cap = sizeof(hrr_flight);
    DtlsServer.process(dtls_server_work);
    int hf = DtlsServerV.n;
    ...
    DtlsPlaintext pt;
    dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, hrr_flight, (size_t)hf, &pt);
    size_t rl = dtls_record_n;
    TEST_ASSERT_TRUE(rl > 0);
    uint8_t hrr[512];
    ...
    h1 = tw_h1;
    Sha256.init(h1);
    Sha256.update(h1, ch1, ch1_len);
    Sha256.final(h1, ch1_hash);
    ...
    size_t mhl = Tls13MsgV.n;
    TEST_ASSERT_TRUE(mhl > 0);
    Sha256.update(tr, mh, mhl);
    Sha256.update(tr, hrr, hrr_len);

    uint8_t ch2[320];
    size_t ch2_len = build_client_hello_ex(ch2, client_pub, PROTO_TRUE, cookie, cookie_len, NULL, 0, PROTO_FALSE,
                                           TLS_GROUP_X25519, TLS_SIG_ED25519);
    Sha256.update(tr, ch2, ch2_len);

    uint8_t f2[380];
    dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, ch2[0], 1, (uint32_t)(ch2_len - 4), 0, ch2 + 4,
                                                (uint32_t)(ch2_len - 4), f2, sizeof(f2));
    size_t f2l = dtls_handshake_n;
    uint8_t r2[420];
    dtls_record_n =
        DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_HANDSHAKE, 0, 1, f2, f2l, r2, sizeof(r2));
    size_t r2l = dtls_record_n;

    uint8_t flight[2048];
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = r2;
    DtlsServerV.process_args.len = r2l;
    DtlsServerV.process_args.out = flight;
    DtlsServerV.process_args.out_cap = sizeof(flight);
    DtlsServer.process(dtls_server_work);
    int fl = DtlsServerV.n;
...
void test_hrr_retry_without_cookie_rejected(void)
{
    ...
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(47, DtlsServerV.value);
}
...
void test_reject_no_tls13(void)
{
    ...
    size_t dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, ch[0], 0, (uint32_t)(ch_len - 4), 0, ch + 4,
                                                       (uint32_t)(ch_len - 4), frag, sizeof(frag));
    size_t fl = dtls_handshake_n;
    size_t dtls_record_n =
        DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_HANDSHAKE, 0, 0, frag, fl, rec, sizeof(rec));
    size_t rl = dtls_record_n;
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = rec;
    DtlsServerV.process_args.len = rl;
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(70, DtlsServerV.value);
}
...
static int drive_server_flight(DtlsConn *conn, DtlsServerConfig *cfg, uint8_t **tr, uint8_t *flight, size_t flight_cap)
{
    ...
    *tr = tw_tr; // NOT tw: that is the shared work borrow every other call here is handed, and
    ...
    DtlsServerV.process_args.len = ch_rl;
    DtlsServerV.process_args.out = flight;
    DtlsServerV.process_args.out_cap = flight_cap;
    DtlsServer.process(dtls_server_work);
    return DtlsServerV.n;
}
...
void test_pto_retransmit_and_recovery(void)
{
    ...
    DtlsServerV.timeout_ms_args.c = &g_dtls;
    DtlsServer.timeout_ms(dtls_server_work);
    TEST_ASSERT_EQUAL_INT((int)PROTOCORE_DTLS_PTO_INITIAL_MS, DtlsServerV.n);

    uint8_t rflight[2048];
    DtlsServerV.on_timeout_args.c = &g_dtls;
    DtlsServerV.on_timeout_args.out = rflight;
    DtlsServerV.on_timeout_args.out_cap = sizeof(rflight);
    DtlsServer.on_timeout(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(0, DtlsServerV.n);
    ...
    DtlsServerV.on_timeout_args.out_cap = sizeof(rflight);
    DtlsServer.on_timeout(dtls_server_work);
    int rfl = DtlsServerV.n;
    TEST_ASSERT_TRUE(rfl > 0);
    DtlsServerV.timeout_ms_args.c = &g_dtls;
    DtlsServer.timeout_ms(dtls_server_work);
    TEST_ASSERT_EQUAL_INT((int)(PROTOCORE_DTLS_PTO_INITIAL_MS * 2), DtlsServerV.n);
    ...
    DtlsPlaintext fpt;
    size_t dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, flight, (size_t)fl, &fpt);
    size_t fskip = dtls_record_n; // the ServerHello goes out in the clear
...
void test_pto_backoff_and_giveup(void)
{
    ...
    DtlsServerV.timeout_ms_args.c = &g_dtls;
    DtlsServer.timeout_ms(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(1000, DtlsServerV.n);
    for (int i = 0; i < PROTOCORE_DTLS_MAX_RETRANSMITS; i++)
    {
        ...
        DtlsServerV.timeout_ms_args.c = &g_dtls;
        DtlsServer.timeout_ms(dtls_server_work);
        TEST_ASSERT_EQUAL_INT(PTO_MS[i], DtlsServerV.n);
    }
    advance_ms(PROTOCORE_DTLS_PTO_MAX_MS + 1000);
    DtlsServerV.on_timeout_args.c = &g_dtls;
    DtlsServerV.on_timeout_args.out = rflight;
    ...
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = rflight;
    DtlsServerV.process_args.len = 1;
    DtlsServerV.process_args.out = rflight;
    DtlsServerV.process_args.out_cap = sizeof(rflight);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
}
...
void test_pto_ack_cancels_retransmit(void)
{
    ...
    DtlsPlaintext pt;
    size_t dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, flight, (size_t)fl, &pt);
    TEST_ASSERT_TRUE(dtls_record_n > 0);
    uint8_t sh[512];
    size_t sh_len = frag_to_tls(pt.fragment, pt.frag_len, sh);
    TEST_ASSERT_TRUE(sh_len > 0);
    uint8_t server_pub[32];
    ...
    t = tw_t;
    Sha256.init(t);
    Sha256.update(t, ch, ch_len);
    Sha256.update(t, sh, sh_len);
    uint8_t h[32];
    Sha256.final(t, h);
    uint8_t ecdhe[32];
    Curve25519V.x25519_args.out = ecdhe;
    ...
    size_t dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, rns, 5, ack_body, sizeof(ack_body));
    size_t bl = dtls_handshake_n;
    TEST_ASSERT_TRUE(bl > 0);
    uint8_t ack_rec[160];
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &cli_write, 0, PROTOCORE_DTLS_CT_ACK, ack_body, bl, ack_rec,
                                       sizeof(ack_rec), NULL, 0);
    size_t ar = dtls_record_n;
    TEST_ASSERT_TRUE(ar > 0);

    uint8_t out[64];
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = ack_rec;
    DtlsServerV.process_args.len = ar;
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    DtlsServerV.timeout_ms_args.c = &g_dtls;
                              ...
                              uint64_t rec_seq)
{
    uint8_t frag[512];
    size_t dtls_handshake_n =
        DtlsHandshake.frag_build(dtls_handshake_work, tls_msg[0], msg_seq, (uint32_t)(tls_len - 4), 0, tls_msg + 4,
    ...
    size_t dtls_record_n =
        DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_HANDSHAKE, 0, rec_seq, frag, fl, out, out_cap);
    return dtls_record_n;
}
...
static proto_bool run_to_finished(DtlsConn *conn, DtlsServerConfig *cfg, ClientSession *st)
{
    ...
    Tls13KsV.bind.kdf = &DTLS13_KDF;
    Tls13KsV.bind.ks = &st->cks;
    Tls13KsV.bind.s = ks_store_1033;
    Tls13Ks.early(NULL);
    Tls13KsV.bind.ks = &st->cks;
    Tls13KsV.step.ecdhe = ecdhe;
    Tls13KsV.step.ecdhe_len = 32;
    Tls13KsV.step.ch_sh_hash = h;
    Tls13Ks.handshake(NULL);
    DtlsRecord.keys_derive(dtls_record_work, &st->srv_hs_read, DTLS_CIPHER_AES_128_GCM_SHA256, 2,
                           st->cks.s + TLS13_KS_SERVER_HS);
    DtlsRecord.keys_derive(dtls_record_work, &st->cli_hs_write, DTLS_CIPHER_AES_128_GCM_SHA256, 2,
                           st->cks.s + TLS13_KS_CLIENT_HS);
    ...
    while (off < (size_t)fl)
    {
        ...
        uint8_t inner[512];
        DtlsCiphertext info;
        proto_bool dtls_record_ok = DtlsRecord.unprotect(dtls_record_work, &st->srv_hs_read, exp_seq, flight + off, crl,
                                                         inner, sizeof(inner), &info, NULL, 0);
        if (!dtls_record_ok)
        {
            return PROTO_FALSE;
        }
    ...
    Tls13KsV.step.ch_sfin_hash = h_sfin;
    Tls13Ks.master(NULL);
    DtlsRecord.keys_derive(dtls_record_work, &st->cli_app_write, DTLS_CIPHER_AES_128_GCM_SHA256, 3,
                           st->cks.s + TLS13_KS_CLIENT_AP);
    DtlsRecord.keys_derive(dtls_record_work, &st->cli_app_read, DTLS_CIPHER_AES_128_GCM_SHA256, 3,
                           st->cks.s + TLS13_KS_SERVER_AP);

    uint8_t verify[32];
    Tls13KsV.bind.ks = &st->cks;
    Tls13KsV.finished_args.base_secret = st->cks.s + TLS13_KS_CLIENT_HS;
    Tls13KsV.finished_args.transcript_hash = h_sfin;
    ...
    size_t dtls_handshake_n =
        DtlsHandshake.frag_build(dtls_handshake_work, cfin[0], 1, (uint32_t)(cfin_len - 4), 0, cfin + 4,
                                 (uint32_t)(cfin_len - 4), st->cfin_frag, sizeof(st->cfin_frag));
    st->cfin_frag_len = dtls_handshake_n;
...
{
    uint8_t rec[128];
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &st->cli_hs_write, seq, PROTOCORE_DTLS_CT_HANDSHAKE,
                                              st->cfin_frag, st->cfin_frag_len, rec, sizeof(rec), NULL, 0);
    size_t rl = dtls_record_n;
    if (!rl)
    {
        return -2;
...
{
    uint8_t rec[192];
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &st->cli_hs_write, seq, PROTOCORE_DTLS_CT_ACK, body,
                                              blen, rec, sizeof(rec), NULL, 0);
    size_t rl = dtls_record_n;
    if (!rl)
    {
        return -2;
...
void test_ciphertext_truncated_header_stops_walk(void)
{
    ...
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = short_len;
    DtlsServerV.process_args.len = sizeof(short_len);
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(0, DtlsServerV.n);
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(0, DtlsServerV.value);

    DtlsServerV.init_args.c = &g_dtls2;
    DtlsServerV.init_args.cfg = &cfg;
    DtlsServerV.init_args.peer_addr = NULL;
    DtlsServerV.init_args.peer_addr_len = 0;
    DtlsServer.init(dtls_server_work);
    const uint8_t long_len[5] = {0x2C, 0x00, 0x01, 0x00, 0xFF};
    DtlsServerV.process_args.c = &g_dtls2;
    DtlsServerV.process_args.dgram = long_len;
    DtlsServerV.process_args.len = sizeof(long_len);
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(0, DtlsServerV.n);
    DtlsServerV.alert_args.c = &g_dtls2;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(0, DtlsServerV.value);
}
...
void test_ciphertext_before_keys_is_discarded(void)
{
    ...
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = with_len;
    DtlsServerV.process_args.len = sizeof(with_len);
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(0, DtlsServerV.n);
    ...
    DtlsServerV.process_args.c = &g_dtls2;
    DtlsServerV.process_args.dgram = no_len;
    DtlsServerV.process_args.len = sizeof(no_len);
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(0, DtlsServerV.n);
...
void test_plaintext_non_handshake_record_ignored(void)
{
    ...
    uint8_t rec[32];
    size_t dtls_record_n = DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_ALERT, 0, 0, alert_body,
                                                      sizeof(alert_body), rec, sizeof(rec));
    size_t rl = dtls_record_n;
    TEST_ASSERT_TRUE(rl > 0);
    uint8_t out[2048];
    DtlsServerV.process_args.c = &g_dtls;
...
void test_truncated_handshake_fragment_ignored(void)
{
    ...
    uint8_t rec[32];
    size_t dtls_record_n = DtlsRecord.plaintext_build(dtls_record_work, PROTOCORE_DTLS_CT_HANDSHAKE, 0, 0, stub,
                                                      sizeof(stub), rec, sizeof(rec));
    size_t rl = dtls_record_n;
    TEST_ASSERT_TRUE(rl > 0);
    uint8_t out[64];
    DtlsServerV.process_args.c = &g_dtls;
...
void test_fragment_for_other_msg_seq_ignored(void)
{
    ...
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = rec;
    DtlsServerV.process_args.len = rl0;
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_TRUE(DtlsServerV.n > 0);
...
void test_oversize_handshake_message_rejected(void)
{
    ...
    uint8_t frag[32];
    size_t dtls_handshake_n = DtlsHandshake.frag_build(dtls_handshake_work, TLS_HS_CLIENT_HELLO, 0,
                                                       (uint32_t)(PROTOCORE_DTLS_CONN_REASM_CAP + 1), 0, body,
    ...
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(50, DtlsServerV.value);
}
...
void test_unexpected_message_in_start_rejected(void)
{
    ...
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(10, DtlsServerV.value);
}
...
void test_client_hello_missing_algorithms_rejected(void)
{
    ...
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = rec;
    DtlsServerV.process_args.len = ra;
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(40, DtlsServerV.value);
    ...
    DtlsServerV.process_args.c = &g_dtls2;
    DtlsServerV.process_args.dgram = rec;
    DtlsServerV.process_args.len = rb;
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
...
void test_oversize_certificate_is_internal_error(void)
{
    ...
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(80, DtlsServerV.value);
}
...
void test_flight_out_cap_too_small_is_internal_error(void)
{
    ...
    DtlsServerV.process_args.dgram = rec;
    DtlsServerV.process_args.len = rl;
    DtlsServerV.process_args.out = tiny;
    DtlsServerV.process_args.out_cap = sizeof(tiny);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    ...
    DtlsServerV.process_args.c = &g_dtls2;
    DtlsServerV.process_args.dgram = rec;
    DtlsServerV.process_args.len = r1;
    DtlsServerV.process_args.out = tiny;
    DtlsServerV.process_args.out_cap = sizeof(tiny);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
...
void test_retransmit_out_cap_too_small(void)
{
    ...
    uint8_t tiny[32];
    DtlsServerV.on_timeout_args.c = &g_dtls;
    DtlsServerV.on_timeout_args.out = tiny;
    DtlsServerV.on_timeout_args.out_cap = sizeof(tiny);
    DtlsServer.on_timeout(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
}
...
void test_timer_idle_when_done_or_failed(void)
{
    ...
    DtlsServerV.on_timeout_args.out_cap = sizeof(out);
    DtlsServer.on_timeout(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(0, DtlsServerV.n);
    ...
    DtlsServerV.timeout_ms_args.c = &g_dtls2;
    DtlsServer.timeout_ms(dtls_server_work);
    TEST_ASSERT_TRUE(DtlsServerV.n >= 0);
    uint8_t ch[256];
    size_t ch_len = build_client_hello(ch, client_pub);
    ...
    DtlsServer.timeout_ms(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(-1, DtlsServerV.n);
    DtlsServerV.on_timeout_args.c = &g_dtls2;
    DtlsServerV.on_timeout_args.out = out;
    DtlsServerV.on_timeout_args.out_cap = sizeof(out);
    DtlsServer.on_timeout(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(0, DtlsServerV.n);
...
void test_client_finished_error_paths(void)
{
    ...
    DtlsServerV.alert_args.c = &g_dtls2;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8(51, DtlsServerV.value);
...
void test_ack_malformed_and_partial_keep_timer(void)
{
    ...
    uint8_t body[2 + 16];
    size_t dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, &one, 1, body, sizeof(body));
    size_t bl = dtls_handshake_n;
    TEST_ASSERT_TRUE(bl > 0);
...
void test_ack_replay_and_late_ack_ignored(void)
{
    ...
    DtlsRecordNumber rns[5] = {{0, 0}, {2, 0}, {2, 1}, {2, 2}, {2, 3}};
    uint8_t body[2 + 5 * 16];
    size_t dtls_handshake_n = DtlsHandshake.ack_build(dtls_handshake_work, rns, 5, body, sizeof(body));
    size_t bl = dtls_handshake_n;
    TEST_ASSERT_TRUE(bl > 0);
    uint8_t rec[192];
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &st.cli_hs_write, 0, PROTOCORE_DTLS_CT_ACK, body, bl,
...
void test_completion_ack_deferred_when_out_full(void)
{
    ...
    uint8_t out[128];
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = tiny;
    DtlsServerV.process_args.len = 0;
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    int n = DtlsServerV.n;
    ...
    uint8_t pt[64];
    DtlsCiphertext info;
    proto_bool dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &st.cli_app_read, 0, out, (size_t)n, pt, sizeof(pt), &info, NULL, 0);
    TEST_ASSERT_TRUE(dtls_record_ok);
...
void test_forged_record_does_not_end_the_association(void)
{
    ...
    junk[4] = 0x2B;
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = junk;
    DtlsServerV.process_args.len = sizeof(junk);
    DtlsServerV.process_args.out = out;
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_EQUAL_INT(0, DtlsServerV.n);
    ...
    uint8_t apprec[64];
    size_t dtls_record_n =
        DtlsRecord.protect(dtls_record_work, &st.cli_app_write, 0, PROTOCORE_DTLS_CT_APPLICATION_DATA, payload,
                           sizeof(payload), apprec, sizeof(apprec), NULL, 0);
    size_t pl = dtls_record_n;
    TEST_ASSERT_TRUE(pl > 0);
    uint8_t plain[64];
    size_t plen = 0;
    DtlsServerV.open_app_args.c = &g_dtls;
    DtlsServerV.open_app_args.rec = apprec;
    DtlsServerV.open_app_args.rec_len = pl;
    DtlsServerV.open_app_args.out = plain;
    DtlsServerV.open_app_args.out_cap = sizeof(plain);
    DtlsServerV.open_app_args.out_len = &plen;
...
void test_app_records_before_and_after_established(void)
{
    ...
    DtlsServerV.open_app_args.c = &g_dtls;
    DtlsServerV.open_app_args.rec = dummy;
    DtlsServerV.open_app_args.rec_len = sizeof(dummy);
    DtlsServerV.open_app_args.out = plain;
    DtlsServerV.open_app_args.out_cap = sizeof(plain);
    DtlsServerV.open_app_args.out_len = &plen;
    DtlsServer.open_app(dtls_server_work);
    TEST_ASSERT_FALSE(DtlsServerV.ok);
    DtlsServerV.seal_app_args.c = &g_dtls;
    DtlsServerV.seal_app_args.data = payload;
    DtlsServerV.seal_app_args.len = sizeof(payload);
    DtlsServerV.seal_app_args.out = plain;
    DtlsServerV.seal_app_args.out_cap = sizeof(plain);
    DtlsServer.seal_app(dtls_server_work);
    size_t sealed = DtlsServerV.n;
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)sealed);
    DtlsServerV.app_write_keys_args.c = &g_dtls;
    DtlsServer.app_write_keys(dtls_server_work);
    TEST_ASSERT_NULL(DtlsServerV.ptr);
    DtlsServerV.app_read_keys_args.c = &g_dtls;
    DtlsServer.app_read_keys(dtls_server_work);
    TEST_ASSERT_NULL(DtlsServerV.ptr);
    uint8_t cid_out[PROTOCORE_DTLS_CID_MAX];
    DtlsServerV.local_cid_args.c = &g_dtls;
    DtlsServerV.local_cid_args.out = cid_out;
    DtlsServer.local_cid(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)DtlsServerV.n);

    ClientSession st;
    TEST_ASSERT_TRUE(run_to_finished(&g_dtls2, &cfg, &st));
    uint8_t out[256];
    TEST_ASSERT_TRUE(feed_client_finished(&g_dtls2, &st, 0, out, sizeof(out)) > 0);
    DtlsServerV.established_args.c = &g_dtls2;
    DtlsServer.established(dtls_server_work);
    TEST_ASSERT_TRUE(DtlsServerV.ok);
    DtlsServerV.app_write_keys_args.c = &g_dtls2;
    DtlsServer.app_write_keys(dtls_server_work);
    TEST_ASSERT_NOT_NULL(DtlsServerV.ptr);
    ...
    junk[3] = 0x00;
    junk[4] = 0x2B;
    DtlsServerV.open_app_args.c = &g_dtls2;
    DtlsServerV.open_app_args.rec = junk;
    DtlsServerV.open_app_args.rec_len = sizeof(junk);
    DtlsServerV.open_app_args.out = plain;
    DtlsServerV.open_app_args.out_cap = sizeof(plain);
    DtlsServerV.open_app_args.out_len = &plen;
    DtlsServer.open_app(dtls_server_work);
    ...
    uint8_t ackrec[64];
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &st.cli_app_write, 0, PROTOCORE_DTLS_CT_ACK, payload,
                                              sizeof(payload), ackrec, sizeof(ackrec), NULL, 0);
    size_t al = dtls_record_n;
    TEST_ASSERT_TRUE(al > 0);
    DtlsServerV.open_app_args.c = &g_dtls2;
    DtlsServerV.open_app_args.rec = ackrec;
    DtlsServerV.open_app_args.rec_len = al;
    DtlsServerV.open_app_args.out = plain;
    DtlsServerV.open_app_args.out_cap = sizeof(plain);
    DtlsServerV.open_app_args.out_len = &plen;
    DtlsServer.open_app(dtls_server_work);
    ...
    uint8_t apprec[64];
    dtls_record_n = DtlsRecord.protect(dtls_record_work, &st.cli_app_write, 1, PROTOCORE_DTLS_CT_APPLICATION_DATA,
                                       payload, sizeof(payload), apprec, sizeof(apprec), NULL, 0);
    size_t pl = dtls_record_n;
    TEST_ASSERT_TRUE(pl > 0);
    DtlsServerV.open_app_args.c = &g_dtls2;
    DtlsServerV.open_app_args.rec = apprec;
    ...
    uint8_t srec[64];
    DtlsServerV.seal_app_args.c = &g_dtls2;
    DtlsServerV.seal_app_args.data = payload;
    DtlsServerV.seal_app_args.len = sizeof(payload);
    DtlsServerV.seal_app_args.out = srec;
    DtlsServerV.seal_app_args.out_cap = sizeof(srec);
    DtlsServer.seal_app(dtls_server_work);
    size_t sl = DtlsServerV.n;
    TEST_ASSERT_TRUE(sl > 0);
    DtlsCiphertext info;
    proto_bool dtls_record_ok =
        DtlsRecord.unprotect(dtls_record_work, &st.cli_app_read, 1, srec, sl, plain, sizeof(plain), &info, NULL, 0);
    TEST_ASSERT_TRUE(dtls_record_ok);
...
void test_conn_id_edge_cases(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0, (uint32_t)DtlsServerV.n);
    DtlsPlaintext pt;
    size_t dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, flight, (size_t)fa, &pt);
    size_t shl = dtls_record_n;
    ...
    DtlsServerV.process_args.out_cap = sizeof(flight);
    DtlsServer.process(dtls_server_work);
    int fb = DtlsServerV.n;
    TEST_ASSERT_TRUE(fb > 0);
    DtlsServerV.local_cid_args.c = &g_dtls2;
    DtlsServerV.local_cid_args.out = cid_out;
    DtlsServer.local_cid(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT32((uint32_t)PROTOCORE_DTLS_CONN_LOCAL_CID_LEN, (uint32_t)DtlsServerV.n);
    TEST_ASSERT_EQUAL_MEMORY(SERVER_RANDOM, cid_out, PROTOCORE_DTLS_CONN_LOCAL_CID_LEN);
    dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, flight, (size_t)fb, &pt);
    size_t shl_b = dtls_record_n;
...
static proto_bool hrr_roundtrip_accepted(const uint8_t *addr, size_t addr_len)
{
    ...
    DtlsServerV.init_args.c = &g_dtls;
    DtlsServerV.init_args.cfg = &cfg;
    DtlsServerV.init_args.peer_addr = addr;
    DtlsServerV.init_args.peer_addr_len = addr_len;
    DtlsServer.init(dtls_server_work);
    ...
    DtlsPlaintext pt;
    size_t dtls_record_n = DtlsRecord.plaintext_parse(dtls_record_work, hrr_flight, (size_t)hf, &pt);
    if (!dtls_record_n)
    {
        return PROTO_FALSE;
    }
    ...
    DtlsServerV.process_args.c = &g_dtls;
    DtlsServerV.process_args.dgram = rec;
    DtlsServerV.process_args.len = r2;
    DtlsServerV.process_args.out = flight;
    DtlsServerV.process_args.out_cap = sizeof(flight);
    DtlsServer.process(dtls_server_work);
    return DtlsServerV.n > 0;
}
...
void test_cookie_is_worthless_to_another_peer(void)
{
    ...
    DtlsServerV.init_args.c = &g_dtls2;
    DtlsServerV.init_args.cfg = &cfg;
    DtlsServerV.init_args.peer_addr = OTHER_ADDR;
    DtlsServerV.init_args.peer_addr_len = sizeof(OTHER_ADDR);
    DtlsServer.init(dtls_server_work);
    uint8_t flight[2048];
    DtlsServerV.process_args.c = &g_dtls2;
    DtlsServerV.process_args.dgram = rec1;
    DtlsServerV.process_args.len = r1b;
    DtlsServerV.process_args.out = flight;
    DtlsServerV.process_args.out_cap = sizeof(flight);
    DtlsServer.process(dtls_server_work);
    TEST_ASSERT_TRUE(DtlsServerV.n > 0);
...
void test_epoch2_other_content_type_ignored(void)
{
    ...
    uint8_t rec[128];
    size_t dtls_record_n = DtlsRecord.protect(dtls_record_work, &st.cli_hs_write, 0, PROTOCORE_DTLS_CT_APPLICATION_DATA,
                                              payload, sizeof(payload), rec, sizeof(rec), NULL, 0);
    size_t rl = dtls_record_n;
    TEST_ASSERT_TRUE(rl > 0);
    uint8_t out[128];
    DtlsServerV.process_args.c = &g_dtls;
...
void test_established_requires_app_keys(void)
{
    ...
    size_t app_len = 0;
    DtlsServerV.open_app_args.c = &g_dtls;
    DtlsServerV.open_app_args.rec = out;
    DtlsServerV.open_app_args.rec_len = 16;
    DtlsServerV.open_app_args.out = app;
    DtlsServerV.open_app_args.out_cap = sizeof(app);
    DtlsServerV.open_app_args.out_len = &app_len;
    DtlsServer.open_app(dtls_server_work);
    TEST_ASSERT_FALSE(DtlsServerV.ok);
}
...
static void low_order_share_is_refused(const uint8_t client_pub[32], const char *what)
{
    ...
    DtlsServerV.process_args.out_cap = sizeof(out);
    DtlsServer.process(dtls_server_work);
    int rc = DtlsServerV.n;
    TEST_ASSERT_EQUAL_INT_MESSAGE(-1, rc, what);
    DtlsServerV.alert_args.c = &g_dtls;
    DtlsServer.alert(dtls_server_work);
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(47, DtlsServerV.value, what);
}
