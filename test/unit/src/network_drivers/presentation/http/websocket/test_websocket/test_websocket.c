...
void test_sha1_empty_string()
{
    ...
    Sha1.hash(tw, (const uint8_t *)"", 0, digest);
...
{
    uint8_t digest[PROTOCORE_SHA1_DIGEST_LEN];
    Sha1.hash(tw, (const uint8_t *)"abc", 3, digest);
...
void test_sha1_rfc6455_handshake_key()
{
    ...
    uint8_t digest[PROTOCORE_SHA1_DIGEST_LEN];
    Sha1.hash(tw, (const uint8_t *)input, strlen(input), digest);
...
void test_sha1_different_inputs_different_digests()
{
    ...
    Sha1.hash(tw, (const uint8_t *)"abc", 3, d1);
    Sha1.hash(tw, (const uint8_t *)"abd", 3, d2);
