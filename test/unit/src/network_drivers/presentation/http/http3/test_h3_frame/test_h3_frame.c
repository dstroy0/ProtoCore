...
void test_rfc9000_sample_varints_as_frame_lengths(void)
{
    ...
    {
        size_t h3_frame_n = H3Frame.write_header(h3_frame_work, g_out, sizeof(g_out), H3_DATA, CASES[i].length);
        size_t n = h3_frame_n;
        ...
        H3FrameHeader f;
        proto_bool h3_frame_ok = H3Frame.parse_header(h3_frame_work, g_out, n, &f);
...
void test_rfc9000_long_spelling_decodes_but_is_not_emitted(void)
{
    ...
    H3FrameHeader f;
    proto_bool h3_frame_ok = H3Frame.parse_header(h3_frame_work, LONG_FORM, sizeof(LONG_FORM), &f);
    TEST_ASSERT_TRUE(h3_frame_ok);
    TEST_ASSERT_EQUAL_HEX64(37u, f.length);
    TEST_ASSERT_EQUAL_UINT(3u, f.header_len);

    size_t h3_frame_n = H3Frame.write_header(h3_frame_work, g_out, sizeof(g_out), H3_DATA, 37);
    TEST_ASSERT_EQUAL_UINT(2u, h3_frame_n);
...
void test_rfc9114_reserved_http2_frame_types(void)
{
    ...
    {
        proto_bool h3_frame_ok = H3Frame.type_reserved(h3_frame_work, RESERVED[i]);
        TEST_ASSERT_TRUE(h3_frame_ok);
    }
    for (size_t i = 0; i < 7; i++)
    {
        proto_bool h3_frame_ok = H3Frame.type_reserved(h3_frame_work, ASSIGNED[i]);
        TEST_ASSERT_FALSE(h3_frame_ok);
    }
    // sec 7.2.8 grease: 0x1f * N + 0x21, here N = 0 and N = 1
    proto_bool h3_frame_ok = H3Frame.type_reserved(h3_frame_work, 0x21);
    TEST_ASSERT_FALSE(h3_frame_ok);
    h3_frame_ok = H3Frame.type_reserved(h3_frame_work, 0x40);
    TEST_ASSERT_FALSE(h3_frame_ok);
}
...
{
    H3Settings s;
    H3Frame.settings_defaults(h3_frame_work, &s);
...
void test_rfc9114_settings_round_trip(void)
{
    ...
    size_t h3_frame_n = H3Frame.build_settings(h3_frame_work, g_out, sizeof(g_out), IDS, VALS, 4);
    size_t n = h3_frame_n;
    ...
    H3Settings s;
    H3Frame.settings_defaults(h3_frame_work, &s);
    h3_frame_ok = H3Frame.parse_settings(h3_frame_work, g_out + f.header_len, (size_t)f.length, &s);
    TEST_ASSERT_TRUE(h3_frame_ok);
...
void test_rfc9114_reserved_settings_identifiers(void)
{
    for (uint8_t id = 0x02; id <= 0x05; id++)
    {
        ...
        H3Settings s;
        H3Frame.settings_defaults(h3_frame_work, &s);
        proto_bool h3_frame_ok = H3Frame.parse_settings(h3_frame_work, pay, sizeof(pay), &s);
        TEST_ASSERT_FALSE(h3_frame_ok);
    }
    // an empty SETTINGS payload is legal and changes nothing
    H3Settings s;
    H3Frame.settings_defaults(h3_frame_work, &s);
    proto_bool h3_frame_ok = H3Frame.parse_settings(h3_frame_work, g_out, 0, &s);
    TEST_ASSERT_TRUE(h3_frame_ok);
    TEST_ASSERT_EQUAL_HEX64(0xFFFFFFFFFFFFFFFFULL, s.max_field_section_size);
}
...
void test_rfc9114_settings_truncated_pair(void)
{
    ...
    H3Settings s;
    H3Frame.settings_defaults(h3_frame_work, &s);
    proto_bool h3_frame_ok = H3Frame.parse_settings(h3_frame_work, ODD, sizeof(ODD), &s);
    TEST_ASSERT_FALSE(h3_frame_ok);

    static const uint8_t SHORT_VAL[2] = {H3_SETTINGS_MAX_FIELD_SECTION_SIZE, 0x40}; // 2-byte value, 1 present
    H3Frame.settings_defaults(h3_frame_work, &s);
    h3_frame_ok = H3Frame.parse_settings(h3_frame_work, SHORT_VAL, sizeof(SHORT_VAL), &s);
    TEST_ASSERT_FALSE(h3_frame_ok);
}
...
void test_rfc9114_data_and_headers_builders(void)
{
    ...
    size_t h3_frame_n = H3Frame.build_data(h3_frame_work, g_out, sizeof(g_out), BODY, sizeof(BODY));
    size_t n = h3_frame_n;
    ...
    h3_frame_n = H3Frame.build_headers(h3_frame_work, g_out, sizeof(g_out), BODY, sizeof(BODY));
    n = h3_frame_n;
    TEST_ASSERT_EQUAL_UINT(2u + 5u, n);
    h3_frame_ok = H3Frame.parse_header(h3_frame_work, g_out, n, &f);
    TEST_ASSERT_TRUE(h3_frame_ok);
    TEST_ASSERT_EQUAL_HEX64(H3_HEADERS, f.type);
    TEST_ASSERT_EQUAL_HEX64(5u, f.length);

    // a zero-length DATA frame is the two header varints and nothing else
    h3_frame_n = H3Frame.build_data(h3_frame_work, g_out, sizeof(g_out), NULL, 0);
    n = h3_frame_n;
...
{
    size_t h3_frame_n = H3Frame.build_goaway(h3_frame_work, g_out, sizeof(g_out), 15293u);
    size_t n = h3_frame_n;
...
void test_truncated_header_is_refused(void)
{
    ...
    proto_bool h3_frame_ok = H3Frame.parse_header(h3_frame_work, TYPE_ONLY, sizeof(TYPE_ONLY), &f);
    TEST_ASSERT_FALSE(h3_frame_ok);
    h3_frame_ok = H3Frame.parse_header(h3_frame_work, SHORT_LEN, sizeof(SHORT_LEN), &f);
    TEST_ASSERT_FALSE(h3_frame_ok);
    h3_frame_ok = H3Frame.parse_header(h3_frame_work, SHORT_TYPE, sizeof(SHORT_TYPE), &f);
    TEST_ASSERT_FALSE(h3_frame_ok);
    h3_frame_ok = H3Frame.parse_header(h3_frame_work, TYPE_ONLY, 0, &f);
    TEST_ASSERT_FALSE(h3_frame_ok);
}
...
void test_builders_refuse_a_short_destination(void)
{
    ...
    size_t h3_frame_n = H3Frame.write_header(h3_frame_work, g_out, 0, H3_DATA, 5);
    TEST_ASSERT_EQUAL_UINT(0u, h3_frame_n);
    h3_frame_n = H3Frame.write_header(h3_frame_work, g_out, 1, H3_DATA, 15293);
    TEST_ASSERT_EQUAL_UINT(0u, h3_frame_n);
    h3_frame_n = H3Frame.build_data(h3_frame_work, g_out, 6, BODY, sizeof(BODY));
    TEST_ASSERT_EQUAL_UINT(0u, h3_frame_n);
    h3_frame_n = H3Frame.build_headers(h3_frame_work, g_out, 6, BODY, sizeof(BODY));
    TEST_ASSERT_EQUAL_UINT(0u, h3_frame_n);
    h3_frame_n = H3Frame.build_settings(h3_frame_work, g_out, 4, IDS, VALS, 1);
    TEST_ASSERT_EQUAL_UINT(0u, h3_frame_n);
    h3_frame_n = H3Frame.build_goaway(h3_frame_work, g_out, 2, 15293u);
    TEST_ASSERT_EQUAL_UINT(0u, h3_frame_n);
}
