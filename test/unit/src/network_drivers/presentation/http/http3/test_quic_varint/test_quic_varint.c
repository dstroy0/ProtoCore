...
static void vector(uint64_t value, const uint8_t *bytes, size_t n)
{
    ...
    size_t quic_varint_n = QuicVarint.len(quic_varint_work, value);
    TEST_ASSERT_EQUAL_UINT(n, quic_varint_n);

    memset(out, 0xAA, sizeof(out));
    quic_varint_n = QuicVarint.encode(quic_varint_work, out, sizeof(out), value);
    TEST_ASSERT_EQUAL_UINT(n, quic_varint_n);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(bytes, out, n);

    proto_bool quic_varint_ok = QuicVarint.decode(quic_varint_work, bytes, n, &v, &consumed);
...
void test_non_minimal_encoding_decodes(void)
{
    ...
    uint64_t v = 0;
    size_t consumed = 0;
    proto_bool quic_varint_ok = QuicVarint.decode(quic_varint_work, NM, sizeof(NM), &v, &consumed);
    TEST_ASSERT_TRUE(quic_varint_ok);
    ...
    uint8_t out[8];
    size_t quic_varint_n = QuicVarint.encode(quic_varint_work, out, sizeof(out), 37u);
    TEST_ASSERT_EQUAL_UINT(1u, quic_varint_n);
...
{
    size_t quic_varint_n = QuicVarint.len(quic_varint_work, 0u);
    TEST_ASSERT_EQUAL_UINT(1u, quic_varint_n);
    quic_varint_n = QuicVarint.len(quic_varint_work, 63u);
    TEST_ASSERT_EQUAL_UINT(1u, quic_varint_n);
    quic_varint_n = QuicVarint.len(quic_varint_work, 64u);
    TEST_ASSERT_EQUAL_UINT(2u, quic_varint_n);
    quic_varint_n = QuicVarint.len(quic_varint_work, 16383u);
    TEST_ASSERT_EQUAL_UINT(2u, quic_varint_n);
    quic_varint_n = QuicVarint.len(quic_varint_work, 16384u);
    TEST_ASSERT_EQUAL_UINT(4u, quic_varint_n);
    quic_varint_n = QuicVarint.len(quic_varint_work, 1073741823u);
    TEST_ASSERT_EQUAL_UINT(4u, quic_varint_n);
    quic_varint_n = QuicVarint.len(quic_varint_work, 1073741824u);
    TEST_ASSERT_EQUAL_UINT(8u, quic_varint_n);
    quic_varint_n = QuicVarint.len(quic_varint_work, 4611686018427387903ull);
    TEST_ASSERT_EQUAL_UINT(8u, quic_varint_n);
...
void test_boundary_encodings_carry_their_prefix(void)
{
    ...
    quic_varint_n = QuicVarint.encode(quic_varint_work, out, sizeof(out), QUIC_VARINT_MAX);
    TEST_ASSERT_EQUAL_UINT(8u, quic_varint_n);
...
{
    uint8_t out[8];
    size_t quic_varint_n = QuicVarint.len(quic_varint_work, QUIC_VARINT_MAX + 1u);
    TEST_ASSERT_EQUAL_UINT(0u, quic_varint_n);
    quic_varint_n = QuicVarint.len(quic_varint_work, 0xFFFFFFFFFFFFFFFFull);
    TEST_ASSERT_EQUAL_UINT(0u, quic_varint_n);
    quic_varint_n = QuicVarint.encode(quic_varint_work, out, sizeof(out), QUIC_VARINT_MAX + 1u);
    TEST_ASSERT_EQUAL_UINT(0u, quic_varint_n);
}
...
void test_encode_refuses_a_short_buffer(void)
{
    uint8_t out[8];
    memset(out, 0xAA, sizeof(out));
    size_t quic_varint_n = QuicVarint.encode(quic_varint_work, out, 1, 16384u);
    TEST_ASSERT_EQUAL_UINT(0u, quic_varint_n);
    TEST_ASSERT_EQUAL_HEX8(0xAA, out[0]);
    quic_varint_n = QuicVarint.encode(quic_varint_work, out, 3, 16384u);
    TEST_ASSERT_EQUAL_UINT(0u, quic_varint_n);
    quic_varint_n = QuicVarint.encode(quic_varint_work, out, 7, QUIC_VARINT_MAX);
    TEST_ASSERT_EQUAL_UINT(0u, quic_varint_n);
    quic_varint_n = QuicVarint.encode(quic_varint_work, out, 0, 0u);
    TEST_ASSERT_EQUAL_UINT(0u, quic_varint_n);

    // cap exactly equal to the encoding length is enough.
    quic_varint_n = QuicVarint.encode(quic_varint_work, out, 4, 16384u);
    TEST_ASSERT_EQUAL_UINT(4u, quic_varint_n);
}
...
void test_decode_refuses_a_truncated_input(void)
{
    ...
    proto_bool quic_varint_ok = QuicVarint.decode(quic_varint_work, TRUNC4, sizeof(TRUNC4), &v, &consumed);
    TEST_ASSERT_FALSE(quic_varint_ok);
    quic_varint_ok = QuicVarint.decode(quic_varint_work, TRUNC8, sizeof(TRUNC8), &v, &consumed);
    TEST_ASSERT_FALSE(quic_varint_ok);
    quic_varint_ok = QuicVarint.decode(quic_varint_work, TRUNC2, sizeof(TRUNC2), &v, &consumed);
    TEST_ASSERT_FALSE(quic_varint_ok);
    quic_varint_ok = QuicVarint.decode(quic_varint_work, TRUNC2, 0, &v, &consumed);
    TEST_ASSERT_FALSE(quic_varint_ok);
}
...
void test_round_trip_over_every_length_class(void)
{
    ...
    for (size_t i = 0; i < sizeof(VALUES) / sizeof(VALUES[0]); i++)
    {
        ...
        uint64_t v = 0;
        size_t consumed = 0;
        size_t quic_varint_n = QuicVarint.encode(quic_varint_work, buf, sizeof(buf), VALUES[i]);
        size_t n = quic_varint_n;
        TEST_ASSERT_TRUE(n == 1u || n == 2u || n == 4u || n == 8u);
        proto_bool quic_varint_ok = QuicVarint.decode(quic_varint_work, buf, n, &v, &consumed);
        TEST_ASSERT_TRUE(quic_varint_ok);
        TEST_ASSERT_EQUAL_UINT(n, consumed);
