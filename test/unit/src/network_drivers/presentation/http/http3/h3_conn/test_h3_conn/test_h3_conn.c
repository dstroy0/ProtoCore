...
void test_request_dispatch_and_response()
{
    ...
    bp += qpack_n;
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, ":path", 5, "/index.html", 11);
    bp += qpack_n;
    ...
    while (off < st->tx_have)
    {
        ...
        {
            Qpack.decode(qpack_work, fp, (size_t)fr.length, scratch, sizeof(scratch), protocore_resp_emit, NULL);
...
void test_post_with_body()
{
    ...
    size_t qpack_n = Qpack.encode_prefix(qpack_work, block, sizeof(block));
    size_t bp = qpack_n;
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, ":method", 7, "POST", 4);
    bp += qpack_n;
...
void test_overlong_field_truncated()
{
    ...
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, ":method", 7, m, 26);
    bp += qpack_n;
...
void test_h3_pseudo_header_name_variants()
{
    ...
    size_t qpack_n = Qpack.encode_prefix(qpack_work, block, sizeof(block));
    size_t bp = qpack_n;
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, ":scheme", 7, "https", 5);
    bp += qpack_n;
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, "hello", 5, "world", 5);
    bp += qpack_n;
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, "user-agent", 10, "curl", 4);
    bp += qpack_n;
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, "abc", 3, "z", 1);
    bp += qpack_n;
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, ":method", 7, "GET", 3);
    bp += qpack_n;
...
void test_h3_request_unknown_frame_and_empty_data()
{
    ...
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, ":method", 7, "POST", 4);
    bp += qpack_n;
    qpack_n = Qpack.encode_header(qpack_work, block + bp, sizeof(block) - bp, ":path", 5, "/u", 2);
    bp += qpack_n;
