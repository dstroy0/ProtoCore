...
{
    QuicFrameHeader f;
    size_t quic_frame_n = QuicFrame.build_ping(quic_frame_work, g_out, sizeof(g_out));
    TEST_ASSERT_EQUAL_UINT(1u, quic_frame_n);
    TEST_ASSERT_EQUAL_HEX8(0x01, g_out[0]);
    quic_frame_n = QuicFrame.parse(quic_frame_work, g_out, 1, &f);
    TEST_ASSERT_EQUAL_UINT(1u, quic_frame_n);
    TEST_ASSERT_EQUAL_HEX64(QUIC_FT_PING, f.type);

    quic_frame_n = QuicFrame.build_handshake_done(quic_frame_work, g_out, sizeof(g_out));
    TEST_ASSERT_EQUAL_UINT(1u, quic_frame_n);
    ...
    quic_frame_n = QuicFrame.build_padding(quic_frame_work, g_out, sizeof(g_out), 5);
    TEST_ASSERT_EQUAL_UINT(5u, quic_frame_n);
    TEST_ASSERT_EQUAL_MEMORY(ZEROS, g_out, 5);
    quic_frame_n = QuicFrame.parse(quic_frame_work, g_out, 5, &f);
    TEST_ASSERT_EQUAL_UINT(1u, quic_frame_n);
...
void test_rfc9000_ack_frame_fields(void)
{
    ...
    QuicFrameHeader f;
    size_t quic_frame_n = QuicFrame.build_ack(quic_frame_work, g_out, sizeof(g_out), 15293u, 37u, 1u);
    TEST_ASSERT_EQUAL_UINT(6u, quic_frame_n);
    TEST_ASSERT_EQUAL_MEMORY(WANT, g_out, 6);

    quic_frame_n = QuicFrame.parse(quic_frame_work, WANT, sizeof(WANT), &f);
    TEST_ASSERT_EQUAL_UINT(6u, quic_frame_n);
...
void test_rfc9000_ack_ranges_and_ecn_are_consumed(void)
{
    ...
    QuicFrameHeader f;
    size_t quic_frame_n = QuicFrame.parse(quic_frame_work, WITH_RANGES, sizeof(WITH_RANGES), &f);
    TEST_ASSERT_EQUAL_UINT(9u, quic_frame_n);
    ...
    quic_frame_n = QuicFrame.parse(quic_frame_work, WITH_ECN, sizeof(WITH_ECN), &f);
    TEST_ASSERT_EQUAL_UINT(8u, quic_frame_n);
    ...
    quic_frame_n = QuicFrame.parse(quic_frame_work, NO_ECN, sizeof(NO_ECN), &f);
    TEST_ASSERT_EQUAL_UINT(5u, quic_frame_n);
}
...
void test_rfc9000_crypto_frame(void)
{
    ...
    QuicFrameHeader f;
    size_t quic_frame_n = QuicFrame.build_crypto(quic_frame_work, g_out, sizeof(g_out), 15293u, DATA, sizeof(DATA));
    TEST_ASSERT_EQUAL_UINT(7u, quic_frame_n);
...
void test_rfc9000_stream_frame_type_bits(void)
{
    ...
    size_t quic_frame_n =
        QuicFrame.build_stream(quic_frame_work, g_out, sizeof(g_out), 4u, 0u, (const uint8_t *)"hi", 2, PROTO_FALSE);
    TEST_ASSERT_EQUAL_UINT(5u, quic_frame_n);
    TEST_ASSERT_EQUAL_MEMORY(LEN_ONLY, g_out, 5);
    quic_frame_n = QuicFrame.parse(quic_frame_work, LEN_ONLY, sizeof(LEN_ONLY), &f);
    TEST_ASSERT_EQUAL_UINT(5u, quic_frame_n);
    ...
    quic_frame_n =
        QuicFrame.build_stream(quic_frame_work, g_out, sizeof(g_out), 4u, 8u, (const uint8_t *)"hi", 2, PROTO_TRUE);
    TEST_ASSERT_EQUAL_UINT(6u, quic_frame_n);
    TEST_ASSERT_EQUAL_MEMORY(ALL_BITS, g_out, 6);
    quic_frame_n = QuicFrame.parse(quic_frame_work, ALL_BITS, sizeof(ALL_BITS), &f);
    TEST_ASSERT_EQUAL_UINT(6u, quic_frame_n);
    ...
    quic_frame_n = QuicFrame.parse(quic_frame_work, NO_LEN, sizeof(NO_LEN), &f);
    TEST_ASSERT_EQUAL_UINT(5u, quic_frame_n);
    TEST_ASSERT_EQUAL_HEX64(4u, f.stream.id);
    TEST_ASSERT_EQUAL_HEX64(0u, f.stream.offset);
    ...
    quic_frame_n = QuicFrame.parse(quic_frame_work, FIN_ONLY, sizeof(FIN_ONLY), &f);
    TEST_ASSERT_EQUAL_UINT(2u, quic_frame_n);
...
void test_rfc9000_max_data(void)
{
    ...
    QuicFrameHeader f;
    size_t quic_frame_n = QuicFrame.build_max_data(quic_frame_work, g_out, sizeof(g_out), 1048576u);
    TEST_ASSERT_EQUAL_UINT(5u, quic_frame_n);
...
void test_rfc9000_connection_close_variants(void)
{
    ...
    size_t quic_frame_n = QuicFrame.build_connection_close(quic_frame_work, g_out, sizeof(g_out), PROTO_FALSE,
                                                           QUIC_ERR_PROTOCOL_VIOLATION, QUIC_FT_CRYPTO, "no", 2);
    TEST_ASSERT_EQUAL_UINT(6u, quic_frame_n);
    TEST_ASSERT_EQUAL_MEMORY(TRANSPORT, g_out, 6);
    quic_frame_n = QuicFrame.parse(quic_frame_work, TRANSPORT, sizeof(TRANSPORT), &f);
    TEST_ASSERT_EQUAL_UINT(6u, quic_frame_n);
    ...
    quic_frame_n = QuicFrame.build_connection_close(quic_frame_work, g_out, sizeof(g_out), PROTO_TRUE, 0x0100u,
                                                    QUIC_FT_CRYPTO, NULL, 0);
    TEST_ASSERT_EQUAL_UINT(4u, quic_frame_n);
    TEST_ASSERT_EQUAL_MEMORY(APP, g_out, 4);
    quic_frame_n = QuicFrame.parse(quic_frame_work, APP, sizeof(APP), &f);
    TEST_ASSERT_EQUAL_UINT(4u, quic_frame_n);
...
void test_rfc9000_unhandled_frames_consume_their_shape(void)
{
    ...
    {
        QuicFrameHeader f;
        size_t quic_frame_n = QuicFrame.parse(quic_frame_work, CASES[i].buf, CASES[i].len, &f);
        TEST_ASSERT_EQUAL_UINT(CASES[i].len, quic_frame_n);
...
void test_rfc9000_fixed_width_frames(void)
{
    ...
    size_t quic_frame_n = QuicFrame.parse(quic_frame_work, buf, sizeof(buf), &f);
    TEST_ASSERT_EQUAL_UINT(24u, quic_frame_n);
    TEST_ASSERT_EQUAL_HEX64(QUIC_FT_NEW_CONNECTION_ID, f.type);

    uint8_t path[9];
    memset(path, 0x5a, sizeof(path));
    path[0] = QUIC_FT_PATH_CHALLENGE;
    quic_frame_n = QuicFrame.parse(quic_frame_work, path, sizeof(path), &f);
    TEST_ASSERT_EQUAL_UINT(9u, quic_frame_n);
...
void test_truncated_frames_are_refused(void)
{
    ...
    {
        size_t quic_frame_n = QuicFrame.parse(quic_frame_work, BAD[i].buf, BAD[i].len, &f);
        TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
    }
    size_t quic_frame_n = QuicFrame.parse(quic_frame_work, g_out, 0, &f);
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
}
...
void test_builders_refuse_a_short_destination(void)
{
    ...
    size_t quic_frame_n = QuicFrame.build_ping(quic_frame_work, g_out, 0);
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
    quic_frame_n = QuicFrame.build_handshake_done(quic_frame_work, g_out, 0);
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
    quic_frame_n = QuicFrame.build_padding(quic_frame_work, g_out, 4, 5);
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
    quic_frame_n = QuicFrame.build_ack(quic_frame_work, g_out, 3, 15293u, 37u, 1u);
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
    quic_frame_n = QuicFrame.build_crypto(quic_frame_work, g_out, 4, 0, DATA, sizeof(DATA));
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
    quic_frame_n = QuicFrame.build_stream(quic_frame_work, g_out, 4, 4u, 0, DATA, sizeof(DATA), PROTO_FALSE);
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
    quic_frame_n = QuicFrame.build_max_data(quic_frame_work, g_out, 1, 1048576u);
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
    quic_frame_n = QuicFrame.build_connection_close(quic_frame_work, g_out, 3, PROTO_FALSE, 0, 0, "reason", 6);
    TEST_ASSERT_EQUAL_UINT(0u, quic_frame_n);
}
