...
void test_rfc9001_published_headers(void)
{
    ...
    proto_bool quic_packet_ok = QuicPacket.is_long_header(quic_packet_work, A2_CLIENT_INITIAL[0]);
    TEST_ASSERT_TRUE(quic_packet_ok);
    quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, A2_CLIENT_INITIAL, sizeof(A2_CLIENT_INITIAL), &h);
    TEST_ASSERT_TRUE(quic_packet_ok);
    ...
    quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, A3_SERVER_INITIAL, sizeof(A3_SERVER_INITIAL), &h);
    TEST_ASSERT_TRUE(quic_packet_ok);
    ...
    quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, A4_RETRY, sizeof(A4_RETRY), &h);
    TEST_ASSERT_TRUE(quic_packet_ok);
...
void test_build_reproduces_the_published_headers(void)
{
    ...
    size_t quic_packet_n = QuicPacket.build_long_header(quic_packet_work, g_out, sizeof(g_out), QUIC_LP_INITIAL,
                                                        QUIC_VERSION_1, A2_DCID, 8, NONE, 0, 4);
    TEST_ASSERT_EQUAL_UINT(15u, quic_packet_n);
    TEST_ASSERT_EQUAL_MEMORY(A2_CLIENT_INITIAL, g_out, 15);

    quic_packet_n = QuicPacket.build_long_header(quic_packet_work, g_out, sizeof(g_out), QUIC_LP_INITIAL,
                                                 QUIC_VERSION_1, NONE, 0, A3_SCID, 8, 2);
    TEST_ASSERT_EQUAL_UINT(15u, quic_packet_n);
...
void test_rfc9000_long_packet_types(void)
{
    ...
    {
        QuicLongHeader h;
        size_t quic_packet_n = QuicPacket.build_long_header(quic_packet_work, g_out, sizeof(g_out), TYPES[i],
                                                            QUIC_VERSION_1, NONE, 0, NONE, 0, 1);
        size_t n = quic_packet_n;
        TEST_ASSERT_EQUAL_UINT(7u, n);
        TEST_ASSERT_EQUAL_HEX8((uint8_t)(0xC0 | (TYPES[i] << 4)), g_out[0]);
        proto_bool quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, g_out, n, &h);
        TEST_ASSERT_TRUE(quic_packet_ok);
...
void test_rfc9000_fixed_bit_is_required(void)
{
    ...
    buf[0] &= (uint8_t)~0x40;
    proto_bool quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, buf, sizeof(buf), &h);
    TEST_ASSERT_FALSE(quic_packet_ok);

    QuicShortHeader s;
    uint8_t sh[2] = {0x42, 0x00};
    quic_packet_ok = QuicPacket.parse_short_header(quic_packet_work, sh, sizeof(sh), 0, &s);
    TEST_ASSERT_TRUE(quic_packet_ok);
...
void test_rfc9001_short_header(void)
{
    ...
    QuicShortHeader s;
    proto_bool quic_packet_ok = QuicPacket.is_long_header(quic_packet_work, A5[0]);
    TEST_ASSERT_FALSE(quic_packet_ok);
    quic_packet_ok = QuicPacket.parse_short_header(quic_packet_work, A5, sizeof(A5), 0, &s);
    TEST_ASSERT_TRUE(quic_packet_ok);
    ...
    quic_packet_ok = QuicPacket.parse_short_header(quic_packet_work, SPUN, sizeof(SPUN), 4, &s);
    TEST_ASSERT_TRUE(quic_packet_ok);
...
void test_rfc9000_version_negotiation(void)
{
    ...
    size_t quic_packet_n =
        QuicPacket.build_version_negotiation(quic_packet_work, g_out, sizeof(g_out), DCID, 2, SCID, 2, VERSIONS, 2);
    size_t n = quic_packet_n;
    ...
    quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, g_out, n, &h);
    TEST_ASSERT_TRUE(quic_packet_ok);
}
...
{
    uint8_t quic_packet_u8 = QuicPacket.pn_length(quic_packet_work, 0xac5c02u, (int64_t)0xabe8b3);
    TEST_ASSERT_EQUAL_UINT8(2, quic_packet_u8);
    quic_packet_u8 = QuicPacket.pn_length(quic_packet_work, 0xace8feu, (int64_t)0xabe8b3);
    TEST_ASSERT_EQUAL_UINT8(3, quic_packet_u8);

    // A.2's encode step is "truncate to the num_bytes least significant bytes", big-endian
    size_t quic_packet_n = QuicPacket.pn_encode(quic_packet_work, g_out, sizeof(g_out), 0xac5c02u, (int64_t)0xabe8b3);
    TEST_ASSERT_EQUAL_UINT(2u, quic_packet_n);
    TEST_ASSERT_EQUAL_HEX8(0x5c, g_out[0]);
    TEST_ASSERT_EQUAL_HEX8(0x02, g_out[1]);
    quic_packet_n = QuicPacket.pn_encode(quic_packet_work, g_out, sizeof(g_out), 0xace8feu, (int64_t)0xabe8b3);
    TEST_ASSERT_EQUAL_UINT(3u, quic_packet_n);
    ...
    quic_packet_u8 = QuicPacket.pn_length(quic_packet_work, 0u, -1);
    TEST_ASSERT_EQUAL_UINT8(1, quic_packet_u8);
    quic_packet_u8 = QuicPacket.pn_length(quic_packet_work, 126u, -1);
    TEST_ASSERT_EQUAL_UINT8(1, quic_packet_u8); // 2 * 127 = 254 fits in 8 bits
    quic_packet_u8 = QuicPacket.pn_length(quic_packet_work, 128u, -1);
    TEST_ASSERT_EQUAL_UINT8(2, quic_packet_u8); // 2 * 129 = 258 does not
    quic_packet_u8 = QuicPacket.pn_length(quic_packet_work, 1u << 24, -1);
    TEST_ASSERT_EQUAL_UINT8(4, quic_packet_u8);
    quic_packet_n = QuicPacket.pn_encode(quic_packet_work, g_out, 1, 0xac5c02u, (int64_t)0xabe8b3);
    TEST_ASSERT_EQUAL_UINT(0u, quic_packet_n);
}
...
{
    uint64_t quic_packet_u64 = QuicPacket.pn_decode(quic_packet_work, 0xa82f30eaULL, 0x9b32ULL, 16);
    TEST_ASSERT_EQUAL_HEX64(0xa82f9b32ULL, quic_packet_u64);
    ...
    quic_packet_u64 = QuicPacket.pn_decode(quic_packet_work, 0x2700bff3ULL, 0x00bff4ULL, 24);
    TEST_ASSERT_EQUAL_HEX64(0x2700bff4ULL, quic_packet_u64);
    ...
    quic_packet_u64 = QuicPacket.pn_decode(quic_packet_work, 0x27fULL, 0x00ULL, 8);
    TEST_ASSERT_EQUAL_HEX64(0x300ULL, quic_packet_u64);
    ...
    quic_packet_u64 = QuicPacket.pn_decode(quic_packet_work, 0x300ULL, 0xffULL, 8);
    TEST_ASSERT_EQUAL_HEX64(0x2ffULL, quic_packet_u64);
}
...
void test_connection_id_bounds(void)
{
    ...
    size_t quic_packet_n = QuicPacket.build_long_header(quic_packet_work, g_out, sizeof(g_out), QUIC_LP_INITIAL,
                                                        QUIC_VERSION_1, big, 21, NONE, 0, 1);
    TEST_ASSERT_EQUAL_UINT(0u, quic_packet_n);
    quic_packet_n = QuicPacket.build_long_header(quic_packet_work, g_out, sizeof(g_out), QUIC_LP_INITIAL,
                                                 QUIC_VERSION_1, NONE, 0, big, 21, 1);
    TEST_ASSERT_EQUAL_UINT(0u, quic_packet_n);
    // pn_len is 1..4 (sec 17.2: the field holds length - 1 in two bits)
    quic_packet_n = QuicPacket.build_long_header(quic_packet_work, g_out, sizeof(g_out), QUIC_LP_INITIAL,
                                                 QUIC_VERSION_1, NONE, 0, NONE, 0, 0);
    TEST_ASSERT_EQUAL_UINT(0u, quic_packet_n);
    quic_packet_n = QuicPacket.build_long_header(quic_packet_work, g_out, sizeof(g_out), QUIC_LP_INITIAL,
                                                 QUIC_VERSION_1, NONE, 0, NONE, 0, 5);
    TEST_ASSERT_EQUAL_UINT(0u, quic_packet_n);
    quic_packet_n =
        QuicPacket.build_long_header(quic_packet_work, g_out, 6, QUIC_LP_INITIAL, QUIC_VERSION_1, NONE, 0, NONE, 0, 1);
    TEST_ASSERT_EQUAL_UINT(0u, quic_packet_n);

    static const uint8_t OVERLONG_DCID[7] = {0xC0, 0x00, 0x00, 0x00, 0x01, 21, 0x00};
    proto_bool quic_packet_ok =
        QuicPacket.parse_long_header(quic_packet_work, OVERLONG_DCID, sizeof(OVERLONG_DCID), &h);
    TEST_ASSERT_FALSE(quic_packet_ok);

    quic_packet_n =
        QuicPacket.build_version_negotiation(quic_packet_work, g_out, sizeof(g_out), big, 21, NONE, 0, NULL, 0);
    TEST_ASSERT_EQUAL_UINT(0u, quic_packet_n);
    quic_packet_n = QuicPacket.build_version_negotiation(quic_packet_work, g_out, 6, NONE, 0, NONE, 0, NULL, 0);
    TEST_ASSERT_EQUAL_UINT(0u, quic_packet_n);
}
...
void test_truncated_headers_are_refused(void)
{
    QuicLongHeader h;
    QuicShortHeader s;
    proto_bool quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, A2_CLIENT_INITIAL, 6, &h);
    TEST_ASSERT_FALSE(quic_packet_ok); // shorter than the fixed part
    quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, A2_CLIENT_INITIAL, 10, &h);
    TEST_ASSERT_FALSE(quic_packet_ok); // DCID runs past the end
    quic_packet_ok = QuicPacket.parse_long_header(quic_packet_work, A2_CLIENT_INITIAL, 14, &h);
    TEST_ASSERT_FALSE(quic_packet_ok); // no room for the SCID length

    static const uint8_t SHORT_ONE[3] = {0x42, 0x11, 0x22};
    quic_packet_ok = QuicPacket.parse_short_header(quic_packet_work, SHORT_ONE, sizeof(SHORT_ONE), 4, &s);
    TEST_ASSERT_FALSE(quic_packet_ok);
    quic_packet_ok = QuicPacket.parse_short_header(quic_packet_work, SHORT_ONE, sizeof(SHORT_ONE), 21, &s);
    TEST_ASSERT_FALSE(quic_packet_ok);
}
