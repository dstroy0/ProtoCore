...
void test_rfc9112_3_request_line(void)
{
    ...
    TEST_ASSERT_EQUAL_STRING("hello", http_get_query(r, "q"));
    TEST_ASSERT_EQUAL_STRING("2", http_get_query(r, "n"));
    TEST_ASSERT_NULL(http_get_query(r, "missing"));

    // A key with no "=" carries an empty value; the key is still present.
    r = feed("GET /?flag&k=v HTTP/1.1\r\n\r\n");
    TEST_ASSERT_EQUAL_STRING("", http_get_query(r, "flag"));
    TEST_ASSERT_EQUAL_STRING("v", http_get_query(r, "k"));
}
...
void test_rfc9112_5_field_lines(void)
{
    ...
    TEST_ASSERT_EQUAL_STRING("example.com", http_get_header(r, "Host"));
    TEST_ASSERT_EQUAL_STRING("example.com", http_get_header(r, "HOST"));
    TEST_ASSERT_EQUAL_STRING("example.com", http_get_header(r, "host"));
    TEST_ASSERT_EQUAL_STRING("abc123", http_get_header(r, "x-trace"));
    TEST_ASSERT_NULL(http_get_header(r, "X-Absent"));
    ...
    r = feed("GET / HTTP/1.1\r\nX-A:\tone\r\nX-B:   two   \r\nX-C:three\r\n\r\n");
    TEST_ASSERT_EQUAL_STRING("one", http_get_header(r, "X-A"));
    TEST_ASSERT_EQUAL_STRING("two", http_get_header(r, "X-B"));
    TEST_ASSERT_EQUAL_STRING("three", http_get_header(r, "X-C"));

    // An empty field value is legal.
    r = feed("GET / HTTP/1.1\r\nX-Empty:\r\n\r\n");
    TEST_ASSERT_EQUAL_INT(PARSE_COMPLETE, r->parse_state);
    TEST_ASSERT_EQUAL_STRING("", http_get_header(r, "X-Empty"));
}
...
void test_segmentation_does_not_change_the_parse(void)
{
    ...
    http_parser_reset(bit);
    for (size_t i = 0; REQ[i]; i++)
    {
        http_parser_feed(bit, (uint8_t)REQ[i]);
    }
...
void test_headers_past_the_cap_still_frame_the_message(void)
{
    ...
    TEST_ASSERT_NULL(http_get_header(r, "Content-Length")); // past the cap: counted, not stored
...
void test_rfc6265_cookie_extraction(void)
{
    ...
    TEST_ASSERT_TRUE(http_get_cookie(r, "sid", out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("abc123", out);
    TEST_ASSERT_TRUE(http_get_cookie(r, "theme", out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("dark", out);

    // Absent, and case-mismatched, both report false and leave an empty string.
    TEST_ASSERT_FALSE(http_get_cookie(r, "nope", out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("", out);
    TEST_ASSERT_FALSE(http_get_cookie(r, "SID", out, sizeof(out)));

    // sec 4.1.1: cookie-value may be a DQUOTE-wrapped span; the quotes are not part of the value.
    r = feed("GET / HTTP/1.1\r\nCookie: q=\"quoted value\"\r\n\r\n");
    TEST_ASSERT_TRUE(http_get_cookie(r, "q", out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("quoted value", out);

    // A name that is a prefix of a stored one must not match it.
    r = feed("GET / HTTP/1.1\r\nCookie: session=one\r\n\r\n");
    TEST_ASSERT_FALSE(http_get_cookie(r, "sess", out, sizeof(out)));
    TEST_ASSERT_TRUE(http_get_cookie(r, "session", out, sizeof(out)));
...
void test_rfc7239_forwarded_client(void)
{
    ...
    TEST_ASSERT_TRUE(http_forwarded_client(r, ip, sizeof(ip), &https));
    ...
    TEST_ASSERT_FALSE(http_forwarded_client(r, ip, sizeof(ip), &https));
...
void test_urlencoded_form_fields(void)
{
    ...
    HttpReq *r = feed("POST /f HTTP/1.1\r\n"
                      ...
                      "user=bob&pass=s3cr3");
    TEST_ASSERT_EQUAL_INT(PARSE_COMPLETE, r->parse_state);
    TEST_ASSERT_TRUE(http_get_form(r, "user", out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("bob", out);
    TEST_ASSERT_TRUE(http_get_form(r, "pass", out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("s3cr3", out);
    TEST_ASSERT_FALSE(http_get_form(r, "missing", out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("", out);
    ...
    r = feed("POST /f HTTP/1.1\r\n"
             ...
             "\r\n"
             "flag&user=bo");
    TEST_ASSERT_TRUE(http_get_form(r, "flag", out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("", out);
    TEST_ASSERT_FALSE(http_get_form(r, "use", out, sizeof(out)));
    ...
    r = feed("POST /f HTTP/1.1\r\n"
             ...
             "\r\n"
             "user=bob");
    TEST_ASSERT_FALSE(http_get_form(r, "user", out, sizeof(out)));
}
...
void test_lookup_helpers_refuse_a_null_destination(void)
{
    ...
    TEST_ASSERT_FALSE(http_get_cookie(r, "sid", NULL, sizeof(out)));
    TEST_ASSERT_FALSE(http_get_cookie(r, "sid", out, 0));
    TEST_ASSERT_FALSE(http_get_cookie(r, NULL, out, sizeof(out)));
    TEST_ASSERT_FALSE(http_get_form(r, "x", NULL, sizeof(out)));
    TEST_ASSERT_FALSE(http_get_form(r, "x", out, 0));
    TEST_ASSERT_FALSE(http_forwarded_client(r, NULL, sizeof(out), NULL));
    TEST_ASSERT_FALSE(http_forwarded_client(r, out, 0, NULL));
    TEST_ASSERT_NULL(http_get_param(r, NULL));
    TEST_ASSERT_NULL(http_get_param(r, "id")); // nothing captured a path parameter
}
