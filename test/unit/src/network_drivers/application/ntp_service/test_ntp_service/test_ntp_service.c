...
static const protocore_net_host_dgram *ask(const char *server)
{
    ...
    NtpService.begin_args.tz = NULL;
    NtpService.begin_args.server1 = server;
    NtpService.begin_args.server2 = NULL;
...
void test_a_matching_reply_sets_the_clock(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(GOOD_EPOCH, (uint32_t)NtpService.value);
    NtpService.time_source(protocore_ntp_service_span());
    TEST_ASSERT_EQUAL_UINT32(GOOD_EPOCH, NtpService.ms);
}
...
void test_an_implausible_clock_is_ignored(void)
{
    ...
    TEST_ASSERT_TRUE(NtpService.ok);
    NtpService.epoch(protocore_ntp_service_span());
    TEST_ASSERT_EQUAL_UINT32(1609459201u, (uint32_t)NtpService.value);
}
...
void test_the_epoch_advances_off_the_monotonic_clock(void)
{
    ...
    NtpService.epoch(protocore_ntp_service_span());
    TEST_ASSERT_EQUAL_UINT32(GOOD_EPOCH + 5u, (uint32_t)NtpService.value);
    ...
    NtpService.epoch(protocore_ntp_service_span());
    TEST_ASSERT_EQUAL_UINT32(GOOD_EPOCH + 6u, (uint32_t)NtpService.value);
}
...
void test_a_server_name_is_refused(void)
{
    protocore_net_host_udp_reset();
    NtpService.begin_args.tz = NULL;
    NtpService.begin_args.server1 = "pool.ntp.org";
    NtpService.begin_args.server2 = NULL;
    NtpService.begin(protocore_ntp_service_span());
    TEST_ASSERT_FALSE(NtpService.ok);
    ...
    TEST_ASSERT_FALSE(NtpService.ok);
    NtpService.begin_args.tz = NULL;
    NtpService.begin_args.server1 = "999.1.1.1";
    NtpService.begin_args.server2 = NULL;
    NtpService.begin(protocore_ntp_service_span());
    TEST_ASSERT_FALSE(NtpService.ok);
}
...
void test_the_http_date_is_empty_until_synced(void)
{
    ...
    NtpService.set_test_epoch_args.epoch = (time_t)784111777;
    NtpService.set_test_epoch(protocore_ntp_service_span());
    NtpService.synced(protocore_ntp_service_span());
    TEST_ASSERT_TRUE(NtpService.ok);
    ...
    TEST_ASSERT_FALSE(NtpService.ok);
    NtpService.epoch(protocore_ntp_service_span());
    TEST_ASSERT_EQUAL_UINT32(0u, (uint32_t)NtpService.value);
    NtpService.time_source(protocore_ntp_service_span());
    TEST_ASSERT_EQUAL_UINT32(0u, NtpService.ms);
    TEST_ASSERT_EQUAL_size_t(0u, protocore_ntp_http_date(buf, sizeof buf));
}
