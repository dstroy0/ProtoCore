...
void test_header_parse_fails_closed(void)
{
    uint8_t buf[PROTOCORE_SMB2_HEADER_SIZE];
    Smb2Header h;
    TEST_ASSERT_EQUAL_size_t(64, protocore_smb2_build_header(buf, sizeof(buf), SMB2_NEGOTIATE, 1, 0, 0, 0));
    TEST_ASSERT_TRUE(protocore_smb2_parse_header(buf, sizeof(buf), &h));

    TEST_ASSERT_FALSE(protocore_smb2_parse_header(buf, 63, &h));

    uint8_t bad[PROTOCORE_SMB2_HEADER_SIZE];
    memcpy(bad, buf, sizeof(bad));
    bad[0] = 0xFF; // not 0xFE
    TEST_ASSERT_FALSE(protocore_smb2_parse_header(bad, sizeof(bad), &h));
...
void test_direct_tcp_transport_framing(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0x123456u, protocore_smb2_transport_len(BIG, sizeof(BIG)));

    // A non-zero first octet is not a Direct TCP frame.
    static const uint8_t BAD[4] = {0x01, 0x00, 0x00, 0x05};
    TEST_ASSERT_EQUAL_UINT32(0, protocore_smb2_transport_len(BAD, sizeof(BAD)));
    TEST_ASSERT_EQUAL_UINT32(0, protocore_smb2_transport_len(out, 3));
    ...
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_transport_frame(out, sizeof(out), MSG, 0x01000000u));
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_transport_frame(out, 8, MSG, sizeof(MSG)));
    TEST_ASSERT_EQUAL_size_t(9, protocore_smb2_transport_frame(out, 9, MSG, sizeof(MSG)));
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_transport_frame(NULL, 16, MSG, sizeof(MSG)));
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_transport_frame(out, 16, NULL, sizeof(MSG)));
}
...
void test_negotiate_request_body(void)
{
    ...
    uint8_t buf[256];
    size_t n = protocore_smb2_build_negotiate(buf, sizeof(buf), GUID, SMB2_NEGOTIATE_SIGNING_ENABLED);
    ...
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_build_negotiate(buf, 107, GUID, 0));
    TEST_ASSERT_EQUAL_size_t(108, protocore_smb2_build_negotiate(buf, 108, GUID, 0));
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_build_negotiate(buf, sizeof(buf), NULL, 0));
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_build_negotiate(NULL, sizeof(buf), GUID, 0));
}
...
void test_negotiate_response_parse(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(64, protocore_smb2_build_header(msg, sizeof(msg), SMB2_NEGOTIATE, 1, 0, 0, 0));
    ...
    TEST_ASSERT_TRUE(protocore_smb2_parse_negotiate_response(msg, sec_off + sizeof(SEC_BUF), &r));
    ...
    b[0] = 64;
    TEST_ASSERT_FALSE(protocore_smb2_parse_negotiate_response(msg, sec_off + sizeof(SEC_BUF), &r));
    b[0] = 65;
    ...
    TEST_ASSERT_FALSE(protocore_smb2_parse_negotiate_response(msg, 64, &r));
    TEST_ASSERT_FALSE(protocore_smb2_parse_negotiate_response(NULL, 128, &r));
    TEST_ASSERT_FALSE(protocore_smb2_parse_negotiate_response(msg, 128, NULL));
}
...
void test_signing_round_trip_and_tamper_detection(void)
{
    ...
    uint8_t msg[128];
    memset(msg, 0, sizeof(msg));
    protocore_smb2_build_header(msg, sizeof(msg), SMB2_TREE_CONNECT, 1, 42, 7, 0xAABBCCDDEEFF0011ull);
    ...
    msg[70] ^= 0x01;
    TEST_ASSERT_FALSE(protocore_smb2_verify(g_work, KEY, msg, sizeof(msg)));
    msg[70] ^= 0x01;
    TEST_ASSERT_TRUE(protocore_smb2_verify(g_work, KEY, msg, sizeof(msg)));
    ...
    other_key[0] ^= 0x01;
    TEST_ASSERT_FALSE(protocore_smb2_verify(g_work, other_key, msg, sizeof(msg)));

    // A message shorter than the header carries no signature field, so it is left untouched.
    uint8_t stub[32];
    memset(stub, 0x5A, sizeof(stub));
    protocore_smb2_sign(g_work, KEY, stub, sizeof(stub));
    for (size_t i = 0; i < sizeof(stub); i++)
    {
        TEST_ASSERT_EQUAL_HEX8(0x5A, stub[i]);
    }
    TEST_ASSERT_FALSE(protocore_smb2_verify(g_work, KEY, stub, sizeof(stub)));
}
...
void test_cmac_signing_is_a_distinct_algorithm(void)
{
    ...
    uint8_t msg[96];
    memset(msg, 0, sizeof(msg));
    protocore_smb2_build_header(msg, sizeof(msg), SMB2_WRITE, 1, 9, 3, 0x1122334455667788ull);
    for (size_t i = 64; i < sizeof(msg); i++)
    {
        msg[i] = (uint8_t)(i * 3);
    }

    protocore_smb2_sign_cmac(g_work, KEY, msg, sizeof(msg));
    TEST_ASSERT_TRUE((le32(msg + 16) & SMB2_FLAGS_SIGNED) != 0);
    TEST_ASSERT_TRUE(protocore_smb2_verify_cmac(g_work, KEY, msg, sizeof(msg)));
    TEST_ASSERT_FALSE(protocore_smb2_verify(g_work, KEY, msg, sizeof(msg)));

    msg[80] ^= 0x40;
    TEST_ASSERT_FALSE(protocore_smb2_verify_cmac(g_work, KEY, msg, sizeof(msg)));
    msg[80] ^= 0x40;
    TEST_ASSERT_TRUE(protocore_smb2_verify_cmac(g_work, KEY, msg, sizeof(msg)));
}
...
void test_transform_round_trip_for_every_cipher(void)
{
    ...
    nonce[0] = 0x01;
    memset(msg, 0, sizeof(msg));
    protocore_smb2_build_header(msg, sizeof(msg), SMB2_READ, 1, 5, 2, 0x0102030405060708ull);
    for (size_t i = 64; i < sizeof(msg); i++)
    {
    ...
    for (size_t c = 0; c < 4; c++)
    {
        ...
        size_t m = protocore_smb2_decrypt(cipher, key, blob, n, back, sizeof(back));
        ...
        TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_decrypt(cipher, key, blob, n, back, sizeof(back)));
        blob[PROTOCORE_SMB2_TRANSFORM_HDR_LEN] ^= 0x01;
        ...
        TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_decrypt(CIPHERS[(c + 1) % 4], key, blob, n, back, sizeof(back)));

        TEST_ASSERT_EQUAL_size_t(m, protocore_smb2_decrypt(cipher, key, blob, n, back, sizeof(back)));
    }
}
...
void test_transform_fails_closed(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_decrypt(0x0099, key, blob, n, back, sizeof(back)));
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_decrypt(SMB2_ENCRYPTION_AES128_GCM, key, blob,
                                                       PROTOCORE_SMB2_TRANSFORM_HDR_LEN - 1, back, sizeof(back)));
    TEST_ASSERT_EQUAL_size_t(0, protocore_smb2_decrypt(SMB2_ENCRYPTION_AES128_GCM, key, NULL, n, back, sizeof(back)));
    TEST_ASSERT_EQUAL_size_t(0,
                             protocore_smb2_decrypt(SMB2_ENCRYPTION_AES128_GCM, key, blob, n, back, sizeof(msg) - 1));
    TEST_ASSERT_EQUAL_size_t(sizeof(msg),
                             protocore_smb2_decrypt(SMB2_ENCRYPTION_AES128_GCM, key, blob, n, back, sizeof(msg)));
}
...
void test_key_derivation_separates_its_outputs(void)
{
    ...
    uint8_t sign_311[16];
    uint8_t sign_300[16];
    TEST_ASSERT_TRUE(protocore_smb3_derive_signing_key(SESSION_KEY, SMB2_DIALECT_0311, preauth, sign_311));
    TEST_ASSERT_TRUE(protocore_smb3_derive_signing_key(SESSION_KEY, SMB2_DIALECT_0300, NULL, sign_300));
    ...
    uint8_t tmp[16];
    TEST_ASSERT_FALSE(protocore_smb3_derive_signing_key(SESSION_KEY, SMB2_DIALECT_0311, NULL, tmp));
    TEST_ASSERT_FALSE(protocore_smb3_derive_signing_key(NULL, SMB2_DIALECT_0300, NULL, tmp));
    ...
    TEST_ASSERT_TRUE(protocore_smb3_derive_signing_key(SESSION_KEY, SMB2_DIALECT_0311, other_preauth, tmp));
    ...
    TEST_ASSERT_TRUE(protocore_smb3_derive_encryption_keys(SESSION_KEY, SMB2_DIALECT_0311, preauth, 16, c2s, s2c));
    ...
    TEST_ASSERT_TRUE(
        protocore_smb3_derive_encryption_keys(SESSION_KEY, SMB2_DIALECT_0311, preauth, 32, c2s256, s2c256));
    TEST_ASSERT_TRUE(memcmp(c2s, c2s256, 16) != 0);

    TEST_ASSERT_FALSE(protocore_smb3_derive_encryption_keys(SESSION_KEY, SMB2_DIALECT_0311, NULL, 16, c2s, s2c));
    TEST_ASSERT_FALSE(protocore_smb3_derive_encryption_keys(SESSION_KEY, SMB2_DIALECT_0311, preauth, 24, c2s, s2c));
    TEST_ASSERT_FALSE(protocore_smb3_derive_encryption_keys(NULL, SMB2_DIALECT_0300, NULL, 16, c2s, s2c));
}
