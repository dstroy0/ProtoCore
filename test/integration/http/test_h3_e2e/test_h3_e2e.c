...
// handshake against a server that is protocore_quic_conn + protocore_h3_conn wired together, then sends a real HTTP/3
// GET (QPACK HEADERS in a STREAM frame). The server's protocore_h3_conn dispatches the request, the app
// responds through protocore_h3_conn_respond, and the client decrypts the 1-RTT response and verifies the
...
{
    fill();
    // Server: protocore_quic_conn + protocore_h3_conn wired together.
    QuicTlsConfig cfg;
    ...
    H3Conn h3;
    protocore_h3_conn_init(&h3, &qc, app_request, NULL); // installs the QUIC callbacks
    ...
    protocore_tls13_ks_early(&TLS13_KDF, &cks, ks_store_290);
    ...
    // protocore_h3_conn dispatched the request to the app.
