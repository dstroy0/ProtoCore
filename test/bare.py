...
REGIONS_ARM = [
    "-DPROTOCORE_LINK_FLASH_ORIGIN=0x08000000",
    "-DPROTOCORE_LINK_FLASH_LEN=1M",
    "-DPROTOCORE_LINK_SRAM_ORIGIN=0x20000000",
    "-DPROTOCORE_LINK_SRAM_LEN=256K",
    "-DPROTOCORE_LINK_STACK_SIZE=16K",
    "-DPROTOCORE_LINK_IRQ_STACK_SIZE=4K",
    "-DPROTOCORE_LINK_DMA_ORIGIN=0x20040000",
    "-DPROTOCORE_LINK_DMA_LEN=64K",
    "-DPROTOCORE_LINK_DMA_ALIGN=32",
]
REGIONS_RISCV = [
    "-DPROTOCORE_LINK_FLASH_ORIGIN=0x20000000",
    "-DPROTOCORE_LINK_FLASH_LEN=1M",
    "-DPROTOCORE_LINK_SRAM_ORIGIN=0x80000000",
    "-DPROTOCORE_LINK_SRAM_LEN=256K",
    "-DPROTOCORE_LINK_STACK_SIZE=16K",
    "-DPROTOCORE_LINK_IRQ_STACK_SIZE=4K",
    "-DPROTOCORE_LINK_DMA_ORIGIN=0x80040000",
    "-DPROTOCORE_LINK_DMA_LEN=64K",
    "-DPROTOCORE_LINK_DMA_ALIGN=32",
]
...
SCHED_PENDING = re.compile(
    r"'(protocore_platform_task|protocore_platform_queue|protocore_platform_mutex|"
    r"protocore_platform_ticks|protocore_platform_status)[a-z_]*'"
)

MAIN_STUB = (
    "// Generated by bare.py: the entry protocore_boot_start() calls.\n"
    "int main(void);\n"
    "int main(void)\n{\n    for (;;)\n    {\n    }\n}\n"
)
...
def link(arch, objs, out_dir, name):
    ...
    subprocess.run(
        [arch["cc"], "-E", "-P", "-x", "c"] + arch["regions"] + [arch["ld"], "-o", ldout],
        capture_output=True,
        text=True,
        cwd=ROOT,
    )
    ...
    l = subprocess.run(
        [arch["cc"]] + arch["march"] + ["-nostdlib", "-nostartfiles", "-T", ldout] + objs + ["-o", elf, "-lgcc"],
        capture_output=True,
        text=True,
        cwd=ROOT,
    )
...
def flash_image(arch, elf, out_dir, name, mb=None):
    ...
    r = subprocess.run(
        [sys.executable, esptool, "--chip", arch["machine"], "elf2image", "--output", app, elf],
        capture_output=True,
        text=True,
        cwd=ROOT,
    )
...
def cmd_sim(a):
    ...
    if "qemu" not in arch:
        print(
            "%s is a shape, not a part: no machine to boot it on. Try: %s"
...
def build_parser():
    ap = argparse.ArgumentParser(
        prog="bare.py", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter
    )
