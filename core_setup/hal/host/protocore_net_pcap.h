...
#include "protocore_net_host.h" // the log this renders

#define PROTOCORE_NET_PCAP_IP4_HDR 20
#define PROTOCORE_NET_PCAP_IP6_HDR 40
#define PROTOCORE_NET_PCAP_UDP_HDR 8
#define PROTOCORE_NET_PCAP_TTL 64
#define PROTOCORE_NET_PCAP_PROTO_UDP 17

/** @brief The largest packet a logged datagram can render to. */
#define PROTOCORE_NET_PCAP_MAX_PKT                                                                                     \
    (PROTOCORE_NET_PCAP_IP6_HDR + PROTOCORE_NET_PCAP_UDP_HDR + PROTOCORE_NET_HOST_DGRAM_LEN)

// Sum @p n bytes into a running one's-complement accumulator, big-endian pairs, odd tail padded.
static inline uint32_t protocore_net_pcap_sum(uint32_t acc, const uint8_t *p, size_t n)
{
    size_t i = 0;
    while (i + 1 < n)
    {
...
static inline uint16_t protocore_net_pcap_fold(uint32_t acc)
{
    while (acc >> 16)
    {
...
static inline void protocore_net_pcap_put16(uint8_t *p, uint16_t v)
{
    p[0] = (uint8_t)(v >> 8);
    p[1] = (uint8_t)v;
}
 ...
 */
static inline size_t protocore_net_pcap_packet(const protocore_net_host_dgram *d, uint8_t *out, size_t cap)
{
    if (!d || !out)
    {
        return 0;
    }
    int v6 = (d->type == PROTOCORE_NET_TYPE_V6);
    if (!v6 && d->type != PROTOCORE_NET_TYPE_V4)
    {
        return 0;
    }
    size_t ip_len = v6 ? PROTOCORE_NET_PCAP_IP6_HDR : PROTOCORE_NET_PCAP_IP4_HDR;
    size_t udp_len = PROTOCORE_NET_PCAP_UDP_HDR + d->len;
    ...
    if (v6)
    {
        ...
        protocore_net_pcap_put16(out + 4, (uint16_t)udp_len);
        out[6] = PROTOCORE_NET_PCAP_PROTO_UDP;
        out[7] = PROTOCORE_NET_PCAP_TTL;
    ...
    else
    {
        ...
        out[1] = d->tos;
        protocore_net_pcap_put16(out + 2, (uint16_t)total);
        out[8] = PROTOCORE_NET_PCAP_TTL;
        out[9] = PROTOCORE_NET_PCAP_PROTO_UDP;
        memcpy(out + 16, d->addr, 4);
        protocore_net_pcap_put16(out + 10,
                                 protocore_net_pcap_fold(protocore_net_pcap_sum(0, out, PROTOCORE_NET_PCAP_IP4_HDR)));
    }

    uint8_t *udp = out + ip_len;
    protocore_net_pcap_put16(udp + 0, d->src_port);
    protocore_net_pcap_put16(udp + 2, d->dst_port);
    protocore_net_pcap_put16(udp + 4, (uint16_t)udp_len);
    memcpy(udp + PROTOCORE_NET_PCAP_UDP_HDR, d->data, d->len);

    size_t addr_len = v6 ? 16u : 4u;
    uint32_t acc = protocore_net_pcap_sum(0, src, addr_len);
    acc = protocore_net_pcap_sum(acc, v6 ? out + 24 : out + 16, addr_len);
    acc += (uint32_t)udp_len + (uint32_t)PROTOCORE_NET_PCAP_PROTO_UDP;
    acc = protocore_net_pcap_sum(acc, udp, udp_len);
    uint16_t ck = protocore_net_pcap_fold(acc);
    if (ck == 0)
    {
        ck = 0xFFFFu;
    }
    protocore_net_pcap_put16(udp + 6, ck);
    return total;
}
...
static inline size_t protocore_net_pcap_render(uint8_t *out, size_t cap)
{
    ...
    size_t w = PROTOCORE_PCAP_GLOBAL_HDR_LEN;
    for (size_t i = 0; i < protocore_net_host_udp_count(); i++)
    {
        const protocore_net_host_dgram *d = protocore_net_host_udp_at(i);
        if (cap < w + PROTOCORE_PCAP_REC_HDR_LEN)
        {
            return 0;
        }
        size_t n =
            protocore_net_pcap_packet(d, out + w + PROTOCORE_PCAP_REC_HDR_LEN, cap - w - PROTOCORE_PCAP_REC_HDR_LEN);
        if (n == 0)
        {
            return 0;
        }
...
#endif // PROTOCORE_PROTOCORE_NET_PCAP_H
