...
| `cpp/constant-comparison`        | `src/server/security/auth_lockout/auth_lockout.c:105`                                        | **Benign (defensive)** - the `dur > (0xFFFFFFFFu >> 1)` overflow guard in the backoff loop is dead under the default `PROTOCORE_AUTH_LOCKOUT_MAX_MS` (the `>= MAX_MS` break fires first) but protects a pathologically large `MAX_MS` config. |
