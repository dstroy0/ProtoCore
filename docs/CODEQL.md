...
| `cpp/constant-comparison`        | `src/services/security/auth_lockout/auth_lockout.c:105`                                        | **Benign (defensive)** - the `dur > (0xFFFFFFFFu >> 1)` overflow guard in the backoff loop is dead under the default `PROTOCORE_AUTH_LOCKOUT_MAX_MS` (the `>= MAX_MS` break fires first) but protects a pathologically large `MAX_MS` config. |
...
| `cpp/poorly-documented-function` | `src/services/fieldbus/modbus/modbus.c:112`                                                    | **Won't fix (style)** - `protocore_modbus_process_pdu` is a function-code dispatcher (documented with a header comment); the heuristic dislikes the size / comment ratio.                                                                     |
