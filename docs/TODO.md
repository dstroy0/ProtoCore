...
- **CDN caching tier** - the RAM-tier reverse-proxy edge cache + cache key / invalidation / purge, the
  ...
  **`https://` origins** (`PROTOCORE_ENABLE_EDGE_ORIGIN_TLS` - since REMOVED, see below), and **cross-device mesh distribution**
  ...
  replication for the mesh, and (TLS origins) an async handshake to keep a cold https MISS from blocking the worker +
...
- **Port forwarding / DNAT** - complete + **HW-verified (2026-07-13)**: the `protocore_relay` bidirectional
  ...
  Throughput ~44 KB/s: a single-NIC relay crosses the only W5500 twice per byte and is round-trip-latency
...
Folded out of `SWEEP_NOTES.md` on 2026-08-08, each item re-measured against the tree that day, none
inherited from the note. The sweep's two large tracks are closed: `src/` is C (349 `.c`, 378
...
- [ ] **Finish the layering move.** RFC 4254 puts the mux in `ssh_channel`, forwarding in
      ...
      `protocore_ssh_connection_channel_build_data`, plus `protocore_ssh_conn_close_channel` (`:187`) and
...
- [x] **Atomic buffer-to-flash layer (the substrate).** _(done)_ A power-loss-safe write path over the
      FS / SD: a write-ahead log with a CRC per record and an A/B superblock commit marker. An
...
- [x] **base64 was slow on-device (mbedTLS).** _(done - hybrid)_ mbedTLS's base64 is slow because it is
      constant-time (side-channel hardened). That is not dropped globally. The path now splits by data
...
- [x] **6. Middleware pipeline (high value / large).** _(done)_ Fixed-size,
      ...
      They are not re-expressed as middlewares - the chain is additive and
...
- [x] **7. Chunked / streaming app responses (medium / medium).** _(done; pull
      ...
      _Follow-up (done):_ `chunk_send_pump` sizes each chunk to `protocore_conn_pool_sndbuf()`
...
- [x] **Architecture pass.** Pluggable per-protocol handler dispatch
      (`server/core/proto_handler.h` - a `ProtoHandler` table; a new
      ...
      `protocore_conn_pool_sndbuf`, context-safe `Tcp.conn->raw_send`), response header+body
...
- [x] **SNMPv3 _inform_** _(done)_ - `protocore_snmp_v3_inform()` (symmetric with
      `protocore_snmp_notify_inform_v2c()` / `protocore_snmp_v3_trap()`) builds + sends an authenticated (authPriv
...
- [x] **CoAP server scope.** `/.well-known/core` resource discovery (RFC 6690) is
      ...
      (RFC 7252 §4.5, the only REQUIRED reliability behavior) is now implemented**: a
      retransmitted CON is re-answered from a small (source endpoint, Message-ID)-keyed
      cache - the FULL address, never a hash - without re-running its handler. A
...
- [~] **Concurrent TLS** (`MAX_TLS_CONNS` > 1). _(library side landed; a `MAX_TLS_CONNS=2`
      ...
      (~122 KB, ROM-reserved both ends - NOT the 320 KB PlatformIO prints). A 2nd
      ...
      the stock precompiled arduino-esp32 2.0.x has it off; a PSRAM/IDF build is
...
- [x] **No authentication attempt limiting (brute-force).** _(done)_
      ...
      A client gets exactly one guess per TCP connection. Cross-connection
...
- [x] **Slow-loris held the pool indefinitely (slot-exhaustion DoS).** _(done - HW-verified)_
      ...
      `Connection: close`. Header-scoped. A slow body upload (`PARSE_BODY`) is never reaped.
...
- [x] **Connection close/abort is driven from L7 holding the raw `tcp_pcb`.** _(done)_
      ...
      then FIN/RST - on a captured pcb pointer. A late lwIP callback finds a freed
...
- [x] **RSA-2048 modexp uses ~7 KB of stack (mbedTLS).** _(done - enforced minimum)_
      ...
      **enforced at build time**: [`PROTOCORE_WORKER_STACK_RSA_MIN`](@ref PROTOCORE_WORKER_STACK_RSA_MIN)
      ...
      [`PROTOCORE_WORKER_TASK_STACK`](@ref PROTOCORE_WORKER_TASK_STACK) is below it. A lowered
...
- [x] **Per-direction NEWKEYS.** _(done)_ The single `ssh_pkt[i].encrypted` flag is
      ...
      reads `enc_out`, the receive path (unpack) reads `enc_in`. A strict peer that
...
- **SNMP agent v1 / v2c / v3.** Zero-heap ASN.1 BER codec + a fixed MIB
  ...
  - [x] v3 (USM, RFC 3414): gated behind `PROTOCORE_ENABLE_SNMP_V3` (default off).
        ...
        [`protocore_snmp_agent_dispatch_pdu()`](@ref protocore_snmp_dispatch_pdu) MIB core. Native tests
