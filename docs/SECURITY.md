...
| Area                        | Caveat                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Transport encryption (HTTP) | Opt-in **HTTPS** ([`PROTOCORE_ENABLE_TLS`](@ref PROTOCORE_ENABLE_TLS)) via mbedTLS on a static memory pool - TLS 1.2+/`ECDHE-ECDSA-AES256-GCM-SHA384`, zero-heap (§6). Default off (plain HTTP). The server authenticates itself by RFC 7250 raw public key and does not request a client certificate, so there is no mutual-TLS mode; `wss://` + TLS-SSE run encrypted over the same TLS record layer. Optional **session resumption** ([`PROTOCORE_ENABLE_TLS_RESUMPTION`](@ref PROTOCORE_ENABLE_TLS_RESUMPTION)) via RFC 5077 tickets gives returning clients an abbreviated handshake. Caveat: one connection at a time (`MAX_TLS_CONNS`=1). On a trusted LAN plain HTTP is fine; the SSH layer is also an encrypted channel. |
| SSH timing side-channels    | The native software bignum/AES/RSA paths are **not constant-time**, but they are **compile-excluded from firmware**: the software Montgomery cluster is under `#ifndef ARDUINO` (`crypto/asymmetric/bignum/bignum.c`) and the software AES / native RSA modexp live in the `#else` of an `#ifdef ARDUINO` (`crypto/cipher/aes256ctr/aes256ctr.c`, `crypto/asymmetric/rsa/rsa.c`). On ESP32 only the hardware/mbedTLS paths are compiled and run; the software paths exist solely for host testing. |
...
| Outbound HTTP client        | **No TLS.** The library ships no client-side TLS engine. An `https://` target URI is refused with `HTTP_CLIENT_ERR_TLS`. It is never sent in the clear. Everything an `http://` request carries - URL, headers, body, any credential in them - travels in cleartext. Use it only on a trusted segment, or put the secret somewhere other than an outbound request. |
| MQTT client                 | Opt-in ([`PROTOCORE_ENABLE_MQTT`](@ref PROTOCORE_ENABLE_MQTT), default off). Plain MQTT sends the broker username/password and all payloads **in cleartext** - use it only on a trusted segment. `mqtts://` is not available - the library ships no client-side TLS engine - and a connection asking for it is refused. It is not downgraded. An untrusted segment needs the broker reached some other way (an SSH tunnel, a VPN, or a local broker that bridges outward). |
...
- **The server does not authenticate clients via TLS.** It presents its own RFC 7250 raw public
  key and never requests a client certificate, so there is no mutual-TLS mode to enable. Use HTTP
  Basic/Digest/JWT auth, or the SSH layer's public-key auth, for client identity.
...
mechanism here is linker symbol isolation, which
raises the attack bar without requiring hardware security features.
...
| --------------------------------------------- | ---------------------------------------------------------------- | ---------------- |
| `crypto_work[1536]`                           | After every [`bn_expmod_group14()`](@ref bn_expmod_group14) call | `crypto/asymmetric/bignum/bignum.c` |
...
| RSA sign bignum temporaries (n/d/m/s)         | Before [`protocore_rsa_sign()`](@ref protocore_rsa_sign) returns       | `crypto/asymmetric/rsa/rsa.c` |
...
| Outbound HTTP client has no TLS | Everything an `http://` request carries travels in cleartext; an `https://` URI is refused | Use it only on a trusted segment, or carry the secret some other way |
