...
| Outbound HTTP client        | **No TLS.** The library ships no client-side TLS engine, so an `https://` target URI is refused with `HTTP_CLIENT_ERR_TLS`. It is never sent in the clear. Everything an `http://` request carries - URL, headers, body, any credential in them - travels in cleartext. Use it only on a trusted segment, or put the secret somewhere other than an outbound request. |
| MQTT client                 | Opt-in ([`PROTOCORE_ENABLE_MQTT`](@ref PROTOCORE_ENABLE_MQTT), default off). Plain MQTT sends the broker username/password and all payloads **in cleartext** - use it only on a trusted segment. `mqtts://` is not available - the library ships no client-side TLS engine - and a connection asking for it is refused. It is not downgraded, so an untrusted segment needs the broker reached some other way (an SSH tunnel, a VPN, or a local broker that bridges outward). |
