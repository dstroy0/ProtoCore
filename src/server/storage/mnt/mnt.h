 ...
 * the accessor (server/storage/filesystem.h), which is the seam that does know.
 */
...
typedef struct
{
    ...
    void (*const point_of)(uint8_t *work);
    void (*const root_of)(uint8_t *work);
    void (*const reset)(uint8_t *work);
    void (*const mount)(uint8_t *work);
