...
typedef struct
{
    uint8_t *span; ///< PROTOCORE_HOTSWAP_BORROW persistent bytes, or null while the pool was short
} HotswapOwnCtx;
...
uint8_t *protocore_hotswap_span(void)
{
    if (s_own.span == NULL)
    {
        protocore_span sp = protocore_secure_persist_span(PROTOCORE_HOTSWAP_BORROW);
        if (span.ok(sp))
        {
            s_own.span = sp.buf;
        }
