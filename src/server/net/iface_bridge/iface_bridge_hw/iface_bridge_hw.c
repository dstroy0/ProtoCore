...
#include "server/net/iface_bridge/iface_bridge_hw/iface_bridge_hw.h"
...
static_assert(IFACE_BRIDGE_HW_OFF_CTX % _Alignof(BridgeGlueCtx) == 0,
              "IFACE_BRIDGE_HW_OFF_CTX is not a multiple of alignof(BridgeGlueCtx) - IFACE_BRIDGE_HW_CTX() would "
              "return a misaligned "
...
static void service_txn(uint8_t slot, const BridgeTarget *t)
{
    ...
    for (;;)
    {
        ...
        {
            ConnPoolV.slot = slot;
            ConnPoolV.close(protocore_conn_pool_span()); // frame exceeds the configured cap - protocol error
            return;
        }
        ...
        {
            ConnPoolV.slot = slot;
            ConnPoolV.close(protocore_conn_pool_span()); // codec disagreed with the header - drop the connection
            return;
        }
        ConnPoolV.slot = slot;
        ConnPoolV.io.count = need;
        ...
        {
            ConnPoolV.slot = slot;
            ConnPoolV.close(protocore_conn_pool_span()); // bus fault
            return;
        }
        ConnPoolV.slot = slot;
        ConnPool.active(protocore_conn_pool_span());
...
static void bridge_on_accept(uint8_t slot)
{
    ...
    {
        ConnPoolV.slot = slot;
        ConnPoolV.close(protocore_conn_pool_span()); // no rule published for this listener
    }
}
...
typedef struct
{
    uint8_t *span; ///< PROTOCORE_IFACE_BRIDGE_HW_BORROW persistent bytes
} IfaceBridgeHwOwnCtx;
...
uint8_t *protocore_iface_bridge_hw_span(void)
{
    if (s_own.span == NULL)
    {
        s_own.span = protocore_secure_persist_span(PROTOCORE_IFACE_BRIDGE_HW_BORROW).buf;
    }
    return s_own.span;
}
...
void protocore_iface_bridge_hw_publish(uint8_t *restrict work)
{
    ...
    IfaceBridgeV.map(protocore_iface_bridge_span());
    ...
    IfaceBridgeV.find(protocore_iface_bridge_span());
