...
// Outside SSH, only the WebSocket deflate path and the OIDC verifier borrow more than the 975-byte
// diag document, so a build with none of the three keeps a 4 KiB transient end.
#ifndef PROTOCORE_PLAINTEXT_SCRATCH
#if PROTOCORE_ENABLE_SSH || PROTOCORE_ENABLE_WEBSOCKET || PROTOCORE_ENABLE_OIDC
#define PROTOCORE_PLAINTEXT_SCRATCH 10240
#else
#define PROTOCORE_PLAINTEXT_SCRATCH 4096
#endif
#endif
...
// The listener is compiled in every build but only these features bind one.
#if PROTOCORE_ENABLE_UDP || PROTOCORE_ENABLE_PROVISIONING || PROTOCORE_ENABLE_SNMP || PROTOCORE_ENABLE_COAP ||         \
    PROTOCORE_ENABLE_HTTP3 || PROTOCORE_ENABLE_DNS_RESOLVER || PROTOCORE_ENABLE_DNS_SERVER || PROTOCORE_ENABLE_MDNS || \
    PROTOCORE_ENABLE_NTP_SERVER || PROTOCORE_ENABLE_NTP || PROTOCORE_ENABLE_DIFFSERV
#define PROTOCORE_PLAINTEXT_WORK_UDPLISTENER PROTOCORE_UDP_LISTENER_BORROW
#else
#define PROTOCORE_PLAINTEXT_WORK_UDPLISTENER 0
#endif
...
#else
#define PROTOCORE_PLAINTEXT_WORK_TCPCLIENT 0
#endif
