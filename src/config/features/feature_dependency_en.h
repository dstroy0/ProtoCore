...
#endif
// An https origin runs the record engine's BIO, and PROTOCORE_PLATFORM_TLS_WANT_READ / _WANT_WRITE
...
#error "ProtoCore: PROTOCORE_ENABLE_EDGE_ORIGIN_TLS needs a TLS engine (PROTOCORE_HAS_VENDOR_TLS). Provide one in\
 test/core_setup/hal/<vendor>, or map plaintext http origins only."
#endif
