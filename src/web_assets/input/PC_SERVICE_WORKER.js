...
// protocore_delivery_swr decides server-side.
//
// The manifest (protocore_delivery_sw_manifest) supplies both the file list and a version tag; the cache is
                ...
                return caches.keys().then(function (keys) {
                    return Promise.all(
                        keys.map(function (k) {
                            ...
                            return k.indexOf(CACHE_PREFIX) === 0 && k !== keep
                                ? caches.delete(k)
                                : null;
                        }),
                    );
