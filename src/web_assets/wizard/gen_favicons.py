...
without hand-drawing each. Each favicon is written as a crisp SVG under src/web_assets/favicons/; a separate
...
packs a ready-to-drop-in tarball, plus a preview PNG for the gallery.
    ...
    python -m src.web_assets.wizard.gen_favicons            # write src/web_assets/favicons/*.svg
    python -m src.web_assets.wizard.gen_favicons --check     # CI: fail if stale
    python -m src.web_assets.wizard.gen_favicons --list       # print every favicon name
"""
...
GALLERY = os.path.join(REPO_ROOT, "docs", "FAVICONS.md")
...
def cmd_gallery():
    ...
    lines = [
        ...
        " [`src/web_assets/favicons/`](../src/web_assets/favicons/). Package any one as a drop-in favicon set (16/32/48/180/"
        "192/512 PNG + `favicon.ico` + `site.webmanifest`, tarballed) with"
        " [`tools/ci_tooling/assets/pack_favicons.sh`](../tools/ci_tooling/assets/pack_favicons.sh) `<name>` (needs `rsvg-convert` + ImageMagick).",
        "",
    ]
    for motif in MOTIFS:
        ...
        lines.append("")
        row = " ".join(
            f'<img src="../src/web_assets/favicons/{motif}-{p}.svg" width="44" alt="{motif}-{p}" title="{motif}-{p}">'
            for p in PAIRS
        )
        lines.append(row)
        lines.append("")
