...
int protocore_audit_format(const protocore_audit_entry *e, char *out, size_t cap)
{
    ...
    protocore_sb_put(&sb_out, "{\"seq\":");
    protocore_sb_u32(&sb_out, (uint32_t)((unsigned long)e->seq));
    protocore_sb_put(&sb_out, ",\"ts\":");
    protocore_sb_u32(&sb_out, (uint32_t)((unsigned long)e->ts));
    protocore_sb_put(&sb_out, ",\"cat\":\"");
    protocore_sb_put(&sb_out, protocore_audit_cat_name(e->category));
    protocore_sb_put(&sb_out, "\",\"msg\":\"");
    int head = (int)protocore_sb_finish(&sb_out);
...
int protocore_audit_dump_json(char *out, size_t cap)
{
    ...
    if (intact)
    {
        ...
        protocore_sb_put(&sb_out2, "{\"intact\":true,\"count\":");
        protocore_sb_u32(&sb_out2, (uint32_t)((unsigned)s_audit.count));
        protocore_sb_put(&sb_out2, ",\"entries\":[");
        head = (int)protocore_sb_finish(&sb_out2);
    }
    else
    {
        protocore_sb sb_out3 = {out, cap, 0, PROTO_TRUE};
        protocore_sb_put(&sb_out3, "{\"intact\":false,\"first_broken\":");
        protocore_sb_u32(&sb_out3, (uint32_t)((unsigned long)broken));
        protocore_sb_put(&sb_out3, ",\"count\":");
        protocore_sb_u32(&sb_out3, (uint32_t)((unsigned)s_audit.count));
        protocore_sb_put(&sb_out3, ",\"entries\":[");
        head = (int)protocore_sb_finish(&sb_out3);
    }
