...
/** @brief The Read resolver registered above, or null while none is. */
...
#define OPCUA_REFTYPE_HAS_PROPERTY 46
#define OPCUA_REFTYPE_HAS_COMPONENT 47
/** @brief The reference an add-in hangs off its host by (Part 3): how DI and Machinery attach Identification. */
#define OPCUA_REFTYPE_HAS_ADDIN 17604
#define OPCUA_TYPEDEF_BASE_OBJECT 58
/** @brief The type of a plain container of other nodes, as every published model types its folders. */
#define OPCUA_TYPEDEF_FOLDER 61
#define OPCUA_TYPEDEF_BASE_DATA_VARIABLE 63
#define OPCUA_TYPEDEF_PROPERTY 68

// ---------------------------------------------------------------------------
// The NamespaceArray
// ---------------------------------------------------------------------------
//
// Part 3 sec 8.2.2: a NodeId's NamespaceIndex is an index into the server's NamespaceArray, and
// index 0 is always the OPC UA core namespace. The index is the SERVER's, not a model's, so a model
// states the URI it publishes and the server answers with the index that URI got. Two models loaded
// together therefore cannot both claim index 1, which is what a per-model compile-time constant did.

/** @brief How many namespaces beyond the core one this server can hold. */
#ifndef PROTOCORE_OPCUA_NAMESPACES
#define PROTOCORE_OPCUA_NAMESPACES 4
#endif

/** @brief Index 0 of every server's NamespaceArray (Part 3 sec 8.2.2). */
#define OPCUA_CORE_NS_URI "http://opcfoundation.org/UA/"

/** @brief The longest namespace URI a compare walks. */
#ifndef PROTOCORE_OPCUA_NS_URI_MAX
#define PROTOCORE_OPCUA_NS_URI_MAX 128
#endif

/** @brief The namespace URI a register names. */
typedef struct
{
    const char *uri; ///< the model's namespace URI, referenced and not copied
} OpcUaNamespaceArgs;

/**
 * @brief Register @p uri and report the index it holds, adding it if it is not there yet.
 *
 * Reports 0 when the table is full: index 0 is the core namespace, which no model owns, so a caller
 * that lands there knows it did not get one.
 */
uint16_t protocore_opcua_namespace_index(const char *uri);

/** @brief The URI at @p index, or NULL past the end of the array. */
const char *protocore_opcua_namespace_uri(uint16_t index);

/** @brief How many namespaces the array holds, the core one included. */
...
typedef struct
{
    ...
    uint16_t type_def_ns;     ///< TypeDefinition namespace index; 0 for the core types below.
...
/** @brief The Browse resolver registered above, or null while none is. */
