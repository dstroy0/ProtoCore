...
typedef struct
{
    uint8_t *span; ///< PROTOCORE_UMATI_BORROW persistent bytes, or null while the pool was short
} UmatiOwnCtx;
...
uint8_t *protocore_umati_span(void)
{
    if (s_own.span == NULL)
    {
        protocore_span sp = protocore_plaintext_persist_span(PROTOCORE_UMATI_BORROW);
        if (span.ok(sp))
        {
            s_own.span = sp.buf;
        }
