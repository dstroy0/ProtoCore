...
static_assert(ROBOTICS_OFF_CTX % _Alignof(RoboticsCtx) == 0,
              "ROBOTICS_OFF_CTX is not a multiple of alignof(RoboticsCtx) - ROBOTICS_CTX() would return a misaligned "
...
typedef struct
{
    uint8_t *span; ///< PROTOCORE_ROBOTICS_BORROW persistent bytes
} RoboticsOwnCtx;
...
uint8_t *protocore_robotics_span(void)
{
    if (s_own.span == NULL)
    {
        s_own.span = protocore_plaintext_persist_span(PROTOCORE_ROBOTICS_BORROW).buf;
    }
    return s_own.span;
}
