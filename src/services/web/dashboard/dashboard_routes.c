...
void protocore_dashboard_begin(const char *path, const protocore_widget *widgets, uint8_t count)
{
    ...
    protocore_sb_put(&sb_layout_path, path);
    protocore_sb_put(&sb_layout_path, "/layout");
    if (protocore_sb_finish(&sb_layout_path) == 0)
    {
        layout_path[0] = '\0';
    }
    protocore_sb sb_stream_path = {s_dashr.stream_path, sizeof(s_dashr.stream_path), 0, PROTO_TRUE};
    protocore_sb_put(&sb_stream_path, path);
    protocore_sb_put(&sb_stream_path, "/stream");
    if (protocore_sb_finish(&sb_stream_path) == 0)
    {
