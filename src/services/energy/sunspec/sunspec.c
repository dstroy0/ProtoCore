...
{
    if (!offset || !protocore_sunspec_check_marker(regs, len))
    {
        return PROTO_FALSE;
    }
...
{
    return protocore_sunspec_write_u16(w, (uint16_t)v);
}
...
{
    return protocore_sunspec_write_u32(w, (uint32_t)v);
}

proto_bool protocore_sunspec_write_marker(SunSpecWriter *w)
{
    return protocore_sunspec_write_u32(w, SUNSPEC_MARKER);
}

proto_bool protocore_sunspec_write_model_header(SunSpecWriter *w, uint16_t id, uint16_t length)
{
    return protocore_sunspec_write_u16(w, id) && protocore_sunspec_write_u16(w, length);
}

proto_bool protocore_sunspec_write_string(SunSpecWriter *w, const char *s, size_t nregs)
{
    if (!s)
    {
        return PROTO_FALSE;
...
proto_bool protocore_sunspec_write_end_model(SunSpecWriter *w)
{
    return protocore_sunspec_write_u16(w, SUNSPEC_END_MODEL) && protocore_sunspec_write_u16(w, 0);
}

size_t protocore_sunspec_writer_finish(SunSpecWriter *w)
{
