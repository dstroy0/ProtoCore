...
#include "services/net/snmp/snmp_ber/snmp_ber.h" // BerEnc: the open encoder a PDU append writes into
...
typedef struct
{
    void (*const build_pdu)(uint8_t *work);
    void (*const build_v2c)(uint8_t *work);
    void (*const trap_v2c)(uint8_t *work);
    void (*const inform_v2c)(uint8_t *work);
} SnmpNotifyNs;
