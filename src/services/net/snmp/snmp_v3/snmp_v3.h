...
#include "services/net/snmp/snmp_notify/snmp_notify.h" // SnmpVarbind: the bindings a v3 notification carries
#endif
...
typedef struct
{
    ...
    void (*const set_user)(uint8_t *work);
    void (*const set_boots)(uint8_t *work);
    void (*const get_boots)(uint8_t *work);
    void (*const process)(uint8_t *work);
    void (*const trap)(uint8_t *work);
