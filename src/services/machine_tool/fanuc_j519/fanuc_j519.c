...
static size_t wr_f32le(uint8_t *p, float v)
{
    uint32_t u;
    mem.cpy(&u, &v, 4);
    return endian.wr32le(p, u);
}
static float rd_f32le(const uint8_t *p)
{
    uint32_t u = endian.rd32le(p);
    float v;
    mem.cpy(&v, &u, 4);
    return v;
}
...
{
    endian.wr32le(buf + 0, (uint32_t)type);
    endian.wr32le(buf + 4, version_no);
}
...
{
    return buf && len == want_len && endian.rd32le(buf + 0) == (uint32_t)want_type;
}
...
proto_bool protocore_j519_peek(const uint8_t *buf, size_t len, uint32_t *type, uint32_t *version_no)
{
    ...
    if (type)
    {
        *type = endian.rd32le(buf + 0);
    }
    if (version_no)
    {
        *version_no = endian.rd32le(buf + 4);
    }
    return PROTO_TRUE;
}
...
size_t protocore_j519_build_motion(uint8_t *buf, size_t cap, const J519MotionCommand *cmd)
{
    ...
    endian.wr32le(buf + 8, cmd->sequence_no);
    buf[12] = cmd->last_data;
    buf[13] = cmd->read_io_type;
    endian.wr16le(buf + 14, cmd->read_io_index);
    endian.wr16le(buf + 16, cmd->read_io_mask);
    buf[18] = cmd->data_style;
    buf[19] = cmd->write_io_type;
    endian.wr16le(buf + 20, cmd->write_io_index);
    endian.wr16le(buf + 22, cmd->write_io_mask);
    endian.wr16le(buf + 24, cmd->write_io_value);
    endian.wr16le(buf + 26, 0); // unused
...
size_t protocore_j519_build_request(uint8_t *buf, size_t cap, const J519Request *req)
{
    ...
    endian.wr32le(buf + 8, req->axis_no);
    endian.wr32le(buf + 12, req->threshold_type);
...
proto_bool protocore_j519_parse_motion(const uint8_t *buf, size_t len, J519MotionCommand *out)
{
    ...
    out->version_no = endian.rd32le(buf + 4);
    out->sequence_no = endian.rd32le(buf + 8);
    out->last_data = buf[12];
    out->read_io_type = buf[13];
    out->read_io_index = endian.rd16le(buf + 14);
    out->read_io_mask = endian.rd16le(buf + 16);
    out->data_style = buf[18];
    out->write_io_type = buf[19];
    out->write_io_index = endian.rd16le(buf + 20);
    out->write_io_mask = endian.rd16le(buf + 22);
    out->write_io_value = endian.rd16le(buf + 24);
...
proto_bool protocore_j519_parse_request(const uint8_t *buf, size_t len, J519Request *out)
{
    ...
    out->version_no = endian.rd32le(buf + 4);
    out->axis_no = endian.rd32le(buf + 8);
    out->threshold_type = endian.rd32le(buf + 12);
    return PROTO_TRUE;
}
...
size_t protocore_j519_build_status(uint8_t *buf, size_t cap, const J519RobotStatus *st)
{
    ...
    endian.wr32le(buf + 8, st->sequence_no);
    buf[12] = st->status;
    buf[13] = st->read_io_type;
    endian.wr16le(buf + 14, st->read_io_index);
    endian.wr16le(buf + 16, st->read_io_mask);
    endian.wr16le(buf + 18, st->read_io_value);
    endian.wr32le(buf + 20, st->time_stamp);
...
size_t protocore_j519_build_ack(uint8_t *buf, size_t cap, const J519Ack *ack)
{
    ...
    endian.wr32le(buf + 8, ack->axis_no);
    endian.wr32le(buf + 12, ack->threshold_type);
    endian.wr32le(buf + 16, ack->max_cart_speed);
    endian.wr32le(buf + 20, ack->unknown0);
...
proto_bool protocore_j519_parse_status(const uint8_t *buf, size_t len, J519RobotStatus *out)
{
    ...
    out->read_io_index = endian.rd16le(buf + 14);
    out->read_io_mask = endian.rd16le(buf + 16);
    out->read_io_value = endian.rd16le(buf + 18);
    out->time_stamp = endian.rd32le(buf + 20);
...
proto_bool protocore_j519_parse_ack(const uint8_t *buf, size_t len, J519Ack *out)
{
    ...
    out->axis_no = endian.rd32le(buf + 8);
    out->threshold_type = endian.rd32le(buf + 12);
    out->max_cart_speed = endian.rd32le(buf + 16);
    out->unknown0 = endian.rd32le(buf + 20);
