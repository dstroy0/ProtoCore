...
typedef struct
{
    uint8_t *span; ///< PROTOCORE_PROMISC_BORROW persistent bytes
} PromiscOwnCtx;
...
uint8_t *protocore_promisc_span(void)
{
    if (s_own.span == NULL)
    {
        s_own.span = protocore_plaintext_persist_span(PROTOCORE_PROMISC_BORROW).buf;
    }
    return s_own.span;
}
...
static_assert(PROMISC_OFF_CTX % _Alignof(PromiscCtx) == 0,
              "PROMISC_OFF_CTX is not a multiple of alignof(PromiscCtx) - PROMISC_CTX() would return a misaligned "
