...
static void free_handle(SftpSession *s, int h)
{
    ...
    FsV.io.handle = s->handles[h].fh; // the bit is what says fh is real, so it needs no second marker
...
static void write_stream_bytes(SftpSession *s, const uint8_t *data, size_t n)
{
    ...
    {
        FsV.io.handle = s->handles[s->wr_handle].fh;
        FsV.io.wbuf = data;
        FsV.io.n = n;
...
static size_t build_entry(uint8_t *restrict work, const protocore_mnt_stat *st, const char *name, size_t name_len)
{
    ...
    SftpV.wr_init(sftp_work); // reserves a 4-byte length prefix we discard below
...
static void do_readdir(uint8_t *restrict work, SftpSession *s, uint32_t id, SftpHandle *H)
{
    ...
    while (!H->readdir_done)
    {
        ...
        FsV.io.handle = H->fh;
        FsV.io.stat = &st;
        FsV.io.name_out = SSH_SFTP_CTX(work)->nm;
        FsV.io.name_cap = sizeof(SSH_SFTP_CTX(work)->nm);
...
static void handle_packet(uint8_t *restrict work, SftpSession *s, const uint8_t *buf, size_t total)
{
    ...
    case PROTOCORE_SSH_FXP_OPEN: {
        ...
        FsV.path.root = SSH_SFTP_CTX(work)->root;
        FsV.path.dir = req;
        FsV.path.name = "";
        FsV.io.mode = mode;
        ...
        if (fh < 0)
        {
            ...
            proto_bool is_dir = FsV.ok && st.is_dir;
    ...
    case PROTOCORE_SSH_FXP_READ: {
        ...
        FsV.io.handle = s->handles[hi].fh;
        FsV.io.buf = SSH_SFTP_CTX(work)->rbuf;
        FsV.io.n = want;
    ...
    case PROTOCORE_SSH_FXP_REMOVE: {
        ...
        send_status(work, s, id, FsV.ok ? PROTOCORE_SSH_FX_OK : PROTOCORE_SSH_FX_FAILURE, "");
        return;
    }
...
void protocore_ssh_sftp_begin(uint8_t *restrict work)
{
    ...
    SSH_SFTP_CTX(work)->root = FsV.i32;
