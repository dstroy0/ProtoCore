 ...
 *
 * @c work is PROTOCORE_MDNS_SERVICE_BORROW bytes the CALLER took, at an address it knows. It is not held past the call,
...
typedef struct
{
    ...
    void (*const txt)(uint8_t *work);
    void (*const add_service)(uint8_t *work);
} MdnsServiceNs;
