...
static void protocore_scp_on_open(uint8_t slot, uint32_t channel, const char *cmd, size_t cmd_len)
{
    ...
    ScpMode scp_value = Scp.parse_cmd(protocore_ssh_scp_span(), cmd, cmd_len, c->dest, sizeof(c->dest));
    ScpMode mode = scp_value;
...
static void protocore_scp_on_data(uint8_t slot, uint32_t channel, const uint8_t *data, size_t len)
{
    ...
    while (len > 0)
    {
        if (c->st == WAIT_CLINE)
        {
            ...
            uint32_t mode = 0;
            uint64_t size = 0;
            proto_bool scp_ok = Scp.parse_cline(protocore_ssh_scp_span(), c->cl, c->cl_len, &mode, &size,
                                                SSH_SCP_CTX(work)->leaf, sizeof(SSH_SCP_CTX(work)->leaf));
            if (!scp_ok)
            {
