...
{
    pc_hkdf_expand_label(work, secret, label, out, out_len, kdf->label_prefix);
}
                            ...
                            uint8_t out[TLS13_SECRET_LEN])
{
    ...
    pc_hkdf_expand_label_ctx(work, secret, label, transcript_hash, TLS13_SECRET_LEN, out, TLS13_SECRET_LEN,
                             kdf->label_prefix);
}
...
proto_bool pc_tls13_ks_early(const Tls13Kdf *kdf, Tls13KeySchedule *ks, uint8_t *s)
{
    ...
    pc_hkdf_extract(ks->s + TLS13_KS_WORK, NULL, 0, ks->s + TLS13_KS_ZEROS, TLS13_SECRET_LEN, ks->s + TLS13_KS_EARLY);
    return PROTO_TRUE;
}
                           ...
                           size_t ecdhe_len)
{
    ...
    pc_sha256(ks->s + TLS13_KS_WORK, NULL, 0, ks->s + TLS13_KS_EMPTY_HASH);
    pc_tls13_derive_secret(ks->kdf, ks->s + TLS13_KS_WORK, ks->s + TLS13_KS_EARLY, "derived",
                           ks->s + TLS13_KS_EMPTY_HASH, ks->s + TLS13_KS_DERIVED);
    pc_hkdf_extract(ks->s + TLS13_KS_WORK, ks->s + TLS13_KS_DERIVED, TLS13_SECRET_LEN, ecdhe, ecdhe_len,
                    ks->s + TLS13_KS_HANDSHAKE);

    pc_tls13_derive_secret(ks->kdf, ks->s + TLS13_KS_WORK, ks->s + TLS13_KS_HANDSHAKE, "c hs traffic", ch_sh_hash,
                           ks->s + TLS13_KS_CLIENT_HS);
    pc_tls13_derive_secret(ks->kdf, ks->s + TLS13_KS_WORK, ks->s + TLS13_KS_HANDSHAKE, "s hs traffic", ch_sh_hash,
...
void pc_tls13_ks_master(Tls13KeySchedule *ks, const uint8_t ch_sfin_hash[TLS13_SECRET_LEN])
{
    ...
    pc_sha256(ks->s + TLS13_KS_WORK, NULL, 0, ks->s + TLS13_KS_EMPTY_HASH);
    pc_tls13_derive_secret(ks->kdf, ks->s + TLS13_KS_WORK, ks->s + TLS13_KS_HANDSHAKE, "derived",
                           ks->s + TLS13_KS_EMPTY_HASH, ks->s + TLS13_KS_DERIVED);
    pc_hkdf_extract(ks->s + TLS13_KS_WORK, ks->s + TLS13_KS_DERIVED, TLS13_SECRET_LEN, ks->s + TLS13_KS_ZEROS,
                    TLS13_SECRET_LEN, ks->s + TLS13_KS_MASTER);

    pc_tls13_derive_secret(ks->kdf, ks->s + TLS13_KS_WORK, ks->s + TLS13_KS_MASTER, "c ap traffic", ch_sfin_hash,
                           ks->s + TLS13_KS_CLIENT_AP);
    pc_tls13_derive_secret(ks->kdf, ks->s + TLS13_KS_WORK, ks->s + TLS13_KS_MASTER, "s ap traffic", ch_sfin_hash,
                           ...
                           const uint8_t transcript_hash[TLS13_SECRET_LEN], uint8_t out[TLS13_SECRET_LEN])
{
    ...
    pc_hkdf_expand_label(ks->s + TLS13_KS_WORK, base_secret, "finished", fk, TLS13_SECRET_LEN, ks->kdf->label_prefix);
    pc_hmac_sha256(ks->s + TLS13_KS_WORK, fk, TLS13_SECRET_LEN, transcript_hash, TLS13_SECRET_LEN, out);
}
