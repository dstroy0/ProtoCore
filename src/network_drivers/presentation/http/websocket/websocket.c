...
static_assert(WS_OFF_CTX % _Alignof(WsCtx) == 0,
              "WS_OFF_CTX is not a multiple of alignof(WsCtx) - WS_CTX() would return a misaligned "
    ...
    if (ws->fin)
    {
        ...
        if (ws->msg_compressed)
        {
            ...
            if (!in || !out || !tbl)
            {
                ...
                protocore_ws_close(work); // arena exhausted: fail closed
