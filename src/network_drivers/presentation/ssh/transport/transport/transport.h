 ...
 *
 * @param i       the SSH slot the exchange belongs to
 * @param ks      the host key blob the server sent
 * @param ks_len  its length
 * @param sig     the signature blob
 * @param sig_len its length
 * @param h       the exchange hash the signature is taken over
 * @param h_len   its length
 * @return true when the signature checks out under the blob's own algorithm.
 */
