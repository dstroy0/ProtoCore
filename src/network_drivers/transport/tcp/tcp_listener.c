...
static proto_bool listener_accept_allowed(uint32_t now_ms)
{
    ...
    if ((uint32_t)(now_ms - s_accept.window_start) >= PROTOCORE_ACCEPT_THROTTLE_WINDOW_MS)
    {
        s_accept.window_start = now_ms;
        s_accept.count = 0;
    }
    if (s_accept.count >= PROTOCORE_ACCEPT_THROTTLE_MAX)
    {
        return PROTO_FALSE;
    }
    s_accept.count++;
...
static proto_bool listener_accept_allowed_ip(const protocore_ip *ip, uint32_t now_ms)
{
    ...
    for (int i = 0; i < PROTOCORE_PER_IP_THROTTLE_SLOTS; i++)
    {
        ...
        if (b->addr.family != PROTOCORE_IP_NONE && Ip.equal(&b->addr, ip))
        {
            // Unsigned subtraction wraps correctly across the millis() rollover.
            if ((uint32_t)(now_ms - b->window_start) >= PROTOCORE_PER_IP_THROTTLE_WINDOW_MS)
            {
        ...
        {
            if (expired < 0 && (uint32_t)(now_ms - b->window_start) >= PROTOCORE_PER_IP_THROTTLE_WINDOW_MS)
            {
                expired = i;
            }
...
static void listener_per_ip_throttle_reset(void)
{
    for (int i = 0; i < PROTOCORE_PER_IP_THROTTLE_SLOTS; i++)
    {
        s_iptt.buckets[i].addr.family = PROTOCORE_IP_NONE;
...
static proto_bool listener_ip_allow_add(const protocore_ip *network, uint8_t prefix_len)
{
    if (!network)
    {
        return PROTO_FALSE;
    }
    int bits = (network->family == PROTOCORE_IP_V4) ? 32 : (network->family == PROTOCORE_IP_V6 ? 128 : -1);
    if (bits < 0 || prefix_len > (uint8_t)bits)
    {
        return PROTO_FALSE; // reject a malformed family or an over-long prefix
    }
    if (s_allow.count >= PROTOCORE_IP_ALLOWLIST_SLOTS)
    {
        return PROTO_FALSE;
    }
...
static proto_bool listener_ip_allowed(const protocore_ip *ip)
{
    if (s_allow.count == 0)
    {
...
static void listener_ip_allowlist_reset(void)
{
    ...
    {
        s_allow.rules[i].network.family = PROTOCORE_IP_NONE;
    }
    s_allow.count = 0;
}
...
static void listener_worker_queues_init(void)
{
    for (int i = 0; i < PROTOCORE_WORKER_COUNT; i++)
    {
        ...
        {
            s_lq.wq[i] =
                protocore_platform_queue_create(EVT_QUEUE_DEPTH, sizeof(TcpEvt), s_lq.wq_storage[i], &s_lq.wq_struct[i]);
        }
    }
}

static protocore_platform_queue listener_worker_queue(int worker_id)
{
...
{
    if (evt == NULL || evt->slot_id >= CONN_POOL_SLOTS)
    {
        return PROTO_FALSE;
    }
#if PROTOCORE_WORKER_COUNT > 1
    ...
    if (owner >= PROTOCORE_WORKER_COUNT || !s_lq.wq[owner])
    {
        return PROTO_FALSE;
    }
    if (protocore_platform_queue_send(s_lq.wq[owner], evt, 0) != PROTOCORE_PLATFORM_OK)
    {
        return PROTO_FALSE;
    }
    ...
    if (protocore_platform_queue_send(lst->queue, evt, 0) != PROTOCORE_PLATFORM_OK)
    {
        return PROTO_FALSE;
    }
 ...
 * the pool is full - PROTOCORE_NET_ERR_ABRT tells lwIP the PCB is already gone from our side.
 *
 ...
 * to drive it through protocore_net_on_accept(), whose mock (test/mocks/lwip/tcp.h) does not
...
protocore_net_err listener_accept_cb(void *arg, protocore_pcb *newpcb, protocore_net_err err)
{
    ...
    if (!listener_accept_allowed(protocore_millis()))
    {
    ...
    // is hardcoded to PROTOCORE_IP_NONE just above (no real lwIP pcb to read an
    ...
    if (!listener_accept_allowed_ip(&remote, protocore_millis()))
    {
        protocore_net_abort(newpcb);
        return PROTOCORE_NET_ERR_ABRT;
    }
    ...
    // pcb is race-free (same context as protocore_net_nagle_disable above).
    {
        uint8_t dscp = (lst->dscp != PROTOCORE_DSCP_UNSET) ? lst->dscp : DiffServ.default_dscp();
        if (dscp)
        {
...
static protocore_net_err listener_lwip_marshal(uint8_t idx, uint16_t port, proto_bool create);
...
static int32_t listener_add(uint8_t idx, uint16_t port, ProtoConn proto, proto_bool tls)
{
    ...
    if (listener_lwip_marshal(idx, port, PROTO_TRUE) != PROTOCORE_NET_OK)
    {
...
static protocore_net_err listener_lwip_do(protocore_net_call *c)
{
...
static protocore_net_err listener_lwip_marshal(uint8_t idx, uint16_t port, proto_bool create)
{
    ...
    k.port = port;
    k.create = create;
    protocore_net_call_marshal(listener_lwip_do, &k.base);
    return k.result;
}
...
static proto_bool set_dscp(uint16_t port, uint8_t dscp)
{
    for (uint8_t i = 0; i < MAX_LISTENERS; i++)
    {
        ...
        if (lst->active && lst->port == port)
        {
            ...
            lst->dscp = (dscp == PROTOCORE_DSCP_UNSET) ? PROTOCORE_DSCP_UNSET : (uint8_t)(dscp & 0x3F);
            return PROTO_TRUE;
        }
    }
    return PROTO_FALSE;
