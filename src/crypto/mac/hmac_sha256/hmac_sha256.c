...
{
    Sha256V.update_args.data = HmacSha256.update_args.data;
    Sha256V.update_args.len = HmacSha256.update_args.len;
    Sha256.update(HMAC_INNER(work));
    HmacSha256.ok = PROTO_TRUE;
}
...
static void hmac_final(uint8_t *restrict work)
{
    ...
    Sha256.update(HMAC_HASH(work));
    Sha256V.final_args.out = HmacSha256.final_args.out;
...
static void hmac_mac(uint8_t *restrict work)
{
    ...
    Sha256V.update_args.len = PROTOCORE_SHA256_BLOCK_LEN;
    Sha256.update(hw);
    Sha256V.update_args.data = HmacSha256.mac_args.data;
    Sha256V.update_args.len = HmacSha256.mac_args.len;
    Sha256.update(hw);
    Sha256V.final_args.out = w->inner_digest;
    ...
    Sha256V.update_args.len = PROTOCORE_SHA256_DIGEST_LEN;
    Sha256.update(hw);
    Sha256V.final_args.out = HmacSha256.mac_args.out;
